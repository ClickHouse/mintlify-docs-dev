...
We recommend users first run a sizing exercise to estimate the storage and memory requirements for this dataset by referring to the [documentation](/core/reference/engines/table-engines/mergetree-family/annindexes).
...
vector similarity search combined with post-filtering/pre-filtering as explained in the [documentation](/core/reference/engines/table-engines/mergetree-family/annindexes)
</Step>
<Step>
...
The parameters and performance considerations for index creation and search are described in the [documentation](/core/reference/engines/table-engines/mergetree-family/annindexes).
...
is then passed as an argument to the [`cosineDistance()`](/core/reference/functions/regular-functions/distance-functions#cosineDistance) function in the `SELECT` query.
...
<Image img="/images/getting-started/example-datasets/laion5b_visualization_1.png" alt="Vector Similarity Search Results" size="md"/>
</Step>
</Steps>
