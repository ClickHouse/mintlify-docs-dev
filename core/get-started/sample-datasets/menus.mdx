...
We use [Decimal](/core/reference/data-types/decimal) data type to store prices.
...
We use [CSVWithNames](/core/reference/formats/CSV/CSVWithNames) format as the data is represented by CSV with header.
...
We disable [input_format_null_as_default](/core/reference/settings/formats#input_format_null_as_default) as our data doesn't have [NULL](/core/reference/settings/formats#input_format_null_as_default). Otherwise ClickHouse will try to parse `\N` sequences and can be confused with `\` in data.

The setting [date_time_input_format best_effort](/core/reference/settings/formats#date_time_input_format) allows to parse [DateTime](/core/reference/data-types/datetime)  fields in wide variety of formats. For example, ISO-8601 without seconds like '2000-01-01 01:02' will be recognized. Without this setting only fixed DateTime format is allowed.

## Denormalize the data {#denormalize-data}

Data is presented in multiple tables in [normalized form](https://en.wikipedia.org/wiki/Database_normalization#Normal_forms). It means you have to perform [JOIN](/core/reference/statements/select/join) if you want to query, e.g. dish names from menu items.
