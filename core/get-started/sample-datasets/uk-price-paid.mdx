...
- ignoring the [UUid](/core/reference/data-types/uuid) field because we don't need it for analysis
- transforming `type` and `duration` to more readable `Enum` fields using the [transform](/core/reference/functions/regular-functions/other-functions#transform) function
- transforming the `is_new` field from a single-character string (`Y`/`N`) to a [UInt8](/core/reference/data-types/int-uint) field with 0 or 1
...
We can speed up these queries with projections. See ["Projections"](/core/concepts/features/projections/projections) for examples with this dataset.
