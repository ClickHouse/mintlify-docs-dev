...
Similar to the [Star Schema Benchmark (SSB)](/core/get-started/sample-datasets/star-schema), TPC-DS is based on [TPC-H](/core/get-started/sample-datasets/tpch), but it took the opposite route, i.e. it expanded the number of joins needed by storing the data in a complex snowflake schema (24 instead of 8 tables).
