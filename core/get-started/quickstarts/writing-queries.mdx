...
slug: /core/guides/writing-queries
...
<Note>
View the [SQL Reference](/core/reference/statements/select) for more details on the syntax and available clauses and options.
</Note>
...
Add a `FORMAT` clause to specify one of the [many supported output formats of ClickHouse](/core/reference/formats#formats-overview):
```sql
SELECT *
FROM helloworld.my_first_table
ORDER BY timestamp
...
need a ClickHouse server up and running to transform data - you can use the `clickhouse-local` tool. View the [docs page of `clickhouse-local`](/core/concepts/features/interfaces/cli) for details.
</Note>
