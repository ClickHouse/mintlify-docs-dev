---
slug: /core/guides/developer/mutations
...
<Tip>
If you need to perform frequent updates, consider using [deduplication](/core/concepts/features/operations/insert/deduplication) in ClickHouse, which allows you to update 
and/or delete rows without generating a mutation event. Alternatively, use [lightweight updates](/core/reference/statements/update)
or [lightweight deletes](/core/concepts/features/operations/delete/lightweight-delete)
</Tip>
...
View the [`DELETE` statement](/core/reference/statements/delete) docs page for more details.
...
- Lightweight deletes are synchronous by default, waiting for all replicas to process the delete. The behavior is controlled by the [`lightweight_deletes_sync` setting](/core/reference/settings/session-settings#lightweight_deletes_sync).
