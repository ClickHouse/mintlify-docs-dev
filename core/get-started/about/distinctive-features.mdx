...
In addition to efficient general-purpose compression codecs with different trade-offs between disk space and CPU consumption, ClickHouse provides [specialized codecs](/core/reference/statements/create/table#specialized-codecs) for specific kinds of data, which allows ClickHouse to compete with and outperform more niche databases, like time-series ones.
...
ClickHouse supports [a declarative query language](/core/reference/home) based on SQL that is mostly compatible with the ANSI SQL standard.

Supported queries include [GROUP BY](/core/reference/statements/select/group-by), [ORDER BY](/core/reference/statements/select/order-by), subqueries in [FROM](/core/reference/statements/select/from), the [JOIN](/core/reference/statements/select/join) clause, the [IN](/core/reference/statements/in) operator, [window functions](/core/reference/functions/window-functions) and scalar subqueries.
...
2.  Running a query based on a part ([SAMPLE](/core/reference/statements/select/sample)) of data and getting an approximated result. In this case, proportionally less data is retrieved from the disk.
...
ClickHouse adaptively chooses how to [JOIN](/core/reference/statements/select/join) multiple tables, by preferring hash join and falling back to merge join if there's more than one large table.
...
For more information, see the section [Data replication](/core/reference/engines/table-engines/mergetree-family/replication).

## Role-Based Access Control {#role-based-access-control}

ClickHouse implements user account management using SQL queries and allows for [role-based access control configuration](/core/concepts/features/security/access-rights) similar to what can be found in the ANSI SQL standard and popular relational database management systems.
