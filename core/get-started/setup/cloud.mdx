...
<Image img="/images/_snippets/signup_page.png" size="md" alt='Select Plan' border/>
<br/>
...
<Image img="/images/_snippets/createservice1.png" size="md" alt='New ClickHouse Service' border/>
<br/>
...
<Image img="/images/_snippets/scaling_limits.png" size="md" alt='Scaling Limits' border/>
<br/>
...
<Image img="/images/_snippets/createservice8.png" size="md" alt='SQL Console' border/>
...
<Image img="/images/_snippets/show_databases.png" size="md" alt='SQL Console' border/>
<br/>
...
<Image img="/images/_snippets/service_connect.png" size="md" alt='Service Connect' border/>
<br/>

If you can't see your language client, you may want to check our list of  [Integrations](/integrations/connectors/home).
</Step>
<Step>
## Add data {#3-add-data}
...
<Image img="/images/_snippets/data_sources.png" size="md" alt='Data sources' border/>
<br/>
...
<Image img="/images/_snippets/select_data_source.png" size="md" alt='Select data source' border/>
<br/>

### Add data using the SQL Console {#add-data-using-the-sql-console}
Like most database management systems, ClickHouse logically groups tables into **databases**. Use the [`CREATE DATABASE`](/core/reference/statements/create/database) command to create a new database in ClickHouse:
...
In the example above, `my_first_table` is a [`MergeTree`](/core/reference/engines/table-engines/mergetree-family/mergetree) table with four columns:

  - `user_id`:  a 32-bit unsigned integer ([UInt32](/core/reference/data-types/int-uint))
  - `message`: a [String](/core/reference/data-types/string) data type, which replaces types like `VARCHAR`, `BLOB`, `CLOB` and others from other database systems
  - `timestamp`: a [DateTime](/core/reference/data-types/datetime) value, which represents an instant in time
  - `metric`: a 32-bit floating point number ([Float32](/core/reference/data-types/float))
...
<br/>
There are many table engines to choose from, but for a simple table on a single-node ClickHouse server, [`MergeTree`](/core/reference/engines/table-engines/mergetree-family/mergetree) is your likely choice.
</Info>
...
For a deep dive into core ClickHouse concepts, see ["Core Concepts"](/core/concepts/core-concepts).

#### Insert data into your table {#insert-data-into-your-table}
You can use the familiar [`INSERT INTO TABLE`](/core/reference/statements/insert-into) command with ClickHouse, but it is important to understand that each insert into a [`MergeTree`](/core/reference/engines/table-engines/mergetree-family/mergetree) table causes a **part** to be created in storage.

<Tip>
**ClickHouse best practice**

Insert a large number of rows per batch - tens of thousands or even millions of
rows at once. Don't worry - ClickHouse can easily handle that type of volume - and it will [save you money](/core/concepts/best-practices/selecting-an-insert-strategy#batch-inserts-if-synchronous) by sending fewer write requests to your service.
</Tip>
...
<Note>
Notice the `timestamp` column is populated using various [**Date**](/core/reference/data-types/date) and [**DateTime**](/core/reference/data-types/datetime) functions. ClickHouse has hundreds of useful functions that you can [view in the **Functions** section](/core/reference/functions).
</Note>
...
You can also connect to your ClickHouse Cloud service using a command-line tool named [**clickhouse client**](/core/concepts/features/interfaces/cli). Click `Connect` on the left menu to access these details. From the dialog select `Native` from the drop-down:

<Image img="/images/_snippets/client_details.png" size="md" alt='clickhouse client connection details' border/>
<br/>

1. Install [ClickHouse](/core/concepts/features/interfaces/cli).
...
4. Add a [`FORMAT`](/core/reference/statements/select/format) clause to specify one of the [many supported output formats of ClickHouse](/core/reference/formats):
...
<Image img="/images/_snippets/new_rows_from_csv.png" size="md" alt='New rows from CSV file' />
<br/>
</Step>
</Steps>
...
- The [Tutorial](/core/get-started/quickstarts/tutorial) has you insert 2 million rows into a table and write some analytical queries
