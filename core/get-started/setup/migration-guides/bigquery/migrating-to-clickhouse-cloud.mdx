...
<Image img="/images/migrations/bigquery-2.png" size="md" alt="ClickHouse vs BigQuery"/>

## Loading data from BigQuery to ClickHouse Cloud {#loading-data-from-bigquery-to-clickhouse-cloud}

### Dataset {#dataset}

As an example dataset to show a typical migration from BigQuery to ClickHouse Cloud, we use the Stack Overflow dataset documented [here](/core/get-started/sample-datasets/stackoverflow). This contains every `post`, `vote`, `user`, `comment`, and `badge` that has occurred on Stack Overflow from 2008 to Apr 2024. The BigQuery schema for this data is shown below:

<Image img="/images/migrations/bigquery-3.png" size="lg" alt="Schema"/>
...
Applying the process [described here](/core/guides/clickhouse/data-modelling/schema-design) results in the following schema:
...
We can populate this table with a simple [`INSERT INTO SELECT`](/core/reference/statements/insert-into), reading the exported data from gcs using the [`gcs` table function](/core/reference/functions/table-functions/gcs). Note that on ClickHouse Cloud you can also use the gcs-compatible [`s3Cluster` table function](/core/reference/functions/table-functions/s3Cluster) to parallelize the loading over multiple nodes:
...
As described [here](/core/get-started/setup/migration-guides/bigquery), like in BigQuery, ClickHouse doesn't enforce uniqueness for a table's primary key column values.
...
In contrast to BigQuery, ClickHouse automatically creates [a (sparse) primary index](/core/guides/clickhouse/data-modelling/sparse-primary-indexes) based on the primary key column values. This index is used to speed up all queries that contain filters on the primary key columns. Specifically:
...
For the considerations and steps in choosing an ordering key, using the posts table as an example, see [here](/core/guides/clickhouse/data-modelling/schema-design#choosing-an-ordering-key).

## Data modeling techniques {#data-modeling-techniques}

We recommend users migrating from BigQuery read [the guide for modeling data in ClickHouse](/core/guides/clickhouse/data-modelling/schema-design). This guide uses the same Stack Overflow dataset and explores multiple approaches using ClickHouse features.
...
You should consider partitioning a data management technique. It is ideal when data needs to be expired from the cluster when operating with time series data e.g. the oldest partition can [simply be dropped](/core/reference/statements/alter/partition#drop-partitionpart).
...
In [ClickHouse data modeling](/core/guides/clickhouse/data-modelling/schema-design), we explore how materialized views can be used
in ClickHouse to pre-compute aggregations, transform rows, and optimize queries 
for different access patterns. For the latter, we [provided an example](/core/concepts/features/materialized-views/incremental-materialized-view#lookup-table) where
...
With an [`EXPLAIN` command](/core/reference/statements/explain), we also confirm the projection was used to serve this query:
...
<Image img="/images/migrations/bigquery-7.png" size="md" alt="Projections"/>
...
details see ["materialized views versus projections"](/core/concepts/features/projections/materialized-views-versus-projections)
...
- Users are comfortable with the associated increase in storage footprint and overhead of writing data twice. Test the impact on insertion speed and [evaluate the storage overhead](/core/guides/clickhouse/data-modelling/compression/compression-in-clickhouse).
...
<Image img="/images/migrations/bigquery-8.png" size="sm" alt="Rewriting BigQuery queries" border/>
...
<Image img="/images/migrations/bigquery-9.png" size="sm" alt="BigQuery 1" border/>
...
Where possible, you should exploit ClickHouse aggregate functions. Below, we show the use of the [`argMax` function](/core/reference/functions/aggregate-functions/reference/argMax) to compute the most viewed question of each year.

_BigQuery_

<Image img="/images/migrations/bigquery-10.png" border size="sm" alt="Aggregate functions 1"/>

<Image img="/images/migrations/bigquery-11.png" border size="sm" alt="Aggregate functions 2"/>
...
<Image img="/images/migrations/bigquery-12.png" size="sm" border alt="Conditionals and Arrays"/>
...
This concludes our basic guide if you're migrating from BigQuery to ClickHouse. We recommend reading the guide for [modeling data in ClickHouse](/core/guides/clickhouse/data-modelling/schema-design) to learn more about advanced ClickHouse features.
