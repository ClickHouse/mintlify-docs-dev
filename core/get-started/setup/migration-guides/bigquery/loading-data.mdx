...
Once the export is complete, we can import this data into a ClickHouse table. You can use the [ClickHouse SQL console](/products/cloud/features/sql-console-features/sql-console) or [`clickhouse-client`](/core/concepts/features/interfaces/cli) to execute the commands below.

You must first [create your table](/core/reference/statements/create/table) in ClickHouse:
...
Finally, we can insert the data from GCS into our ClickHouse table using the [`INSERT INTO SELECT` command](/core/reference/statements/insert-into#inserting-the-results-of-select), which inserts data into a table based on the results from a `SELECT` query.

To retrieve the data to `INSERT`, we can use the [s3Cluster function](/core/reference/functions/table-functions/s3Cluster) to retrieve data from our GCS bucket since GCS is interoperable with [Amazon S3](https://aws.amazon.com/s3/). If you only have one ClickHouse node, you can use the [s3 table function](/core/reference/functions/table-functions/s3) instead of the `s3Cluster` function.
...
In the above query, we use the [`ifNull` function](/core/reference/functions/regular-functions/functions-for-nulls#ifNull) with the `some_text` column to insert data into our ClickHouse table with a default value. You can also make your columns in ClickHouse [`Nullable`](/core/reference/data-types/nullable), but this isn't recommended as it may affect negatively performance.
