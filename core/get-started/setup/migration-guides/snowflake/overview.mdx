...
<Image img="/images/cloud/onboard/discover/use_cases/snowflake_architecture.png" size="md" alt="Snowflake architecture" />
...
<Image img="/images/cloud/onboard/discover/use_cases/cloud_architecture.png" size="md" alt="ClickHouse Cloud architecture" />
...
* **Query latency**: Snowflake queries have a higher query latency even
  ...
  with the ability to control its use on a [per-query basis](/core/reference/settings/session-settings#use_query_cache),
  its [precise size](/core/reference/settings/session-settings#query_cache_max_size_in_bytes),
  whether a [query is cached](/core/reference/settings/session-settings#enable_writes_to_query_cache)
