...
<Image img="/images/integrations/data-ingestion/dbms/postgres-replicas.png" size="md" alt="Eventual consistency"/>
...
To achieve this behavior in OSS depends on your shard and replica topology and if you're using a [Distributed table](/core/reference/engines/table-engines/special/distributed) for querying.
...
In this case, you should ensure consistent node routing is performed based on a property e.g. `session_id` or `user_id`. The settings [`prefer_localhost_replica=0`](/core/reference/settings/session-settings#prefer_localhost_replica), [`load_balancing=in_order`](/core/reference/settings/session-settings#load_balancing) should be [set in the query](/core/concepts/features/configuration/settings/settings-query-level). This will ensure any local replicas of shards are preferred, with replicas preferred as listed in the configuration otherwise - provided they have the same number of errors - failover will occur with random selection if errors are higher. [`load_balancing=nearest_hostname`](/core/reference/settings/session-settings#load_balancing) can also be used as an alternative for this deterministic shard selection.
...
1. **Read/Write to the same node** - If you're using native protocol, or a [session to do your write/read via HTTP](/core/concepts/features/interfaces/http#default-database), you should then be connected to the same replica: in this scenario you're reading directly from the node where you're writing, then your read will always be consistent.
...
1. **Enable sequential consistency** - via the query setting [`select_sequential_consistency = 1`](/core/reference/settings/session-settings#select_sequential_consistency). In OSS, the setting `insert_quorum = 'auto'` must also be specified.
...
ClickHouse provides ACID properties under [limited configurations](/core/concepts/features/operations/insert/transactions) - most simply when using a non-replicated instance of the MergeTree table engine with one partition. You shouldn't expect these properties outside of these cases and ensure these aren't a requirement.
...
Further details on optimizing and measuring compression can be found [here](/core/guides/clickhouse/data-modelling/compression/compression-in-clickhouse).
...
| `TEXT, CHAR, BPCHAR` | [String](/core/reference/data-types/string) |
| `INTEGER` | Nullable([Int32](/core/reference/data-types/int-uint)) |
| `ARRAY` | [Array](/core/reference/data-types/array) |
| `FLOAT4` | [Float32](/core/reference/data-types/float) |
| `BOOLEAN` | [Bool](/core/reference/data-types/boolean) |
| `VARCHAR` | [String](/core/reference/data-types/string) |
| `BIT` | [String](/core/reference/data-types/string) |
| `BIT VARYING` | [String](/core/reference/data-types/string) |
| `BYTEA` | [String](/core/reference/data-types/string) |
| `NUMERIC` | [Decimal](/core/reference/data-types/decimal) |
| `GEOGRAPHY` | [Point](/core/reference/data-types/geo#point), [Ring](/core/reference/data-types/geo#ring), [Polygon](/core/reference/data-types/geo#polygon), [MultiPolygon](/core/reference/data-types/geo#multipolygon) |
| `GEOMETRY` | [Point](/core/reference/data-types/geo#point), [Ring](/core/reference/data-types/geo#ring), [Polygon](/core/reference/data-types/geo#polygon), [MultiPolygon](/core/reference/data-types/geo#multipolygon) |
| `INET` | [IPv4](/core/reference/data-types/ipv4), [IPv6](/core/reference/data-types/ipv6) |
| `MACADDR` | [String](/core/reference/data-types/string) |
| `CIDR` | [String](/core/reference/data-types/string) |
| `HSTORE` | [Map(K, V)](/core/reference/data-types/map), [Map](/core/reference/data-types/map)(K,[Variant](/core/reference/data-types/variant)) |
| `UUID` | [UUID](/core/reference/data-types/uuid) |
| `ARRAY<T>` | [ARRAY(T)](/core/reference/data-types/array) |
| `JSON` | [String](/core/reference/data-types/string), [Variant](/core/reference/data-types/variant), [Nested](/core/reference/data-types/nested-data-structures#nestedname1-type1-name2-type2-), [Tuple](/core/reference/data-types/tuple) |
| `JSONB` | [String](/core/reference/data-types/string) |
