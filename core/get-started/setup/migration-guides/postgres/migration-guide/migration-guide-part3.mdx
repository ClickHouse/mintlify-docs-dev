...
We recommend users migrating from Postgres read [the guide for modeling data in ClickHouse](/core/guides/clickhouse/data-modelling/schema-design). This guide uses the same Stack Overflow dataset and explores multiple approaches using ClickHouse features.
...
- Postgres primary keys are, by definition, unique per row. The use of [B-tree structures](/core/guides/clickhouse/data-modelling/sparse-primary-indexes#an-index-design-for-massive-data-scales) allows the efficient lookup of single rows by this key. While ClickHouse can be optimized for the lookup of a single row value, analytics workloads will typically require the reading of a few columns but for many rows. Filters will more often need to identify **a subset of rows** on which an aggregation will be performed.
...
For more details, we recommend this [in-depth guide](/core/guides/clickhouse/data-modelling/sparse-primary-indexes).

<Image img="/images/migrations/postgres-b-tree.png" size="lg" alt="PostgreSQL B-Tree Index"/>

<Image img="/images/migrations/postgres-sparse-index.png" size="lg" alt="PostgreSQL Sparse Index"/>
...
<Image img="/images/migrations/postgres-partitions.png" size="md" alt="PostgreSQL partitions to ClickHouse partitions"/>
...
For a full description of partitioning see ["Table partitions"](/core/concepts/core-concepts/partitions).
...
In ClickHouse [data modeling docs](/core/guides/clickhouse/data-modelling/schema-design), we explore how materialized views can be used in ClickHouse to pre-compute aggregations, transform rows, and optimize queries for different access patterns.

For the latter of these, we provided [an example](/core/concepts/features/materialized-views/incremental-materialized-view#lookup-table) where the materialized view sends rows to a target table with a different ordering key than the original table receiving inserts.
...
with a [projection](/core/concepts/features/projections/projections). The command below adds a 
...
<Image img="/images/migrations/postgres-projections.png" size="md" alt="PostgreSQL projections in ClickHouse"/>
...
Despite these advantages, projections come with some [inherent limitations](/core/concepts/features/projections/projections#when-to-use-projections) 
...
- Users are comfortable with the associated increase in storage footprint and 
  ...
  [evaluate the storage overhead](/core/guides/clickhouse/data-modelling/compression/compression-in-clickhouse).
...
For more details see ["Projections"](/core/concepts/features/projections/projections)
</Note>
...
You can refer to this [guide](/core/guides/clickhouse/data-modelling/denormalization) that shows the benefits of denormalizing the Stack Overflow dataset in ClickHouse.

This concludes our basic guide if you're migrating from Postgres to ClickHouse. We recommend reading the [guide for modeling data in ClickHouse](/core/guides/clickhouse/data-modelling/schema-design) to learn more about advanced ClickHouse features.
