...
For the SSL [`chain of trust`](https://en.wikipedia.org/wiki/Chain_of_trust) to work correctly, it is also important to make sure that the [`caConfig`](/core/reference/settings/server-settings/settings#openssl) parameter is configured properly.
