...
When [SQL-driven Access Control and Account Management](/core/concepts/features/security/access-rights#access-control-usage) is enabled in ClickHouse, users identified by HTTP authentication can also be created using SQL statements.
