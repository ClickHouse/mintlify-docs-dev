...
This section configures the [trace_log](/core/reference/system-tables/trace_log) system table containing the results of the profiler functioning.
...
Set up the [`query_profiler_cpu_time_period_ns`](/core/reference/settings/session-settings#query_profiler_cpu_time_period_ns) or [`query_profiler_real_time_period_ns`](/core/reference/settings/session-settings#query_profiler_real_time_period_ns) settings.
...
To analyze the `trace_log` system table allow introspection functions with the [`allow_introspection_functions`](/core/reference/settings/session-settings#allow_introspection_functions) setting:
...
Use the `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` and `demangle` [introspection functions](/core/reference/functions/regular-functions/introspection) to get function names and their positions in ClickHouse code.
...
ClickHouse provides the [`flameGraph`](/core/reference/functions/aggregate-functions/reference/flame_graph) aggregate function which builds a flame graph directly from stack traces stored in `trace_log`.
...
- `traces` — a stacktrace. [`Array(UInt64)`](/core/reference/data-types/array).
- `size` — an allocation size for memory profiling. [`Int64`](/core/reference/data-types/int-uint).
- `ptr` — an allocation address. [`UInt64`](/core/reference/data-types/int-uint).
