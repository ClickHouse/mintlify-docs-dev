...
This section is similar to [<http_handlers>](/core/concepts/features/interfaces/http) but works for prometheus protocols:
...
| `url` / `headers` / `method` | none       | Filters used to find a matching handler for a request. Similar to the fields with the same names in the [`<http_handlers>`](/core/concepts/features/interfaces/http) section.                                    |
