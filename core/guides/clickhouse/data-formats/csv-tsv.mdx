...
Note that we use [FORMAT CSV](/core/reference/formats/CSV/CSV) to let ClickHouse know we're ingesting CSV formatted data. Alternatively, we can load data from a local file using the [FROM INFILE](/core/reference/statements/insert-into#inserting-data-from-a-file) clause:
...
Here, we use the `FORMAT CSV` clause so ClickHouse understands the file format. We can also load data directly from URLs using [url()](/core/reference/functions/table-functions/url) function or from S3 files using [s3()](/core/reference/functions/table-functions/s3) function.
...
To import data from this file, we can use [CSVWithNames](/core/reference/formats/CSV/CSVWithNames) format:
...
In case the CSV file uses other than comma delimiter, we can use the [format_csv_delimiter](/core/reference/settings/formats#format_csv_delimiter) option to set the relevant symbol:
...
Sometimes, we might skip a certain number of lines while importing data from a CSV file. This can be done using [input_format_csv_skip_first_lines](/core/reference/settings/formats#input_format_csv_skip_first_lines) option:
...
<Tip>
When using the `file()` function, with ClickHouse Cloud you will need to run the commands in `clickhouse client` on the machine where the file resides. Another option is to use [`clickhouse-local`](/core/concepts/features/tools-and-utilities/clickhouse-local) to explore files locally.
</Tip>

### Treating NULL values in CSV files {#treating-null-values-in-csv-files}

Null values can be encoded differently depending on the application that generated the file. By default, ClickHouse uses `\N` as a Null value in CSV. But we can change that using the [format_csv_null_representation](/core/reference/settings/formats#format_tsv_null_representation) option.
...
Tab-separated data format is widely used as a data interchange format. To load data from a [TSV file](/assets/data_small.tsv) to ClickHouse, the [TabSeparated](/core/reference/formats/TabSeparated/TabSeparated) format is used:

```bash
clickhouse-client -q "INSERT INTO sometable FORMAT TabSeparated" < data_small.tsv
```

There's also a [TabSeparatedWithNames](/core/reference/formats/TabSeparated/TabSeparatedWithNames) format to allow working with TSV files that have headers. And, like for CSV, we can skip the first X lines using the [input_format_tsv_skip_first_lines](/core/reference/settings/formats#input_format_tsv_skip_first_lines) option.

### Raw TSV {#raw-tsv}

Sometimes, TSV files are saved without escaping tabs and line breaks. We should use [TabSeparatedRaw](/core/reference/formats/TabSeparated/TabSeparatedRaw) to handle such files.
...
To add a header to the CSV file, we use the [CSVWithNames](/core/reference/formats/CSV/CSVWithNames) format:
...
To save exported data to a file, we can use the [INTO...OUTFILE](/core/reference/statements/select/into-outfile) clause:
...
If we want to have other than comma delimiters, we can use the [format_csv_delimiter](/core/reference/settings/formats#format_csv_delimiter) settings option for that:
...
If we want a CSV file to work fine in a Windows environment, we should consider enabling [output_format_csv_crlf_end_of_line](/core/reference/settings/formats#output_format_csv_crlf_end_of_line) option. This will use `\r\n` as a line breaks instead of `\n`:
...
ClickHouse also allows explicitly setting column types when exporting data using [CSVWithNamesAndTypes](/core/reference/formats/CSV/CSVWithNamesAndTypes) (and other *WithNames formats family):
...
In sophisticated cases, text data can be formatted in a highly custom manner but still have a structure. ClickHouse has a special [CustomSeparated](/core/reference/formats/CustomSeparated/CustomSeparated) format for such cases, which allows setting custom escaping rules, delimiters, line separators, and starting/ending symbols.
...
We can also use [CustomSeparatedWithNames](/core/reference/formats/CustomSeparated/CustomSeparatedWithNames) to get headers exported and imported correctly. Explore [regex and template](/core/guides/clickhouse/data-formats/templates-regex) formats to deal with even more complex cases.
...
- [Regex and templates](/core/guides/clickhouse/data-formats/templates-regex)
