...
We can use a [Template](/core/reference/formats/Template/Template) format to import this data. We have to define a template string with values placeholders for each row of input data:
...
We define a name of a column and escaping rule in a `${name:escaping}` format. Multiple options are available here, like CSV, JSON, Escaped, or Quoted, which implement [respective escaping rules](/core/reference/formats/Template/Template).
...
Consider using [TemplateIgnoreSpaces](/core/reference/formats/Template/TemplateIgnoreSpaces), which allows skipping whitespaces between delimiters in a template:
```text
