...
To import from an [avro file](/assets/data.avro), we should use [Avro](/core/reference/formats/Avro/Avro) format in the `INSERT` statement:
...
With the [file()](/core/reference/functions/regular-functions/files#file) function, we can also explore Avro files before actually importing data:
...
Consider [data types matching](/core/reference/formats/Avro/Avro#data-type-mapping) when importing or exporting Avro files. Use explicit type casting to convert when loading data from Avro files:
...
When Kafka messages use Avro format, ClickHouse can read such streams using [AvroConfluent](/core/reference/formats/Avro/AvroConfluent) format and [Kafka](/core/reference/engines/table-engines/integrations/kafka) engine:
...
Another columnar format is [Apache Arrow](https://arrow.apache.org/), also supported by ClickHouse for import and export. To import data from an [Arrow file](/assets/data.arrow), we use the [Arrow](/core/reference/formats/Arrow/Arrow) format:
...
Also, check [data types matching](/core/reference/formats/Arrow/Arrow#data-types-matching) to know if any should be converted manually.

### Arrow data streaming {#arrow-data-streaming}

The [ArrowStream](/core/reference/formats/Arrow/ArrowStream) format can be used to work with Arrow streaming (used for in-memory processing). ClickHouse can read and write Arrow streams.
...
[Apache ORC](https://orc.apache.org/) format is a columnar storage format typically used for Hadoop. ClickHouse supports importing as well as exporting [Orc data](/assets/data.orc) using [ORC format](/core/reference/formats/ORC):
...
Also, check [data types matching](/core/reference/formats/ORC) as well as [additional settings](/core/reference/formats/Parquet/Parquet#format-settings) to tune export and import.
