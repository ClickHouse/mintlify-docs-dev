...
While [schema inference](/core/guides/clickhouse/data-formats/json/inference) can be used to establish an initial schema for JSON data and query JSON data files in place, e.g., in S3, you should aim to establish an optimized versioned schema for your data. We discuss the recommended approach for modeling JSON structures below.
...
1. **Primitive types** - If the key's value is a primitive type, irrespective of whether it is part of a sub-object or on the root, ensure you select its type according to general schema [design best practices](/core/guides/clickhouse/data-modelling/schema-design) and [type optimization rules](/core/guides/clickhouse/data-modelling/schema-design#optimizing-types). Arrays of primitives, such as `phone_numbers` below, can be modeled as `Array(<type>)` e.g., `Array(String)`.
2. **Static vs dynamic** - If the key's value is a complex object i.e. either an object or an array of objects, establish whether it is subject to change. Objects that rarely have new keys, where the addition of a new key can be predicted and handled with a schema change via [`ALTER TABLE ADD COLUMN`](/core/reference/statements/alter/column#add-column), can be considered **static**. This includes objects where only a subset of the keys may be provided on some JSON documents. Objects where new keys are added frequently and/or aren't predictable should be considered **dynamic**. **The exception here is structures with hundreds or thousands of sub keys which can be considered dynamic for convenience purposes**. 

To establish whether a value is **static** or **dynamic**, see the relevant sections [**Handling static objects**](/core/guides/clickhouse/data-formats/json/schema#handling-static-structures) and [**Handling dynamic objects**](/core/guides/clickhouse/data-formats/json/schema#handling-semi-structured-dynamic-structures) below.

<p></p>

**Important:** The above rules should be applied recursively. If a key's value is determined to be dynamic, no further evaluation is required and the guidelines in [**Handling dynamic objects**](/core/guides/clickhouse/data-formats/json/schema#handling-semi-structured-dynamic-structures) can be followed. If the object is static, continue to assess the subkeys until either key values are primitive or dynamic keys are encountered.
...
If you need to differentiate between a value being empty and not provided, the [Nullable](/core/reference/data-types/nullable) type can be used. This [should be avoided](/core/concepts/best-practices/select-data-type#avoid-nullable-columns) unless absolutely required, as it will negatively impact storage and query performance on these columns.
</Info>
...
Columns can be added to a schema using the [`ALTER TABLE ADD COLUMN`](/core/reference/statements/alter/column#add-column) command. A default can be specified via the `DEFAULT` clause, which will be used if it isn't specified during the subsequent inserts. Rows for which this value isn't present (as they were inserted prior to its creation) will also return this default value. If no `DEFAULT` value is specified, the default value for the type will be used.
...
If JSON data is semi-structured where keys can be dynamically added and/or have multiple types, the [`JSON`](/core/reference/data-types/newjson) type is recommended.
...
Consider our [earlier person JSON](/core/guides/clickhouse/data-formats/json/schema#static-vs-dynamic-json) where the `company.labels` object was determined to be dynamic.
...
- **Simpler introspection of paths and types** - Although the JSON type supports [introspection functions](/core/reference/data-types/newjson#introspection-functions) to determine the types and paths that have been inferred, static structures can be simpler to explore e.g. with `DESCRIBE`.
...
We can determine the inferred sub columns and their types using [introspection functions](/core/reference/data-types/newjson#introspection-functions). For example:
...
For a complete list of introspection functions, see the ["Introspection functions"](/core/reference/data-types/newjson#introspection-functions)

[Sub paths can be accessed](/core/reference/data-types/newjson#reading-json-paths-as-sub-columns) using `.` notation e.g.
...
[Introspection functions](/core/reference/data-types/newjson#introspection-functions) can be used to determine the inferred paths and types for the `company.labels` column.
...
Additionally, we can skip paths within JSON that we don't want to store with the [`SKIP` and `SKIP REGEXP`](/core/reference/data-types/newjson) parameters in order to minimize storage and avoid unnecessary inference on unneeded paths. For example, suppose we use a single JSON column for the above data. We can skip the `address` and `company` paths:
...
Type hints offer more than just a way to avoid unnecessary type inference - they eliminate storage and processing indirection entirely, as well as allowing [optimal primitive types](/core/guides/clickhouse/data-modelling/schema-design#optimizing-types) to be specified. JSON paths with type hints are always stored just like traditional columns, bypassing the need for [**discriminator columns**](https://clickhouse.com/blog/a-new-powerful-json-data-type-for-clickhouse#storage-extension-for-dynamically-changing-data) or dynamic resolution during query time. 
...
<Image img="/images/integrations/data-ingestion/data-formats/json_column_per_type.png" size="md" alt="Column per JSON path" />
...
<Image img="/images/integrations/data-ingestion/data-formats/json_offsets.png" size="md" alt="JSON offsets" />
...
<Image img="/images/integrations/data-ingestion/data-formats/json_shared_column.png" size="md" alt="Shared JSON column" />

This threshold is controlled by the [`max_dynamic_paths`](/core/reference/data-types/newjson#reaching-the-limit-of-dynamic-paths-inside-json) parameter in the JSON type declaration.
