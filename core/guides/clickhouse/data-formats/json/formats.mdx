...
Earlier examples of loading JSON data assume the use of [`JSONEachRow`](/core/reference/formats/JSON/JSONEachRow) (`NDJSON`). This format reads the keys in each JSON line as columns. For example:
...
In contrast, `JSONAsObject` treats each line as a single `JSON` object and stores it in a single column, of type [`JSON`](/core/reference/data-types/newjson), making it better suited for nested JSON payloads and cases where the keys are dynamic and have potentially more than one type. 

Use `JSONEachRow` for row-wise inserts, and [`JSONAsObject`](/core/reference/formats/JSON/JSONAsObject) when storing flexible or dynamic JSON data.
...
To import a list of JSON objects, we can use a [`JSONEachRow`](/core/reference/formats/JSON/JSONEachRow) format (inserting data from [list.json](/assets/list.json) file):
...
We have used a [FROM INFILE](/core/reference/statements/insert-into#inserting-data-from-a-file) clause to load data from the local file, and we can see the import was successful:
...
ClickHouse can load data from this kind of data using the [`JSONObjectEachRow`](/core/reference/formats/JSON/JSONObjectEachRow) format:
...
Let's say we also want to save values in parent object keys to the table. In this case, we can use the [following option](/core/reference/settings/formats#format_json_object_each_row_column_for_object_name) to define the name of the column we want key values to be saved to:

```sql
SET format_json_object_each_row_column_for_object_name = 'id'
```

Now, we can check which data is going to be loaded from the original JSON file using [`file()`](/core/reference/functions/regular-functions/files#file) function:
...
Sometimes, for the sake of saving space, JSON files are encoded in arrays instead of objects. In this case, we deal with a [list of JSON arrays](/assets/arrays.jsonl){/* MIGRATE: unresolved relative link */}:
...
In this case, ClickHouse will load this data and attribute each value to the corresponding column based on its order in the array. We use [`JSONCompactEachRow`](/core/reference/formats/JSON/JSONCompactEachRow) format for this:
...
ClickHouse uses the [`JSONColumns`](/core/reference/formats/JSON/JSONColumns) format to parse data formatted like that:
...
A more compact format is also supported when dealing with an [array of columns](/assets/columns-array.json) instead of an object using [`JSONCompactColumns`](/core/reference/formats/JSON/JSONCompactColumns) format:
...
Now we can load data from the file into this table using [`JSONAsString`](/core/reference/formats/JSON/JSONAsString) format to keep JSON objects instead of parsing them:
...
And we can use [JSON functions](/core/reference/functions/regular-functions/json-functions) to query saved objects:
...
In cases when we're dealing with [nested JSON objects](/assets/list-nested.json), we can additionally define an explicit schema and use complex types ([`Array`](/core/reference/data-types/array), [`JSON`](/core/guides/clickhouse/data-formats/json/intro) or [`Tuple`](/core/reference/data-types/tuple)) to load data:
...
We can refer to [nested JSON keys](/assets/list-nested.json) by enabling the [following settings option](/core/reference/settings/formats#input_format_import_nested_json):
...
ClickHouse will ignore unknown columns while importing. This can be disabled with the [input_format_skip_unknown_fields](/core/reference/settings/formats#input_format_skip_unknown_fields) settings option:
...
To import BSON data, we use the [BSONEachRow](/core/reference/formats/BSONEachRow) format. Let's import data from [this BSON file](/assets/data.bson):
