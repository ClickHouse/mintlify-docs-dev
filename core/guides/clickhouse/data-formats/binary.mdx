...
The most efficient data format to export and import data between ClickHouse nodes is [Native](/core/reference/formats/Native) format. Exporting is done using `INTO OUTFILE` clause:
...
To import data, we can use [file()](/core/reference/functions/table-functions/file) for smaller files or exploration purposes:
...
Another binary format supported is [RowBinary](/core/reference/formats/RowBinary/RowBinary), which allows importing and exporting data in binary-represented rows:
...
Consider using [RowBinaryWithNames](/core/reference/formats/RowBinary/RowBinaryWithNames), which also adds a header row with a columns list. [RowBinaryWithNamesAndTypes](/core/reference/formats/RowBinary/RowBinaryWithNamesAndTypes) will also add an additional header row with column types.
...
This is the case when the [RawBLOB format](/core/reference/formats/RawBLOB) can be used. This format can be directly used with a single-column table only:
...
ClickHouse supports importing and exporting to [MessagePack](https://msgpack.org/) using the [MsgPack](/core/reference/formats/MsgPack). To export to MessagePack format:
...
To work with [Protocol Buffers](/core/reference/formats/Protobuf/Protobuf) we first need to define a [schema file](/assets/schema.proto):
...
Path to this schema file (`schema.proto` in our case) is set in a `format_schema` settings option for the [Protobuf](/core/reference/formats/Protobuf/Protobuf) format:
...
This saves data to the [proto.bin](/assets/proto.bin) file. ClickHouse also supports importing Protobuf data as well as nested messages. Consider using [ProtobufSingle](/core/reference/formats/Protobuf/ProtobufSingle) to work with a single Protocol Buffer message (length delimiters will be omitted in this case).
...
Now we can import and export using [CapnProto](/core/reference/formats/CapnProto) format and this schema:
...
Note that we had to cast the `Date` column as `UInt32` to [match corresponding types](/core/reference/formats/CapnProto#data_types-matching-capnproto).
