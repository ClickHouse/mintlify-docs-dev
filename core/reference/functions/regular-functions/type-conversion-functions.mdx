...
`to<type>` functions and [cast](#CAST) behave differently in some cases, for example in case of [LowCardinality](/core/reference/data-types/lowcardinality): [cast](#CAST) removes [LowCardinality](/core/reference/data-types/lowcardinality) trait `to<type>` functions don't. The same with [Nullable](/core/reference/data-types/nullable), this behaviour is not compatible with SQL standard, and it can be changed using [cast_keep_nullable](/core/reference/settings/session-settings#cast_keep_nullable) setting.
