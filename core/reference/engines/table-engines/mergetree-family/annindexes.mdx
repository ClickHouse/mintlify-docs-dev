...
The optional GRANULARITY parameter `<N>` refers to the size of the index granules (see [here](/core/concepts/features/performance/skip-indexes/skipping-indexes)).
...
More specifically, an error is returned if `<N>` is bigger than the value of setting [max_limit_for_vector_search_queries](/core/reference/settings/session-settings#max_limit_for_vector_search_queries) with default value 100.
