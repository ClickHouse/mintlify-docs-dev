...
export async function getStaticPaths() {
  ...
  for (const sourceLocale of ACTIVE_LOCALES) {
    ...
    for (const section of sectionsOf(await entriesFor(localeCollectionName(sourceLocale)))) paths.push({ params: { locale, section } });
  }
  return paths;
}

export async function GET({ params }: { params: { locale: string; section: string } }) {
  const entries = (await entriesFor(localeCollectionName(params.locale))).filter((i) => sectionOf(i) === params.section);
