...
Si controlas el pipeline, usa uno de los [clientes oficiales de lenguaje](/es/integrations/language-clients). Se encargan de la serialización, el procesamiento por lotes, TLS, la compresión y el pool de conexiones. Tú pasas primitivas del runtime; el cliente se encarga del formato de transmisión.
...
* **Código de la aplicación:** use los mismos [clientes oficiales de lenguaje](/es/integrations/language-clients) que para la ingestión
* **Herramientas de BI y SQL:** ClickHouse incluye un [driver JDBC v2](/es/integrations/language-clients/java) oficial (Java) y un [driver ODBC](/es/concepts/features/interfaces/odbc). Tableau, Looker, Power BI, Metabase, Apache Superset y Grafana se integran mediante estos drivers o conectores específicos mantenidos por ClickHouse y sus partners
