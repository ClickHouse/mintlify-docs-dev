...
<Image img="/images/cloud/reference/add_marketplace.png" size="md" alt="Interfaz de ClickHouse Cloud que muestra cómo añadir una suscripción de Marketplace a una organización existente" border />
...
<Image img="/images/cloud/reference/beta_dashboards.png" size="lg" alt="Interfaz de ClickHouse Cloud que muestra la nueva función de Dashboards en Beta con visualizaciones" border />
...
<Image img="/images/cloud/reference/api_endpoints.png" size="lg" alt="Interfaz de ClickHouse Cloud que muestra la configuración de API Endpoints con varias opciones" border />
...
<Image img="/images/cloud/reference/cross-vpc-clickpipes.png" size="lg" alt="Diagrama que muestra la arquitectura de acceso a recursos entre VPC para ClickPipes al conectarse a AWS MSK" border />
...
<Image img="/images/cloud/reference/nov-22-dashboard.png" size="lg" alt="Panel de observabilidad avanzada de ClickHouse Cloud que muestra métricas del servidor y uso de recursos" border />
...
<Image img="/images/cloud/reference/nov-8-notifications.png" size="lg" alt="Interfaz del centro de notificaciones de ClickHouse Cloud que muestra opciones de configuración para distintos tipos de notificaciones" border />
...
<Image img="/images/cloud/reference/oct-4-latency-insights.png" size="lg" alt="Interfaz de ClickPipes que muestra un gráfico de métricas de latencia para el rendimiento de la ingestión de datos" border />
...
Nuestra nueva funcionalidad Compute-compute separation le permite crear varios grupos de nodos de compute, cada uno con su propio endpoint, que utilizan la misma carpeta de object storage y, por lo tanto, las mismas tablas, vistas, etc. Obtenga más información sobre [Compute-compute separation aquí](/es/products/cloud/features/infrastructure/warehouses). Si desea acceder a esta funcionalidad en vista previa privada, [contacte con el equipo de soporte](https://clickhouse.com/support/program).

<Image img="/images/cloud/reference/aug-15-compute-compute.png" size="lg" alt="Diagrama que muestra una arquitectura de ejemplo de compute-compute separation con grupos de servicios de lectura y escritura y de solo lectura" border />
...
<Image img="/images/cloud/reference/july-18-table-inspector.png" size="lg" alt="Interfaz del Inspector de tablas de ClickHouse Cloud que muestra información detallada sobre el esquema y el almacenamiento" border />
...
<Image img="/images/cloud/reference/june-28-query-insights.png" size="lg" alt="Interfaz de Query Insights de ClickHouse Cloud que muestra métricas y análisis del rendimiento de las consultas" border />
...
<Image img="/images/cloud/reference/june-28-prometheus.png" size="lg" alt="Panel de Grafana que muestra métricas de Prometheus de ClickHouse Cloud" border />
...
Hasta hace poco, cada vez que configuraba un nuevo [Kafka Connector para ClickPipes](/es/integrations/clickpipes/kafka), este siempre consumía datos desde el inicio del topic de Kafka. En algunos casos, esto podía no ser lo bastante flexible para casos de uso específicos en los que necesita reprocesar datos históricos, supervisar nuevos datos entrantes o reanudar desde un punto concreto.
...
<Image img="/images/cloud/reference/june-13-kafka-config.png" size="lg" alt="Interfaz de configuración del conector Kafka de ClickPipes que muestra opciones de selección de offset" border />
...
<Image img="/images/cloud/reference/june-13-fast-releases.png" size="lg" alt="Página de configuración de ClickHouse Cloud que muestra la opción de inscribirse en lanzamientos rápidos" border />
...
<Image img="/images/cloud/reference/may-30-share-queries.png" size="lg" alt="Editor de consultas de ClickHouse Cloud que muestra la función de compartir con opciones de permisos" border />
...
<div id="set-up-private-link-via-the-cloud-console">
  ### Configurar Private Link a través de la consola de ClickHouse Cloud
</div>
...
<Image img="/images/cloud/reference/may-30-private-endpoints.png" size="lg" alt="Consola de ClickHouse Cloud que muestra la interfaz de configuración de endpoints privados en la configuración de seguridad" border />
...
<Image img="/images/cloud/reference/may-17-kinesis.png" size="lg" alt="Interfaz de ClickPipes que muestra las opciones de configuración de la integración de Amazon Kinesis" border />
...
<Image img="/images/cloud/reference/may-17-query-endpoints.png" size="lg" alt="Interfaz de ClickHouse Cloud que muestra la configuración de Query Endpoints con opciones de formato de salida" border />
...
<Image img="/images/cloud/reference/clickpipes-s3-gcs.png" size="lg" alt="Interfaz de ClickPipes que muestra opciones de configuración para cargar datos desde buckets de S3 y GCS" border />
...
<Image img="/images/cloud/reference/create-tokyo-service.png" size="lg" alt="Interfaz de creación de un servicio de ClickHouse Cloud que muestra la selección de la región de Tokio" border />
...
<Image img="/images/cloud/reference/new-cloud-console.gif" size="lg" alt="Animación que muestra la interfaz de la nueva consola de ClickHouse Cloud con editor SQL integrado y funciones de administración" border />
...
* Cambio incompatible con versiones anteriores visible para el usuario: Anteriormente, dos funciones ([is&#95;deleted](/es/reference/engines/table-engines/mergetree-family/replacingmergetree#is_deleted) y `OPTIMIZE CLEANUP`), en determinadas condiciones, podían provocar la corrupción de los datos en ClickHouse. Para proteger la integridad de los datos de nuestros usuarios sin alterar la esencia de esta funcionalidad, ajustamos su funcionamiento. En concreto, la configuración de MergeTree `clean_deleted_rows` ahora está en desuso y ya no tiene ningún efecto. La palabra clave `CLEANUP` no está permitida de forma predeterminada (para usarla, tendrás que habilitar `allow_experimental_replacing_merge_with_cleanup`). Si decides usar `CLEANUP`, debes asegurarte de que siempre se utilice junto con `FINAL` y debes garantizar que no se inserten filas con versiones anteriores después de ejecutar `OPTIMIZE FINAL CLEANUP`.