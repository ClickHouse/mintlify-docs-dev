...
<Image img="/images/clickstack/performance_guide/trace_filtering.png" size="lg" alt="Filtrado de trazas" />
...
<Image img="/images/clickstack/performance_guide/trace_filtering_v2.png" size="lg" alt="Filtrado de trazas v2" />
...
<Steps>
  ...
  <Step>
    ...
    La instrucción `EXCHANGE` se usa para intercambiar los nombres de las tablas de forma [atómica](/es/concepts/core-concepts/glossary#atomicity). La tabla temporal (ahora la antigua tabla predeterminada) se puede eliminar.
...
<Steps>
  <Step>
    ...
    Crea la nueva tabla con la clave primaria deseada. Observa el sufijo `_23_01_2025`: adáptalo para que sea la fecha actual. Por ejemplo:
  ...
  <Step>
    ...
    El [motor Merge](/es/reference/engines/table-engines/special/merge) (no debe confundirse con MergeTree) no almacena datos por sí mismo, pero permite leer al mismo tiempo de cualquier número de tablas.
  ...
  <Step>
    #### Actualizar HyperDX para leer desde la tabla Merge

    Configura HyperDX para usar `otel_logs_merge` como tabla de la fuente de datos de logs.

    <Image img="/images/clickstack/performance_guide/select_merge_table.png" size="lg" alt="Seleccionar tabla Merge" />

    En este punto, las escrituras continúan en `otel_logs` con la clave primaria original, mientras que las lecturas usan la tabla Merge. No hay ningún cambio visible para los usuarios ni impacto en la ingestión.
  </Step>

  <Step>
    #### Intercambiar las tablas

    Ahora se usa una instrucción `EXCHANGE` para intercambiar atómicamente los nombres de las tablas `otel_logs` y `otel_logs_23_01_2025`.

    ```sql
    EXCHANGE TABLES otel_logs AND otel_logs_23_01_2025
    ```

    Las escrituras ahora van a la nueva tabla `otel_logs` con la clave primaria actualizada. Los datos existentes permanecen en `otel_logs_23_01_2025` y siguen siendo accesibles a través de la tabla Merge. El sufijo indica la fecha en que se aplicó el cambio y representa el timestamp más reciente contenido en esa tabla.
