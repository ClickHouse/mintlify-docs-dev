...
Para definir `interval`, utilice los operadores de [intervalo de tiempo](/es/reference/operators#operators-for-working-with-dates-and-times), por ejemplo:
...
* Durante las fusiones en segundo plano y las [mutaciones](/es/reference/statements/alter#mutations).
...
* `NullCount`
  ...
  Lleva un seguimiento del número de valores `NULL` en columnas `Nullable`. Se usa para estimar con precisión la selectividad de los predicados `IS NULL`/`IS NOT NULL` en la optimización de PREWHERE y permite la poda de partes según la presencia de NULL.
