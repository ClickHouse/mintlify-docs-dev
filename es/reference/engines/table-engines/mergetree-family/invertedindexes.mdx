...
La función [mapContains](/es/reference/functions/regular-functions/tuple-map-functions#mapcontains)(alias de: `mapContainsKey`) busca coincidencias con un único token en las claves de un mapa.
...
El [operador de acceso `operator[]`](/es/reference/operators#access-operators) se puede usar con el índice de texto para filtrar claves y valores.

Ejemplo:

```sql
SELECT count() FROM tab WHERE map['engine'] = 'clickhouse'; -- usará el índice de texto si está definido
```
