...
En este caso, la expresión del preprocesador transforma cada elemento del array por separado.
...
Para definir un preprocesador en un índice de texto creado sobre columnas de tipo [Map](/es/reference/data-types/map), los usuarios deben decidir si el índice está
creado sobre las claves o los valores del mapa.
...
│↳    `k` UInt64,                                                       ↴│
│↳    `s` String,                                                       ↴│
│↳    INDEX idx s TYPE text(tokenizer = ngrams(2)) GRANULARITY 100000000↴│ <-- aquí
│↳)                                                                     ↴│
│↳ENGINE = MergeTree                                                    ↴│
│↳ORDER BY k                                                            ↴│
...
El [operator[]](/es/reference/operators#access-operators) de acceso puede usarse con el índice de texto para filtrar claves y valores. El índice de texto solo se utiliza si se crea sobre las expresiones `mapKeys(map)` o `mapValues(map)`, o sobre ambas.
...
-- Materializar el índice para los datos ya existentes
