...
El motor `Join` permite especificar la configuración [join&#95;use&#95;nulls](/es/reference/settings/session-settings#join_use_nulls) en la sentencia `CREATE TABLE`. La consulta [SELECT](/es/reference/statements/select) debe tener el mismo valor de `join_use_nulls`.
