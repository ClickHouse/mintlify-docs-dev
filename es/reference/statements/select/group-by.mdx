...
* Todas las expresiones de las cláusulas [SELECT](/es/reference/statements/select), [HAVING](/es/reference/statements/select/having) y [ORDER BY](/es/reference/statements/select/order-by) **deben** calcularse a partir de expresiones de clave **o** de [funciones de agregación](/es/reference/functions/aggregate-functions) sobre expresiones que no sean de clave (incluidas las columnas simples). En otras palabras, cada columna seleccionada de la tabla debe usarse o bien en una expresión de clave o dentro de una función de agregación, pero no en ambas.
