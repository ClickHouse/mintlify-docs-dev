...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Nueva configuración para habilitar la expansión de comodines en la función de tabla `url` mediante la enumeración de páginas de índice HTTP."}]}]} />

Permite la expansión experimental de comodines para `url()` y `ENGINE = URL` a partir de páginas de índice HTTP.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Nueva comprobación de seguridad: `ALTER TABLE ... REPLACE PARTITION ... FROM ...` ahora lanza una excepción cuando la tabla de origen no tiene partes en la partición solicitada (corrige la pérdida silenciosa de datos en [#23727](https:\/\/github.com\/ClickHouse\/ClickHouse\/issues\/23727)). El comportamiento anterior, que eliminaba silenciosamente la partición de destino, se conserva configurando `allow_replace_partition_from_empty_source = 1`."}]}]} />

Permite que `ALTER TABLE ... REPLACE PARTITION ... FROM ...` elimine silenciosamente la partición de destino cuando el origen no tiene partes en esa partición.

De forma predeterminada, esto no está permitido: `REPLACE PARTITION` desde un origen que no tiene datos en la partición solicitada lanza una excepción, porque en este caso la operación en la práctica se convierte en un `DROP PARTITION` silencioso en el destino (se eliminan los datos del destino y nada los reemplaza), una causa habitual de pérdida accidental de datos (consulta [#23727](https://github.com/ClickHouse/ClickHouse/issues/23727)).

Habilita esta configuración para restaurar el comportamiento anterior, por ejemplo, cuando uses intencionadamente una partición de origen vacía para borrar datos en el destino. Para una eliminación incondicional, es preferible usar `ALTER TABLE ... DROP PARTITION ...`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.3"},{"label": "0"},{"label": "Prohíbe PRIMARY KEY/ORDER BY sospechosos para MergeTree (es decir, SimpleAggregateFunction)"}]}]} />

Permite `PRIMARY KEY`/`ORDER BY` sospechosos para MergeTree (es decir, SimpleAggregateFunction).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.4"},{"label": "1"},{"label": "Nueva configuración para habilitar\/deshabilitar la inferencia de tipos durante CAST de String a Variant"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Nueva configuración para mostrar las columnas `certificate` y `signature` en `system.distributed_cache_registry`."}]}]} />

Solo tiene efecto en ClickHouse Cloud. Muestra las columnas `certificate` y `signature` de la tabla `system.distributed_cache_registry`. De forma predeterminada, estas columnas están vacías para mantener compacta la salida; habilita esta configuración para inspeccionarlas.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Nueva configuración experimental para indicar cuántos workers sin estado se reservan para una consulta distribuida. Cero deshabilita la reserva."}]}]} />

Cuántos workers sin estado se usarán para ejecutar esta consulta. Cero deshabilita la reserva de workers sin estado para planes distribuidos.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "A partir de la versión 26.7, `EXPLAIN PLAN` usa por defecto `actions=1, compact=1, pretty=1`. Establezca este valor en `legacy` para restaurar la salida anterior a la 26.7."}]}]} />

Formato predeterminado que usa `EXPLAIN PLAN`.

Posibles valores:

* `pretty` (predeterminado desde la versión 26.7) — `actions`, `compact` y `pretty` toman el valor `true` de forma predeterminada, lo que genera un plan compacto, `pretty` y anotado con acciones.
* `legacy` — salida anterior a la 26.7.

Si especifica explícitamente las opciones `actions`, `compact` o `pretty` en la instrucción `EXPLAIN` (por ejemplo, `EXPLAIN actions = 0, compact = 0, pretty = 0 SELECT ...`), este ajuste siempre queda anulado.

`EXPLAIN PLAN` con `json = 1` o `distributed = 1` mantiene los valores predeterminados de `legacy` (anteriores a la 26.7) independientemente de este ajuste, a menos que `actions`, `compact` o `pretty` se establezcan explícitamente. La salida `pretty` no puede representar resultados JSON ni planes distribuidos por segmento, por lo que esos modos solo se muestran correctamente en formato `legacy`.
...
El formateador &#39;%e&#39; de la función &#39;formatDateTime&#39; imprime los días de un solo dígito con un espacio delante, por ejemplo, &#39; 2&#39; en lugar de &#39;2&#39;.
...
El formateador &#39;%M&#39; en las funciones &#39;formatDateTime&#39; y &#39;parseDateTime&#39; imprime/analiza el nombre del mes en lugar de los minutos.
...
<Note>
  `insert_deduplication_token` se rastrea por partición, por lo que varias particiones escritas por una sola inserción pueden llevar el mismo token. Sin un token, la suma de comprobación del contenido predeterminada (`insert_deduplication_version = new_unified_hash`) se calcula sobre todo el bloque insertado, por lo que una inserción se deduplica solo cuando todos sus datos coinciden con una inserción anterior (un reintento), no cuando las filas de una sola partición coinciden por casualidad con una inserción diferente.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Nueva configuración para aplicar `LIMIT` a la agregación en orden y permitir la finalización anticipada cuando `ORDER BY` es un prefijo de la descripción de ordenación de `GROUP BY`."}]}]} />

Cuando está habilitada y la agregación en orden está activa, aplica `LIMIT` en la etapa de agregación para permitir la finalización anticipada tras generar suficientes grupos. Esto reduce la cantidad de datos leídos cuando `ORDER BY` coincide con el prefijo de la clave de `GROUP BY`. Puede reducir el valor informado por `rows_before_limit_at_least`; use `exact_rows_before_limit` si se necesitan recuentos exactos.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "Nueva configuración que limita el trabajo de la optimización `optimize_and_compare_chain` (medido en nodos del árbol de consultas procesados con hash) para que no llegue a dominar el análisis de consultas con cadenas `AND` de comparaciones muy numerosas o muy grandes. El valor anterior `0` (ilimitado) reproduce el comportamiento previo a la versión 26.7, en el que la optimización no tenía límite, por lo que `compatibility` configurado con una versión anterior sigue derivando predicados transitivos sin presupuesto. Establézcalo en `0` para desactivar el presupuesto."}]}]} />

Presupuesto de trabajo para la optimización `optimize_and_compare_chain` durante el análisis de consultas, medido por el número de nodos del árbol de consultas a los que `getTreeHash` calcula un hash (el coste dominante de esta optimización). Cuando una consulta ha procesado con hash más nodos que este valor al aplicar la optimización, deja de aplicarla al resto de la consulta. Esto limita el tiempo de análisis de consultas con cadenas `AND` de comparaciones muy numerosas o muy grandes, en las que, de otro modo, la optimización podría dominar el análisis sin simplificar nada. Detenerse antes siempre es seguro: solo se renuncia a una optimización y nunca se modifican los resultados. Establézcalo en `0` para desactivar el presupuesto (ilimitado).
...
Se usa en la planificación de cargas de trabajo. Cantidad mínima de RAM reservada para ejecutar una consulta en un solo servidor. La reserva se realiza a través de la jerarquía WORKLOAD mediante el valor de la configuración de consulta `workload`.
Si no hay suficiente memoria disponible para la carga de trabajo, se impide que la consulta se inicie y esta permanece en estado pendiente hasta que pueda completarse la reserva.
Un valor de `0` significa que no hay reserva.
Esta configuración surte efecto solo si se crea el recurso MEMORY RESERVATION.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "100000"},{"label": "Nueva configuración para limitar la cantidad de directorios leídos al expandir comodines en la función de tabla `url`."}]}]} />

Cantidad máxima de directorios que se pueden recorrer al expandir comodines de URL desde páginas de índice.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Use la nueva función `toTime` (que convierte valores al tipo de datos `Time`) de forma predeterminada en lugar de la `toTime` heredada (que sigue estando disponible como `toTimeWithFixedDate`)."}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "Nueva configuración. Permite al usuario usar la lógica anterior de la función toTime, que funciona como toTimeWithFixedDate."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Se habilitó globalmente la caché del encabezado del índice de texto."}]}, {"id": "row-2","items": [{"label": "25.11"},{"label": "0"},{"label": "Nueva opción"}]}]} />

Indica si se almacenan en caché en memoria los encabezados deserializados del índice de texto.
...
Si se deben almacenar en caché en memoria las listas de postings deserializadas del índice de texto.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Se habilitó globalmente la caché de tokens del índice de texto."}]}, {"id": "row-2","items": [{"label": "26.3"},{"label": "0"},{"label": "Nueva configuración"}]}]} />

Indica si se debe almacenar en caché en memoria la información deserializada de los tokens del índice de texto.
