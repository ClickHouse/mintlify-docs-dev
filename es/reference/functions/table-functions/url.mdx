...
La función `url` crea una tabla a partir de la `URL` con el `format` y la structure especificados.

La función `url` puede usarse en consultas `SELECT` e `INSERT` sobre datos de tablas [URL](/es/reference/engines/table-engines/special/url).
