...
El proveedor `apache-airflow-providers-clickhousedb` conecta Airflow con ClickHouse, lo que le permite ejecutar consultas, crear tablas y cargar datos como parte de un DAG. Se conecta a través de la [interfaz HTTP](/es/interfaces/http) mediante el client [`clickhouse-connect`](/es/integrations/python), y expone ClickHouse a través del marco SQL común de Airflow, por lo que el `SQLExecuteQueryOperator` estándar gestiona consultas DDL, DML y analíticas sin necesidad de un operador específico de ClickHouse.
...
Para [ClickHouse Cloud](/es/cloud/overview) o cualquier clúster autohospedado con TLS habilitado, establezca `secure` en `true` en el campo **Extra** y use el puerto TLS (`8443`).
...
| `client_name`          | Nombre del cliente                      | (vacío)        | Una etiqueta que se añade al identificador de versión de Airflow en el `User-Agent` de ClickHouse y en la columna `client_name` de [`system.query_log`](/es/operations/system-tables/query_log). |
| `session_settings`     | Configuración de sesión (JSON)          | (vacío)        | [Configuración de sesión de ClickHouse](/es/operations/settings/settings) aplicada a cada consulta de la conexión; por ejemplo, `{"max_execution_time": 300, "max_threads": 8}`.                 |
| `client_kwargs`        | `kwargs` del cliente (JSON)             | (vacío)        | Argumentos de palabra clave adicionales que se redirigen a `clickhouse_connect.get_client()`, por ejemplo `http_proxy`.                                                                       |
...
Pasa la [configuración de sesión](/es/operations/settings/settings) al crear el hook, ya sea directamente o mediante el `hook_params` del operador. La configuración que se pasa al constructor se superpone a cualquier `session_settings` definido en el campo **Extra** de la conexión, y los valores del constructor prevalecen cuando hay conflicto entre claves:
...
* [`clickhouse-connect` client para Python](/es/integrations/python)
* [interfaz HTTP de ClickHouse](/es/interfaces/http)
* [referencia de la configuración de sesión de ClickHouse](/es/operations/settings/settings)
