...
1. Supongamos que tenemos un archivo TSV llamado `comments.tsv` que contiene algunos comentarios de Hacker News y que la fila de encabezado contiene nombres de columnas. Debes especificar un [formato de entrada](/es/reference/formats) al insertar los datos, que en nuestro caso es `TabSeparatedWithNames`:
...
Consulta la [página de documentación de `clickhouse-client`](/es/concepts/features/interfaces/cli) para obtener más información sobre cómo instalar `clickhouse-client` en tu sistema operativo local.