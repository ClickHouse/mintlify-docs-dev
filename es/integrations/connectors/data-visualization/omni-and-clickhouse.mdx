---
sidebarTitle: 'Omni'
slug: /integrations/omni
keywords: ['clickhouse', 'Omni', 'conectar', 'integrar', 'UI']
description: 'Omni es una plataforma empresarial para BI, aplicaciones de datos y analítica integrada que permite explorar y compartir información en tiempo real.'
title: 'Omni'
doc_type: 'guide'
integration:
  - support_level: 'partner'
  - category: 'data_visualization'
---

import ConnectionDetails from '/snippets/es/_gather_your_details_http.mdx';
import { Image } from "/snippets/components/Image.jsx";

import PartnerBadge from "/snippets/components/PartnerBadge/PartnerBadge.jsx";

<PartnerBadge />

Omni puede conectarse a ClickHouse Cloud o a una implementación local mediante la fuente de datos oficial de ClickHouse.

<div id="1-gather-your-connection-details">
  ## 1. Reúne los datos de conexión
</div>

<ConnectionDetails />

<div id="2-create-a-clickhouse-data-source">
  ## 2. Crear una fuente de datos de ClickHouse
</div>

Ve a Admin -&gt; Connections y haz clic en el botón &quot;Add Connection&quot;, en la esquina superior derecha.

<Image size="lg" img="/images/integrations/data-visualization/omni_01.png" alt="Interfaz de administración de Omni que muestra el botón Add Connection en la sección Connections" border />

<br />

Selecciona `ClickHouse`. Introduce tus credenciales en el formulario.

<Image size="lg" img="/images/integrations/data-visualization/omni_02.png" alt="Interfaz de configuración de la conexión de Omni para ClickHouse que muestra los campos del formulario de credenciales" border />

<br />

Ahora ya deberías poder consultar y visualizar datos de ClickHouse en Omni.