---
title: 'Despliegue de infraestructura adicional'
slug: /cloud/reference/byoc/onboarding/new_region
sidebarTitle: 'Infraestructura adicional'
keywords: ['BYOC', 'nube', 'trae tu propia nube', 'configuración inicial', 'infraestructura adicional', 'multirregión', 'multicuenta']
description: 'Despliegue infraestructura adicional de BYOC en nuevas regiones o cuentas'
doc_type: 'referencia'
---

import { Image } from "/snippets/components/Image.jsx";

Después de completar la configuración inicial, es posible que desee implementar infraestructura BYOC adicional en una Región diferente o en otra cuenta de AWS o proyecto de GCP.

Para agregar una nueva implementación de BYOC:

1. Vaya a la página &quot;Infrastructure&quot; de su organización en la consola de ClickHouse Cloud.

<Image img="/images/cloud/reference/byoc-new-infra-1.png" size="lg" alt="Página de infraestructura de BYOC" />

2. Seleccione &quot;Add new account&quot; o &quot;Add new infrastructure&quot; y siga el asistente guiado para completar el proceso de configuración.

<Image img="/images/cloud/reference/byoc-new-infra-2.png" size="lg" alt="Página de infraestructura de BYOC" />