...
1. **Registro de direcciones tailnet**: Cada endpoint registra una dirección tailnet única (p. ej., `k8s.xxxx.us-east-1.aws.byoc.clickhouse-prd.com` para el servidor de API de Kubernetes)
...
3. **Pod de Kubernetes de Nginx**: Un pod de Kubernetes de Nginx que:
...
Los ingenieros de ClickHouse Cloud requieren acceso para solucionar problemas a través de Tailscale. Se les proporciona autenticación basada en certificados just-in-time para implementaciones BYOC. Consulta [acceso a los datos de ClickHouse](/es/products/bring-your-own-cloud/reference/clickhouse-data-access) para conocer la política de acceso completa.
