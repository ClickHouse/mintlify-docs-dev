...
<Note>
  Google Cloud Storage (GCS) también es compatible mediante el tipo `s3`. Consulte [GCS backed MergeTree](/es/integrations/connectors/data-ingestion/GCP).
</Note>
...
No se admiten las [mutaciones](/es/reference/statements/alter#mutations) ni la replicación de tablas.
...
[`ALTER TABLE`](/es/reference/statements/alter), [`RENAME TABLE`](/es/reference/statements/rename#rename-table),
