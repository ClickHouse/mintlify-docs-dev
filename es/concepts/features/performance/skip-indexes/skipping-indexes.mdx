...
Si, al ejecutar una consulta y leer los archivos de columnas pertinentes, alguna parte de la condición de filtrado de la cláusula WHERE coincide con la expresión del índice de omisión, ClickHouse usará los datos del archivo de índice para determinar si cada bloque de datos relevante debe procesarse o puede omitirse (suponiendo que el bloque no haya sido excluido previamente al aplicar la clave primaria). Para usar un ejemplo muy simplificado, considere la siguiente tabla cargada con datos predecibles.
...
Ahora agrega un índice de omisión muy básico:

```sql
ALTER TABLE skip_table ADD INDEX vix my_value TYPE set(100) GRANULARITY 2;
```

Normalmente, los índices de omisión solo se aplican a los datos recién insertados, por lo que añadir el índice no afectará a la consulta anterior.
...
<Image img="/images/guides/best-practices/simple_skip.png" size="md" alt="Omisión simple" />

Puede acceder a información detallada sobre el uso del índice de omisión habilitando la traza al ejecutar consultas.  Desde
...
anterior, el registro de depuración muestra que el índice de omisión descartó todos los gránulos salvo dos:
...
<Image img="/images/guides/best-practices/bad_skip.png" size="md" alt="Omisión deficiente" />
...
* [Modificación de índices de omisión de datos](/es/reference/statements/alter/skipping-index)
