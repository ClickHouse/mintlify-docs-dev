...
También podemos usar el modificador [`APPLY`](/es/reference/statements/select) para aplicar funciones a todas las columnas.
...
También podemos excluir un campo con el modificador [`EXCEPT`](/es/reference/statements/select). Por ejemplo, para quitar la columna `tolls_amount`, escribiríamos la siguiente consulta:
