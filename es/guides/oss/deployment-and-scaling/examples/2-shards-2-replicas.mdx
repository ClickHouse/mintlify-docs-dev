...
<Image img="/images/deployment-guides/replication-sharding-examples/both.png" size="md" alt="Diagrama de arquitectura para 2 segmentos y 1 réplica" />
...
<Steps>
  ...
  <Step>
    ...
    ```xml
    <remote_servers>
       <!-- nombre del clúster (no debe contener puntos) -->
      <cluster_2S_2R>
          <!-- <allow_distributed_ddl_queries>false</allow_distributed_ddl_queries> -->
          <shard>
              <!-- Opcional. Indica si se deben escribir datos en una sola réplica. Valor predeterminado: false (escribir datos en todas las réplicas). -->
              <internal_replication>true</internal_replication>
              <replica>
                  <host>clickhouse-01</host>
                  <port>9000</port>
...
La ventaja de esta topología de clúster con 2 segmentos y 2 réplicas es que proporciona tanto escalabilidad como tolerancia a fallos.
Los datos se distribuyen entre hosts independientes, lo que reduce los requisitos de almacenamiento y de E/S por nodo, mientras que las consultas se procesan en paralelo en ambos segmentos para mejorar el rendimiento y la eficiencia de memoria.
Lo más importante es que el clúster puede tolerar la pérdida de un nodo y seguir respondiendo consultas sin interrupciones, ya que cada segmento dispone de una réplica de respaldo en otro nodo.
...
Además, aunque el clúster puede soportar el fallo de un único nodo, la pérdida simultánea de dos nodos puede dejarlo inoperativo, según qué nodos fallen y cómo se distribuyan los segmentos.
Esta topología ofrece un equilibrio entre disponibilidad y coste, por lo que resulta adecuada para entornos de producción en los que se requiere cierto grado de tolerancia a fallos sin asumir el coste de factores de replicación más altos.

Para saber cómo ClickHouse Cloud procesa las consultas y ofrece tanto escalabilidad como tolerancia a fallos, consulte la sección [&quot;Réplicas paralelas&quot;](/es/products/cloud/features/infrastructure/parallel-replicas).