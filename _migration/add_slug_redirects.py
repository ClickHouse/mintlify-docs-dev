...
Outputs:
  _site/redirects.json       — merged deployable output (docs.json $ref target)
  _site/redirects-ru.json    — Russian locale source
  _site/redirects-jp.json    — Japanese locale source (jp -> ja code rename)
  _site/redirects-ko.json    — Korean locale source
  _site/redirects-zh.json    — Chinese locale source

The locale files are the per-locale source of truth; redirects.json is
regenerated by merging them with the English entries on each run.
...
def main() -> None:
    ...
    # --- Merged output for docs.json ---
    all_locale = [e for _, _, entries in locale_files for e in entries]
    merged = all_en + all_locale
    merged.sort(key=lambda r: r["source"])
    merged = [{"source": r["source"], "destination": r["destination"]} for r in merged]
    write_json(REDIRECTS_EN, merged)
