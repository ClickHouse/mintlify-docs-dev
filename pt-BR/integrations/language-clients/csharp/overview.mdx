...
description: 'O cliente oficial em C# para se conectar ao ClickHouse.'
title: 'Driver C# do ClickHouse'
doc_type: 'guide'
integration:
  - support_level: 'core'
...
Formatos comuns: `JSONEachRow`, `CSV`, `TSV`, `Parquet`, `Native`. Consulte a [documentação sobre formatos](/pt-BR/reference/formats) para ver todas as opções.
...
Use `InsertRawStreamAsync` para inserir dados diretamente de streams de arquivo ou de memória em formatos como CSV, JSON, Parquet ou qualquer [formato compatível do ClickHouse](/pt-BR/reference/formats).
