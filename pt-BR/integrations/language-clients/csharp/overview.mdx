...
| ReadBufferSize          | `int`                    | `8192` (8 KiB) | `ReadBufferSize`           | Tamanho em bytes do buffer usado para ler respostas HTTP de consultas. Aumente-o para reduzir o reabastecimento do buffer em grandes result sets. Valores iguais ou superiores a 85.000 bytes são alocados no Large Object Heap, o que pode causar fragmentação do LOH e pausas mais longas do GC. |
...
| AcceptEncoding        | `string`                      | Substituição do cabeçalho `Accept-Encoding` por consulta (por exemplo, `"zstd"`, `"zstd, gzip;q=0.5"`); também força `enable_http_compression=1` na URL. Consulte [Compactação de transporte por consulta](#per-query-accept-encoding). |
...
<Image img="/images/_snippets/cloud-connect-button.png" size="md" alt="Botão Connect do serviço do ClickHouse Cloud" border />

Escolha **C#**. Os detalhes da conexão são exibidos abaixo.

<Image img="/images/_snippets/connection-details-csharp.png" size="md" alt="Detalhes da conexão do ClickHouse Cloud para C#" border />
...
Não há conversões automáticas, e uma incompatibilidade de tipo lança `InvalidOperationException`.

<div id="poco-read-mapto">
  ##### Materializando uma única linha
</div>

Ao iterar manualmente sobre um reader, use `ClickHouseDataReader.MapTo<T>()` para materializar a linha atual em um POCO registrado sem avançar o reader:
...
Por padrão, o cliente negocia `gzip, deflate` quando `Compression=true` (o padrão da string de conexão), e o cliente HTTP descomprime o fluxo de forma transparente.
...
O `HttpClient` padrão que o driver cria tem `AutomaticDecompression = GZip | Deflate`, o que descomprime esses algoritmos de forma transparente e remove `Content-Encoding` da resposta. Isso é o ideal para consultas normais, mas não se você quiser processar por conta própria os dados brutos e comprimidos.

Nesse caso, passe um `HttpClient` ou `HttpClientFactory` para o ClickHouseClient com `AutomaticDecompression = DecompressionMethods.None`

<Warning>
  Se `AcceptEncoding` solicitar um codec que o `HttpClient` configurado não consegue descomprimir automaticamente, somente `ExecuteRawResultAsync` é seguro. Chamar `ExecuteReaderAsync`, `ExecuteScalarAsync` ou `ExecuteNonQueryAsync` nesse mesmo fluxo de consulta fará com que o sistema tente analisar os bytes comprimidos como se fossem o formato normal do resultado, produzindo dados sem sentido.
</Warning>
...
Quando o servidor responde com um 4xx/5xx e `enable_http_compression=1` foi definido, ele compacta o corpo do erro com o mesmo codec que usaria em uma resposta bem-sucedida. O driver descompacta esses corpos para os codecs incluídos na BCL (`gzip`, `deflate`, `br`/`brotli`), para que a mensagem exibida em `ClickHouseServerException` seja legível. Para codecs que ele não consegue decodificar (`zstd`, `lz4`, …), o driver retorna uma mensagem substituta que informa o codec e aponta para `system.query_log`, onde está o texto original do erro.
