...
1. Suponha que temos um arquivo TSV chamado `comments.tsv` que contém alguns comentários do Hacker News, e que a linha de cabeçalho contém os nomes das colunas. Você precisa especificar um [formato de entrada](/pt-BR/reference/formats) ao inserir os dados, que, no nosso caso, é `TabSeparatedWithNames`:
...
Visite a [página da documentação sobre o `clickhouse-client`](/pt-BR/concepts/features/interfaces/cli) para saber como instalar o `clickhouse-client` no seu sistema operacional local.