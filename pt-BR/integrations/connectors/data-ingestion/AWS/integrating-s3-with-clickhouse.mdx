...
* path — URL do bucket com o path para o arquivo. Isso oferece suporte aos seguintes curingas no modo somente leitura: `*`, `?`, `{abc,def}` e `{N..M}`, em que `N`, `M` são números e `'abc'`, `'def'` são strings. Para mais informações, consulte a documentação sobre [uso de curingas no path](/pt-BR/reference/engines/table-engines/integrations/s3#wildcards-in-path).
...
<Image img="/images/integrations/data-ingestion/s3/s3-j.png" size="lg" border alt="Visualização do bucket do S3 no console da AWS mostrando arquivos de dados do ClickHouse armazenados no S3" />
...
<Image img="/images/integrations/data-ingestion/s3/bucket1.png" size="lg" border alt="Tamanho dos dados no primeiro bucket do S3 mostrando métricas de uso de armazenamento" />

<Image img="/images/integrations/data-ingestion/s3/bucket2.png" size="lg" border alt="Tamanho dos dados no segundo bucket do S3 mostrando métricas de uso de armazenamento" />
