...
<Steps>
  <Step>
    ### Reúna os dados de conexão do ClickHouse

    <ConnectionDetails />
  </Step>

  <Step>
    ### Pré-requisitos do Apify
  ...
  <Step>
    ...
    <Note>
      O Apify também fornece um [Python client](https://docs.apify.com/api/client/python). Se você preferir Python, instale o `apify-client` via pip e use o [clickhouse-connect](/pt-BR/integrations/language-clients/python) com o ClickHouse.
    </Note>
  </Step>

  <Step>
    ### Crie uma tabela de destino no ClickHouse
  ...
  <Step>
    ### Busque o dataset do Apify e carregue-o no ClickHouse
...
Para inserções com alta taxa de transferência a partir do cliente JavaScript, siga as [dicas de otimização de desempenho](/pt-BR/integrations/language-clients/js#tips-for-performance-optimizations). Agrupe várias linhas em inserções maiores, em vez de inserir uma linha de cada vez, e considere usar [async inserts](/pt-BR/concepts/features/operations/insert/asyncinserts) quando o agrupamento no lado do cliente não for prático.

<div id="security">
  ### Segurança
</div>

Os exemplos desta página usam o usuário e o banco de dados `default` para simplificar. Em produção, crie um usuário dedicado com os privilégios mínimos necessários para inserir dados na tabela de destino e armazene as credenciais de forma segura (por exemplo, em variáveis de ambiente ou em um gerenciador de segredos, em vez de fazer commit delas no código-fonte). Consulte [Cloud access management](/pt-BR/products/cloud/guides/security/cloud-access-management) para obter orientações.
...
* [Cliente JavaScript da Apify](https://docs.apify.com/api/client/js)
...
* [Cliente JavaScript do ClickHouse](/pt-BR/integrations/language-clients/js)