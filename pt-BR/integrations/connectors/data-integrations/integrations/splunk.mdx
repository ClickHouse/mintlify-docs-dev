---
sidebarTitle: 'Splunk'
slug: /integrations/audit-splunk
keywords: ['clickhouse', 'Splunk', 'auditoria', 'cloud']
description: 'Armazene os logs de auditoria do ClickHouse Cloud no Splunk.'
title: 'Armazenando os logs de auditoria do ClickHouse Cloud no Splunk'
doc_type: 'guide'
---

import { Image } from "/snippets/components/Image.jsx";

import PartnerBadge from "/snippets/components/PartnerBadge/PartnerBadge.jsx";

<PartnerBadge />

[Splunk](https://www.splunk.com/) é uma plataforma de análise de dados e monitoramento.

Este add-on permite armazenar os [logs de auditoria do ClickHouse Cloud](/pt-BR/products/cloud/reference/security/audit-logging) no Splunk. Ele usa a [ClickHouse Cloud API](/pt-BR/products/cloud/features/admin-features/api/api-overview) para baixar os logs de auditoria.

Este add-on contém apenas uma entrada modular e não inclui nenhuma UI adicional.

<div id="installation">
  # Instalação
</div>

<div id="for-splunk-enterprise">
  ## Para o Splunk Enterprise
</div>

Baixe o ClickHouse Cloud Audit Add-on for Splunk no [Splunkbase](https://splunkbase.splunk.com/app/7709).

<Image img="/images/integrations/tools/data-integration/splunk/splunk_001.png" size="lg" alt="Site do Splunkbase mostrando a página de download do ClickHouse Cloud Audit Add-on for Splunk" border />

No Splunk Enterprise, navegue até Apps -&gt; Manage. Em seguida, clique em Install app from file.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_002.png" size="lg" alt="Interface do Splunk Enterprise mostrando a página de gerenciamento de Apps com a opção Install app from file" border />

Selecione o arquivo compactado baixado do Splunkbase e clique em Upload.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_003.png" size="lg" alt="Janela de instalação de aplicativo do Splunk para enviar o add-on do ClickHouse" border />

Se tudo correr bem, agora você verá o aplicativo ClickHouse Audit logs instalado. Caso contrário, consulte os logs do Splunkd para identificar possíveis erros.

<div id="modular-input-configuration">
  # Configuração da entrada modular
</div>

Para configurar a entrada modular, primeiro você precisará de informações da sua implantação no ClickHouse Cloud:

* O ID da organização
* Uma [API Key](/pt-BR/products/cloud/features/admin-features/api/openapi) de Admin

<div id="getting-information-from-clickhouse-cloud">
  ## Obtendo informações do ClickHouse Cloud
</div>

Faça login no [console do ClickHouse Cloud](https://console.clickhouse.cloud/).

Navegue até Organization -&gt; Organization details. Lá, você pode copiar o Organization ID.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_004.png" size="lg" alt="Console do ClickHouse Cloud mostrando a página Organization details com o Organization ID" border />

Em seguida, no menu à esquerda, acesse API Keys.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_005.png" size="lg" alt="Console do ClickHouse Cloud mostrando a seção API Keys no menu de navegação à esquerda" border />

Crie uma API Key, dê um nome significativo e selecione os privilégios de `Admin`. Clique em Generate API Key.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_006.png" size="lg" alt="Console do ClickHouse Cloud mostrando a interface de criação de API Key com privilégios de Admin selecionados" border />

Salve a API Key e o Secret em um local seguro.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_007.png" size="lg" alt="Console do ClickHouse Cloud mostrando a API Key gerada e o Secret para ser salvo" border />

<div id="configure-data-input-in-splunk">
  ## Configure a entrada de dados no Splunk
</div>

No Splunk, volte para Settings -&gt; Data inputs.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_008.png" size="lg" alt="Interface do Splunk mostrando o menu Settings com a opção Data inputs" border />

Selecione a entrada de dados ClickHouse Cloud Audit Logs.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_009.png" size="lg" alt="Página Data inputs do Splunk mostrando a opção ClickHouse Cloud Audit Logs" border />

Clique em &quot;New&quot; para configurar uma nova instância da entrada de dados.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_010.png" size="lg" alt="Interface do Splunk para configurar uma nova entrada de dados ClickHouse Cloud Audit Logs" border />

Depois de inserir todas as informações, clique em Next.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_011.png" size="lg" alt="Página de configuração do Splunk com as configurações da entrada de dados do ClickHouse concluídas" border />

A entrada de dados está configurada, e você já pode começar a consultar os logs de auditoria.

<div id="usage">
  # Uso
</div>

A entrada modular armazena dados no Splunk. Para visualizar esses dados, você pode usar a visualização de pesquisa geral do Splunk.

<Image img="/images/integrations/tools/data-integration/splunk/splunk_012.png" size="lg" alt="Interface de pesquisa do Splunk mostrando dados de logs de auditoria do ClickHouse" border />