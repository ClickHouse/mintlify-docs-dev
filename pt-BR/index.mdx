...
export const HeroCard = ({ filename, darkFilename: darkFilenameProp, title, description, body, href, links, icon, showArrow, arrowBottom, extraStyle, logo, logoInvert }) => {
    const darkFilename = darkFilenameProp || (filename ? filename.replace('-light.', '-dark.') : null);
    const cardClassName = `ch-hero-card relative ${arrowBottom ? 'flex flex-col' : 'block'} p-4 rounded-lg no-underline bg-[#f9f9f9] border border-[#e5e7eb] text-inherit`;
    ...
    return (
        <Tabs>
            <Tab title="Claude Code" icon="/images/logo-claudecode-color.svg">
                <p className="mb-2 text-[#9ca3af] text-[13px]">Execute no seu terminal:</p>
            ...
            <Tab title="Claude Desktop" icon="/images/logo-claude.svg">
                <p className="mb-2 text-[#9ca3af] text-[13px]">Adicione ao <code>claude_desktop_config.json</code>:</p>
                <CodeBlock language="json">{jsonConfig}</CodeBlock>
            </Tab>
            <Tab title="Cursor" icon="/images/logo-cursor.png">
                <p className="mb-2 text-[#9ca3af] text-[13px]">Adicione ao <code>.cursor/mcp.json</code>:</p>
                <CodeBlock language="json">{jsonConfig}</CodeBlock>
            </Tab>
            <Tab title="Windsurf" icon="/images/logo-windsurf.svg">
                <p className="mb-2 text-[#9ca3af] text-[13px]">Adicione ao <code>~/.codeium/windsurf/mcp_config.json</code>:</p>
                <CodeBlock language="json">{jsonConfig}</CodeBlock>
            </Tab>
        </Tabs>
    );
  ...
  <div className="relative z-10 px-0 py-16 lg:py-12 lg:pb-40 max-w-6xl mx-auto">
    ...
    <div className="space-y-4 mt-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <HeroCard filename="thumbnail-chdb-light.png" title="chDB" description="Um banco de dados in-process rápido, confiável e escalável. Experimente o poder do ClickHouse in-process." href="/pt-BR/products/chdb" showArrow />
      </div>
    </div>
