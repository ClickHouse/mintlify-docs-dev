...
* Um serviço do ClickHouse Cloud em execução com o [ClickStack ativado](/pt-BR/use-cases/observability/clickstack/deployment/clickstack-clickhouse-cloud)
* [MCP ativado](/pt-BR/use-cases/AI/MCP/remote_mcp#enable-remote-mcp-server) no serviço — abra o console do Cloud, clique em **Connect**, selecione **Connect with MCP** e ative-o
...
Cada cliente gerencia automaticamente o fluxo OAuth ao se conectar pela primeira vez.
...
<Tabs>
  <Tab title="Claude Code">
    ...
    Abra o Claude Code e execute `/mcp`; depois, selecione `clickstack-cloud` para concluir o fluxo OAuth.
  </Tab>
...
<Accordion title="O MCP está habilitado, mas o cliente MCP não consegue se conectar">
  ...
  * Verifique se o [MCP está habilitado](/pt-BR/use-cases/AI/MCP/remote_mcp#enable-remote-mcp-server) no serviço no console do Cloud.
</Accordion>

<Accordion title="As solicitações vão para o serviço errado">
  Sem o header `x-service-id`, as solicitações são direcionadas por padrão para o primeiro serviço do ClickStack provisionado e usado pela sua conta. Envie o header para direcioná-las a um serviço específico. Consulte [Direcionar para um serviço específico](#managed-service-override).
</Accordion>
