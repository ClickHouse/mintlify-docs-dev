...
A tabela abaixo mostra todos os tipos de dados compatíveis com o formato Apache Avro e seus correspondentes tipos de dados do ClickHouse [tipos de dados](/pt-BR/reference/data-types) em consultas `INSERT` e `SELECT`.
...
* `bytes` é o padrão e é controlado pela configuração [`output_format_avro_string_column_pattern`](/pt-BR/reference/settings/formats#output_format_avro_string_column_pattern)

**  O [tipo Variant](/pt-BR/reference/data-types/variant) aceita implicitamente `null` como valor de um campo; assim, por exemplo, o `union(T1, T2, null)` do Avro será convertido em `Variant(T1, T2)`.
Como resultado, ao gerar Avro a partir do ClickHouse, precisamos sempre incluir o tipo `null` no conjunto de tipos `union` do Avro, pois não sabemos se algum valor é de fato `null` durante a inferência de esquema.
