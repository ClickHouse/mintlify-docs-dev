---
alias: []
description: 'Documentação do formato JSONCompactStringsEachRowWithProgress'
input_format: true
...
Semelhante a [`JSONCompactEachRowWithProgress`](/pt-BR/reference/formats/JSON/JSONCompactEachRowWithProgress), mas todos os valores são convertidos para strings.
Isso é útil quando você precisa de uma representação textual consistente para todos os tipos de dados.

Principais recursos:

* Mesma estrutura de `JSONCompactEachRowWithProgress`
* Todos os valores são representados como strings (números, arrays etc. ficam todos entre aspas)
* Inclui atualizações de progresso, totais e tratamento de exceções
* Útil para clientes que preferem ou exigem dados em formato de string
