...
O primeiro argumento é uma coleção nomeada que especifica o provedor, o modelo, o endpoint e, opcionalmente, uma chave de API.
...
* `collection` — Nome de uma coleção nomeada que contém as credenciais do provedor e a configuração. [`String`](/pt-BR/reference/data-types/string)
