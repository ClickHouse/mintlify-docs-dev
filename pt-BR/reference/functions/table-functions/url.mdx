...
A função `url` cria uma tabela a partir da `URL` com o `format` e a `structure` fornecidos.

A função `url` pode ser usada em consultas `SELECT` e `INSERT` em dados de tabelas [URL](/pt-BR/reference/engines/table-engines/special/url).
