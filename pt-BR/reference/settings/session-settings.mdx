...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Nova configuração para habilitar a expansão de curingas na função de tabela `url` por meio da listagem de páginas de índice HTTP."}]}]} />

Permite a expansão experimental de curingas para `url()` e `ENGINE = URL` a partir de páginas de índice HTTP.
...
Permite argumentos de fuso horário não constantes em determinadas funções relacionadas a tempo, como toTimeZone(), fromUnixTimestamp*() e snowflakeIDToDateTime*().
...
Por padrão, isso não é permitido: `REPLACE PARTITION` a partir de uma origem sem dados na partição solicitada lança uma exceção, porque, nesse caso, a operação efetivamente se torna um `DROP PARTITION` silencioso no destino (os dados do destino são removidos e nada os substitui), uma causa comum de perda acidental de dados (veja [#23727](https://github.com/ClickHouse/ClickHouse/issues/23727)).

Ative essa configuração para restaurar o comportamento anterior, por exemplo, quando você usa intencionalmente uma partição de origem vazia para limpar os dados no destino. Para uma remoção incondicional, prefira `ALTER TABLE ... DROP PARTITION ...`.
...
Tem efeito apenas no ClickHouse Cloud. Exibe as colunas `certificate` e `signature` na tabela `system.distributed_cache_registry`. Por padrão, essas colunas ficam vazias para manter a saída compacta; habilite essa configuração para inspecioná-las.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Nova configuração experimental para definir quantos workers sem estado devem ser alocados para uma consulta distribuída. Zero desabilita a alocação."}]}]} />

Quantos workers sem estado serão usados para executar esta consulta. Zero desabilita a alocação de workers sem estado para planos distribuídos.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "A partir da versão 26.7, `EXPLAIN PLAN` usa por padrão `actions=1, compact=1, pretty=1`. Defina esse valor como `legacy` para restaurar a saída anterior à 26.7."}]}]} />

Formato padrão usado por `EXPLAIN PLAN`.

Valores possíveis:

* `pretty` (padrão desde a 26.7) — `actions`, `compact` e `pretty` assumem `true` por padrão, produzindo um plano compacto, bem formatado e anotado com ações.
...
`EXPLAIN PLAN` com `json = 1` ou `distributed = 1` mantém os padrões legados (anteriores à 26.7), independentemente dessa configuração, a menos que `actions`, `compact` ou `pretty` sejam definidos explicitamente. A saída `pretty` não pode representar resultados JSON nem planos distribuídos por shard, portanto esses modos só são exibidos corretamente na forma legada.
...
O formatador &#39;%f&#39; na função &#39;formatDateTime&#39; imprime um único zero em vez de seis zeros se o valor formatado não tiver frações de segundo.
...
O formatador &#39;%M&#39; nas funções &#39;formatDateTime&#39; e &#39;parseDateTime&#39; exibe/interpreta o nome do mês em vez dos minutos.
...
<Note>
  `insert_deduplication_token` é rastreado por partição, portanto várias partições gravadas por um único insert podem carregar o mesmo token. Sem um token, o checksum de conteúdo padrão (`insert_deduplication_version = new_unified_hash`) é calculado sobre todo o bloco inserido, de modo que um insert é desduplicado apenas quando todos os seus dados correspondem aos de um insert anterior (uma nova tentativa), e não quando as linhas de uma única partição por acaso coincidem com as de um insert diferente.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.4"},{"label": "0"},{"label": "Adicionada uma configuração separada para controlar o número de threads na análise em paralelo de arquivos"}]}]} />

O número máximo de threads para fazer a análise de dados em formatos de entrada que suportam análise em paralelo. Por padrão, ele é determinado automaticamente.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Nova configuração para aplicar `LIMIT` à agregação em ordem, permitindo encerramento antecipado quando `ORDER BY` for um prefixo da descrição de ordenação de `GROUP BY`."}]}]} />

Quando habilitada e com a agregação em ordem ativa, aplica `LIMIT` à etapa de agregação para permitir o encerramento antecipado após produzir grupos suficientes. Isso reduz a quantidade de dados lidos quando `ORDER BY` corresponde ao prefixo da chave de `GROUP BY`. Pode reduzir o valor informado por `rows_before_limit_at_least`; use `exact_rows_before_limit` se forem necessárias contagens exatas.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "Nova configuração que limita o trabalho da otimização `optimize_and_compare_chain` (medido em nós da árvore de consulta com hash calculado), para que ela não passe a dominar a análise de consultas com cadeias `AND` de comparações muito numerosas ou muito grandes. O valor anterior `0` (ilimitado) reproduz o comportamento anterior à versão 26.7, em que a otimização não tinha limite; assim, `compatibility` definido para uma versão anterior continua derivando predicados transitivos sem orçamento. Defina como `0` para desabilitar o orçamento."}]}]} />

Orçamento de trabalho para a otimização `optimize_and_compare_chain` durante a análise da consulta, medido pelo número de nós da árvore de consulta cujo hash é calculado por `getTreeHash` (o custo dominante dessa otimização). Quando uma consulta tiver calculado o hash de mais nós do que esse limite ao aplicar a otimização, ela deixa de aplicá-la no restante da consulta. Isso limita o tempo de análise de consultas com cadeias `AND` de comparações muito numerosas ou muito grandes, nas quais a otimização poderia, de outra forma, dominar a análise sem consolidar nada. Interromper isso antecipadamente é sempre seguro: apenas abre mão de uma otimização e nunca altera os resultados. Defina como `0` para desabilitar o orçamento (ilimitado).
...
Usado no agendamento de workloads. A quantidade mínima de RAM reservada para a execução de uma consulta em um único servidor. A reserva é feita por meio da hierarquia de WORKLOAD usando o valor da configuração de consulta `workload`.
Se não houver memória suficiente disponível para o workload, a consulta é impedida de iniciar e fica em estado pendente até que a reserva possa ser atendida.
Um valor de `0` significa que não há reserva.
Essa configuração só tem efeito se o recurso MEMORY RESERVATION for criado.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "100000"},{"label": "Nova configuração para limitar o número de diretórios percorridos ao expandir curingas na função de tabela `url`."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Use a nova função `toTime` (convertendo valores para o tipo de dado `Time`) por padrão em vez da `toTime` legada (que ainda está disponível como `toTimeWithFixedDate`)."}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "Nova configuração. Permite ao usuário usar a lógica antiga da função toTime, que funciona como toTimeWithFixedDate."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "O text index agora está em GA"}]}, {"id": "row-2","items": [{"label": "26.1"},{"label": "0"},{"label": "Habilitado por padrão"}]}, {"id": "row-3","items": [{"label": "25.9"},{"label": "0"},{"label": "Nova configuração"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Habilitou globalmente o cache do cabeçalho do índice de texto."}]}, {"id": "row-2","items": [{"label": "25.11"},{"label": "0"},{"label": "Nova configuração"}]}]} />

Define se os cabeçalhos desserializados do índice de texto devem ser armazenados em cache na memória.
...
Define se listas de postings desserializadas do índice de texto devem ser armazenadas em cache na memória.
...
Define se informações desserializadas de tokens do índice de texto devem ser armazenadas em cache na memória.
Usar o cache de tokens do índice de texto pode reduzir significativamente a latência e aumentar a taxa de transferência ao trabalhar com um grande número de consultas de índice de texto.
