...
Permite criar índices de texto com o argumento experimental `positions`, que
armazena as posições dos tokens para permitir a correspondência exata de frases.
...
configuração [replicated&#95;deduplication&#95;window](#replicated_deduplication_window)), incluindo a
granularidade de `insert_deduplication_version` (bloco inteiro inserido com o
`new_unified_hash` padrão, por parte criada nas versões legacy). As somas de hash são gravadas em
um arquivo local em um disco em vez de no ClickHouse Keeper.
...
No padrão `insert_deduplication_version = new_unified_hash`, a soma de hash abrange o
bloco inserido inteiro, portanto uma inserção é desduplicada apenas quando todos os seus dados correspondem a uma
inserção anterior (uma nova tentativa), e não para cada parte individualmente. Já no legado `old_separate_hashes` /
`compatible_double_hashes`, a soma de hash é calculada por parte criada, a partir da
composição dos nomes e tipos dos campos e dos dados dessa parte (fluxo de bytes).

Um valor alto para `replicated_deduplication_window` torna os `Inserts` mais lentos porque mais
...
Esta configuração se aplica apenas quando `insert_deduplication_version = old_separate_hashes` ou
`compatible_double_hashes`, que mantêm os hashes de async inserts em um diretório `async_blocks`
separado. No modo padrão `new_unified_hash`, os async inserts compartilham o diretório
`deduplication_hashes` unificado com os inserts síncronos e, em vez disso, são regidos por
...
Assim como `replicated_deduplication_window_for_async_inserts`, isso se aplica apenas com
`insert_deduplication_version = old_separate_hashes` / `compatible_double_hashes`; com o
`new_unified_hash` padrão, async inserts usam `replicated_deduplication_window_seconds`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "Habilitar merges coordenadas por padrão"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "0"},{"label": "Nova configuração"}]}]} />

Habilita a estratégia de merges coordenadas
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "Habilita mesclagens coordenadas por padrão"}]}, {"id": "row-2","items": [{"label": "25.3"},{"label": "0"},{"label": "Nova configuração"}]}]} />
...
(`OutdatedPartsQuorumThread`): enquanto uma réplica estiver inativa por menos do que essa quantidade de segundos,
ela ainda impede a remoção de partes desatualizadas e o truncamento de mutações concluídas. Um valor de 0
...
Tamanho da janela (em números de blocos) usado para distribuir a atribuição de merge entre
réplicas. Partes dentro da mesma janela são atribuídas às mesmas réplicas.
Disponível apenas no ClickHouse Cloud
...
Se estiver definido como true, a tabela ficará em modo somente leitura e não realizará modificações no disco.
...
(`ATTACH`/`MOVE`/`DROP`/`DROP DETACHED`/`FETCH`/`REPLACE PARTITION`, bem como `MOVE PARTITION ... TO TABLE` direcionado a esta tabela). Operações
que não modificam os dados da tabela, como `FREEZE`/`UNFREEZE` e `FORGET PARTITION`, permanecem permitidas.

Nenhuma tarefa em segundo plano é agendada: mesclagens regulares, mesclagens de TTL (`DELETE`/`MOVE`/recompressão), mesclagens de recompressão, mutações em segundo plano
e movimentações de partes em segundo plano são todos suprimidos. Como consequência, uma tabela com TTL deixa de remover ou mover seus dados expirados enquanto esta configuração
estiver habilitada.

A configuração sempre pode ser alternada de volta com `ALTER TABLE ... MODIFY SETTING table_readonly = 0` (ou `RESET SETTING`). Ela não é compatível com `ReplicatedMergeTree`.
