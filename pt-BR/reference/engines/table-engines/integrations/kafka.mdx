...
* `kafka_format` — Formato da mensagem. Usa a mesma notação da função SQL `FORMAT`, como `JSONEachRow`. Para mais informações, consulte a seção [Formatos](/pt-BR/reference/formats).
...
O motor Kafka oferece suporte a todos os [formatos](/pt-BR/reference/formats) compatíveis com o ClickHouse.
