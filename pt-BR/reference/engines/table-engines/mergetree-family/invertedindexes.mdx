...
A função [mapContains](/pt-BR/reference/functions/regular-functions/tuple-map-functions#mapcontains)(sinônimo de: `mapContainsKey`) faz correspondência com um único token nas chaves de um map.
...
O [operator[]](/pt-BR/reference/operators#access-operators) de acesso pode ser usado com o índice de texto para filtrar chaves e valores.

Exemplo:

```sql
SELECT count() FROM tab WHERE map['engine'] = 'clickhouse'; -- usará o índice de texto se definido
```
