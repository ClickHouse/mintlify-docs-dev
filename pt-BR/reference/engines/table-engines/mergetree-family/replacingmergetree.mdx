...
O motor difere de [MergeTree](/pt-BR/reference/engines/table-engines/mergetree-family/versionedcollapsingmergetree) por remover entradas duplicadas com o mesmo valor da [chave de ordenação](/pt-BR/reference/engines/table-engines/mergetree-family/mergetree) (seção `ORDER BY` da tabela, não `PRIMARY KEY`).
