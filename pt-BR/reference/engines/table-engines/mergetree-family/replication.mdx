...
Os dados comprimidos de consultas `INSERT` e `ALTER` são replicados (para mais informações, consulte a documentação de [ALTER](/pt-BR/reference/statements/alter).
...
O ClickHouse usa o [ClickHouse Keeper](/pt-BR/guides/oss/deployment-and-scaling/keeper) para armazenar os metadados das réplicas. É possível usar o ZooKeeper versão 3.4.5 ou mais recente, mas o ClickHouse Keeper é recomendado.
...
<Note>
  Não deixe de configurar a segurança. O ClickHouse oferece suporte ao esquema de ACL `digest` do subsistema de segurança do ZooKeeper [esquema ACL](https://zookeeper.apache.org/doc/current/zookeeperProgrammers.html#sc_ZooKeeperAccessControl).
</Note>
...
O ClickHouse também oferece suporte ao armazenamento das metainformações das réplicas em um cluster ZooKeeper auxiliar. Para isso, informe o nome e o caminho do cluster ZooKeeper como argumentos do motor.
...
Para armazenar os metadados da tabela em um cluster auxiliar do ZooKeeper em vez do cluster ZooKeeper padrão, podemos usar SQL para criar a tabela com o
