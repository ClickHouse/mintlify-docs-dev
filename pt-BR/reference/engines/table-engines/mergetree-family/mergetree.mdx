...
Para definir `interval`, use os operadores de [intervalo de tempo](/pt-BR/reference/operators#operators-for-working-with-dates-and-times), por exemplo:
...
* Durante mesclagens em segundo plano e [mutações](/pt-BR/reference/statements/alter#mutations).
...
Atualmente, as estatísticas `MinMax` e `NullCount` dão suporte à poda de partes. Quando estatísticas `MinMax` são definidas para uma coluna, o ClickHouse acompanha os valores mínimo e máximo dessa coluna em cada parte. Quando estatísticas `NullCount` são definidas para uma coluna `Nullable`, o ClickHouse acompanha o número de valores NULL em cada parte, permitindo a poda com base em predicados `IS NULL` / `IS NOT NULL` e melhorando a precisão da poda com filtros de intervalo para colunas com valores NULL.
...
```sql
-- Cria uma tabela com estatísticas MinMax na coluna 'value'
...
-- Insere dados em inserções separadas para criar múltiplas partes
INSERT INTO test_stats SELECT number, number FROM numbers(1000); -- Parte 1: intervalo de value [0, 999]
INSERT INTO test_stats SELECT number, number + 10000 FROM numbers(1000); -- Parte 2: intervalo de value [10000, 10999]

SET use_statistics_for_part_pruning = 1;

-- Esta consulta ignorará a Parte 1 completamente porque seu valor máximo (999) < 5000
SELECT count() FROM test_stats WHERE value > 5000;

-- Use EXPLAIN para ver o efeito do pruning
EXPLAIN indexes = 1 SELECT count() FROM test_stats WHERE value > 5000;
-- A saída mostrará "Parts: 1/2" indicando que uma parte foi removida pelo pruning
```
...
* `NullCount`
  ...
  Rastreia o número de valores `NULL` em colunas `Nullable`. É usado para estimar com precisão a seletividade de predicados `IS NULL`/`IS NOT NULL` na otimização PREWHERE e permite a poda de parte com base na presença de NULL.
...
|           | (U)Int*, Float*, Decimal(*), Date*, Boolean, Enum* | String ou FixedString | Nullable(*) / LowCardinality(Nullable(*)) |
| --------- | -------------------------------------------------- | --------------------- | ----------------------------------------- |
| CountMin  | ✔                                                  | ✔                     | ✗                                         |
| MinMax    | ✔                                                  | ✗                     | ✔                                         |
| NullCount | ✗                                                  | ✗                     | ✔                                         |
...
|           | Filtros por igualdade (==) | Filtros por intervalo (`>, >=, <, <=`) | `IS NULL` / `IS NOT NULL` |
| --------- | -------------------------- | -------------------------------------- | ------------------------- |
| CountMin  | ✔                          | ✗                                      | ✗                         |
| MinMax    | ✗                          | ✔                                      | ✗                         |
| NullCount | ✗                          | ✗                                      | ✔                         |
