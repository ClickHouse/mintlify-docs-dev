...
A expressão do pré-processador deve transformar um valor de entrada do tipo [String](/pt-BR/reference/data-types/string) ou [FixedString](/pt-BR/reference/data-types/fixedstring) em um valor do mesmo tipo.
Se o índice de texto foi criado em uma coluna do tipo `Nullable(T)` ou `LowCardinality(T)`, então a expressão do pré-processador deve aceitar valores anuláveis ou de baixa cardinalidade (ou seja, sem lançar uma exceção).
...
Neste caso, a expressão de pré-processamento transforma os elementos do array individualmente.
...
Para definir um pré-processador em um índice de texto na compilação de colunas do tipo [Map](/pt-BR/reference/data-types/map), os usuários precisam decidir se o índice é
compilado com base nas chaves ou nos valores do map.
...
O operador de acesso [operator[]](/pt-BR/reference/operators#access-operators) pode ser usado com o índice de texto para filtrar as chaves e os valores. O índice de texto só é usado se for criado nas expressões `mapKeys(map)` ou `mapValues(map)`, ou em ambas.
