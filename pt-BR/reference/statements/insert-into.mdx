...
Você pode especificar uma lista de colunas para inserir usando `(c1, c2, c3)`. Também é possível usar uma expressão com [matcher](/pt-BR/reference/statements/select#asterisk) de colunas, como `*`, e/ou [modificadores](/pt-BR/reference/statements/select#select-modifiers), como [APPLY](/pt-BR/reference/statements/select/apply_modifier), [EXCEPT](/pt-BR/reference/statements/select/except_modifier), [REPLACE](/pt-BR/reference/statements/select/replace_modifier).
...
Os dados podem ser fornecidos ao INSERT em qualquer [formato](/pt-BR/reference/formats) compatível com o ClickHouse. O formato deve ser especificado explicitamente na consulta:
...
Use a sintaxe acima para inserir dados de um arquivo, ou de vários arquivos, armazenados no **cliente**. `file_name` e `type` são literais de string. O [formato](/pt-BR/reference/formats) do arquivo de entrada deve ser definido na cláusula `FORMAT`.
...
Os dados podem ser inseridos em tabelas referenciadas por [funções de tabela](/pt-BR/reference/functions/table-functions).
