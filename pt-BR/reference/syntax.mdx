...
Como já mencionado, a consulta `INSERT` usa ambos os analisadores.
...
<Accordion title="Ativando o analisador completo">
  ...
  Se isso falhar, o ClickHouse tenta usar o analisador completo para os dados, tratando-os como uma [expressão](#expressions) SQL.
</Accordion>
...
<Note>
  Para mais informações sobre analisadores de formato, consulte a seção [Formatos](/pt-BR/reference/formats).
</Note>
...
* [Funções](#functions).
* [Tipos de dados](/pt-BR/reference/data-types).
...
Para mais informações, consulte [Tipos de dados](/pt-BR/reference/data-types).
...
<Note>
  ...
  * Dependendo do formato de dados (de entrada ou de saída), `NULL` pode ter uma representação diferente. Para mais informações, consulte [formatos de dados](/pt-BR/reference/formats).
...
* [CREATE](/pt-BR/reference/statements/create).
...
Para mais informações, consulte a seção [SELECT](/pt-BR/reference/statements/select#asterisk).