...
```python
# Sessão com configurações personalizadas
sess = session.Session(
...
Veja também: [test&#95;stateful.py](https://github.com/chdb-io/chdb/blob/main/tests/test_stateful.py).
...
class DatabaseReader(chdb.PyReader):
    """Leitor personalizado para fontes de dados semelhantes a bancos de dados"""
    ...
    def get_schema(self) -> List[Tuple[str, str]]:
        """Define o esquema da tabela com tipos explícitos"""
        return [
            ("id", "UInt64"),
            ("name", "String"),
            ("score", "Int64"),
            ("metadata", "String")  # JSON armazenado como string
        ]
    ...
    def read(self, col_names: List[str], count: int) -> List[List[Any]]:
        ...
        # Retornar dados para as colunas solicitadas
        result = []
        for col in col_names:
            if col in self.data:
                result.append(self.data[col][self.cursor:end_pos])
            else:
                # Tratar colunas ausentes
...
# DataFrame com objetos JSON mistos
...
# Controlar a inferência de JSON com configurações
result = chdb.query("""
    SELECT 
        user_id,
    ...
    SETTINGS pandas_analyze_sample = 1000  -- Analisar todas as linhas para detecção de JSON
""", "Pretty")
print(result)

# Operações avançadas de JSON
