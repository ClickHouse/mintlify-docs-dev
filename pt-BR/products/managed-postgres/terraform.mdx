...
Consulte [Como gerenciar chaves de API](/pt-BR/cloud/manage/openapi) para ver instruções sobre como criar uma chave de API para usar com o provider.
...
`ha_type` pode ser alterado após a criação; mudá-lo aciona uma transição de HA. Consulte [Alta disponibilidade](/pt-BR/cloud/managed-postgres/high-availability) para mais detalhes.
...
Consulte [réplicas de leitura](/pt-BR/cloud/managed-postgres/read-replicas) para obter mais detalhes.
...
O bloco inteiro só pode ser usado na criação: alterar `source_id` ou `restore_target`, ou remover o bloco, destrói e recria o serviço. Consulte [Backup e restauração](/pt-BR/cloud/managed-postgres/backup-and-restore) para mais detalhes.
