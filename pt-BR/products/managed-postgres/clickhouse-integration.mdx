...
<Image img="/images/managed-postgres/clickhouse-integration-intro.png" alt="Página inicial da integração com o ClickHouse mostrando a opção de integração na barra lateral" size="md" border />
...
<Steps>
  <Step>
    ## Configure o serviço de replicação
    ...
    * **Replication method**: Escolha uma das opções:
    ...
    <Image img="/images/managed-postgres/replication-service-step.png" alt="Configuração do serviço de replicação mostrando nome da integração, serviço de destino e opções de método de replicação" size="md" border />
  ...
  <Step>
    ...
    * **Snapshot number of tables in parallel**: Quantas tabelas têm snapshot feito em paralelo (padrão: 1)
    ...
    <Image img="/images/managed-postgres/integration-replication-settings.png" alt="Etapa de configurações de replicação mostrando intervalo de sincronização, threads em paralelo, tamanho do batch de extração e opções de configuração de snapshot" size="md" border />
  ...
  <Step>
    ...
    * **Preserve NULL values from source**: Mantém os valores NULL em vez de convertê-los em valores padrão
    * **Remove deleted rows during merges**: Para tabelas [ReplacingMergeTree](/pt-BR/reference/engines/table-engines/mergetree-family/replacingmergetree), remove fisicamente as linhas excluídas durante os merges em segundo plano
    ...
    <Image img="/images/managed-postgres/select-tables-step.png" alt="Etapa de seleção de tabelas mostrando a seleção do banco de dados, as opções de replicação e o seletor de tabelas agrupado por schema" size="md" border />

    Clique em **Replicate data to ClickHouse** para iniciar a replicação.
  </Step>

  <Step>
    ## Monitore seu ClickPipe
    ...
    <Image img="/images/managed-postgres/integration-running.png" alt="Lista da integração com o ClickHouse mostrando um ClickPipe em execução com seu serviço de destino e status" size="md" border />
