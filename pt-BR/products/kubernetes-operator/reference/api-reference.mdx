...
| `keeperClusterRef` | [KeeperClusterReference](#keeperclusterreference) | Referência ao KeeperCluster utilizado para coordenação do ClickHouse.<br />Quando o Espaço de nomes é omitido, o Espaço de nomes do ClickHouseCluster é utilizado. | true |  |
...
| `additionalVolumeClaimTemplates` | [PersistentVolumeClaimTemplate](#persistentvolumeclaimtemplate) array | Templates de PVC adicionais por pod do Kubernetes para armazenamento JBOD / multi-disco.<br />Cada entrada é propagada no volumeClaimTemplate do StatefulSet, montada em /var/lib/clickhouse/disks/<name> e<br />adicionada à storage policy JBOD gerada.<br />O conjunto de disks é fixado na criação. | false |  |
| `labels` | object (keys:string, values:string) | Labels adicionais que são adicionados aos recursos. | false |  |
