---
description: 'Instalar o ClickHouse no Linux Redhat/CentOS'
keywords: ['ClickHouse', 'instalar', 'Redhat', 'CentOS', 'rpm']
sidebarTitle: 'Redhat/CentOS'
slug: /install/redhat
title: 'Instalar o ClickHouse em distribuições Linux baseadas em RPM'
hide_title: true
doc_type: 'guide'
---

import Recommendations from '/snippets/pt-BR/recommendations.mdx';

<div id="from-rpm-packages">
  # Instalar o ClickHouse em distribuições baseadas em rpm
</div>

> Recomenda-se usar os pacotes `rpm` oficiais pré-compilados para **CentOS**, **RedHat** e todas as outras
> distribuições Linux baseadas em rpm.

<Steps>
  <Step>
    ## Revise as recomendações

    <Recommendations />
  </Step>

  <Step>
    ## Configure o repositório RPM

    Adicione o repositório oficial executando o comando a seguir:

    ```bash
    sudo yum install -y yum-utils
    sudo yum-config-manager --add-repo https://packages.clickhouse.com/rpm/clickhouse.repo
    ```

    Em sistemas com o gerenciador de pacotes `zypper` (openSUSE, SLES), execute:

    ```bash
    sudo zypper addrepo -r https://packages.clickhouse.com/rpm/clickhouse.repo -g
    sudo zypper --gpg-auto-import-keys refresh clickhouse-stable
    ```

    Nas etapas abaixo, `yum install` pode ser substituído por `zypper install`, dependendo
    do gerenciador de pacotes que você estiver usando.
  </Step>

  <Step>
    ## Instale o servidor e o cliente ClickHouse

    Para instalar o ClickHouse, execute os seguintes comandos:

    ```bash
    sudo yum install -y clickhouse-server clickhouse-client
    ```

    * Você pode substituir `stable` por `lts` para usar diferentes [tipos de release](/pt-BR/resources/support-center/knowledge-base/setup-installation/production), conforme sua necessidade.
    * Você pode baixar e instalar os pacotes manualmente em [packages.clickhouse.com/rpm](https://packages.clickhouse.com/rpm/stable).
    * Para especificar uma versão específica, adicione `-$version` ao final do nome do pacote,
      por exemplo:

    ```bash
    sudo yum install clickhouse-server-22.8.7.34
    ```
  </Step>

  <Step>
    ## Inicie o servidor ClickHouse

    Para iniciar o servidor ClickHouse, execute:

    ```bash
    sudo systemctl enable clickhouse-server
    sudo systemctl start clickhouse-server
    sudo systemctl status clickhouse-server
    ```

    Para iniciar o cliente ClickHouse, execute:

    ```bash
    clickhouse-client
    ```

    Se você configurou uma senha para o servidor, precisará executar:

    ```bash
    clickhouse-client --password
    ```
  </Step>

  <Step>
    ## Instale o ClickHouse Keeper standalone

    <Tip>
      Em ambientes de produção, recomendamos fortemente executar o ClickHouse Keeper em nós dedicados.
      Em ambientes de teste, se você decidir executar o ClickHouse Server e o ClickHouse Keeper no mesmo servidor,
      não será necessário instalar o ClickHouse Keeper, pois ele já está incluído no servidor ClickHouse.
    </Tip>

    Para instalar o `clickhouse-keeper` em servidores ClickHouse Keeper standalone, execute:

    ```bash
    sudo yum install -y clickhouse-keeper
    ```
  </Step>

  <Step>
    ## Habilite e inicie o ClickHouse Keeper

    ```bash
    sudo systemctl enable clickhouse-keeper
    sudo systemctl start clickhouse-keeper
    sudo systemctl status clickhouse-keeper
    ```
  </Step>
</Steps>
