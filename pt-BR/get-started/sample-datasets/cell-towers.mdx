---
description: 'Aprenda a carregar dados do OpenCelliD no ClickHouse, conectar o Apache Superset
  ao ClickHouse e criar um dashboard com base nesses dados'
sidebarTitle: 'Torres de celular'
slug: /getting-started/example-datasets/cell-towers
title: 'Dados geoespaciais usando o conjunto de dados de torres de celular'
keywords: ['dados de torres de celular', 'dados geoespaciais', 'OpenCelliD', 'conjunto de dados geoespacial', 'primeiros passos']
doc_type: 'guide'
---
...
import ActionsMenu from '/snippets/pt-BR/_service_actions_menu.mdx';
import SQLConsoleDetail from '/snippets/pt-BR/_launch_sql_console.mdx';
import SupersetDocker from '/snippets/pt-BR/_add_superset_detail.mdx';
...
Veja uma prévia do dashboard criado neste guia:

<Image img="/images/getting-started/example-datasets/superset-cell-tower-dashboard.png" size="md" alt="Dashboard de torres de telefonia celular por tipo de rádio no mcc 204" />

<div id="get-the-dataset">
  ## Obtenha o conjunto de dados
</div>

Este conjunto de dados é do [OpenCelliD](https://www.opencellid.org/) — o maior banco de dados aberto de torres de celular do mundo.

Em 2021, ele continha mais de 40 milhões de registros sobre torres de celular (GSM, LTE, UMTS etc.) em todo o mundo, com suas coordenadas geográficas e metadados (código do país, rede etc.).

O projeto OpenCelliD é licenciado sob a Creative Commons Attribution-ShareAlike 4.0 International License, e redistribuímos um snapshot desse conjunto de dados nos termos da mesma licença. A versão mais atual do conjunto de dados está disponível para download após o login.

<Tabs>
  <Tab title="ClickHouse Cloud">
    ### Carregue os dados de exemplo

    O ClickHouse Cloud oferece uma forma simples de carregar esse conjunto de dados a partir do S3. Faça login na sua organização do ClickHouse Cloud ou crie uma avaliação gratuita em [ClickHouse.cloud](https://clickhouse.cloud).

    <ActionsMenu menu="Carregar dados" />

    Escolha o conjunto de dados **Cell Towers** na aba **Dados de exemplo** e clique em **Carregar dados**:

    <Image img="/images/_snippets/cloud-load-data-sample.png" size="md" alt="Carregar conjunto de dados de torres de celular" />

    ### Examine o schema da tabela cell_towers
    ...
    Esta é a saída de `DESCRIBE`. Mais adiante neste guia, explicaremos as escolhas dos tipos de campo.
  ...
  <Tab title="Autogerenciado">
    ...
    2. Importe o conjunto de dados de um bucket público do S3 (686 MB):
...
<div id="examples">
  ## Execute algumas consultas de exemplo
</div>

1. O número de torres de celular por tipo:
...
2. Torres de celular por [código de país móvel (MCC)](https://en.wikipedia.org/wiki/Mobile_country_code):
...
Com base na consulta acima e na [lista de MCC](https://en.wikipedia.org/wiki/Mobile_country_code), os países com mais torres de celular são: os EUA, a Alemanha e a Rússia.

Talvez você queira criar um [Dicionário](/pt-BR/reference/statements/create/dictionary) no ClickHouse para decodificar esses valores.

<div id="use-case">
  ## Caso de uso: incorporar dados geoespaciais
</div>

Usando a função [`pointInPolygon`](/pt-BR/reference/functions/regular-functions/geo/coordinates#pointinpolygon).

1. Crie uma tabela na qual armazenaremos polígonos:
...
2. Este é um contorno aproximado de Moscou (sem a &quot;nova Moscou&quot;):
...
3. Verifique quantas torres de celular existem em Moscou:
...
Antes de criar visualizações no Superset, dê uma olhada nas colunas que você vai usar. Este conjunto de dados fornece principalmente a localização (Longitude e Latitude) e as tecnologias de rádio de torres de celular em todo o mundo. As descrições das colunas podem ser encontradas no [fórum da comunidade](https://community.opencellid.org/t/documenting-the-columns-in-the-downloadable-cells-database-csv/186). As colunas usadas nas visualizações que serão criadas estão descritas abaixo.

Aqui está uma descrição das colunas retirada do fórum do OpenCelliD:
...
<Tip>
  **mcc**
  ...
  Para encontrar seu MCC, consulte [Mobile network codes](https://en.wikipedia.org/wiki/Mobile_country_code) e use os três dígitos na coluna **Mobile country code**.
</Tip>

O schema desta tabela foi projetado para armazenamento compacto em disco e rapidez nas consultas.

* Os dados de `radio` são armazenados como `Enum8` (`UInt8`) em vez de uma string.
* `mcc`, ou Mobile country code, é armazenado como `UInt16`, pois sabemos que o intervalo vai de 1 a 999.
* `lon` e `lat` são `Float64`.

Nenhum dos outros campos é usado nas consultas ou visualizações deste guia, mas eles estão descritos no fórum indicado acima caso você tenha interesse.

<div id="build-visualizations-with-apache-superset">
  ## Crie visualizações com o Apache Superset
</div>

O Superset é fácil de executar no Docker. Se você já estiver com o Superset em execução, basta adicionar o ClickHouse Connect com `pip install clickhouse-connect`. Se precisar instalar o Superset, abra a seção **Launch Apache Superset in Docker** logo abaixo.

<SupersetDocker />

Para criar um dashboard no Superset usando o conjunto de dados OpenCelliD, você deve:

* Adicionar seu serviço ClickHouse como um **banco de dados** no Superset
...
<Image img="/images/getting-started/example-datasets/superset-add.png" size="md" alt="Adicionar um banco de dados" />

Escolha **ClickHouse Connect** na lista:

<Image img="/images/getting-started/example-datasets/superset-choose-a-database.png" size="md" alt="Escolher ClickHouse Connect como tipo de banco de dados" />

<Note>
  Se **ClickHouse Connect** não estiver entre as opções, será necessário instalá-lo. O comando é `pip install clickhouse-connect`, e mais informações estão [disponíveis aqui](https://pypi.org/project/clickhouse-connect/).
</Note>

<div id="add-your-connection-details">
  #### Adicione os detalhes da sua conexão
</div>

<Tip>
  Certifique-se de ativar o **SSL** ao se conectar ao ClickHouse Cloud ou a outros sistemas ClickHouse que exigem o uso de SSL.
</Tip>

<Image img="/images/getting-started/example-datasets/superset-connect-a-database.png" size="md" alt="Adicionar o ClickHouse como fonte de dados no Superset" />
...
No Superset, um **conjunto de dados** corresponde a uma tabela em um banco de dados. Clique em adicionar um conjunto de dados e escolha seu serviço do ClickHouse, o banco de dados que contém sua tabela (`default`) e a tabela `cell_towers`:

<Image img="/images/getting-started/example-datasets/superset-add-dataset.png" size="md" alt="Adicionar a tabela cell_towers como um conjunto de dados no Superset" />

<div id="create-some-charts">
  ### Crie alguns **gráficos**
</div>

Ao adicionar um gráfico no Superset, você precisa especificar o conjunto de dados (`cell_towers`) e o tipo de gráfico. Como o conjunto de dados OpenCelliD fornece coordenadas de longitude e latitude das torres de celular, vamos criar um gráfico de **mapa**. O tipo **deck.gL Scatterplot** é adequado para esse conjunto de dados, pois funciona bem com pontos de dados densos em um mapa.

<Image img="/images/getting-started/example-datasets/superset-create-map.png" size="md" alt="Crie um mapa no Superset" />

<div id="specify-the-query-used-for-the-map">
  #### Especifique a consulta usada para o mapa
</div>

Um gráfico de dispersão do deck.gl requer longitude e latitude, e também é possível aplicar um ou mais filtros à consulta. Neste exemplo, são aplicados dois filtros: um para torres de celular com rádio UMTS e outro para o código de país móvel atribuído aos Países Baixos.

Os campos `lon` e `lat` contêm a longitude e a latitude:

<Image img="/images/getting-started/example-datasets/superset-lon-lat.png" size="md" alt="Especifique os campos de longitude e latitude" />

Adicione um filtro com `mcc` = `204` (ou substitua por qualquer outro valor de `mcc`):

<Image img="/images/getting-started/example-datasets/superset-mcc-204.png" size="md" alt="Filtrar por MCC 204" />
...
<Image img="/images/getting-started/example-datasets/superset-radio-umts.png" size="md" alt="Filtrar por radio igual a UMTS" />

Esta é a configuração completa do gráfico que filtra por `radio = 'UMTS'` e `mcc = 204`:

<Image img="/images/getting-started/example-datasets/superset-umts-netherlands.png" size="md" alt="Gráfico para rádios UMTS no MCC 204" />

Clique em **UPDATE CHART** para exibir a visualização.

<div id="add-the-charts-to-a-dashboard">
  ### Adicione os gráficos a um **dashboard**
</div>

Esta captura de tela mostra a localização de torres de celular com rádios LTE, UMTS e GSM. Os gráficos são todos criados da mesma forma e adicionados a um dashboard.

<Image img="/images/getting-started/example-datasets/superset-cell-tower-dashboard.png" size="md" alt="Dashboard de torres de celular por tipo de rádio no mcc 204" />

<Tip>
  Os dados também estão disponíveis para consultas interativas no [Playground](https://sql.clickhouse.com).

  Este [exemplo](https://sql.clickhouse.com?query_id=UV8M4MAGS2PWAUOAYAAARM) já preencherá o username e até a consulta para você.

  Embora você não possa criar tabelas no Playground, pode executar todas as consultas e até mesmo usar o Superset (ajuste o nome do host e o número da porta).
</Tip>