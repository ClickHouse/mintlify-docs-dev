...
<div id="designing-a-schema-for-observability">
  # Projetando um schema para observabilidade
</div>
...
[visão materializada](/pt-BR/concepts/features/materialized-views) oferecem uma forma mais poderosa de aplicar filtragem e transformações SQL a logs e traces.
...
<Image img="/images/use-cases/observability/observability-10.png" alt="Visão materializada" size="md" />
...
Para garantir que não persistamos os dados duas vezes (nas tabelas de origem e de destino), podemos alterar a tabela de origem para usar um [Null table engine](/pt-BR/reference/engines/table-engines/special/null), preservando o schema original. Nossos OTel collectors continuarão enviando dados para essa tabela. Por exemplo, para logs, a tabela `otel_logs` se torna:
...
<Image img="/images/use-cases/observability/observability-11.png" alt="Otel MV" size="md" />
...
1 linha no conjunto. Elapsed: 0.010 sec.
```
...
<Image img="/images/use-cases/observability/observability-12.png" alt="Observabilidade e dicionários" size="md" />
...
Embora junções raramente sejam necessárias em casos de uso de observabilidade, os dicionários ainda podem ser úteis para fins de enriquecimento, tanto no momento da inserção quanto no momento da consulta. Fornecemos exemplos de ambos abaixo.

<Info>
  **Acelerando junções**

  Usuários interessados em acelerar junções com dicionários podem encontrar mais detalhes [aqui](/pt-BR/concepts/features/dictionaries).
</Info>
...
* **Tempo de inserção** - Em geral, isso é apropriado quando o valor de enriquecimento não muda e existe em uma fonte externa que pode ser usada para preencher o dicionário. Nesse caso, enriquecer a linha no momento da inserção evita a consulta ao dicionário no momento da consulta. Isso tem um custo em termos de desempenho de inserção, além de uma sobrecarga adicional de armazenamento, já que os valores enriquecidos serão armazenados como colunas.
* **Tempo de consulta** - Se os valores em um dicionário mudam com frequência, as consultas no momento da consulta costumam ser mais adequadas. Isso evita a necessidade de atualizar colunas (e reescrever dados) se os valores mapeados mudarem. Essa flexibilidade vem com o custo de uma consulta no momento da consulta. Esse custo normalmente só é perceptível quando a consulta é necessária para muitas linhas, por exemplo, ao usar uma consulta ao dicionário em uma cláusula de filtro. Para enriquecimento de resultados, ou seja, no `SELECT`, essa sobrecarga normalmente não é perceptível.
...
Para exemplos simples de enriquecimento, veja o guia sobre Dicionários [aqui](/pt-BR/concepts/features/dictionaries). Abaixo, focamos em tarefas comuns de enriquecimento de observabilidade.
...
* [Tópicos avançados sobre dicionários](/pt-BR/concepts/features/dictionaries#advanced-dictionary-topics)
...
<Image img="/images/use-cases/observability/observability-13.png" alt="Observabilidade e projeções" size="md" />
...
1 linha no conjunto. Elapsed: 0.008 sec.
```
...
Além de reduzir drasticamente os custos de armazenamento, ter menos dados em disco significa menos I/O e consultas e inserções mais rápidas. A redução de I/O compensará com folga a sobrecarga de qualquer algoritmo de compressão em termos de CPU. Portanto, melhorar a compressão dos dados deve ser o primeiro foco ao buscar garantir que as consultas no ClickHouse sejam rápidas.
