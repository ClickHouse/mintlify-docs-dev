...
O ClickHouse oferece suporte a uma ampla variedade de [formatos de dados de entrada e saída](/pt-BR/reference/formats). Entre eles, há várias variações de JSON, mas a mais comum para ingestão de dados é o [JSONEachRow](/pt-BR/reference/formats/JSON/JSONEachRow). Ele espera um objeto JSON por linha, com cada objeto separado por uma quebra de linha.
...
* `input_format_import_nested_json` permite inserir objetos JSON aninhados em colunas do tipo [Nested](/pt-BR/reference/data-types/nested-data-structures).
