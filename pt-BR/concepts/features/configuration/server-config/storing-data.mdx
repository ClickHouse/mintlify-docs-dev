...
<Note>
  O Google Cloud Storage (GCS) também é compatível com o tipo `s3`. Consulte [GCS backed MergeTree](/pt-BR/integrations/connectors/data-ingestion/GCP).
</Note>
...
[Mutações](/pt-BR/reference/statements/alter#mutations) e a replicação de tabelas não são suportadas.
...
[`ALTER TABLE`](/pt-BR/reference/statements/alter), [`RENAME TABLE`](/pt-BR/reference/statements/rename#rename-table),
