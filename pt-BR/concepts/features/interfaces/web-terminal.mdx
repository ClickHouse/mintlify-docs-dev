...
<Note>
  O terminal web é um recurso experimental e vem desativado por padrão; consulte [Como habilitar o recurso](#enabling-the-feature) abaixo.
</Note>
...
O endpoint `/webterminal` é controlado pela configuração de servidor `allow_experimental_webterminal`. Quando essa configuração está como `false` (o padrão), as solicitações para `/webterminal` retornam o status HTTP `403 Forbidden`.
