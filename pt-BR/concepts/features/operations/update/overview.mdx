...
Também há outros [tipos de motores](/pt-BR/reference/engines/table-engines/mergetree-family), mas estes são os mais usados.
...
Esses métodos funcionam com tabelas que usam motores da [família MergeTree](/pt-BR/reference/engines/table-engines/mergetree-family).
...
| [Atualizações leves](#lightweight-updates) | `UPDATE ... SET ... WHERE` | Pequenas atualizações (~0.1-10% das linhas); atualizações frequentes com necessidade de desempenho             | Adiciona sobrecarga ao SELECT; patch parts contam para os limites |
