...
Blocos idênticos gerados durante a transformação em uma visão materializada não são deduplicados, porque se baseiam em dados inseridos diferentes.
...
Aqui vemos que 2 partes foram inseridas na tabela `mv_dst`. Essas partes contêm os mesmos dados, no entanto, não são deduplicadas.
...
A nova tentativa de inserção é deduplicada como esperado.
...
Inserimos dados diferentes a cada vez. No entanto, os mesmos dados são inseridos na tabela `mv_dst`. Os dados não são deduplicados porque os dados de origem eram diferentes.
...
Essa operação de retry é deduplicada em ambas as tabelas `dst` e `mv_dst`.