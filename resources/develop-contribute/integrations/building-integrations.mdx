...
- For throughput, batch **10K–100K rows** and aim for roughly **one insert per second** as an upper bound for synchronous inserts. If client-side batching is impractical, use [asynchronous inserts](/core/concepts/features/operations/insert/asyncinserts) to shift batching to the server

See also: [Bulk inserts](/core/concepts/features/operations/insert/bulkinserts).
...
- **BI and SQL tools:** ClickHouse ships an official [JDBC v2 driver](/integrations/language-clients/java) (Java) and an [ODBC driver](/core/concepts/features/interfaces/odbc). Tableau, Looker, Power BI, Metabase, Apache Superset, and Grafana integrate via these drivers or dedicated connectors maintained by ClickHouse and partners
