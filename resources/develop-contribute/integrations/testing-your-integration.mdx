...
- **Self-hosted (open source):** use the latest stable release from [GitHub releases](https://github.com/ClickHouse/ClickHouse/releases). The [install guide](/core/get-started/setup/install) is the fastest path to a local instance with Docker
...
The full set can be found in the [**Example datasets**](/core/get-started/sample-datasets) section. The following four datasets cover most integration testing needs:

- **[GitHub events](/core/get-started/sample-datasets/github-events):** 3.1B rows with nested event payloads. Best for arrays, tuples, and nested types
- **[NYC taxi data](/core/get-started/sample-datasets/nyc-taxi):** billions of rows with a well-known schema. Good for throughput and read-path testing
- **[Stack Overflow](/core/get-started/sample-datasets/stackoverflow):** multi-table relational data for JOIN-heavy BI scenarios
- **[Hacker News](/core/get-started/sample-datasets/hacker-news):** 28M rows, fast to load, useful for iteration

For extreme-scale validation, use **[WikiStat](/core/get-started/sample-datasets/wikistat)** (~0.5 trillion records).
