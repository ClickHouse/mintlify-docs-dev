...
Even if a DBMS started as a pure OLAP or pure OLTP, they're forced to move towards that HTAP direction to keep up with their competition. And ClickHouse is no exception, initially, it has been designed as [fast-as-possible OLAP system](/core/get-started/about/why-clickhouse-is-so-fast) and it still doesn't have full-fledged transaction support, but some features like consistent read/writes and mutations for updating/deleting data had to be added.

The fundamental trade-off between OLAP and OLTP systems remains:

- To build analytical reports efficiently it's crucial to be able to read columns separately, thus most OLAP databases are [columnar](/resources/support-center/knowledge-base/general-faqs/columnar-database),
