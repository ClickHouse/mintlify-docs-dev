...
## Useful queries for troubleshooting <a id="useful-queries-for-troubleshooting"></a>
...
## View which settings have been changed from the default <a id="view-which-settings-have-been-changed-from-the-default"></a>
```sql
SELECT
    name,
    value
...
## Get the size of all your tables <a id="get-the-size-of-all-your-tables"></a>
```sql
SELECT table,
...
## Row count and average day size of your table <a id="row-count-and-average-day-size-of-your-table"></a>
```sql
SELECT
    table,
...
## Compression columns percentage as well as the size of primary index in memory <a id="compression-columns-percentage-as-well-as-the-size-of-primary-index-in-memory"></a>
...
## Number of queries sent by client in the last 10 minutes <a id="number-of-queries-sent-by-client-in-the-last-10-minutes"></a>
...
## Number of parts in each partition <a id="number-of-parts-in-each-partition"></a>
```sql
SELECT
...
## Finding long running queries <a id="finding-long-running-queries"></a>
...
## View the most recent errors <a id="view-the-most-recent-errors"></a>
```
SELECT *
FROM system.errors
...
## Top 10 queries that are using the most CPU and memory <a id="top-10-queries-that-are-using-the-most-cpu-and-memory"></a>
```sql
SELECT
    type,
    event_time,
...
## How much disk space are my projection using <a id="how-much-disk-space-are-my-projection-using"></a>
```sql
SELECT
    name,
    parent_name,
...
## Show disk storage, number of parts, number of rows in system.parts and marks across databases <a id="show-disk-storage-number-of-parts-number-of-rows-in-systemparts-and-marks-across-databases"></a>
```sql
SELECT
    database,
    table,
...
## List details of recently written new parts <a id="list-details-of-recently-written-new-parts"></a>
...
## Cluster-wide monitoring queries <a id="cluster-wide-monitoring-queries"></a>
...
### Average new parts created per minute and second (last hour) <a id="avg-new-parts-per-minute-and-second"></a>
```sql
WITH
    PER_MINUTE AS
    (
...
### CPU and memory intensive queries (cluster-wide) <a id="cpu-and-memory-intensive-queries-cluster-wide"></a>
```sql
SELECT
    type,
    event_time,
...
### Merges in progress with ETA <a id="merges-in-progress-with-eta"></a>
...
### Most common queries by normalized hash <a id="most-common-queries-by-normalized-hash"></a>
...
### Error counts by event type and date <a id="error-counts-by-event-type-and-date"></a>
...
### Number of tables by node <a id="number-of-tables-by-node"></a>
...
### Check for async insert operations <a id="check-for-async-insert-operations"></a>
...
## Parts and merges analysis <a id="parts-and-merges-analysis"></a>
### Currently active parts by table <a id="currently-active-parts-by-table"></a>
...
### Partitions with too many parts <a id="partitions-with-too-many-parts"></a>
...
### Detached parts <a id="detached-parts"></a>
...
## System information queries <a id="system-information-queries"></a>
### Cluster-wide memory usage by node <a id="cluster-wide-memory-usage-by-node"></a>
...
### Running queries on the cluster <a id="running-queries-on-the-cluster"></a>
...
### Modified settings from defaults <a id="modified-settings-from-defaults"></a>
...
### Replication queue status <a id="replication-queue-status"></a>
