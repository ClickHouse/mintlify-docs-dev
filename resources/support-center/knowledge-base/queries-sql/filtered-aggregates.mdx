...
ClickHouse provides a simple and intuitive way to write _filtered aggregates_.
 ...
 For example, compare the standard SQL way to write filtered aggregates (which work fine in ClickHouse) with the shorthand syntax using the `-If` [aggregate function combinator](/core/reference/functions/aggregate-functions/combinators), which can be appended to any aggregate function:
...
Check out the [aggregate function combinator](/core/reference/functions/aggregate-functions/combinators) page in the docs
for more details.
