...
Similarly, you can execute the same [SYSTEM statement](/reference/statements/system) on all nodes with a single statement, by using the [ON CLUSTER](/snippets/distributed-ddl) clause:
```sql
