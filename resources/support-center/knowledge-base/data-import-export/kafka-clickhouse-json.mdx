...
With the introduction of the new [`JSON`](/core/reference/data-types/newjson) data type, ClickHouse is now a good choice of [database for doing JSON analytics](https://clickhouse.com/engineering-resources/json-database).
...
Note that we're using the [`JSONAsObject`](/core/reference/formats/JSON/JSONAsObject) format, which will ensure that incoming messages are made available as a JSON object.
