...
이 단계별 가이드는 [MySQL ClickPipe](/ko/integrations/clickpipes/mysql)를 사용하여 Azure Flexible Server for MySQL의 데이터를 ClickHouse Cloud로 복제하도록 구성하는 방법을 설명합니다. 이 서비스에서는 **일회성 수집**만 지원합니다. MySQL CDC와 관련된 일반적인 질문은 [MySQL FAQ 페이지](/ko/integrations/clickpipes/mysql/faq)를 참조하십시오.
...
<Tabs>
  <Tab title="ClickPipes IP 허용">
    ...
    3. **Firewall rules** 섹션에서 서비스가 배포된 Region에 해당하는 [ClickPipes 고정 IP 주소 목록](/ko/integrations/clickpipes/home#list-of-static-ips)을 입력합니다.
       ...
       <Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/azure-flexible-server-mysql/1_configure_network_security.png" alt="IP 허용 목록을 사용해 퍼블릭 액세스용 네트워킹을 구성합니다" size="lg" border />
...
이제 [ClickPipe를 생성](/ko/integrations/clickpipes/mysql)하고 Azure Flexible Server for MySQL 인스턴스의 데이터를 ClickHouse Cloud로 수집하기 시작할 수 있습니다. 인스턴스를 설정할 때 사용한 연결 정보를 반드시 기록해 두십시오. 이 정보는 ClickPipe 생성 과정에서 필요합니다.