...
<Image img="/images/integrations/data-ingestion/clickpipes/clickpipes_stack.png" alt="ClickPipes 스택" size="lg" border />
...
| ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Apache Kafka](/ko/integrations/clickpipes/kafka)              | <img src="/images/integrations/logos/kafka.svg" alt="Apache Kafka 로고" width="32" />                  | 스트리밍    | 안정      | ClickPipes를 구성하고 Apache Kafka의 스트리밍 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                                 |
...
| Redpanda                                                    | <img src="/images/integrations/logos/logo_redpanda.png" alt="Redpanda 로고" width="32" />              | 스트리밍    | 안정      | ClickPipes를 구성하고 Redpanda의 스트리밍 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                                     |
| AWS MSK                                                     | <img src="/images/integrations/logos/msk.svg" alt="AWS MSK 로고" width="32" />                         | 스트리밍    | 안정      | ClickPipes를 구성하고 AWS MSK의 스트리밍 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                                      |
| Azure Event Hubs                                            | <img src="/images/integrations/logos/azure_event_hubs.svg" alt="Azure Event Hubs 로고" width="32" />   | 스트리밍    | 안정      | ClickPipes를 구성하고 Azure Event Hubs의 스트리밍 데이터를 ClickHouse Cloud로 수집하기 시작합니다. 자세한 내용은 [Azure Event Hubs FAQ](/ko/integrations/clickpipes/kafka/faq#azure-eventhubs)를 참조하십시오. |
| WarpStream                                                  | <img src="/images/integrations/logos/warpstream.svg" alt="WarpStream 로고" width="32" />               | 스트리밍    | 안정      | ClickPipes를 구성하고 WarpStream의 스트리밍 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                                   |
| Amazon S3                                                   | <img src="/images/integrations/logos/amazon_s3_logo.svg" alt="Amazon S3 로고" width="32" />            | 객체 스토리지 | 안정      | ClickPipes를 구성하여 객체 스토리지의 대용량 데이터를 수집합니다.                                                                                                                              |
| Google Cloud Storage                                        | <img src="/images/integrations/logos/gcs.svg" alt="Google Cloud Storage 로고" width="32" />            | 객체 스토리지 | 안정      | ClickPipes를 구성하여 객체 스토리지의 대용량 데이터를 수집합니다.                                                                                                                              |
| DigitalOcean Spaces                                         | <img src="/images/integrations/logos/digitalocean.svg" alt="Digital Ocean 로고" width="32" />          | 객체 스토리지 | 안정      | ClickPipes를 구성하여 객체 스토리지의 대용량 데이터를 수집합니다.                                                                                                                              |
| Azure Blob Storage                                          | <img src="/images/integrations/logos/azureblobstorage.svg" alt="Azure Blob Storage 로고" width="32" /> | 객체 스토리지 | 안정      | ClickPipes를 구성하여 객체 스토리지의 대용량 데이터를 수집합니다.                                                                                                                              |
| [Amazon Kinesis](/ko/integrations/clickpipes/kinesis/overview) | <img src="/images/integrations/logos/amazon_kinesis_logo.svg" alt="Amazon Kenesis 로고" width="32" />  | 스트리밍    | 안정      | ClickPipes를 구성하고 Amazon Kinesis의 스트리밍 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                               |
| [Postgres](/ko/integrations/clickpipes/postgres)               | <img src="/images/integrations/logos/postgresql.svg" alt="Postgres 로고" width="32" />                 | DBMS    | 안정      | ClickPipes를 구성하고 Postgres 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                                           |
| [MySQL](/ko/integrations/clickpipes/mysql)                     | <img src="/images/integrations/logos/mysql.svg" alt="MySQL 로고" width="32" />                         | DBMS    | 공개 베타   | ClickPipes를 구성하고 MySQL 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                                              |
| [MongoDB](/ko/integrations/clickpipes/mongodb)                 | <img src="/images/integrations/logos/mongodb.svg" alt="MongoDB 로고" width="32" />                     | DBMS    | 비공개 프리뷰 | ClickPipes를 구성하고 MongoDB 데이터를 ClickHouse Cloud로 수집하기 시작합니다.                                                                                                            |
...
아래 Google Cloud 표에 나열된 Google Cloud 리전의 서비스는 2026년 5월 27일 이후에 생성된 경우에만 해당 Google Cloud IP 주소를 사용합니다. 해당 리전에서 2026년 5월 27일 이전에 생성된 서비스는 아래에 나열된 기본 리전 IP 주소를 계속 사용합니다.

그 외 서비스의 경우 ClickPipes 트래픽은 서비스 위치에 따라 기본 리전에서 발생합니다.

* **eu-central-1**: 명시적으로 나열되지 않은 모든 EU 리전, Azure EU 리전, 그리고 2026년 5월 27일 이전에 생성된 Google Cloud EU 서비스에 적용됩니다.
...
* **us-east-2**: 위 규칙에 해당하지 않는 그 밖의 모든 리전에 적용되며, 여기에는 Azure 리전과 2026년 5월 27일 이전에 생성된 Google Cloud 서비스가 포함됩니다.
...
<Image img="/images/integrations/data-ingestion/clickpipes/cp_custom_role.png" alt="사용자 지정 역할 할당" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/cp_advanced_settings.png" alt="ClickPipes의 고급 설정" size="lg" border />
...
* **ClickPipes를 사용하면 추가 비용이 발생합니까?**
  ...
  ClickPipes는 수집된 데이터와 컴퓨트의 2가지 차원으로 청구됩니다. 가격에 대한 자세한 내용은 [이 페이지](/ko/products/cloud/reference/billing/clickpipes)에서 확인할 수 있습니다. 또한 ClickPipes를 실행하면 다른 수집 워크로드와 마찬가지로 대상 ClickHouse Cloud 서비스에서 간접적인 컴퓨트 및 스토리지 비용이 발생할 수 있습니다.
