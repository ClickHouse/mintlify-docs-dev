---
sidebarTitle: 'Luzmo'
slug: /integrations/luzmo
keywords: ['ClickHouse', 'Luzmo', '연결', '통합', 'UI', '임베디드']
description: 'Luzmo는 소프트웨어 및 SaaS 애플리케이션에 특화된, 네이티브 ClickHouse 통합을 제공하는 임베디드 애널리틱스 플랫폼입니다.'
title: 'Luzmo를 ClickHouse와 통합하기'
sidebar: 'integrations'
doc_type: 'guide'
integration:
  - support_level: 'community'
  - category: 'data_visualization'
---

import { Image } from "/snippets/components/Image.jsx";

import CommunityMaintainedBadge from "/snippets/components/CommunityMaintainedBadge/CommunityMaintainedBadge.jsx";

<CommunityMaintainedBadge />

<div id="1-setup-a-clickhouse-connection">
  ## 1. ClickHouse 연결 설정
</div>

ClickHouse에 연결하려면 **Connections 페이지**로 이동한 다음 **New Connection**을 선택하고, New Connection 모달에서 ClickHouse를 선택하십시오.

<Image img="/images/integrations/data-visualization/luzmo_01.png" size="md" alt="ClickHouse가 선택된 Create a New Connection 대화 상자를 보여주는 Luzmo 인터페이스" border />

그러면 **host**, **username**, **password**를 입력하라는 메시지가 표시됩니다.

<Image img="/images/integrations/data-visualization/luzmo_02.png" size="md" alt="ClickHouse host, username, password 입력 필드를 보여주는 Luzmo 연결 구성 양식" border />

* **Host**: ClickHouse 데이터베이스가 노출된 호스트입니다. 데이터를 wire 상에서 안전하게 전송하기 위해 여기서는 `https`만 허용됩니다. host URL은 다음 구조를 따라야 합니다: `https://url-to-clickhouse-db:port/database`
  기본적으로 플러그인은 &#39;default&#39; 데이터베이스와 443 포트에 연결됩니다. &#39;/&#39; 뒤에 데이터베이스를 지정하면 연결할 데이터베이스를 구성할 수 있습니다.
* **Username**: ClickHouse 클러스터에 연결할 때 사용할 사용자 이름입니다.
* **Password**: ClickHouse 클러스터에 연결할 때 사용할 비밀번호입니다.

API를 통해 [ClickHouse 연결을 생성](https://developer.luzmo.com/api/createAccount?exampleSection=AccountCreateClickhouseRequestBody)하는 방법은 개발자 문서의 예시를 참조하십시오.

<div id="2-add-datasets">
  ## 2. 데이터셋 추가
</div>

ClickHouse를 연결한 후에는 [여기](https://academy.luzmo.com/article/ldx3iltg)에 설명된 대로 데이터셋을 추가할 수 있습니다. ClickHouse에서 사용할 수 있는 데이터셋을 하나 이상 선택한 다음, Luzmo에서 이를 [연결](https://academy.luzmo.com/article/gkrx48x5)해 하나의 대시보드에서 함께 사용할 수 있도록 하십시오. 또한 [분석용 데이터 준비하기](https://academy.luzmo.com/article/u492qov0) 문서도 확인해 보십시오.

API를 사용해 데이터셋을 추가하는 방법은 [개발자 문서의 이 예시](https://developer.luzmo.com/api/createDataprovider?exampleSection=DataproviderCreateClickhouseRequestBody)를 참조하십시오.

이제 데이터셋을 활용해 보기 좋은 (임베디드) 대시보드를 만들거나, 고객의 질문에 답할 수 있는 AI 데이터 분석가([Luzmo IQ](https://luzmo.com/iq))를 구동할 수도 있습니다.

<Image img="/images/integrations/data-visualization/luzmo_03.png" size="md" alt="ClickHouse 데이터를 여러 시각화로 보여주는 Luzmo 대시보드 예시" border />

<div id="usage-notes">
  ## 사용 시 참고 사항
</div>

1. Luzmo ClickHouse 커넥터는 연결에 HTTP API 인터페이스(일반적으로 8123 포트에서 실행됨)를 사용합니다.
2. `Distributed` 테이블 엔진을 사용하는 테이블을 사용할 때 `distributed_product_mode`가 `deny`로 설정되어 있으면 일부 Luzmo 차트가 작동하지 않을 수 있습니다. 다만 이는 테이블을 다른 테이블에 연결하고, 그 연결을 차트에서 사용할 때만 발생합니다. 이 경우 ClickHouse 클러스터에서 `distributed_product_mode`를 환경에 맞는 다른 옵션으로 설정하십시오. ClickHouse Cloud를 사용하는 경우 이 설정은 무시해도 됩니다.
3. 예를 들어 Luzmo 애플리케이션만 ClickHouse 인스턴스에 접근할 수 있도록 하려면 [Luzmo의 고정 IP 주소 범위](https://academy.luzmo.com/article/u9on8gbm)를 **허용 목록에 추가**하는 것을 강력히 권장합니다. 또한 서비스용 읽기 전용 사용자 계정을 사용하는 것도 권장합니다.
4. ClickHouse 커넥터는 현재 다음 데이터 타입을 지원합니다.

   | ClickHouse 유형 | Luzmo 유형  |
   | ------------- | --------- |
   | UInt          | numeric   |
   | Int           | numeric   |
   | Float         | numeric   |
   | Decimal       | numeric   |
   | Date          | datetime  |
   | DateTime      | datetime  |
   | String        | hierarchy |
   | Enum          | hierarchy |
   | FixedString   | hierarchy |
   | UUID          | hierarchy |
   | Bool          | hierarchy |