...
<Image img="/images/integrations/data-ingestion/dbms/jdbc-01.png" size="lg" alt="ClickHouse JDBC Bridge 아키텍처 다이어그램" background="white" />

외부 데이터 소스에 네이티브로 제공되는 내장 [integration 엔진](/ko/reference/engines/table-engines/integrations), 테이블 함수 또는 외부 딕셔너리가 없더라도, 해당 데이터 소스용 JDBC 드라이버가 있다면 유용하게 사용할 수 있습니다.
...
ClickHouse JDBC Bridge를 사용하는 가장 쉬운 방법은 ClickHouse가 실행 중인 동일한 호스트에 이를 설치하고 실행하는 것입니다:<Image img="/images/integrations/data-ingestion/dbms/jdbc-02.png" size="lg" alt="로컬에 배포된 ClickHouse JDBC Bridge 다이어그램" background="white" />
...
분산 ClickHouse 클러스터(ClickHouse 호스트가 2개 이상인 클러스터)에서는 ClickHouse JDBC Bridge를 별도의 전용 호스트에 외부 설치하여 실행하는 것이 적절합니다.

<Image img="/images/integrations/data-ingestion/dbms/jdbc-03.png" size="lg" alt="ClickHouse JDBC Bridge 외부 배포 다이어그램" background="white" />
...
2. 각 ClickHouse 호스트에서 다음 구성 블록을 <a href="/ko/concepts/features/configuration/server-config/configuration-files#configuration_files" target="_blank">ClickHouse 서버 구성</a>에 추가합니다(선택한 구성 포맷에 따라 XML 또는 YAML 버전을 사용하십시오).
