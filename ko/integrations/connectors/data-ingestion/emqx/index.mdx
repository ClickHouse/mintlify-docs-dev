...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_1.png" size="sm" border alt="AWS 리전 선택을 보여주는 ClickHouse Cloud 서비스 배포 인터페이스" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_2.png" size="sm" border alt="IP 액세스 구성을 보여주는 ClickHouse Cloud 연결 설정" />

그런 다음 나중에 사용할 수 있도록 사용자 이름과 비밀번호를 저장해야 합니다.

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_3.png" size="sm" border alt="사용자 이름과 비밀번호를 보여주는 ClickHouse Cloud 자격 증명 화면" />

그 후 실행 중인 ClickHouse 인스턴스를 확인할 수 있습니다. ClickHouse Cloud 인스턴스의 연결 주소를 확인하려면 &quot;Connect&quot;를 클릭하십시오.

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_4.png" size="lg" border alt="연결 옵션이 있는 ClickHouse Cloud 실행 인스턴스 dashboard" />

EMQX Cloud와 통합하기 위한 데이터베이스와 테이블을 생성하려면 &quot;Connect to SQL Console&quot;을 클릭하십시오.

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_5.png" size="lg" border alt="ClickHouse Cloud SQL 콘솔 인터페이스" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_6.png" size="lg" border alt="ClickHouse Cloud에서 데이터베이스 및 테이블 CREATE SQL 쿼리 실행" />
...
EMQX Cloud를 처음 사용하는 경우 [EMQX Cloud sign up](https://accounts.emqx.com/signup?continue=https%3A%2F%2Fwww.emqx.com%2Fen%2Fcloud) 페이지로 이동한 다음 start free를 클릭하여 계정을 등록하세요.

<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_sign_up.png" size="lg" border alt="등록 양식이 있는 EMQX Cloud 가입 페이지" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_create_1.png" size="lg" border alt="배포 옵션을 보여주는 EMQX Cloud 배포 생성 1단계" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_overview.png" size="lg" border alt="브로커 메트릭을 보여주는 EMQX Cloud 개요 대시보드" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_auth.png" size="lg" border alt="자격 증명을 추가하기 위한 EMQX Cloud Authentication Setup 인터페이스" />
...
ClickHouse 통합 설정을 시작하기 전에 먼저 NAT gateway를 활성화해야 합니다. 기본적으로 MQTT 브로커는 프라이빗 VPC에 배포되므로 공용 네트워크를 통해 서드파티 시스템으로 데이터를 전송할 수 없습니다.
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_nat_gateway.png" size="lg" border alt="EMQX Cloud NAT Gateway 구성 패널" />
...
[EMQX Cloud Data Integrations](https://docs.emqx.com/en/cloud/latest/rule_engine/introduction.html#general-flow)는 EMQX 메시지 흐름과 디바이스 이벤트를 처리하고 대응하는 규칙을 구성하는 데 사용됩니다. Data Integrations는 명확하고 유연한 &quot;구성 가능한&quot; 아키텍처를 제공할 뿐만 아니라, 개발 과정을 간소화하고 사용 편의성을 높이며 비즈니스 시스템과 EMQX Cloud 간의 결합도를 낮춥니다. 또한 EMQX Cloud의 고유 capability를 맞춤화할 수 있는 뛰어난 인프라도 제공합니다.
...
EMQX Cloud는 널리 사용되는 데이터 시스템과 30개 이상의 네이티브 통합을 제공합니다. ClickHouse도 그중 하나입니다.

<Image img="/images/integrations/data-ingestion/emqx/data_integration_clickhouse.png" size="lg" border alt="EMQX Cloud ClickHouse Data Integration 커넥터 세부 정보" />
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_resource.png" size="lg" border alt="연결 정보가 표시된 EMQX Cloud ClickHouse 리소스 설정 양식" />
...
EMQX는 강력한 [규칙 엔진](https://docs.emqx.com/en/cloud/latest/rule_engine/rules.html)을 제공하며, 이를 통해 원시 MQTT 메시지를 서드파티 시스템으로 전송하기 전에 변환하고 보강할 수 있습니다.
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_1.png" size="md" border alt="SQL 쿼리를 보여주는 EMQX Cloud 데이터 통합 규칙 생성 1단계" />

SQL 테스트를 실행해 결과를 확인할 수 있습니다.

<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_2.png" size="md" border alt="테스트 결과를 보여주는 EMQX Cloud 데이터 통합 규칙 생성 2단계" />
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_action.png" size="md" border alt="SQL 템플릿을 사용한 EMQX Cloud 데이터 통합 규칙 작업 설정" />
...
&quot;Confirm&quot; 및 &quot;View Details&quot;를 클릭하세요. 이제 모든 설정이 정상적으로 완료됩니다. 규칙 세부 정보 페이지에서 데이터 통합이 정상적으로 작동하는지 확인할 수 있습니다.

<Image img="/images/integrations/data-ingestion/emqx/data_integration_details.png" size="md" border alt="구성 요약을 보여주는 EMQX Cloud 데이터 통합 규칙 세부 정보" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-overview.png" size="lg" border alt="클라이언트 인터페이스를 보여주는 MQTTX Overview" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-new.png" size="lg" border alt="연결 세부 정보를 포함한 MQTTX 새 연결 설정 양식" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-publish.png" size="lg" border alt="메시지 작성을 보여주는 MQTTX의 MQTT 메시지 게시 인터페이스" />
...
<Image img="/images/integrations/data-ingestion/emqx/rule_monitor.png" size="lg" border alt="메시지 처리 메트릭을 보여주는 EMQX Cloud 규칙 모니터링 대시보드" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_result.png" size="lg" border alt="저장된 IoT 데이터가 표시된 ClickHouse 쿼리 결과" />
