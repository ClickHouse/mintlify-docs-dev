...
<Image img="/images/integrations/data-ingestion/s3/insert_mechanics.png" size="lg" border alt="ClickHouse의 삽입 블록 크기 메커니즘" />
...
<Image img="/images/integrations/data-ingestion/s3/pull.png" size="lg" border alt="ClickHouse에서 외부 소스의 데이터를 가져오는 방식" />
...
<Image img="/images/integrations/data-ingestion/s3/merges.png" size="lg" border alt="ClickHouse의 백그라운드 머지" />
...
더 큰 파트로 병합된 파트는 [비활성](/ko/reference/system-tables/parts) 상태로 표시되며, [설정 가능한](/ko/reference/settings/merge-tree-settings#old_parts_lifetime) 시간이 몇 분 지난 뒤 최종적으로 삭제됩니다. 시간이 지나면서 병합된 파트로 이루어진 트리가 형성되며(그래서 [`MergeTree`](/ko/reference/engines/table-engines/mergetree-family) 테이블이라는 이름이 붙었습니다).

<div id="insert-parallelism">
  ### 삽입 병렬성
</div>

<Image img="/images/integrations/data-ingestion/s3/resource_usage.png" size="lg" border alt="삽입 병렬성에 따른 리소스 사용량" />
...
<Image img="/images/integrations/data-ingestion/s3/insert_threads.png" size="lg" border alt="ClickHouse의 병렬 삽입 스레드" />
...
<Image img="/images/integrations/data-ingestion/s3/hardware_size.png" size="lg" border alt="하드웨어 크기가 ClickHouse 성능에 미치는 영향" />
...
ClickHouse는 `s3` 함수와 `S3` 엔진을 사용해 [지원되는 포맷](/ko/reference/formats#formats-overview)으로 S3 버킷에 저장된 파일을 읽을 수 있습니다. 원시 파일을 읽는 경우, 이러한 포맷 중 일부는 뚜렷한 장점이 있습니다.
...
<Image img="/images/integrations/data-ingestion/s3/s3Cluster.png" size="lg" border alt="ClickHouse의 s3Cluster 함수" />
