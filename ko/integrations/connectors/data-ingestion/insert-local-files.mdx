...
1. `comments.tsv`라는 TSV 파일에 Hacker News 댓글 일부가 들어 있고, 헤더 행에 컬럼 이름이 포함되어 있다고 가정하겠습니다. 데이터를 삽입할 때는 [입력 형식](/ko/reference/formats)을 지정해야 하며, 이 예시에서는 `TabSeparatedWithNames`를 사용합니다:
...
로컬 운영 체제에 `clickhouse-client`를 설치하는 방법에 대한 자세한 내용은 [`clickhouse-client` 문서 페이지](/ko/concepts/features/interfaces/cli)에서 확인하십시오.