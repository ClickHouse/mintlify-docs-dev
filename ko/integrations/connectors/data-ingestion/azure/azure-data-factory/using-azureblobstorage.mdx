...
keywords: ['azure data factory', 'azure', 'microsoft', '데이터', 'azureBlobStorage']
title: 'ClickHouse의 azureBlobStorage 테이블 함수를 사용해 Azure 데이터를 ClickHouse로 가져오기'
doc_type: 'guide'
integration:
   - support_level: 'core'
...
`TSV`, `ORC`, `Avro` 등을 포함한 널리 사용되는 다양한 파일 포맷을 지원합니다. 전체 목록은 [&quot;데이터 포맷&quot;](/ko/reference/formats)을 참조하십시오.
...
ClickHouse가 Azure Blob Storage에 액세스할 수 있도록 하려면 액세스 키가 포함된 연결 문자열이 필요합니다.

1. Azure Portal에서 **스토리지 계정**으로 이동합니다.

2. 왼쪽 메뉴에서 **Security +
   networking** 섹션 아래의 **Access keys**를 선택합니다.
   <Image img="/images/integrations/data-ingestion/azure-data-factory/azure-data-store-settings.png" size="lg" alt="Azure 데이터 저장소 설정" border />

3. **key1** 또는 **key2** 중 하나를 선택한 다음,
   **연결 문자열** 필드 옆의 **Show** 버튼을 클릭합니다.
   <Image img="/images/integrations/data-ingestion/azure-data-factory/azure-data-store-access-keys.png" size="lg" alt="Azure 데이터 저장소 액세스 키" border />
...
이 문서는 azureBlobStorage 함수를 사용하는 방법을 간단히 소개합니다. 더
고급 옵션과 구성 세부 정보는 공식
문서를 참조하십시오:

* [azureBlobStorage 테이블 함수](/ko/reference/functions/table-functions/azureBlobStorage)
* [입력 및 출력 데이터 포맷](/ko/reference/formats)
