...
<Steps>
  ...
  <Step>
    ...
    <Note>
      Apify는 [Python client](https://docs.apify.com/api/client/python)도 제공합니다. Python을 선호한다면 pip로 `apify-client`를 설치하고, ClickHouse에는 [clickhouse-connect](/ko/integrations/language-clients/python)를 사용하십시오.
    </Note>
  </Step>

  <Step>
    ### ClickHouse에 대상 테이블을 생성하세요
  ...
  <Step>
    ### Apify 데이터셋을 가져와 ClickHouse에 적재하세요
  ...
  <Step>
    ...
    payload 세부 정보와 구성 옵션은 [Apify webhook documentation](https://docs.apify.com/platform/integrations/webhooks)을 참고하십시오.
...
JavaScript client에서 높은 처리량으로 데이터를 삽입하려면 [성능 최적화를 위한 팁](/ko/integrations/language-clients/js#tips-for-performance-optimizations)을 따르십시오. 행을 한 번에 하나씩 삽입하는 대신 여러 행을 묶어 더 큰 단위로 삽입하고, 클라이언트 측 배칭이 실용적이지 않은 경우 [async inserts](/ko/concepts/features/operations/insert/asyncinserts)를 고려하십시오.

<div id="security">
  ### 보안
</div>

이 페이지의 예시는 설명을 단순하게 하기 위해 `default` 사용자와 데이터베이스를 사용합니다. 프로덕션 환경에서는 대상 테이블(target table)에 삽입하는 데 필요한 최소 권한만 가진 전용 사용자를 생성하고, 자격 증명은 안전하게 저장하십시오(예: 소스 코드에 커밋하지 말고 환경 변수나 시크릿 관리 도구에 저장). 자세한 내용은 [Cloud 접근 관리](/ko/products/cloud/guides/security/cloud-access-management)를 참조하십시오.
...
* [ClickHouse JavaScript client](/ko/integrations/language-clients/js)