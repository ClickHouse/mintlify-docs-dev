...
이 가이드는 [문서](/ko/integrations/connectors/data-ingestion/etl-tools/dbt), [기능 및 구성](/ko/integrations/connectors/data-ingestion/etl-tools/dbt/features-and-configurations), [materializations 참고](/ko/integrations/connectors/data-ingestion/etl-tools/dbt/materializations)와 함께 사용할 수 있도록 작성되었습니다.

<div id="setup">
  ## 설정
</div>

환경을 준비하려면 [dbt 및 ClickHouse 어댑터 설정](/ko/integrations/connectors/data-ingestion/etl-tools/dbt) 섹션의 안내를 따르십시오.
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_01.png" size="lg" alt="IMDB 테이블 스키마" />
...
3. 이제 원하는 텍스트 편집기가 필요합니다. 아래 예시에서는 널리 사용되는 VS Code를 사용합니다. IMDB 디렉터리를 열면 yml 파일과 sql 파일이 있는 것을 확인할 수 있습니다.
   ...
   <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_02.png" size="lg" alt="새 dbt 프로젝트" />

4. `dbt_project.yml` 파일을 업데이트하여 첫 번째 model인 `actor_summary`를 지정하고, profile을 `clickhouse_imdb`로 설정합니다.

   <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_03.png" size="lg" alt="dbt 프로필" />

   <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_04.png" size="lg" alt="dbt 프로필" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_05.png" size="lg" alt="dbt 증분 업데이트" />

이 전략은 매우 큰 모델에서는 문제가 될 수 있습니다. 자세한 내용은 [제한 사항](/ko/integrations/connectors/data-ingestion/etl-tools/dbt#limitations)을 참조하십시오.
...
ClickHouse는 그동안 비동기 [뮤테이션](/ko/reference/statements/alter) 형태로만 업데이트와 삭제를 제한적으로 지원해 왔습니다. 이는 IO 사용량이 매우 클 수 있으므로 일반적으로 피하는 것이 좋습니다.
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_06.png" size="lg" alt="경량한 삭제 증분" />
...
dbt는 CSV 파일에서 데이터를 로드하는 기능을 제공합니다. 이 기능은 데이터베이스의 대규모 내보내기 데이터를 로드하는 데는 적합하지 않으며, 일반적으로 코드 테이블과 [dictionaries](/ko/concepts/features/dictionaries)에 사용하는 작은 파일에 더 적합합니다. 예를 들어 국가 코드를 국가 이름에 매핑할 때 사용할 수 있습니다. 간단한 예시로, seed 기능을 사용해 장르 코드 목록을 생성한 다음 업로드해 보겠습니다.
