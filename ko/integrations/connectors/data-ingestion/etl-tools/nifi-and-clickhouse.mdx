...
<Steps>
  ...
  <Step>
    ...
    1. Apache NiFi에서 Controller Service를 구성하려면 &quot;gear&quot; 버튼을 클릭해 NiFi Flow Configuration 페이지로 이동하세요
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_01.png" size="sm" border alt="gear 버튼이 강조 표시된 NiFi Flow Configuration 페이지" />

    2. Controller Services 탭을 선택한 뒤, 오른쪽 상단의 `+` 버튼을 클릭해 새 Controller Service를 추가하세요

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_02.png" size="lg" border alt="추가 버튼이 강조 표시된 Controller Services 탭" />

    3. `DBCPConnectionPool`을 검색한 다음 &quot;Add&quot; 버튼을 클릭하세요

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_03.png" size="lg" border alt="DBCPConnectionPool이 강조 표시된 Controller Service 선택 대화 상자" />

    4. 새로 추가한 `DBCPConnectionPool`은 기본적으로 Invalid 상태입니다. 구성을 시작하려면 &quot;gear&quot; 버튼을 클릭하세요

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_04.png" size="lg" border alt="gear 버튼이 강조 표시된 Invalid 상태의 DBCPConnectionPool이 표시된 Controller Services 목록" />
    ...
    6. &quot;Settings&quot; 섹션에서 나중에 쉽게 식별할 수 있도록 Controller Service 이름을 &quot;ClickHouse JDBC&quot;로 변경하세요
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_05.png" size="lg" border alt="속성이 입력된 DBCPConnectionPool 구성 대화 상자" />

    7. &quot;lightning&quot; 버튼을 클릭한 다음 &quot;Enable&quot; 버튼을 클릭해 `DBCPConnectionPool` Controller Service를 활성화하세요

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_06.png" size="lg" border alt="lightning 버튼이 강조 표시된 Controller Services 목록" />

       <br />

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_07.png" size="lg" border alt="Controller Service 활성화 확인 대화 상자" />

    8. Controller Services 탭에서 해당 Controller Service가 활성화되었는지 확인하세요

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_08.png" size="lg" border alt="활성화된 ClickHouse JDBC 서비스가 표시된 Controller Services 목록" />
  </Step>
  ...
  <Step>
    ...
    1. 적절한 업스트림 및 다운스트림 프로세서와 함께 `ExecuteSQL` 프로세서를 추가합니다
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_09.png" size="md" border alt="워크플로에 ExecuteSQL 프로세서가 표시된 NiFi 캔버스" />
    ...
    3. `ExecuteSQL` 프로세서를 시작합니다
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_10.png" size="lg" border alt="속성이 입력된 ExecuteSQL 프로세서 구성" />

    4. 쿼리가 성공적으로 처리되었는지 확인하려면 출력 큐에 있는 `FlowFile` 중 하나를 확인합니다

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_11.png" size="lg" border alt="검사할 준비가 된 FlowFile이 표시된 큐 목록 대화상자" />

    5. 출력 `FlowFile`의 결과를 보려면 보기를 &quot;formatted&quot;로 전환합니다

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_12.png" size="lg" border alt="formatted 보기에서 쿼리 결과를 표시하는 FlowFile 콘텐츠 뷰어" />
  </Step>
  ...
  <Step>
    ...
    3. 여러 레코드가 하나로 머지되었는지 확인하려면 `MergeRecord` 프로세서의 입력과 출력을 확인합니다. 출력은 여러 입력 레코드로 이루어진 배열이라는 점에 유의하십시오.
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_13.png" size="sm" border alt="단일 레코드를 보여주는 MergeRecord 프로세서 입력" />

       출력

       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_14.png" size="sm" border alt="머지된 레코드 배열을 보여주는 MergeRecord 프로세서 출력" />
    ...
    5. 각 삽입에 여러 행이 포함되는지 확인하려면 테이블의 행 수가 `MergeRecord`에서 정의한 &quot;Minimum Number of Records&quot; 값 이상씩 증가하는지 확인합니다.
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/nifi_15.png" size="sm" border alt="대상 테이블의 행 수를 보여주는 쿼리 결과" />
