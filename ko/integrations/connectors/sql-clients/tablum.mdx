---
sidebarTitle: 'TABLUM.IO'
slug: /integrations/tablumio
description: 'TABLUM.IO는 ClickHouse를 기본적으로 지원하는 데이터 관리 SaaS입니다.'
title: 'ClickHouse에 TABLUM.IO 연결하기'
doc_type: 'guide'
keywords: ['tablum', 'SQL 클라이언트', '데이터베이스 도구', '쿼리 도구', '데스크톱 앱']
integration:
  - support_level: 'community'
  - category: 'sql_client'
---

import { Image } from "/snippets/components/Image.jsx";

import CommunityMaintainedBadge from "/snippets/components/CommunityMaintainedBadge/CommunityMaintainedBadge.jsx";

<CommunityMaintainedBadge />

<div id="open-the-tablumio-startup-page">
  ## TABLUM.IO 시작 페이지 열기
</div>

<Note>
  Linux 서버에서 Docker로 TABLUM.IO의 자체 호스팅 버전을 설치할 수 있습니다.
</Note>

<div id="1-sign-up-or-sign-in-to-the-service">
  ## 1. 서비스에 가입하거나 로그인합니다
</div>

먼저 이메일로 TABLUM.IO에 가입하거나 Google 또는 Facebook 계정으로 간편하게 로그인합니다.

<Image img="/images/integrations/sql-clients/tablum-ch-0.png" size="md" border alt="TABLUM.IO 로그인 페이지" />

<div id="2-add-a-clickhouse-connector">
  ## 2. ClickHouse 커넥터 추가
</div>

ClickHouse 연결 정보를 준비한 다음 **Connector** 탭으로 이동하여 호스트 URL, 포트, 사용자 이름, 비밀번호, 데이터베이스 이름, 커넥터 이름을 입력합니다. 이 필드를 모두 입력한 후 **Test connection** 버튼을 클릭해 정보를 확인하고, 이어서 **Save connector for me**를 클릭해 저장합니다.

<Tip>
  연결 정보에 맞는 올바른 **HTTP** 포트를 지정하고, **SSL** 모드도 그에 맞게 설정했는지 확인하세요.
</Tip>

<Tip>
  일반적으로 TLS를 사용하는 경우 포트는 8443이고, TLS를 사용하지 않는 경우 8123입니다.
</Tip>

<Image img="/images/integrations/sql-clients/tablum-ch-1.png" size="lg" border alt="TABLUM.IO에서 ClickHouse 커넥터 추가" />

<div id="3-select-the-connector">
  ## 3. 커넥터 선택
</div>

**데이터셋** 탭으로 이동한 다음, 드롭다운에서 최근에 생성한 ClickHouse 커넥터를 선택하세요. 오른쪽 패널에 사용 가능한 테이블과 스키마 목록이 표시됩니다.

<Image img="/images/integrations/sql-clients/tablum-ch-2.png" size="lg" border alt="TABLUM.IO에서 ClickHouse 커넥터 선택" />

<div id="4-input-a-sql-query-and-run-it">
  ## 4. SQL 쿼리를 입력하고 실행하세요
</div>

SQL 콘솔에 쿼리를 입력한 다음 **Run Query**를 클릭하세요. 결과는 스프레드시트 형태로 표시됩니다.

<Tip>
  컬럼 이름을 마우스 오른쪽 버튼으로 클릭하면 정렬, 필터 및 기타 작업이 있는 드롭다운 메뉴가 열립니다.
</Tip>

<Image img="/images/integrations/sql-clients/tablum-ch-3.png" size="lg" border alt="TABLUM.IO에서 SQL 쿼리 실행" />

<Note>
  TABLUM.IO를 사용하면 다음 작업을 수행할 수 있습니다.

  * TABLUM.IO 계정에서 여러 ClickHouse 커넥터를 생성하고 사용할 수 있습니다.
  * 데이터 소스와 관계없이 로드된 모든 데이터에 대해 쿼리를 실행할 수 있습니다.
  * 결과를 새 ClickHouse 데이터베이스로 공유할 수 있습니다.
</Note>

<div id="learn-more">
  ## 더 알아보기
</div>

TABLUM.IO에 대한 자세한 내용은 https://tablum.io에서 확인할 수 있습니다.