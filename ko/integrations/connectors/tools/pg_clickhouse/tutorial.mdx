...
이 튜토리얼은 [ClickHouse 튜토리얼]을 기반으로 하지만, 모든 쿼리는
...
먼저 ClickHouse 데이터베이스가 아직 없다면 생성하십시오. 빠르게 시작하는 방법 중 하나는 Docker 이미지를 사용하는 것입니다:
...
[ClickHouse 튜토리얼]을 참고하여 New York
City 택시 데이터셋으로 간단한 데이터베이스를 생성하겠습니다:
...
[PGXN] 또는 [GitHub]에서 pg&#95;clickhouse를 빌드하고 설치하세요. 또는
[pg&#95;clickhouse image]를 사용해 Docker 컨테이너를 실행할 수 있습니다. 이 이미지는
Docker [Postgres image]에 pg&#95;clickhouse를 추가한 것입니다:
...
다음으로, PostgreSQL 사용자를 ClickHouse 사용자에 매핑합니다. 가장 간단한 방법은 현재 PostgreSQL 사용자를 foreign
...
이제 테이블(table)이 임포트되어 있어야 합니다: [psql]에서 `\det+`를 사용해 확인하세요:
...
쿼리가 얼마나 빠르게 실행되었는지 확인해 보십시오. pg&#95;clickhouse는 `COUNT()` 집계를 포함한 전체
쿼리를 푸시다운하므로 ClickHouse에서 실행되고 Postgres에는 단일 행만
반환합니다. 이를 보려면 [EXPLAIN]을 사용하십시오:
...
&quot;Foreign Scan&quot;이 계획의 최상위에 표시된다는 점에 유의하십시오. 이는
...
몇 가지 쿼리를 실행해 데이터를 분석해 보십시오. 다음 예시를 살펴보거나
직접 SQL 쿼리를 시도해 보십시오.
...
테이블과 딕셔너리는 뉴욕시의 각 지역별로 하나의 행을 포함하는 CSV 파일을
기반으로 합니다.
...
1. 계속해서 Postgres에서 `clickhouse_raw_query` 함수를 사용해
   `taxi_zone_dictionary`라는 ClickHouse [딕셔너리]를 생성하고
   S3의 CSV 파일에서 딕셔너리 데이터를 채우십시오.
...
이 쿼리는 도착지가 LaGuardia 또는 JFK 공항인 택시 운행 건수를 자치구별로
합산합니다. 승차 지역을 알 수 없는 운행도 꽤 많다는 점에
유의하십시오.
...
2. 이 쿼리는 팁 금액이 가장 높은 상위 1000건의 운행에 해당하는 행을 반환한 다음,
...
[ClickHouse tutorial]: /get-started/quickstarts/tutorial "ClickHouse 고급 튜토리얼"
