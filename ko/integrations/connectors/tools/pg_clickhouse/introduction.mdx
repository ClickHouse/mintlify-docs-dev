---
sidebarTitle: '소개'
description: 'SQL을 다시 작성할 필요 없이 PostgreSQL에서 바로 ClickHouse 분석 쿼리를 실행하세요'
...
오픈소스 PostgreSQL 확장인 [pg&#95;clickhouse]는 SQL을 다시 작성할 필요 없이
...
시작하면 pg&#95;clickhouse를 사용해 PostgreSQL 스키마로 [외부 테이블 가져오기]를
...
pg&#95;clickhouse를 가장 간단하게 사용해 보는 방법은 [Docker 이미지]를 이용하는 것으로, 여기에는
pg&#95;clickhouse와 [re2] 확장이 포함된 표준 PostgreSQL Docker 이미지가 들어 있습니다:
...
이 표는 일반 PostgreSQL
테이블과 ClickHouse에 연결된 pg&#95;clickhouse의 [TPC-H] 쿼리 성능을 비교한 것입니다.
두 환경 모두 스케일링
...
{/* XXX DSO는 현재 비활성화되어 있습니다.
  기본적으로 `make`는 `clickhouse-cpp` 라이브러리를 동적으로 링크합니다(macOS는
  예외이며, macOS에서는 동적 `clickhouse-cpp` 라이브러리가 아직 지원되지
  않습니다). ClickHouse 라이브러리를 `pg_clickhouse`에 정적으로 컴파일하려면
  `CH_BUILD=static`을 지정하세요:
