...
<Steps>
  ...
  <Step title="모델 선택">
    ...
    <Image img="/images/agentic-data-stack/select-model.png" alt="왼쪽 상단에 있는 LibreChat 모델 셀렉터에 기본 모델과 「Select a model」 툴팁이 표시된 모습" size="md" />
    ...
    <Accordion title="UI에서 provider 키 설정">
      ...
      <Image img="/images/agentic-data-stack/set-api-key.png" alt="Anthropic provider 옆에 Set API Key 버튼이 표시된 LibreChat 모델 셀렉터" size="md" />

      대화 상자에 키를 붙여넣고 **Submit**을 클릭하십시오. 만료 시간을 설정하거나, 키가 만료되지 않도록 유지할 수 있습니다.

      <Image img="/images/agentic-data-stack/set-api-key-modal.png" alt="만료 드롭다운, Key 필드, Submit 및 Revoke 버튼이 있는 Anthropic용 Set API Key 대화 상자" size="md" />
    </Accordion>
  </Step>
  ...
  <Step title="MCP 서버 선택">
    ...
    <Image img="/images/agentic-data-stack/select-clickhouse-local.png" alt="LibreChat 컴포저에서 MCP Servers 메뉴가 열려 있으며, 녹색 연결 점이 있는 ClickHouse-Local과 ClickHouse-Cloud가 표시된 모습" size="md" />
  </Step>
  ...
  <Step title="첫 번째 질문하기">
    ...
    <Image img="/images/agentic-data-stack/prompt-chat.png" alt="ClickHouse-Local MCP 도구를 호출해 예시 질문에 답변하고, 사용 가능한 데이터베이스와 테이블을 나열하는 LibreChat" size="md" />
  </Step>
</Steps>
