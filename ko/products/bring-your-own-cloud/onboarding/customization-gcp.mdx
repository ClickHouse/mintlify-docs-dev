...
<Steps>
  <Step>
    ...
    <Image img="/images/cloud/reference/byoc-gcp-subnet.png" size="lg" alt="Private Google Access가 활성화된 기본 및 보조 IPv4 범위를 보여주는 BYOC GCP 서브넷 세부 정보" />
  </Step>
  ...
  <Step>
    ...
    **DNS Resolution**
    VPC에서 DNS 확인이 정상적으로 작동하고 있으며, 표준 DNS 이름을 차단, 방해 또는 덮어쓰지 않는지 확인하십시오. ClickHouse BYOC는 DNS를 사용해 Tailscale 컨트롤 서버와 ClickHouse 서비스 엔드포인트를 확인합니다. DNS를 사용할 수 없거나 구성이 잘못된 경우 BYOC 서비스가 연결되지 않거나 제대로 작동하지 않을 수 있습니다.
  </Step>
  ...
  <Step>
    ...
    * (선택 사항) ClickHouse 전용 보조 IPv4 범위 이름. 이는 프라이빗 서브넷에 여러 보조 IPv4 범위가 있고, 그중 모든 범위를 ClickHouse용으로 사용할 예정이 아닌 경우에만 필요합니다.
