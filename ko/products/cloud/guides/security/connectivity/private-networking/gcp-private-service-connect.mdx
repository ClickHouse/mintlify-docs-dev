...
description: '이 문서에서는 Google Cloud Platform(GCP) Private Service Connect(PSC)를 사용해 ClickHouse Cloud에 연결하는 방법과, ClickHouse Cloud IP 액세스 목록을 사용해 GCP PSC 주소 이외의 주소에서 ClickHouse Cloud 서비스로의 액세스를 비활성화하는 방법을 설명합니다.'
...
<Image img="/images/cloud/security/gcp-psc-overview.png" size="lg" alt="Private Service Connect 개요" border />
...
<Image img="/images/cloud/security/gcp-psc-open.png" size="lg" alt="Google Cloud console에서 Private Service Connect 열기" border />
...
<Image img="/images/cloud/security/gcp-psc-enable-global-access.png" size="md" alt="Private Service Connect에 대해 Global Access 활성화" border />

PSC Endpoint를 생성하려면 **ADD ENDPOINT** 버튼을 사용합니다.

연결이 승인되면 **Status** 컬럼이 **Pending**에서 **Accepted**로 변경됩니다.

<Image img="/images/cloud/security/gcp-psc-copy-connection-id.png" size="lg" alt="PSC Connection ID 복사" border />
...
<Image img="/images/cloud/security/gcp-pe-remove-private-endpoint.png" size="lg" alt="ClickHouse Cloud에서 Private Endpoint 제거" border />
...
<Image img="/images/cloud/security/gcp-privatelink-pe-filters.png" size="lg" alt="Private Endpoints 필터" border />
...
ClickHouse Cloud에서 [MySQL](/ko/reference/functions/table-functions/mysql) 또는 [PostgreSQL](/ko/reference/functions/table-functions/postgresql) 테이블 함수를 사용해 GCP에 호스팅된 데이터베이스에 연결하려는 경우를 가정해 보겠습니다. GCP PSC는 이 연결을 안전하게 설정하는 용도로는 사용할 수 없습니다. PSC는 단방향 연결이기 때문입니다. 즉, 내부 네트워크 또는 GCP VPC에서 ClickHouse Cloud로 안전하게 연결할 수는 있지만, ClickHouse Cloud에서 내부 네트워크로 연결할 수는 없습니다.
...
이렇게 하려면 ClickHouse Cloud에서 내부/프라이빗 데이터베이스 서비스로의 연결을 허용하도록 GCP VPC 방화벽 규칙을 구성하십시오. [ClickHouse Cloud 리전의 기본 egress IP 주소](/ko/products/cloud/guides/data-sources/cloud-endpoints-api)와 [사용 가능한 고정 IP 주소](https://api.clickhouse.cloud/static-ips.json)도 함께 확인하십시오.
