...
**Endpoint services that use NLBs and GWLBs**를 선택한 다음, **Service Name** 필드에 [Endpoint &quot;Service name&quot; 가져오기 ](#obtain-endpoint-service-info) 단계에서 얻은 `Service name`<sup>console</sup> 또는 `endpointServiceId`<sup>API</sup>를 입력하세요. 그런 다음 **Verify service**를 클릭하세요.

<Image img="/images/cloud/security/aws-privatelink-endpoint-settings.png" size="md" alt="AWS PrivateLink 엔드포인트 설정" border />
...
<Image img="/images/cloud/security/aws-privatelink-select-vpc-and-subnets.png" size="md" alt="VPC 및 서브넷 선택" border />

선택 사항으로 보안 그룹/태그를 할당하세요.
...
<Image img="/images/cloud/security/aws-privatelink-vpc-endpoint-id.png" size="md" alt="VPC 엔드포인트 ID" border />
...
<Image img="/images/cloud/security/aws-privatelink-pe-filters.png" size="md" alt="Private Endpoints 필터" border />
...
<Image img="/images/cloud/security/aws-privatelink-pe-dns-name.png" size="md" alt="Private Endpoint DNS 이름" border />
...
ClickHouse Cloud에서 [MySQL](/ko/reference/functions/table-functions/mysql) 또는 [PostgreSQL](/ko/reference/functions/table-functions/postgresql) 테이블 함수를 사용해 Amazon Web Services (AWS) VPC에 호스팅된 데이터베이스에 연결하려는 상황을 가정해 보겠습니다. 이 연결을 안전하게 설정하는 데는 AWS PrivateLink를 사용할 수 없습니다. PrivateLink는 단방향 연결입니다. 내부 네트워크 또는 Amazon VPC에서 ClickHouse Cloud로는 안전하게 연결할 수 있지만, ClickHouse Cloud에서 내부 네트워크로 연결할 수는 없습니다.

[AWS PrivateLink documentation](https://docs.aws.amazon.com/whitepapers/latest/building-scalable-secure-multi-vpc-network-infrastructure/aws-privatelink.html)에는 다음과 같이 설명되어 있습니다.
...
이렇게 하려면 AWS Security Group을 구성하여 ClickHouse Cloud에서 내부 프라이빗 데이터베이스 서비스로의 연결을 허용하십시오. [ClickHouse Cloud 리전의 기본 egress IP 주소](/ko/products/cloud/guides/data-sources/cloud-endpoints-api)와 [사용 가능한 고정 IP 주소](https://api.clickhouse.cloud/static-ips.json)를 함께 확인하십시오.