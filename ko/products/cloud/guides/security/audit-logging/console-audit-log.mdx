---
sidebarTitle: '콘솔 감사 로그'
slug: /cloud/security/audit-logging/console-audit-log
title: '콘솔 감사 로그'
description: '이 페이지에서는 클라우드 감사 로그를 검토하는 방법을 설명합니다.'
doc_type: 'guide'
keywords: ['감사 로그']
---

import { Image } from "/snippets/components/Image.jsx";

<div id="console-audit-log">
  # 콘솔 감사 로그
</div>

콘솔 활동은 감사 로그에 기록됩니다. 조직 역할이 관리자 또는 개발자인 경우 로그를 검토하고 로깅 시스템과 통합할 수 있습니다.

<div id="console-audit-log-ui">
  ## 사용자 인터페이스를 통해 콘솔 감사 로그에 액세스하기
</div>

<Steps>
  <Step>
    ## 조직 선택

    ClickHouse Cloud에서 조직 세부 정보로 이동합니다.

    <Image img="/images/cloud/security/activity_log1.png" size="md" alt="ClickHouse Cloud 활동 탭" border />

    <br />
  </Step>

  <Step>
    ## Audit 선택

    왼쪽 메뉴에서 **Audit** 탭을 선택하면 ClickHouse Cloud 조직에서 어떤 변경이 이루어졌는지, 누가 변경했는지, 그리고 언제 발생했는지 확인할 수 있습니다.

    **Activity** 페이지에는 조직에 대해 기록된 이벤트 목록이 테이블로 표시됩니다. 기본적으로 이 목록은 최신순(가장 최근 이벤트가 맨 위)으로 정렬됩니다. 컬럼 헤더를 클릭하여 테이블의 정렬 순서를 변경할 수 있습니다. 테이블의 각 항목에는 다음 필드가 포함됩니다.

    * **Activity:** 이벤트를 설명하는 짧은 텍스트
    * **User:** 이벤트를 시작한 사용자
    * **IP Address:** 해당하는 경우, 이 필드에는 이벤트를 시작한 사용자의 IP Address가 표시됩니다
    * **Time:** 이벤트의 타임스탬프

    <Image img="/images/cloud/security/activity_log2.png" size="md" alt="ClickHouse Cloud 활동 테이블" border />

    <br />
  </Step>

  <Step>
    ## 검색 창 사용

    제공된 검색 창을 사용하여 서비스 이름 또는 IP 주소와 같은 기준에 따라 이벤트를 추려낼 수 있습니다. 또한 이 정보를 CSV 형식으로 내보내 외부 도구에서 배포하거나 분석할 수 있습니다.
  </Step>
</Steps>

<div class="eighty-percent">
  <Image img="/images/cloud/security/activity_log3.png" size="lg" alt="ClickHouse Cloud 활동 CSV 내보내기" border />
</div>

<div id="console-audit-log-api">
  ## API를 통해 콘솔 감사 로그에 액세스하기
</div>

ClickHouse Cloud API의 `activity` 엔드포인트를 사용하여 감사 이벤트 내보내기 데이터를 가져올 수 있습니다. 자세한 내용은 [API 참조](/ko/api-reference/organization/get-list-of-available-organizations)에서 확인할 수 있습니다.

<div id="log-integrations">
  ## 로그 통합
</div>

API를 사용하여 원하는 로깅 플랫폼과 통합할 수 있습니다. 다음은 기본으로 지원되는 커넥터입니다:

* [Splunk용 ClickHouse Cloud Audit 추가 기능](/ko/integrations/connectors/data-integrations/integrations/splunk)