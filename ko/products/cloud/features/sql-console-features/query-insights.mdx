...
description: 'system.query_log 데이터를 시각화해 쿼리 디버깅과 성능 최적화를 더 쉽게 수행할 수 있습니다'
...
<Image img="/images/cloud/sqlconsole/insights_latency.png" size="md" alt="쿼리 인사이트 UI 지연 시간 차트" border />

<div id="recent-queries">
  ## 최근 쿼리
</div>

상위 메트릭 아래에는 선택한 시간 범위 동안의 쿼리 로그 항목(정규화된 쿼리 해시와 사용자별로 그룹화됨)을 표시하는 테이블이 있습니다:

<Image img="/images/cloud/sqlconsole/insights_recent.png" size="md" alt="쿼리 인사이트 UI 최근 쿼리 테이블" border />
...
<Image img="/images/cloud/sqlconsole/insights_drilldown.png" size="md" alt="Query Insights UI 쿼리 드릴다운" border />

플라이아웃에서 확인할 수 있듯이, 이 쿼리는 지난 24시간 동안 3000회 이상 실행되었습니다. **쿼리 정보** 탭의 모든 메트릭은 집계 메트릭이지만, **쿼리 이력** 탭을 선택하면 개별 실행의 메트릭도 확인할 수 있습니다:

<Image img="/images/cloud/sqlconsole/insights_query_info.png" size="sm" alt="Query Insights UI 쿼리 정보" border />
