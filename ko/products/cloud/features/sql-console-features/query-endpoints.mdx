---
sidebar_title: 'Query API endpoint'
slug: /cloud/features/query-api-endpoints
description: '저장된 쿼리에서 REST API 엔드포인트를 손쉽게 생성합니다'
keywords: ['api', 'query api endpoints', 'query endpoints', 'query rest api']
title: 'Query API endpoint'
doc_type: 'guide'
---

import { Image } from "/snippets/components/Image.jsx";

<div id="query-api-endpoints">
  # Query API 엔드포인트
</div>

대화형 데이터 기반 애플리케이션을 구축하려면 빠른 데이터베이스뿐만 아니라, 구조가 잘 잡힌 데이터와 최적화된 쿼리도 필요합니다.
프런트엔드와 마이크로서비스도 이러한 쿼리가 반환하는 데이터를 쉽게 활용할 수 있어야 하며, 가능하면 구조화가 잘 된 API를 통해 활용하는 것이 좋습니다.

**Query API endpoint** 기능을 사용하면 ClickHouse Cloud 콘솔에 저장된 모든 SQL 쿼리에서 API 엔드포인트를 직접 생성할 수 있습니다.
네이티브 드라이버로 ClickHouse Cloud 서비스에 연결하지 않고도 HTTP를 통해 API 엔드포인트에 접근하여 저장된 쿼리를 실행할 수 있습니다.

<div id="ip-access-control">
  ## IP 액세스 제어
</div>

Query API 엔드포인트는 API Key 수준의 IP 허용 목록을 따릅니다. SQL 콘솔과 마찬가지로 Query API 엔드포인트는 ClickHouse 인프라 내부에서 요청을 프록시하므로 서비스 수준의 IP 허용 목록 설정은 적용되지 않습니다.

Query API 엔드포인트를 호출할 수 있는 클라이언트를 제한하려면 다음 단계를 수행하세요.

<Steps>
  <Step>
    #### API Key 설정 열기

    1. ClickHouse Cloud 콘솔에서 **Organization** → **API Keys**로 이동합니다

    <Image img="/images/cloud/guides/query-endpoints/console-api-keys.png" size="md" alt="API Keys" />

    2. Query API 엔드포인트에 사용하는 API Key 옆의 **Edit**를 클릭합니다

    <Image img="/images/cloud/guides/query-endpoints/api-key-edit.png" size="md" alt="Edit" />
  </Step>

  <Step>
    #### 허용할 IP 주소 추가

    1. **Allow access to this API Key** 섹션에서 **Specific locations**를 선택합니다
    2. IP 주소 또는 CIDR 범위(예: `203.0.113.1` 또는 `203.0.113.0/24`)를 입력합니다
    3. 필요에 따라 여러 항목을 추가합니다

    <Image img="/images/cloud/guides/query-endpoints/specific-locations.png" size="md" alt="Specific locations" />

    Query API 엔드포인트를 생성하려면 관리자 Console Role과 적절한 권한이 부여된 API Key가 필요합니다.
  </Step>
</Steps>

<Tip>
  **가이드**

  몇 단계만으로 Query API 엔드포인트를 설정하는 방법은 [Query API endpoint guide](/ko/products/cloud/guides/sql-console/query-endpoints)를 참조하십시오.
</Tip>