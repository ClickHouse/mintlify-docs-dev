...
* **Special variables** - **Special variables** 버튼을 클릭해 플레이스홀더를 삽입하거나 `{{name}}` 형식의 마커를 직접 입력할 수 있습니다. 선택기에서는 삽입 전에 값을 입력하라는 메시지가 표시됩니다.
* **Category**, **Description**, **Command** (선택 사항) - 라이브러리 정리, 선택기 미리보기 텍스트, 빠른 실행 바로가기 설정에 사용됩니다.

그런 다음 오른쪽 아래의 **Create Prompt**를 클릭하십시오.

<Image img="/images/cloud/agent-builder/prompts/create-prompt.png" alt="왼쪽에서 + 버튼이 강조 표시된 Prompts 패널과 오른쪽에서 Prompt Name, Text, Category, Special variables, Description, Command 필드 및 Create Prompt 버튼이 표시된 Create Prompt 양식" size="lg" />
...
<Image img="/images/cloud/agent-builder/prompts/preview.png" alt="Prompts 패널에서 프롬프트가 선택되어 있고, 오른쪽에 세부 정보가 표시되며, Preview 및 Edit 옵션이 있는 컨텍스트 메뉴가 열린 모습" size="lg" />

미리 보기에는 프롬프트 텍스트와 작성자, 날짜가 표시됩니다. **Use Prompt**를 클릭하면 본문이 컴포저에 삽입됩니다. 프롬프트에 변수가 있으면 먼저 값을 입력하세요.

<Image img="/images/cloud/agent-builder/prompts/use-prompt-modal.png" alt="프롬프트 제목, 작성자, 날짜, 본문 텍스트와 Use Prompt 버튼이 표시된 프롬프트 미리 보기 모달" size="md" />
