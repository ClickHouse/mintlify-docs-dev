...
메모리는 에이전트가 여러 대화에 걸쳐 사용자별로 기억해 둘 수 있는 저장소입니다. 여러 항목으로 구성되며, 각 항목은 키-값 쌍입니다. 예를 들어 선호하는
날짜 포맷, 주로 쿼리하는 데이터베이스, 응답이 얼마나 간결하길 원하는지 같은 정보가 여기에 해당합니다. 에이전트는 이러한 정보를 필요할 때 컨텍스트에 가져와 활용합니다.
...
<Image img="/images/cloud/agent-builder/memory/memories.png" alt="왼쪽 탐색 메뉴에서 뇌 아이콘이 강조 표시된 Memories 패널로, 필터 입력란, Add 버튼, 메모리 사용 체크박스, 편집 및 삭제 컨트롤이 있는 메모리 항목, 그리고 Admin Settings 버튼이 표시되어 있습니다" size="sm" />
...
<Image img="/images/cloud/agent-builder/memory/create.png" alt="메모리 패널에서 + 버튼이 강조 표시된 Create Memory 화면" size="sm" />
...
<Image img="/images/cloud/agent-builder/memory/filter.png" alt="메모리 패널에서 **메모리 필터링** 입력란이 강조 표시되어 있고 'demo'가 입력되어 있음" size="sm" />

<div id="edit-memory">
  ### 메모리 편집
</div>

메모리 항목의 연필 아이콘을 클릭하여 **메모리 편집** 대화상자를 여십시오. 키 또는 값을 수정한 후 **저장**을 클릭하십시오.

<Image img="/images/cloud/agent-builder/memory/edit.png" alt="메모리 항목에서 메모리 편집 연필 아이콘이 강조 표시된 모습" size="sm" />
...
<Image img="/images/cloud/agent-builder/memory/delete.png" alt="Delete Memory 휴지통 아이콘이 강조 표시된 메모리 항목" size="sm" />

<div id="toggle-memory">
  ## 메모리 켜기/끄기
</div>

메모리 패널 상단의 **메모리 사용** 체크박스로 메모리를 켜거나 끌 수 있습니다. 저장하지 않으려는 민감한 주제이거나, 개인화가 도움이 되지 않는 일회성 대화의 경우 비활성화하세요.

메모리가 꺼져 있으면 에이전트는 메모리 저장소를 읽거나 쓰지 않습니다.

<Image img="/images/cloud/agent-builder/memory/toggle.png" alt="상단의 메모리 사용 체크박스가 강조 표시된 메모리 패널" size="sm" />
