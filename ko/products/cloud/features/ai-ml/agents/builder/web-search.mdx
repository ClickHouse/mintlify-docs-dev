...
Agent Builder의 **Capabilities** 섹션에서 **웹 검색**을 활성화하세요. 활성화하면 사용자 질문과 에이전트 지침에 따라 에이전트가 검색을 수행할 시점을 결정합니다. 검색이 실행되고 결과가 스크레이프되면, 가장 관련성 높은 콘텐츠가 다시 모델 컨텍스트로 전달됩니다.

<Image img="/images/cloud/agent-builder/web-search/web-search.png" alt="웹 검색 체크박스가 표시된 웹 검색 섹션이 강조된 Capabilities 패널" size="sm" />
