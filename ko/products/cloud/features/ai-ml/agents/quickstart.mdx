...
<Steps>
  <Step>
    ...
    <Image img="/images/cloud/agent-builder/launch-ch-agents.png" alt="ClickHouse agents(베타)가 선택된 Cloud service 탐색 화면으로, Launch ClickHouse agents 버튼이 있는 런치패드를 보여줍니다" size="lg" />
  </Step>
  ...
  <Step>
    ...
    * **Model** - 드롭다운에서 모델을 선택하십시오. 샘플링 온도와 기타 생성 설정은 [model parameters](/ko/products/cloud/features/ai-ml/agents/builder/model-parameters)에서 조정하십시오.

    <Image img="/images/cloud/agent-builder/agent-builder.png" alt="Create New Agent 드롭다운, 양식 필드(Name, Description, Category, Instructions, Model), 그리고 Capabilities 섹션이 표시된 Agent Builder 패널" size="lg" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/cloud/agent-builder/capabilities.png" alt="Run Code, 웹 검색, File Context, Artifacts, MCP 서버, 스킬 토글이 표시된 Agent Builder 패널의 Capabilities 섹션" size="sm" />
    ...
    <Image img="/images/cloud/agent-builder/tools-button.png" alt="Add Tools 버튼이 강조 표시된 Agent Builder 패널 하단" size="sm" />

    카탈로그를 탐색하려면 **Add Tools**를 클릭하십시오:

    <Image img="/images/cloud/agent-builder/tools-modal.png" alt="Google, OpenAI Image Tools, Wolfram, DALL-E-3, Tavily Search, Calculator, Stable Diffusion 등 서드파티 통합 그리드가 표시된 Agent Tools 모달" size="lg" />
  ...
  <Step>
    ...
    <Image img="/images/cloud/agent-builder/chat-query.png" alt="'What are my top 10 tables by row count this week?'라는 질문과 에이전트의 응답이 표시된 채팅 화면으로, 행 수 기준 서비스 전반의 상위 10개 테이블 순위를 보여주는 Markdown 표와 그 아래 Key Observations가 포함되어 있습니다" size="lg" />
  </Step>
</Steps>
