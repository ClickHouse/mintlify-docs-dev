...
| `versionProbeRevision`  | string                                                                                                 | VersionProbeRevision은 마지막으로 성공한 version probe의 image hash입니다.<br />Deprecated: Keeper version probe Jobs는 사용되지 않으며, 이 필드는 이전 버전과의 호환성을 위해 유지됩니다. | false |     |
