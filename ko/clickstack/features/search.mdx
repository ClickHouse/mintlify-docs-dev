...
* 검색은 기본적으로 완전한 단어 단위로 일치합니다(예: `Error`는 `Error here`에는 일치하지만
  `Errors here`에는 일치하지 않습니다). 단어의 일부만 일치시키려면 단어를 와일드카드로 감쌀 수 있습니다
...
<Image img="/images/use-cases/observability/hyperdx-27.png" alt="검색" size="md" />
...
<Image img="/images/use-cases/observability/clickstack-saved-search-listing.png" alt="저장된 검색 목록 페이지" size="lg" />

검색을 저장하려면 검색 쿼리를 설정한 다음 저장 버튼을 클릭하면 됩니다. 나중에 쉽게 구분할 수 있도록 저장된 검색에 설명적인 이름을 지정할 수 있습니다.

<Image img="/images/use-cases/observability/clickstack-saved-search.png" alt="검색 저장" size="md" />
