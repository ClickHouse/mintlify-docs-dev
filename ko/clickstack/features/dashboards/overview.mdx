...
<Steps>
  ...
  <Step>
    ...
    상단 메뉴에서 `Line/Bar` 시각화 유형을 선택한 다음 `Traces` 데이터셋([play-clickstack.clickhouse.com](https://play-clickstack.clickhouse.com)을 사용하는 경우 `Demo Traces`)을 선택합니다. 다음 값을 입력합니다:
    ...
    * Alias: `Average Time`
    ...
    <Image img="/images/use-cases/observability/hyperdx-visualization-2.png" alt="단순 시각화" size="lg" />
    ...
    <Image img="/images/use-cases/observability/hyperdx-visualization-3.png" alt="단순 시각화 2" size="lg" />
...
<Info>
  **필수 시스템 테이블 액세스**
  ...
  이 대시보드는 주요 메트릭을 표시하기 위해 ClickHouse [시스템 테이블](/ko/reference/system-tables)을 조회합니다. 다음 권한이 필요합니다.
