...
<Accordion title="ClickStack은 매개변수화된 대시보드 또는 대시보드 변수를 지원합니까?">
  ClickStack은 ClickHouse에서 쿨리한 데이터로 채워지는 대시보드용 사용자 지정 드롭다운 필터를 지원합니다. 이러한 필터를 사용하면 대시보드의 모든 타일을 특정 값(예: 서비스 이름, 환경 또는 호스트)에 맞춰 동적으로 한정할 수 있습니다.
...
<Accordion title="어떤 드릴다운 기능을 사용할 수 있습니까?">
  ...
  * 클릭하여 이벤트 보기 — 대시보드 타일의 데이터를 클릭한 뒤 **View Events**를 선택하면 로그 및 트레이스 데이터에 대한 관련 필터가 적용된 [Search](/ko/clickstack/features/search) 페이지로 이동합니다.
  * [사전 구성된 대시보드 드릴다운](/ko/clickstack/features/dashboards/overview#presets) — [Services](/ko/clickstack/features/dashboards/overview#services-dashboard), [ClickHouse](/ko/clickstack/features/dashboards/overview#clickhouse-dashboard), [Kubernetes](/ko/clickstack/features/dashboards/overview#kubernetes-dashboard) 대시보드에는 탭 전반에 걸쳐 더 풍부한 기본 제공 드릴다운 탐색 기능이 포함되어 있습니다.
  ...
  한 사용자 지정 대시보드에서 다른 대시보드로 이어지는 다단계 드릴다운(대시보드 → 대시보드 → 상세 보기)은 현재 지원되지 않습니다.

  <Note>
    **View Events** 드릴다운은 로그 및 트레이스 데이터에서 가장 잘 작동합니다. 메트릭 데이터는 [Search](/ko/clickstack/features/search) 페이지에서 볼 수 없으므로, 메트릭 타일에서 드릴다운하면 대신 선택한 시간대를 기준으로 인접한 로그로 연결됩니다.
  </Note>
</Accordion>
...
<Accordion title="메트릭을 찾아보거나 검색할 수 있는 UI가 있습니까?">
  ![메트릭 속성 탐색기](/images/clickstack/faq/metrics-explorer.png)
...
* [알림](/ko/clickstack/features/alerts) — 검색 알림, 대시보드 차트 알림, 웹훅 통합.
* [대시보드](/ko/clickstack/features/dashboards/overview) — 시각화 생성, 대시보드 구축, 필터 적용.
* [검색](/ko/clickstack/features/search) — Lucene 및 SQL 구문으로 로그와 트레이스를 쿼리.
* [스키마](/ko/clickstack/ingesting-data/schemas) — 로그, 트레이스, 메트릭용 OpenTelemetry 데이터 스키마.
* [아키텍처](/ko/clickstack/architecture) — ClickStack 구성 요소와 각 요소가 어떻게 함께 작동하는지.