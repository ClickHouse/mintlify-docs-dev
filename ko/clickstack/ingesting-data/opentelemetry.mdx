...
<Tabs>
  <Tab title="관리형 ClickStack">
    ...
    * **gRPC (OTLP):** `localhost:4317`
    ...
    OpenTelemetry를 지원하는 [language SDKs](/ko/clickstack/ingesting-data/sdks) 및 telemetry 라이브러리의 경우 애플리케이션에서 `OTEL_EXPORTER_OTLP_ENDPOINT` 환경 변수를 설정하면 됩니다:
  ...
  <Tab title="Open Source ClickStack">
    ...
    * **gRPC (OTLP):** `localhost:4317`
    ...
    OpenTelemetry를 지원하는 [language SDKs](/ko/clickstack/ingesting-data/sdks) 및 텔레메트리 라이브러리의 경우, 애플리케이션에서 `OTEL_EXPORTER_OTLP_ENDPOINT` 환경 변수를 설정하기만 하면 됩니다.
