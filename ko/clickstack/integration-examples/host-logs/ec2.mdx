...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/host-logs/ec2/search-view.png" alt="EC2 로그 Search view" />

    <Image img="/images/clickstack/host-logs/ec2/log-view.png" alt="메타데이터가 표시된 EC2 로그 상세 보기" />
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/host-logs/ec2/search-view-demo.png" alt="EC2 로그 검색 뷰" />

    <Image img="/images/clickstack/host-logs/ec2/log-view-demo.png" alt="메타데이터가 포함된 EC2 로그 상세 보기" />
...
<Steps>
  <Step>
    #### <TrackedLink href={'/ko/examples/host-logs-dashboard.json'} download="host-logs-dashboard.json" eventName="docs.ec2_host_logs_monitoring.dashboard_download">다운로드</TrackedLink> 대시보드 구성 파일
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/import-dashboard.png" alt="대시보드 가져오기 버튼" />

    3. `host-logs-dashboard.json` 파일을 업로드한 다음 **Finish Import**를 클릭합니다

    <Image img="/images/clickstack/host-logs/import-dashboard.png" alt="가져오기 완료" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/host-logs/host-logs-dashboard.png" alt="EC2 로그 대시보드" />
...
이 가이드는 호스트 수준 모니터링에 권장되는 프로덕션 배포 방식으로, OpenTelemetry Collector를 EC2 인스턴스에 직접 설치합니다. 여러 인스턴스에 걸쳐 collector를 관리하려면 구성 관리 도구(Ansible, Chef, Puppet) 또는 Kubernetes 환경의 OpenTelemetry Operator 사용을 고려하십시오. 프로덕션 구성에 대해서는 [OpenTelemetry 데이터 전송](/ko/clickstack/ingesting-data/opentelemetry)을 참조하십시오.