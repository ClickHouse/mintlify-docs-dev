...
<Steps>
  ...
  <Step>
    ## Kubernetes 속성을 애플리케이션으로 전달하기 (권장)
  ...
  <Step>
    ...
    <Image img="/images/use-cases/observability/clickstack-cloud-v2.png" size="lg" alt="ClickStack 시작" border />
    ...
    **검색** 뷰에서 소스를 `Logs`로 전환하고 시간 범위를 **Last 15 minutes**로 설정합니다. 그러면 클러스터 전반의 컨테이너 로그가 몇 초 안에 표시되며, `k8s.namespace.name`, `k8s.pod.name`, `k8s.node.name`과 같은 속성으로 보강되어 나타납니다.
    ...
    <Image img="/images/use-cases/observability/clickstack-search-with-k8-logs.png" size="lg" alt="Kubernetes 로그가 포함된 ClickStack 검색 뷰" />

    인프라 메트릭과 이벤트를 함께 보려면 **Dashboards** -&gt; **Kubernetes**로 이동해 기본 제공 **Kubernetes** 대시보드를 엽니다. `Pods`, `Nodes`, `Namespaces` 탭에 모두 데이터가 표시되어야 합니다.

    <Image img="/images/use-cases/observability/clickstack-dashboard-kubernetes.png" size="lg" alt="ClickStack Kubernetes 대시보드" border />
