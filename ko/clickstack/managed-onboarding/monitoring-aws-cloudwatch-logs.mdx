...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/use-cases/observability/clickstack-cloud-v2.png" size="lg" alt="ClickStack 실행" border />

    **검색** 보기에서 소스를 `Logs`로 전환하고 시간 범위를 **최근 15분**으로 설정합니다. CloudWatch 이벤트는 두어 번의 폴링 주기 내에 표시됩니다.

    <Image img="/images/clickstack/cloudwatch/log-search-view-clickstack.png" size="lg" alt="CloudWatch 로그가 표시된 ClickStack 검색 보기" />
    ...
    * `Body`: 원본 로그 한 줄
    ...
    <Image img="/images/clickstack/cloudwatch/log-search-attributes-clickstack.png" size="lg" alt="CloudWatch 로그와 속성이 표시된 ClickStack 검색 보기" />

    메타데이터를 확인하려면 로그 항목을 선택합니다:

    <Image img="/images/clickstack/cloudwatch/error-log-column-values-clickstack.png" size="lg" alt="로그 상세 보기의 CloudWatch 속성" />
  ...
  <Step>
    ...
    <TrackedLink href={'/ko/examples/cloudwatch-logs-dashboard.json'} download="cloudwatch-logs-dashboard.json" eventName="docs.cloudwatch_logs_monitoring.dashboard_download">`cloudwatch-logs-dashboard.json` 다운로드</TrackedLink>한 다음 ClickStack UI에서 **Dashboards**로 이동해 **Import**를 클릭합니다.

    <Image img="/images/clickstack/clickstack-import-dashboard.png" size="lg" alt="대시보드 가져오기 버튼" />

    JSON 파일을 업로드하고 **Finish Import**를 클릭합니다.

    <Image img="/images/clickstack/cloudwatch/finish-clickstack-import.png" size="lg" alt="가져오기 완료 대화 상자" />
  </Step>
