...
<profiles>
    <default>
        <!-- 사용자/프로필 'default'의 최대 캐시 크기(바이트) -->
        <query_cache_max_size_in_bytes>10000</query_cache_max_size_in_bytes>
        <!-- 사용자/프로필 'default'의 캐시에 저장되는 SELECT 쿼리 결과의 최대 개수 -->
        <query_cache_max_entries>100</query_cache_max_entries>
        <!-- 사용자가 변경할 수 없도록 두 설정을 읽기 전용으로 설정 -->
        <constraints>
...
```sql
SELECT 1 SETTINGS use_query_cache = true; -- query_cache_tag는 암묵적으로 '' (빈 문자열)
SELECT 1 SETTINGS use_query_cache = true, query_cache_tag = 'tag 1';
SELECT 1 SETTINGS use_query_cache = true, query_cache_tag = 'tag 2';
```

쿼리 캐시에서 태그 `tag`가 있는 항목만 제거하려면 `SYSTEM CLEAR QUERY CACHE TAG 'tag'` 문을 사용할 수 있습니다.
