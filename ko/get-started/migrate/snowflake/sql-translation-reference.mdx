...
`Object`(현재는 네이티브 `JSON` 유형으로 대체되어 더 이상 권장되지 않음)와 [`Array`](/ko/reference/data-types/array)
...
ClickHouse는 또한 이름이 있는 [`Tuple`s](/ko/reference/data-types/tuple)와 Tuples의 배열을
[`Nested`](/ko/reference/data-types/nested-data-structures) 유형으로 지원하여,
...
| [`VARIANT`](https://docs.snowflake.com/en/sql-reference/data-types-semistructured#variant)                                                                                                                                                                                                                                                                                                                                                                                      | [`JSON`, `Tuple`, `Nested`](/ko/reference/formats)                                                                                    | ClickHouse에서 `JSON` 타입은 실험적입니다. 이 타입은 삽입 시점에 컬럼 타입을 추론합니다. 대안으로 `Tuple`, `Nested`, `Array`를 사용해 타입을 명시한 구조를 만들 수도 있습니다.                                                                                                                          |   |
| [`OBJECT`](https://docs.snowflake.com/en/sql-reference/data-types-semistructured#object)                                                                                                                                                                                                                                                                                                                                                                                        | [`Tuple`, `Map`, `JSON`](/ko/reference/formats)                                                                                       | `OBJECT`와 `Map`은 모두 키가 `String`인 ClickHouse의 `JSON` 유형과 유사합니다. ClickHouse에서는 값의 유형이 일관되고 엄격하게 지정되어야 하지만, Snowflake는 `VARIANT`를 사용합니다. 즉, 서로 다른 키의 값이 서로 다른 유형일 수 있습니다. ClickHouse에서 이것이 필요하다면 `Tuple`을 사용해 계층 구조를 명시적으로 정의하거나 `JSON` 유형을 사용하십시오. |   |
| [`ARRAY`](https://docs.snowflake.com/en/sql-reference/data-types-semistructured#array)                                                                                                                                                                                                                                                                                                                                                                                          | [`Array`](/ko/reference/data-types/array), [`Nested`](/ko/reference/data-types/nested-data-structures)                                   | Snowflake의 `ARRAY`는 요소에 상위 타입인 `VARIANT`를 사용합니다. 반면 ClickHouse에서는 이들이 강한 타입으로 지정됩니다.                                                                                                                                                             |   |
