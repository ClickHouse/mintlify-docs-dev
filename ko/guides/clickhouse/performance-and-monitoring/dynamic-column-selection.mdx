...
[동적 컬럼 선택](/ko/reference/statements/select#dynamic-column-selection)은 각 컬럼을 하나씩 지정하지 않고 정규식을 사용해 컬럼을 선택할 수 있는 강력하지만 잘 활용되지 않는 ClickHouse 기능입니다. 또한 `APPLY` 수정자를 사용하면 일치하는 컬럼에 함수를 적용할 수 있어 데이터 분석 및 변환 작업에 매우 유용합니다.
...
[`APPLY`](/ko/reference/statements/select) 수정자를 사용하면 모든 컬럼에 함수를 적용할 수도 있습니다.
...
지금까지는 잘 따라왔습니다. 하지만 값 하나만 조정하고 나머지는 그대로 두고 싶을 수 있습니다. 예를 들어 총금액은 2배로 늘리고 MTA tax는 1.1로 나누려는 경우입니다. 이때는 [`REPLACE`](/ko/reference/statements/select) 수정자를 사용하면 됩니다. 이 수정자는 다른 컬럼은 그대로 둔 채 특정 컬럼을 대체합니다.
...
[`EXCEPT`](/ko/reference/statements/select) 수정자를 사용해 필드를 제외할 수도 있습니다. 예를 들어 `tolls_amount` 컬럼을 제거하려면 다음과 같이 쿼리를 작성합니다:
