...
[Nested 타입](/ko/reference/data-types/nested-data-structures)은 거의 변경되지 않는 정적 객체를 모델링하는 데 사용할 수 있으며, `Tuple` 및 `Array(Tuple)`의 대안이 될 수 있습니다. 일반적으로 이 타입은 동작 방식이 종종 혼란스러울 수 있으므로 JSON에는 사용하지 않는 것을 권장합니다. `Nested`의 주요 장점은 서브컬럼을 정렬 키에 사용할 수 있다는 점입니다.
