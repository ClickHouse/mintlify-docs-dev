...
이 가이드는 ClickHouse Cloud를 검토하는 엔터프라이즈 팀을 위해 프로덕션 배포의 모니터링 및 관측성 기능에 대한 포괄적인 정보를 제공합니다. 엔터프라이즈 고객은 기본 제공 모니터링 기능, Datadog 및 AWS CloudWatch와 같은 도구를 포함한 기존 관측성 스택과의 통합, 그리고 ClickHouse의 모니터링이 자체 호스팅 배포와 비교해 어떤 차이가 있는지에 대해 자주 문의합니다.
