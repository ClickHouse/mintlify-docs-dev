...
description: '이 가이드에서는 Jan.ai와 ClickHouse MCP 서버를 설정하는 방법을 설명합니다.'
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/use-cases/AI_ML/MCP/0_janai_openai.png" alt="OpenAI 모델 활성화" size="md" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/use-cases/AI_ML/MCP/1_janai_mcp_servers.png" alt="MCP 서버 활성화" size="md" />
  ...
  <Step>
    ...
    <Image img="/images/use-cases/AI_ML/MCP/2_janai_mcp_servers_list.png" alt="MCP 서버 목록" size="md" />

    이 서버들은 기본적으로 모두 비활성화되어 있지만, 토글을 클릭해 활성화할 수 있습니다.

    ClickHouse MCP 서버를 설치하려면 `+` 아이콘을 클릭한 다음 아래와 같이 양식을 입력합니다:

    <Image img="/images/use-cases/AI_ML/MCP/3_janai_add_mcp_server.png" alt="MCP 서버 추가" size="md" />

    입력을 마친 후 아직 켜져 있지 않다면 ClickHouse Server 토글을 켭니다:

    <Image img="/images/use-cases/AI_ML/MCP/4_janai_toggle.png" alt="MCP 서버 활성화" size="md" />

    이제 채팅 창에서 ClickHouse MCP 서버의 도구를 확인할 수 있습니다:

    <Image img="/images/use-cases/AI_ML/MCP/5_jani_tools.png" alt="ClickHouse MCP 서버 도구" size="md" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/use-cases/AI_ML/MCP/6_janai_question.png" alt="질문" size="md" />

    Jan.ai는 도구를 호출하기 전에 확인을 요청합니다:

    <Image img="/images/use-cases/AI_ML/MCP/7_janai_tool_confirmation.png" alt="도구 확인" size="md" />

    그런 다음 수행된 도구 호출 목록이 표시됩니다:

    <Image img="/images/use-cases/AI_ML/MCP/8_janai_tools_called.png" alt="호출된 도구" size="md" />

    도구 호출을 클릭하면 호출 세부 정보를 확인할 수 있습니다:

    <Image img="/images/use-cases/AI_ML/MCP/9_janai_tools_called_expanded.png" alt="확장된 도구 호출" size="md" />

    그 아래에는 결과가 표시됩니다:

    <Image img="/images/use-cases/AI_ML/MCP/10_janai_result.png" alt="결과" size="md" />
  </Step>
</Steps>