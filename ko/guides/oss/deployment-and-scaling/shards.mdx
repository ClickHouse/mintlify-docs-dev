...
<Image img="/images/managing-data/core-concepts/shards_03.png" size="lg" alt="세그먼트" />
...
나머지 과정은 레플리카가 없는 구성과 [동일하므로](#select-forwarding) 위 다이어그램에는 표시하지 않았습니다. 처음 대상으로 지정된 분산 테이블을 호스팅하는 ClickHouse 서버는 모든 로컬 결과를 수집하고, 이를 머지하여 최종 전역 결과를 만든 다음 쿼리 요청자에게 반환합니다.
