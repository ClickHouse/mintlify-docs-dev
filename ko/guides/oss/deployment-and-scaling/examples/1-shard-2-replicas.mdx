...
<Image img="/images/deployment-guides/replication-sharding-examples/replication.png" size="md" alt="1개 세그먼트와 2개 레플리카로 구성된 ReplicatedMergeTree의 아키텍처 다이어그램" />
...
<Steps>
  ...
  <Step>
    ...
    ```xml
    <remote_servers>
        <!-- 클러스터 이름 (점(.)을 포함하면 안 됩니다) -->
        <cluster_1S_2R>
            <!-- <allow_distributed_ddl_queries>false</allow_distributed_ddl_queries> -->
            <shard>
                <!-- 선택 사항. 레플리카 중 하나에만 데이터를 쓸지 여부입니다. 기본값: false (모든 레플리카에 데이터를 씁니다). -->
                <internal_replication>true</internal_replication>
                <replica>
                    <host>clickhouse-01</host>
                    <port>9000</port>
    ...
    <Note>
      이 예시에서는 cluster의 모든 노드에서 `users.xml` 파일이 동일합니다.
    </Note>
  </Step>
  ...
  <Step>
    ...
    `clickhouse-01`에서 아래 쿼리를 사용하여 더 작은 데이터 하위 집합을 삽입하십시오:
