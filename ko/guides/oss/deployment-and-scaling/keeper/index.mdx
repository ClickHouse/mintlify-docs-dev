...
7. 새 Keeper cluster를 가리키도록 ClickHouse 구성을 업데이트합니다.
...
1. 2개의 세그먼트와 2개 노드에 각각 레플리카 1개만 있는 단순한 클러스터를 구성해 보겠습니다. 세 번째 노드는 ClickHouse Keeper 요구 사항의 쿼럼을 충족하는 데 사용됩니다. `chnode1` 및 `chnode2`의 구성을 업데이트하십시오. 다음 클러스터는 각 노드에 1개의 세그먼트를 정의하므로 총 2개의 세그먼트가 되며 복제는 없습니다. 이 예시에서는 일부 데이터는 한 노드에, 나머지 데이터는 다른 노드에 저장됩니다:
   ...
   | 매개변수     | 설명                                   | 예시                   |
   | -------- | ------------------------------------ | -------------------- |
   | shard    | 클러스터 정의에 포함된 레플리카 목록                 | 각 세그먼트의 레플리카 목록      |
   | replica  | 각 레플리카에 대한 설정 목록                     | 각 레플리카의 설정 항목        |
   | host     | 레플리카 세그먼트를 호스팅할 서버의 호스트명, IP 또는 FQDN | `chnode1.domain.com` |
   | port     | 네이티브 TCP 프로토콜로 통신할 때 사용하는 포트         | 9000                 |
   | user     | 클러스터 인스턴스에 인증할 때 사용할 사용자 이름          | default              |
   | password | 클러스터 인스턴스 연결을 허용하도록 정의된 사용자의 비밀번호    | `ClickHouse123!`     |

2. ClickHouse를 다시 시작하고 클러스터가 생성되었는지 확인하십시오:
...
1. `chnode1`에서 ClickHouse client를 사용해 새 클러스터에 새 데이터베이스를 생성합니다. `ON CLUSTER` 절을 사용하면 두 노드 모두에 데이터베이스가 자동으로 생성됩니다.
   ```sql
   CREATE DATABASE db1 ON CLUSTER 'cluster_2S_1R';
   ```

2. `db1` 데이터베이스에 새 테이블을 생성합니다. 여기서도 `ON CLUSTER`를 사용하면 두 노드 모두에 테이블이 생성됩니다.
   ```sql
...
5. 각 노드에서 `SELECT` 문을 실행하면 해당 노드에 있는 데이터만 표시됩니다. 예를 들어 `chnode1`에서는 다음과 같습니다:
...
7. 두 세그먼트의 데이터를 나타내는 `Distributed` 테이블을 생성할 수 있습니다. `Distributed` 테이블 엔진을 사용하는 테이블은 자체 데이터를 저장하지 않지만, 여러 서버에 대해 분산 쿼리 처리를 수행할 수 있게 해줍니다. 읽기는 모든 세그먼트로 전송되며, 쓰기는 여러 세그먼트에 분산할 수 있습니다. `chnode1`에서 다음 쿼리를 실행합니다:
   ```sql
...
8. `dist_table`을 쿼리하면 두 세그먼트의 데이터 4개 행이 모두 반환됩니다:
