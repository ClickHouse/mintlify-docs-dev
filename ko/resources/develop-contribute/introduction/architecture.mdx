...
<Note>
  대부분의 외부 애플리케이션에는 HTTP 인터페이스 사용을 권장합니다. 단순하고 사용하기 쉽기 때문입니다. TCP 프로토콜은 내부 데이터 구조와 더 밀접하게 연결되어 있습니다. 데이터 블록을 전달할 때 내부 포맷을 사용하고, 압축된 데이터에는 사용자 정의 프레이밍을 사용합니다. 이 프로토콜용 C 라이브러리는 공개하지 않았는데, ClickHouse 코드베이스 대부분을 링크해야 하므로 실용적이지 않기 때문입니다.
</Note>
