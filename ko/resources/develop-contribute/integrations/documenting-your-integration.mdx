...
4. **인증(authentication).** 지원하는 인증 방식을 설명합니다(TLS를 통한 사용자 이름 및 비밀번호는 최소 요구 사항이며, 해당하는 경우 mTLS, SSL 클라이언트 인증서, IP 허용 목록 관련 참고 사항도 포함)
5. **엔드투엔드 예시.** 연결부터 의미 있는 결과까지 이어지는 현실적인 예시를 최소 1개 포함합니다. 독자가 재현할 수 있도록 [ClickHouse 예시 데이터셋](/ko/get-started/sample-datasets)을 사용하십시오
