...
ClickHouse는 [입력 및 출력용 데이터 포맷](/ko/reference/formats)을 폭넓게 지원합니다. 이 중에는 여러 JSON 변형이 있으며, 데이터 수집에 가장 일반적으로 사용되는 포맷은 [JSONEachRow](/ko/reference/formats/JSON/JSONEachRow)입니다. 이 포맷은 각 행에 JSON 객체 하나가 있어야 하며, 각 객체는 줄바꿈으로 구분됩니다.
...
* `input_format_import_nested_json`를 사용하면 중첩된 JSON 객체를 [Nested](/ko/reference/data-types/nested-data-structures) 타입 컬럼에 삽입할 수 있습니다.
