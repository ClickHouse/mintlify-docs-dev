---
title: Kafka에서 ClickHouse로 데이터 수집하기
...
이 문서에서는 Kafka 토픽의 데이터를 ClickHouse 테이블(table)로 전송하는 과정을 설명합니다. 예제로는 다양한 Wikimedia 속성에서 이루어진 변경 사항을 나타내는 [이벤트 스트림](https://stream.wikimedia.org/v2/stream/recentchange)을 제공하는 Wiki recent changes feed를 사용합니다. 다음 단계를 다룹니다:

1. Ubuntu에 Kafka를 설정하는 방법
2. 데이터 스트림을 Kafka 토픽으로 수집하기
3. 토픽을 구독하는 ClickHouse 테이블(table) 생성하기
