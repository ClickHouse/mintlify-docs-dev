...
백업 대상은 `Disk`, `S3`, `File`과 같은 유효한 백업 [대상](/ko/concepts/features/backup-restore/local-disk#configure-backup-destinations-for-disk)일 수 있습니다.

`Disk` 백업 대상을 사용할 경우, 백업에서 데이터베이스를 생성하는 쿼리는 다음과 같습니다.
