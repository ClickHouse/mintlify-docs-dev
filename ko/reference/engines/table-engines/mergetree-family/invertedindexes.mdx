...
함수 [mapContains](/ko/reference/functions/regular-functions/tuple-map-functions#mapcontains)(별칭: `mapContainsKey`)은 맵의 키에 있는 단일 토큰과 일치하는지 확인합니다.
...
접근 [operator[]](/ko/reference/operators#access-operators) 연산자를 텍스트 인덱스와 함께 사용하면 키와 값을 필터링할 수 있습니다.

예시:

```sql
SELECT count() FROM tab WHERE map['engine'] = 'clickhouse'; -- 정의된 경우 텍스트 인덱스를 사용합니다
```
