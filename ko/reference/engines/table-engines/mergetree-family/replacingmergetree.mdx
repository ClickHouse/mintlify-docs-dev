...
이 엔진은 동일한 [정렬 키](/ko/reference/engines/table-engines/mergetree-family/mergetree) 값(`ORDER BY` 테이블 절, `PRIMARY KEY` 아님)을 가진 중복 레코드를 제거한다는 점에서 [MergeTree](/ko/reference/engines/table-engines/mergetree-family/versionedcollapsingmergetree)와 다릅니다.
