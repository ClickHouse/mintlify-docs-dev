...
정렬 키에 대한 [ALTER](/ko/reference/statements/alter)는 가벼운 작업입니다. 새 컬럼을 테이블과 정렬 키에 동시에 추가하더라도 기존 데이터 파트는 변경할 필요가 없기 때문입니다. 이전 정렬 키는 새 정렬 키의 접두사이고, 새로 추가된 컬럼에는 데이터가 없으므로, 테이블 수정 시점의 데이터는 이전 정렬 키와 새 정렬 키를 모두 기준으로 정렬된 상태가 됩니다.
...
`interval`을 정의하려면 [시간 인터벌](/ko/reference/operators#operators-for-working-with-dates-and-times) 연산자를 사용하십시오. 예를 들면 다음과 같습니다:
...
* [`gcs` for GCS](/ko/reference/functions/table-functions/gcs#creating-a-disk)
...
* 백그라운드 머지 및 [뮤테이션](/ko/reference/statements/alter#mutations) 중에 기록됩니다.
...
통계 선언은 `*MergeTree*` 계열 테이블의 `CREATE` 쿼리에서 컬럼 섹션에 위치합니다:
...
현재 `MinMax` 및 `NullCount` 통계는 파트 프루닝을 지원합니다. 컬럼에 MinMax 통계가 정의되면 ClickHouse는 각 파트에서 해당 컬럼의 최솟값과 최댓값을 추적합니다. `Nullable` 컬럼에 NullCount 통계가 정의되면 ClickHouse는 각 파트의 NULL 값 개수를 추적하므로 `IS NULL` / `IS NOT NULL` 프레디케이트를 기반으로 프루닝할 수 있으며, NULL 값이 있는 컬럼의 범위 필터 프루닝 정확도도 향상됩니다.
파트 프루닝을 사용하면 쿼리 필터 조건과 일치하는 행이 전혀 없는 파트는 전체를 읽지 않고 건너뛸 수 있습니다.

**예시:**

```sql
-- 'value' 컬럼에 MinMax 통계를 적용한 테이블을 생성합니다
...
-- 여러 파트를 만들기 위해 데이터를 나누어 삽입합니다
INSERT INTO test_stats SELECT number, number FROM numbers(1000); -- 파트 1: value 값 범위 [0, 999]
INSERT INTO test_stats SELECT number, number + 10000 FROM numbers(1000); -- 파트 2: value 값 범위 [10000, 10999]

SET use_statistics_for_part_pruning = 1;

-- 이 쿼리는 최대값(999)이 5000보다 작으므로 파트 1을 완전히 건너뜁니다
SELECT count() FROM test_stats WHERE value > 5000;

-- 프루닝 효과를 확인하려면 EXPLAIN을 사용합니다
EXPLAIN indexes = 1 SELECT count() FROM test_stats WHERE value > 5000;
-- 출력에 "Parts: 1/2"가 표시되며, 이는 파트 1개가 프루닝되었음을 의미합니다
```
...
* `MinMax`
  ...
  숫자 컬럼의 범위 필터 선택도를 추정할 수 있게 해주는 컬럼의 최솟값과 최댓값입니다.
...
* `Uniq`
  ...
  컬럼에 포함된 고유값의 개수를 추정해 주는 [HyperLogLog](https://en.wikipedia.org/wiki/HyperLogLog) 스케치입니다.

  구문: `uniq`

* `NullCount`

  널 허용 컬럼에서 `NULL` 값의 개수를 추적합니다. PREWHERE 최적화에서 `IS NULL`/`IS NOT NULL` 프레디케이트의 선택도를 정확하게 추정하는 데 사용되며, `NULL` 존재 여부를 기반으로 한 파트 프루닝도 가능하게 합니다.
...
|           | 동등성 필터 (==) | 범위 필터 (`>, >=, <, <=`) | `IS NULL` / `IS NOT NULL` |
| --------- | ----------- | ---------------------- | ------------------------- |
| CountMin  | ✔           | ✗                      | ✗                         |
| MinMax    | ✗           | ✔                      | ✗                         |
| NullCount | ✗           | ✗                      | ✔                         |
