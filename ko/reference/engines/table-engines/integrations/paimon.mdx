...
아래 예시에서는 `PaimonLocal`을 사용합니다. 스토리지 백엔드에 맞게 엔진을 `PaimonS3`, `PaimonAzure`, `PaimonHDFS` 또는 `Paimon` 별칭으로 대체하십시오:
...
-- S3 스토리지 (Paimon은 PaimonS3의 별칭(alias))
CREATE TABLE paimon_mv_source
...
테이블 엔진 `Paimon`은 이제 `PaimonS3`의 별칭으로 사용됩니다.
