...
* `format` - 사용 가능한 파일 포맷 중 하나를 지정합니다. `SELECT` 쿼리를 실행하려면 해당 포맷이 입력을 지원해야 하며, `INSERT` 쿼리를 실행하려면 출력을 지원해야 합니다. 사용 가능한 포맷은 [포맷](/ko/reference/formats#formats-overview) 섹션에 나와 있습니다.
* [PARTITION BY expr]
