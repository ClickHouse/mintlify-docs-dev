...
* `partition_strategy` – 옵션: `WILDCARD` 또는 `HIVE`입니다. `WILDCARD`는 경로에 `{_partition_id}`가 있어야 하며, 이 값은 파티션 키로 대체됩니다. `HIVE`는 와일드카드를 허용하지 않으며, 경로를 테이블 루트로 간주하고, 파일 이름은 Snowflake ID, 확장자는 파일 포맷으로 하는 Hive 스타일의 파티션 디렉터리를 생성합니다. 기본값은 `WILDCARD`입니다.
...
`WILDCARD` (기본값): 파일 경로의 `{_partition_id}` 와일드카드를 실제 파티션 키로 대체합니다. 읽기는 지원하지 않습니다.

`HIVE`는 읽기 및 쓰기에 Hive 스타일 파티셔닝을 사용합니다. 읽기는 재귀적 glob pattern을 사용해 구현되며, 이는 `SELECT * FROM s3('table_root/**.parquet')`와 동일합니다.
쓰기는 다음 포맷으로 파일을 생성합니다: `<prefix>/<key1=val1/key2=val2...>/<snowflakeid>.<toLower(file_format)>`.

참고: `HIVE` 파티션 전략을 사용할 때 `use_hive_partitioning` 설정은 효과가 없습니다.
