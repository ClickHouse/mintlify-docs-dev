...
| `fail_on_connection_loss` | 연결이 끊어졌을 때 서버 동작을 제어합니다. `true`이면 클라이언트와 서버 간 연결이 끊어진 경우 즉시 예외가 발생합니다. `false`이면 ClickHouse 서버는 예외를 발생시키기 전에 쿼리 실행을 세 번 재시도합니다. 재시도하면 응답 시간이 늘어날 수 있습니다. 기본값은 `false`입니다. |
