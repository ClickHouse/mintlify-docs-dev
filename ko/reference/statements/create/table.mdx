...
지정된 [테이블 함수](/ko/reference/functions/table-functions)와 동일한 결과를 반환하는 테이블을 생성합니다. 생성된 테이블은 지정한 해당 테이블 함수와 동일한 방식으로 동작합니다.
...
쿼리에서는 `ENGINE` 절 뒤에 다른 절이 올 수 있습니다. 테이블을 생성하는 방법에 대한 자세한 내용은 [테이블 엔진](/ko/reference/engines/table-engines) 설명서를 참조하십시오.
...
* [Log](/ko/reference/engines/table-engines/log-family) 계열. 기본적으로 `lz4` 압축 방식을 사용하며 컬럼 압축 코덱을 지원합니다.
