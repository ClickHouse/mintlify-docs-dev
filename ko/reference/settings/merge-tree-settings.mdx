...
토큰 위치를 저장해 정확한 구문 일치를 지원하는 실험적 `positions` 인수를 사용하여
텍스트 인덱스를 생성할 수 있도록 허용합니다.
...
[replicated&#95;deduplication&#95;window](#replicated_deduplication_window) 설정). 여기에는
`insert_deduplication_version` 세분화 수준도 포함됩니다(기본
`new_unified_hash`에서는 삽입된 전체 블록, legacy 버전에서는 생성된 각 파트 단위).
해시 합은 ClickHouse Keeper가 아니라 디스크의 로컬 파일에 기록됩니다.
...
복제된 테이블에 쓸 때 ClickHouse는 중복 제거 해시 합을
...
기본 `insert_deduplication_version = new_unified_hash`에서는 해시 합이
삽입된 전체 블록을 대상으로 계산되므로, 이전 삽입(재시도)과 전체 데이터가
완전히 일치할 때만 해당 삽입이 중복으로 처리되며, 개별 파트 단위로는
중복 제거되지 않습니다. 레거시 `old_separate_hashes` /
`compatible_double_hashes`에서는 대신 해시 합이 생성된 각 파트별로,
필드 이름과 타입의 구성 및 해당 파트의 데이터(바이트 스트림)로부터
계산됩니다.

`replicated_deduplication_window` 값이 크면 `Inserts`는 비교해야 하는
항목이 더 많아지므로 속도가 느려집니다.
...
비교해야 하는 항목이 더 많아지므로 `비동기 삽입`이 느려집니다.
해시 합은 필드 이름과 타입의 구성,
그리고 삽입 데이터(바이트 스트림)를 기준으로 계산됩니다.

이 설정은 비동기 삽입 해시를 별도의 `async_blocks`
디렉터리에 유지하는 `insert_deduplication_version = old_separate_hashes` 또는
`compatible_double_hashes`에서만 적용됩니다. 기본값인 `new_unified_hash`에서는 비동기 삽입이
sync 삽입과 통합된 `deduplication_hashes` 디렉터리를 공유하므로, 대신
`replicated_deduplication_window` / `replicated_deduplication_window_seconds`의 적용을 받습니다.
...
`replicated_deduplication_window_for_async_inserts`와 마찬가지로, 이는
`insert_deduplication_version = old_separate_hashes` / `compatible_double_hashes`에서만 적용됩니다. 기본값인
`new_unified_hash`에서는 비동기 삽입에 `replicated_deduplication_window_seconds`가 사용됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "기본적으로 조정된 머지 전략을 사용하도록 설정"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "0"},{"label": "새로운 설정"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "기본적으로 조정된 머지를 활성화"}]}, {"id": "row-2","items": [{"label": "25.3"},{"label": "0"},{"label": "새로운 설정"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "백그라운드 정리 작업에서 비활성 레플리카를 얼마나 오랫동안 고려할지 제어하는 새로운 설정(0은 ZooKeeper session timeout 2회를 의미함)"}]}]} />

비활성 레플리카를 백그라운드 정리 작업
(`OutdatedPartsQuorumThread`)에서 얼마나 오랫동안 계속 고려할지 지정합니다. 레플리카가 이 값보다 짧은 시간 동안 비활성 상태였다면,
오래된 파트 삭제와 완료된 뮤테이션의 truncation이 계속 지연됩니다. 값이 0이면
기본값인 ZooKeeper session timeout 2회를 사용한다는 뜻입니다.
ClickHouse Cloud에서만 사용할 수 있습니다
...
레플리카 전체에 걸쳐 머지 할당을 분산하는 데 사용되는 윈도우 크기(블록 번호 기준)입니다.
동일한 윈도우 내의 파트는 동일한 레플리카에 할당됩니다.
ClickHouse Cloud에서만 사용할 수 있습니다.
...
true로 설정하면 테이블이 읽기 전용 모드가 되며, 디스크에 대한 어떠한 수정도 수행하지 않습니다.

테이블을 수정하는 모든 포그라운드 작업은 거부됩니다: 삽입, 뮤테이션, `OPTIMIZE`, 그리고 데이터를 변경하는 파티션 명령
(`ATTACH`/`MOVE`/`DROP`/`DROP DETACHED`/`FETCH`/`REPLACE PARTITION`, 그리고 이 테이블을 대상으로 하는 `MOVE PARTITION ... TO TABLE`). 테이블의 데이터를 수정하지 않는 작업인
`FREEZE`/`UNFREEZE` 및 `FORGET PARTITION`은 계속 허용됩니다.

백그라운드 작업도 예약되지 않습니다. 일반 머지, TTL 머지(`DELETE`/`MOVE`/재압축), 재압축 머지, 백그라운드 뮤테이션,
그리고 백그라운드 파트 이동 작업이 모두 억제됩니다. 그 결과, 이 설정이 활성화되어 있는 동안 TTL이 있는 테이블은 만료된 데이터를 더 이상 회수하거나 이동하지 않습니다.

이 설정은 `ALTER TABLE ... MODIFY SETTING table_readonly = 0`(또는 `RESET SETTING`)으로 언제든지 다시 전환할 수 있습니다. `ReplicatedMergeTree`에는 지원되지 않습니다.
