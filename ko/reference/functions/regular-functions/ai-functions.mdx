...
AI 함수는 프로바이더 자격 증명과 구성을 저장하는 **명명된 컬렉션**을 참조합니다. 각 함수의 첫 번째 인수는 이 컬렉션의 이름입니다.

프로바이더 자격 증명이 포함된 명명된 컬렉션을 생성하는 예시 구문:
...
첫 번째 인수는 프로바이더, model, endpoint와 선택적으로 API Key를 지정하는 명명된 컬렉션입니다.
...
* `collection` — 공급자 자격 증명과 구성이 포함된 명명된 컬렉션의 이름입니다. [`String`](/ko/reference/data-types/string)
...
첫 번째 인수는 프로바이더, 모델, 엔드포인트 및 선택적으로 API Key를 지정하는 명명된 컬렉션입니다.
...
* `collection` — 프로바이더 자격 증명과 구성을 포함하는 명명된 컬렉션의 이름입니다. [`String`](/ko/reference/data-types/string)
...
첫 번째 인수는 프로바이더, model, endpoint 및 선택적으로 API Key를 지정하는 명명된 컬렉션입니다.
...
* `temperature` — 무작위성을 제어하는 샘플링 온도입니다. 기본값: `0.0`입니다. [`const Float64`](/ko/reference/data-types/float)
...
첫 번째 인수는 프로바이더, model, endpoint와 선택적 API Key를 지정하는 명명된 컬렉션입니다.
...
* `collection` — 프로바이더 자격 증명 및 구성을 포함하는 명명된 컬렉션의 이름입니다. [`String`](/ko/reference/data-types/string)
...
지정된 LLM 프로바이더를 사용해 주어진 텍스트를 지정한 대상 언어로 번역합니다.

추가 스타일 또는 방언 지침은 네 번째 인수로 전달할 수 있습니다(예: `'기술 용어는 번역하지 않기'`).
