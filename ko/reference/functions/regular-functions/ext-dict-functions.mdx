...
* 둘 이상의 속성으로 이루어진 **복합 키**를 사용하는 딕셔너리에서는 키 값을 튜플로 전달합니다:

```sql
SELECT dictGet('complex_key_dictionary', 'attr_name', ('value_for_field1', 42));
```

* **복합 키가 단일 속성으로 구성된 경우**, 키 값을 `tuple`로 감싸지 않고 직접 전달할 수 있습니다. 다음 두 가지는 모두 유효하며 동일합니다:
...
이는 키가 단일 속성인 `ip_trie` 딕셔너리에도 동일하게 적용됩니다. 조회할 IP 주소를 직접 전달할 수 있습니다:
...
* `id_expr` — 키 값입니다. 단순 키를 사용하는 딕셔너리의 경우 `UInt64` 값을 반환하는 표현식입니다. 복합 키를 사용하는 딕셔너리의 경우 키 값의 튜플을 반환하는 표현식입니다. 복합 키가 단일 속성으로만 구성된 경우 `tuple`로 감싸지 않고 해당 값을 직접 전달할 수 있습니다. [`UInt64`](/ko/reference/data-types/int-uint) 또는 [`Tuple(T)`](/ko/reference/data-types/tuple)
...
해당 키의 직계 하위 항목을 반환합니다. [`Array(UInt64)`](/ko/reference/data-types/array)
...
<Note>
  이 함수는 비결정적입니다. 같은 인수에 대해서도 다른 결과를 반환할 수 있습니다.
</Note>
...
* `id_expr` — 키 값입니다. 단순 키를 사용하는 딕셔너리에서는 `UInt64` 값을 반환하는 표현식입니다. 복합 키를 사용하는 딕셔너리에서는 키 값의 튜플을 반환하는 표현식입니다. 복합 키가 단일 속성으로만 구성된 경우 `tuple`로 감싸지 않고 해당 값을 직접 전달할 수 있습니다. [`UInt64`](/ko/reference/data-types/int-uint) 또는 [`Tuple(T)`](/ko/reference/data-types/tuple)
...
* `dict_name` — 딕셔너리 이름입니다. 문자열 리터럴입니다. - `attr_name` — 조회할 컬럼 이름입니다. 문자열 리터럴입니다. - `id_expr` — 키 값입니다. 딕셔너리 키 타입 값을 반환하는 표현식입니다.
