...
2. [`user_defined_executable_function_drivers_config`](/ko/reference/settings/server-settings/settings#user_defined_executable_function_drivers_config)이 하나 이상의 드라이버 구성 파일(glob 지원)을 가리키도록 설정하고, 필요에 따라 생성된 실행형 UDF 구성이 저장되는 디렉터리인 [`dynamic_user_defined_executable_functions_path`](/ko/reference/settings/server-settings/settings#dynamic_user_defined_executable_functions_path)도 설정합니다.
