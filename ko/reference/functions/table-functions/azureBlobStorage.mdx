...
`WILDCARD`: 파일 경로의 `{_partition_id}` 와일드카드를 실제 파티션 키(partition key)로 대체합니다. `26.6` 이전의 `compatibility` 설정에서만 기본값으로 선택되며, 그 외에는 기본값이 `HIVE`입니다(`file_like_engine_default_partition_strategy` 설정 참조).
