...
| `partition_strategy`                    | 선택 사항인 매개변수입니다. 지원되는 값은 `WILDCARD` 또는 `HIVE`입니다. `WILDCARD`는 경로에 `{_partition_id}`가 있어야 하며, 이 값은 파티션 키로 대체됩니다. `HIVE`는 와일드카드를 허용하지 않고, 경로를 테이블 루트로 간주하며, 파일 이름으로 Snowflake ID를 사용하고 파일 포맷을 확장자로 하여 Hive 스타일의 파티션 디렉터리를 생성합니다. 기본값은 `WILDCARD`입니다. |
