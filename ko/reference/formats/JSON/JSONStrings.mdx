...
[JSON](/ko/reference/formats/JSON/JSON) 포맷과의 차이점은 데이터 필드가 타입이 지정된 JSON 값이 아니라 문자열로 출력된다는 점뿐입니다.
...
다음 데이터가 포함된 `football.json`이라는 이름의 JSON 파일을 사용합니다:
...
다음과 같이 `JSONStrings` 포맷을 사용해 데이터를 읽습니다:
