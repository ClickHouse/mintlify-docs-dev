...
이 포맷은 `JSONEachRow`, `NDJSON`(Newline Delimited JSON), 또는 `JSONL`(`JSONLines`)라고도 합니다. 이 이름들은 모두 동일한 포맷의 별칭이며 서로 바꿔서 사용할 수 있습니다.
...
이름을 알 수 없는 데이터 컬럼은 [input&#95;format&#95;skip&#95;unknown&#95;fields](/ko/reference/settings/formats#input_format_skip_unknown_fields) 설정이 1로 설정된 경우 가져오지 않고 건너뜁니다.
