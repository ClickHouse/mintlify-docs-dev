...
`AT TIME ZONE`의 연산자 우선순위는 13입니다(`*`/`/`/`%`의 12보다 높고 `+`/`-`의 11보다 높음). 이는 PostgreSQL과 일치합니다. 즉, `a * ts AT TIME ZONE 'tz'`는 `a * (ts AT TIME ZONE 'tz')`로 묶이고, `ts + interval AT TIME ZONE 'tz'`는 `ts + (interval AT TIME ZONE 'tz')`로 묶입니다. 산술 연산 후에 시간대 변환을 적용하려면 명시적으로 괄호를 사용하십시오:
