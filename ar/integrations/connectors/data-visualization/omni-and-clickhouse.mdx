---
sidebarTitle: 'Omni'
slug: /integrations/omni
keywords: ['clickhouse', 'Omni', 'connect', 'integrate', 'ui']
description: 'Omni هي منصة مؤسسية لذكاء الأعمال وتطبيقات البيانات والتحليلات المضمنة، تساعدك على استكشاف الرؤى ومشاركتها لحظيًا.'
title: 'Omni'
doc_type: 'guide'
integration:
  - support_level: 'partner'
  - category: 'data_visualization'
---

import ConnectionDetails from '/snippets/ar/_gather_your_details_http.mdx';
import { Image } from "/snippets/components/Image.jsx";

import PartnerBadge from "/snippets/components/PartnerBadge/PartnerBadge.jsx";

<PartnerBadge />

يمكن لـ Omni الاتصال بـ ClickHouse Cloud أو بعملية نشر محلية عبر مصدر بيانات ClickHouse الرسمي.

<div id="1-gather-your-connection-details">
  ## 1. اجمع تفاصيل الاتصال الخاصة بك
</div>

<ConnectionDetails />

<div id="2-create-a-clickhouse-data-source">
  ## 2. أنشئ مصدر بيانات ClickHouse
</div>

انتقل إلى Admin -&gt; Connections وانقر على زر &quot;Add Connection&quot; في الزاوية العلوية اليمنى.

<Image size="lg" img="/images/integrations/data-visualization/omni_01.png" alt="واجهة Admin في Omni تعرض زر Add Connection في قسم Connections" border />

<br />

حدِّد `ClickHouse`. أدخل بيانات الاعتماد الخاصة بك في النموذج.

<Image size="lg" img="/images/integrations/data-visualization/omni_02.png" alt="واجهة إعداد الاتصال في Omni لـ ClickHouse تعرض حقول نموذج بيانات الاعتماد" border />

<br />

يمكنك الآن تنفيذ query على البيانات من ClickHouse وتصورها في Omni.