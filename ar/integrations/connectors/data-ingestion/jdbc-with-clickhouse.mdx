...
<Image img="/images/integrations/data-ingestion/dbms/jdbc-01.png" size="lg" alt="مخطط معمارية ClickHouse JDBC Bridge" background="white" />

ويكون ذلك مفيدًا عندما لا يتوفر [integration engine](/ar/reference/engines/table-engines/integrations/index) أصلي مدمج، أو table function، أو قاموس خارجي لمصدر البيانات الخارجي، بينما يتوفر JDBC driver لهذا المصدر.
...
أسهل طريقة لاستخدام ClickHouse JDBC Bridge هي تثبيته وتشغيله على نفس المضيف الذي يعمل عليه ClickHouse:<Image img="/images/integrations/data-ingestion/dbms/jdbc-02.png" size="lg" alt="مخطط نشر ClickHouse JDBC Bridge محليًا" background="white" />
...
<Note>
  ...
  * في قيمة `driverUrls`، اكتفينا بتحديد URL يمكن تنزيل <a href="https://repo1.maven.org/maven2/mysql/mysql-connector-java/" target="_blank">الإصدار الحالي</a> من MySQL JDBC driver منه. هذا كل ما نحتاج إلى فعله، وسيتولى ClickHouse JDBC Bridge تنزيل JDBC driver هذا تلقائيًا (إلى دليل خاص بنظام التشغيل).
</Note>
...
<Image img="/images/integrations/data-ingestion/dbms/jdbc-03.png" size="lg" alt="مخطط نشر خارجي لـ ClickHouse JDBC Bridge" background="white" />
...
2. على كل مضيف ClickHouse، نضيف كتلة الإعداد التالية إلى <a href="/ar/concepts/features/configuration/server-config/configuration-files#configuration_files" target="_blank">إعدادات خادم ClickHouse</a> (بحسب تنسيق الإعداد الذي اخترته، استخدم إما إصدار XML أو YAML):
