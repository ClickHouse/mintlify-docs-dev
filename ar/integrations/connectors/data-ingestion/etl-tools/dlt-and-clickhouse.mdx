...
<Steps>
  <Step>
    ...
    يُنشئ الأمر أعلاه عدة ملفات ومجلدات، بما في ذلك `.dlt/secrets.toml` وملف متطلبات خاص بـ ClickHouse. يمكنك تثبيت التبعيات اللازمة المحددة في ملف المتطلبات بتنفيذ الأمر التالي:
  ...
  <Step>
    ...
    port = 9000                              # منفذ ClickHouse، والقيمة الافتراضية هي 9000
    ...
    dataset_table_separator = "___"          # الفاصل المستخدم في أسماء جداول مجموعة بيانات المشتقة من مجموعة بيانات.
    ```
    ...
    <Info>
      **HTTP&#95;PORT**
      ...
      يجب عليك تعيين `http_port` إذا كنت لا تستخدم تخزينًا مرحليًا خارجيًا (أي إذا كنت لا تعيّن معلمة تخزين مرحلي في مسار البيانات لديك). ويعود ذلك إلى أن التخزين المرحلي المدمج للتخزين المحلي في ClickHouse يستخدم مكتبة <a href="https://github.com/ClickHouse/clickhouse-connect">clickhouse-connect</a>، التي تتواصل مع ClickHouse عبر HTTP.

      تأكد من أن خادم ClickHouse لديك مُعدّ لقبول اتصالات HTTP على المنفذ المحدد بواسطة `http_port`. على سبيل المثال، إذا عيّنت `http_port = 8443`، فيجب أن يكون ClickHouse في وضع الاستماع لطلبات HTTP على المنفذ 8443. وإذا كنت تستخدم تخزينًا مرحليًا خارجيًا، يمكنك حذف المعلمة `http_port`، لأن clickhouse-connect لن يُستخدم في هذه الحالة.
    </Info>
