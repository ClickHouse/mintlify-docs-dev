...
يربط موفّر `apache-airflow-providers-clickhousedb` بين Airflow وClickHouse، مما يتيح لك تشغيل الاستعلامات وإنشاء الجداول وتحميل البيانات كجزء من DAG. ويتصل عبر [واجهة HTTP](/ar/interfaces/http) باستخدام عميل [`clickhouse-connect`](/ar/integrations/python)، كما يوفّر ClickHouse من خلال إطار SQL الشائع في Airflow، بحيث يتولى `SQLExecuteQueryOperator` القياسي تنفيذ استعلامات DDL وDML والاستعلامات التحليلية من دون الحاجة إلى مشغّل خاص بـ ClickHouse.
...
بالنسبة إلى [ClickHouse Cloud](/ar/cloud/overview) أو أي عنقود مستضاف ذاتيًا مع تمكين TLS، اضبط `secure` على `true` في حقل **Extra** واستخدم منفذ TLS (`8443`).
...
| `send_receive_timeout` | Query Timeout (seconds)      | `300`   | مهلة القراءة/الكتابة الخاصة بـ query بالثواني. زِد هذه القيمة للاستعلامات التحليلية طويلة التشغيل.                                                           |
| `compress`             | تفعيل ضغط LZ4                | `true`  | يفعّل ضغط نتائج LZ4.                                                                                                                                         |
| `client_name`          | اسم Client                   | (empty) | تسمية تُلحَق بمعرّف إصدار Airflow في `User-Agent` الخاص بـ ClickHouse وفي العمود `client_name` من [`system.query_log`](/ar/operations/system-tables/query_log). |
| `session_settings`     | إعدادات الجلسة (JSON)        | (empty) | [إعدادات الجلسة في ClickHouse](/ar/operations/settings/settings) التي تُطبَّق على كل query في الاتصال، مثل `{"max_execution_time": 300, "max_threads": 8}`. |
...
مرِّر [إعدادات الجلسة](/ar/operations/settings/settings) عند إنشاء الـ خطاف، إما مباشرةً أو عبر `hook_params` الخاص بالمشغّل. وتُدمَج الإعدادات المُمرَّرة إلى المُنشئ مع أي `session_settings` مُعرَّفة في حقل **Extra** الخاص بالاتصال، وتكون لقيم المُنشئ الأولوية عند تعارض المفاتيح:
...
* [`clickhouse-connect` عميل بايثون](/ar/integrations/python)
* [واجهة ClickHouse HTTP](/ar/interfaces/http)
* [مرجع إعدادات جلسة ClickHouse](/ar/operations/settings/settings)
