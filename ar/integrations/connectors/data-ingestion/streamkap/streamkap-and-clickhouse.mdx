...
تُعد <a href="https://www.streamkap.com/" target="_blank">Streamkap</a> منصة لتكامل البيانات في الوقت الفعلي، متخصصة في بث CDC (التقاط بيانات التغيير) ومعالجة التدفقات. وهي مبنية على مكدس عالي الإنتاجية وقابل للتوسع باستخدام Apache Kafka وApache Flink وDebezium، وتُقدَّم كخدمة مُدارة بالكامل ضمن نماذج نشر SaaS أو BYOC (Bring your own Cloud).
...
* تفاصيل الاتصال بعنقود ClickHouse الخاص بك: اسم المضيف، والمنفذ، واسم المستخدم، وكلمة المرور.
* قاعدة بيانات مصدر (مثل PostgreSQL أو SQL Server) مُعدّة للسماح باستخدام CDC. يمكنك العثور على أدلة إعداد مفصلة في وثائق Streamkap.

<div id="configure-clickhouse-source">
  ### الخطوة 1: تكوين المصدر في Streamkap
</div>

1. سجّل الدخول إلى حسابك في Streamkap.
2. من الشريط الجانبي، انتقل إلى **Connectors** ثم اختر علامة التبويب **Sources**.
3. انقر على **+ Add** وحدّد نوع قاعدة البيانات المصدر (مثل SQL Server RDS).
4. أدخل بيانات الاتصال، بما في ذلك نقطة النهاية والمنفذ واسم قاعدة البيانات وبيانات اعتماد المستخدم.
5. احفظ الموصّل.

<div id="configure-clickhouse-dest">
  ### الخطوة 2: تكوين وجهة ClickHouse
</div>
...
<div id="run-pipeline">
  ### الخطوة 3: إنشاء مسار بيانات وتشغيله
</div>

1. انتقل إلى **Pipelines** في الشريط الجانبي وانقر على **+ Create**.
2. حدِّد الـSource والوجهة اللذين أعددتهما للتو.
3. اختر المخططات والجداول التي تريد بثها.
4. سمِّ مسار بيانات ثم انقر على **Save**.

بمجرد إنشائه، سيصبح مسار بيانات نشطًا. سيأخذ Streamkap أولًا لقطة من البيانات الحالية، ثم يبدأ ببث أي تغييرات جديدة فور حدوثها.

<div id="verify-data-clickhoouse">
  ### الخطوة 4: التحقّق من البيانات في ClickHouse
</div>

اتصل بعنقود ClickHouse لديك ونفّذ استعلامًا للاطّلاع على البيانات التي تصل إلى الجدول الهدف.
