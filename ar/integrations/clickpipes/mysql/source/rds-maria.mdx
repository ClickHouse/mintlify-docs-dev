...
السجل الثنائي عبارة عن مجموعة من ملفات السجل التي تتضمن معلومات عن تعديلات البيانات التي أُجريت على مثيل خادم MySQL. وتُعد ملفات السجل الثنائي مطلوبة للنسخ المتماثل. يجب اتباع كلتا الخطوتين أدناه:

<div id="enable-binlog-logging-rds">
  ### 1. تمكين التسجيل الثنائي عبر النسخ الاحتياطي الآلي
</div>

تحدّد ميزة النسخ الاحتياطية الآلية ما إذا كان التسجيل الثنائي مفعّلًا أو معطّلًا في MySQL. ويمكن ضبطها من وحدة تحكم AWS:

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/rds-backups.png" alt="تمكين النسخ الاحتياطية الآلية في RDS" size="lg" border />

يُنصح بضبط مدة الاحتفاظ بالنسخ الاحتياطية على قيمة طويلة بشكل معقول، وفقًا لسيناريو النسخ المتماثل المستخدم.

<div id="binlog-retention-hours-rds">
  ### 2. ساعات الاحتفاظ بـ binlog
</div>
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/rds_config.png" alt="مكان العثور على مجموعة المعلمات في RDS" size="lg" border />

عند النقر على رابط مجموعة المعلمات، ستنتقل إلى صفحة مجموعة المعلمات. وسترى زر `Edit` في أعلى اليمين:

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/edit_button.png" alt="تحرير مجموعة المعلمات" size="lg" border />

يجب ضبط الإعدادات `binlog_format` و`binlog_row_metadata` و`binlog_row_image` على النحو التالي:

1. اضبط `binlog_format` على `ROW`.

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_format.png" alt="تنسيق binlog إلى ROW" size="lg" border />

2. اضبط `binlog_row_metadata` على `FULL`

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_metadata.png" alt="البيانات الوصفية لصف binlog إلى FULL" size="lg" border />

3. اضبط `binlog_row_image` على `FULL`

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_image.png" alt="صورة صف binlog إلى FULL" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/security-group-in-rds-mysql.png" alt="أين تجد مجموعة الأمان في RDS؟" size="lg" border />

<Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/rds/edit_inbound_rules.png" alt="تعديل قواعد Inbound rules لمجموعة الأمان المذكورة أعلاه" size="lg" border />

<div id="private-access-via-aws-privatelink">
  ### الوصول الخاص عبر AWS PrivateLink
</div>

للاتصال بمثيل RDS الخاص بك عبر private network، يمكنك استخدام AWS PrivateLink. اتبع [setup guide الخاص بنا لـ AWS PrivateLink مع ClickPipes](/ar/resources/support-center/knowledge-base/cloud-services/aws-privatelink-setup-for-clickpipes) لإعداد الاتصال.