...
وهي الخيار المفضل في الحالات التالية:
...
* الاتصال عبر المناطق لـ عنقود MSK. يُرجى التواصل مع فريق دعم ClickHouse للحصول على المساعدة.
...
<Info>
  ...
  لـ ClickPipes. أضِف [منطقة ClickPipe الخاصة بك](#aws-privatelink-regions) إلى المناطق المسموح بها في VPC endpoint service الخاصة بك.
</Info>
...
1. انتقل إلى SQL Console الخاصة بـ ClickHouse Cloud Service.

<Image img="/images/integrations/data-ingestion/clickpipes/cp_service.png" alt="خدمة ClickPipes" size="md" border />

2. حدّد زر `Data Sources` من القائمة الجانبية اليسرى، ثم انقر على &quot;Set up a ClickPipe&quot;

<Image img="/images/integrations/data-ingestion/clickpipes/cp_step0.png" alt="تحديد الواردات" size="lg" border />

3. حدّد Kafka أو Postgres كمصدر بيانات.

<Image img="/images/integrations/data-ingestion/clickpipes/cp_rpe_select.png" alt="تحديد مصدر البيانات" size="lg" border />

4. حدّد خيار `Reverse private endpoint`.

<Image img="/images/integrations/data-ingestion/clickpipes/cp_rpe_step0.png" alt="تحديد نقطة نهاية خاصة عكسية" size="lg" border />

5. حدّد أي نقطة نهاية خاصة عكسية موجودة أو أنشئ واحدة جديدة.

<Info>
  إذا كان الوصول عبر المناطق مطلوبًا لـ RDS، فستحتاج إلى إنشاء VPC endpoint service، ويمكن أن يكون
...
<Image img="/images/integrations/data-ingestion/clickpipes/cp_rpe_step1.png" alt="تحديد نقطة نهاية خاصة عكسية" size="lg" border />

6. أدخل المعلمات المطلوبة لنوع endpoint المحدد.

<Image img="/images/integrations/data-ingestion/clickpipes/cp_rpe_step2.png" alt="تحديد نقطة نهاية خاصة عكسية" size="lg" border />

* بالنسبة إلى مورد VPC، أدخل configuration share ARN وconfiguration ID.
  * بالنسبة إلى MSK multi-VPC، أدخل cluster ARN وطريقة المصادقة المستخدمة مع endpoint الذي أنشأته.
  * بالنسبة إلى VPC endpoint service، أدخل اسم الخدمة.

7. انقر على `Create` وانتظر حتى تصبح نقطة نهاية خاصة عكسية جاهزة.

   إذا كنت تُنشئ endpoint جديدًا، فسيستغرق إعداده بعض الوقت.
   ستُحدَّث الصفحة تلقائيًا بمجرد أن يصبح endpoint جاهزًا.
   قد يتطلب VPC endpoint service قبول طلب الاتصال في وحدة تحكم AWS.

<Image img="/images/integrations/data-ingestion/clickpipes/cp_rpe_step3.png" alt="تحديد نقطة نهاية خاصة عكسية" size="lg" border />

8. بعد أن يصبح endpoint جاهزًا، يمكنك استخدام اسم DNS للاتصال بمصدر البيانات.

   في قائمة endpoints، يمكنك رؤية اسم DNS الخاص بالـ endpoint المتاح.
   وقد يكون هذا إما اسم DNS وفّرته ClickPipes داخليًا أو اسم DNS خاصًا توفّره خدمة PrivateLink.
   اسم DNS ليس عنوان شبكة كاملًا.
   أضف Port وفقًا لمصدر البيانات.

   يمكن العثور على connection string الخاص بـ MSK في وحدة تحكم AWS.

   للاطلاع على القائمة الكاملة لأسماء DNS، انتقل إليها من إعدادات الخدمة السحابية.

<div id="managing-existing-endpoints">
  ## إدارة نقاط النهاية الخاصة العكسية الحالية
</div>

يمكنك إدارة نقاط النهاية الخاصة العكسية الحالية في إعدادات خدمة ClickHouse Cloud:

1. في الشريط الجانبي، ابحث عن زر `Settings` وانقر عليه.

   <Image img="/images/integrations/data-ingestion/clickpipes/cp_rpe_settings0.png" alt="إعدادات ClickHouse Cloud" size="lg" border />

2. انقر على `Reverse private endpoints` في قسم `ClickPipe reverse private endpoints`.

   <Image img="/images/integrations/data-ingestion/clickpipes/cp_rpe_settings1.png" alt="إعدادات ClickHouse Cloud" size="md" border />

   تُعرض المعلومات التفصيلية الخاصة بنقطة النهاية الخاصة العكسية في اللوحة المنبثقة.
