...
<Image img="/images/cloud/security/aws-privatelink-pe-create.png" size="md" alt="نقاط النهاية الخاصة" border />

دوّن `Service name` و`DNS name`، ثم [انتقل إلى الخطوة التالية](#create-aws-endpoint).
...
حدِّد **Endpoint services that use NLBs and GWLBs**، ثم استخدم `Service name`<sup>console</sup> أو `endpointServiceId`<sup>API</sup> الذي حصلت عليه من خطوة [Obtain Endpoint &quot;Service name&quot; ](#obtain-endpoint-service-info) في حقل **Service Name**. انقر على **Verify service**:

<Image img="/images/cloud/security/aws-privatelink-endpoint-settings.png" size="md" alt="إعدادات نقطة نهاية AWS PrivateLink" border />
...
<Image img="/images/cloud/security/aws-privatelink-select-vpc-and-subnets.png" size="md" alt="تحديد VPC والشبكات الفرعية" border />
...
<Image img="/images/cloud/security/aws-privatelink-vpc-endpoint-id.png" size="md" alt="معرّف VPC Endpoint" border />
...
<Image img="/images/cloud/security/aws-privatelink-pe-filters.png" size="md" alt="مرشّح نقاط النهاية الخاصة" border />
...
<Image img="/images/cloud/security/aws-privatelink-pe-dns-name.png" size="md" alt="اسم DNS لنقطة النهاية الخاصة" border />
...
لنفترض أنك تحاول استخدام table functions الخاصة بـ [MySQL](/ar/reference/functions/table-functions/mysql) أو [PostgreSQL](/ar/reference/functions/table-functions/postgresql) في ClickHouse Cloud للاتصال بقاعدة بياناتك المستضافة داخل Amazon Web Services (AWS) VPC. لا يمكن استخدام AWS PrivateLink لتأمين هذا الاتصال. فـ PrivateLink اتصال أحادي الاتجاه. إذ يتيح لشبكتك الداخلية أو لـ Amazon VPC الاتصال بـ ClickHouse Cloud بشكل آمن، لكنه لا يتيح لـ ClickHouse Cloud الاتصال بشبكتك الداخلية.
...
> استخدم AWS PrivateLink عندما يكون لديك إعداد عميل/خادم وتريد منح VPC مستهلكة واحدة أو أكثر وصولًا أحادي الاتجاه إلى خدمة محددة أو مجموعة من المثيلات في VPC الخاصة بمزوّد الخدمة. ولا يمكن إلا للعملاء الموجودين في VPC المستهلكة بدء اتصال بالخدمة الموجودة في VPC الخاصة بمزوّد الخدمة.

للقيام بذلك، اضبط Security Groups في AWS للسماح باتصالات من ClickHouse Cloud إلى خدمة قاعدة البيانات الداخلية/الخاصة لديك. راجع [عناوين IP الصادرة الافتراضية لمناطق ClickHouse Cloud](/ar/products/cloud/guides/data-sources/cloud-endpoints-api)، بالإضافة إلى [عناوين IP الثابتة المتاحة](https://api.clickhouse.cloud/static-ips.json).