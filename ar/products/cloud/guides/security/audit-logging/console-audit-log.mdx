---
sidebarTitle: 'سجل تدقيق Console'
slug: /cloud/security/audit-logging/console-audit-log
title: 'سجل تدقيق Console'
description: 'توضح هذه الصفحة كيفية مراجعة سجل تدقيق السحابة'
doc_type: 'guide'
keywords: ['سجل التدقيق']
---

import { Image } from "/snippets/components/Image.jsx";

تُسجَّل أنشطة Console في سجل التدقيق. إذا كان لديك دور Admin أو Developer على مستوى مؤسسة، يمكنك مراجعة السجل ودمجه مع أنظمة التسجيل لديك.

<div id="console-audit-log-ui">
  ## الوصول إلى سجل التدقيق عبر واجهة المستخدم
</div>

<Steps>
  <Step>
    ## حدِّد المؤسسة

    في ClickHouse Cloud، انتقل إلى تفاصيل مؤسستك.

    <Image img="/images/cloud/security/activity_log1.png" size="md" alt="علامة تبويب Activity في ClickHouse Cloud" border />

    <br />
  </Step>

  <Step>
    ## حدِّد Audit

    حدِّد علامة التبويب **Audit** من القائمة اليمنى لمعرفة التغييرات التي أُجريت على مؤسستك في ClickHouse Cloud، بما في ذلك من أجرى التغيير ووقت حدوثه.

    تعرض صفحة **Activity** جدولًا يتضمن قائمة بالأحداث المسجّلة المتعلقة بمؤسستك. وتكون هذه القائمة افتراضيًا مرتبة ترتيبًا زمنيًا عكسيًا (أحدث حدث في الأعلى). ويمكنك تغيير ترتيب الجدول بالنقر على رؤوس الأعمدة. ويحتوي كل صف في الجدول على الحقول التالية:

    * **Activity:** مقتطف نصي يصف الحدث
    * **User:** المستخدم الذي بدأ الحدث
    * **IP Address:** عند الاقتضاء، يعرض هذا الحقل عنوان IP للمستخدم الذي بدأ الحدث
    * **Time:** الطابع الزمني للحدث

    <Image img="/images/cloud/security/activity_log2.png" size="md" alt="جدول Activity في ClickHouse Cloud" border />

    <br />
  </Step>

  <Step>
    ## استخدم شريط البحث

    يمكنك استخدام شريط البحث المتاح لتصفية الأحداث استنادًا إلى معايير معينة، مثل اسم الخدمة أو عنوان IP. كما يمكنك تصدير هذه المعلومات بتنسيق CSV لتوزيعها أو تحليلها باستخدام أداة خارجية.
  </Step>
</Steps>

<div class="eighty-percent">
  <Image img="/images/cloud/security/activity_log3.png" size="lg" alt="تصدير Activity بتنسيق CSV في ClickHouse Cloud" border />
</div>

<div id="console-audit-log-api">
  ## الوصول إلى سجل تدقيق Console عبر واجهة برمجة تطبيقات
</div>

يمكنك استخدام نقطة النهاية `activity` في واجهة برمجة تطبيقات ClickHouse Cloud للحصول على نسخة مُصدَّرة
من أحداث التدقيق. ويمكنك العثور على مزيد من التفاصيل في [مرجع واجهة برمجة تطبيقات](/ar/api-reference/organization/get-list-of-available-organizations).

<div id="log-integrations">
  ## تكاملات السجلات
</div>

يمكنك استخدام واجهة برمجة التطبيقات للتكامل مع منصة تسجيل السجلات التي تختارها. وفيما يلي موصلات مدعومة وجاهزة للاستخدام:

* [الوظيفة الإضافية للتدقيق في ClickHouse Cloud لـ Splunk](/ar/integrations/connectors/data-integrations/integrations/splunk)