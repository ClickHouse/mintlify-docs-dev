...
**البديل**: `allow_delta_lake_writes`

<SettingsInfoBlock type="Bool" default_value="0" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "نُقلت ميزة الكتابة في Delta Lake إلى Beta."}]}, {"id": "row-2","items": [{"label": "25.9"},{"label": "0"},{"label": "إعداد جديد."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "إعداد جديد لتمكين توسيع أحرف البدل في table function ‏`url` عبر سرد صفحات فهرس HTTP."}]}]} />

يسمح بتوسيع أحرف البدل تجريبيًا لكلٍّ من `url()` و `ENGINE = URL` استنادًا إلى صفحات فهرس HTTP.
...
السماح بوسائط المنطقة الزمنية غير الثابتة في بعض الدوال المرتبطة بالوقت مثل toTimeZone() وfromUnixTimestamp*() وsnowflakeIDToDateTime*().
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "فحص أمان جديد: يُطلِق `ALTER TABLE ... REPLACE PARTITION ... FROM ...` الآن استثناءً عندما لا يحتوي الجدول المصدر على أي أجزاء في القسم المطلوب (ما يعالج الفقدان الصامت للبيانات في [#23727](https:\/\/github.com\/ClickHouse\/ClickHouse\/issues\/23727)). ويمكن الإبقاء على السلوك السابق، وهو إسقاط قسم الوجهة بصمت، عبر ضبط `allow_replace_partition_from_empty_source = 1`."}]}]} />

اسمح لـ `ALTER TABLE ... REPLACE PARTITION ... FROM ...` بإسقاط قسم الوجهة بصمت عندما لا يحتوي المصدر على أي أجزاء في ذلك القسم.

يكون هذا غير مسموح به افتراضيًا: إذ إن `REPLACE PARTITION` من مصدر لا يحتوي على أي بيانات في القسم المطلوب يُطلِق استثناءً، لأن العملية في هذه الحالة تصبح فعليًا `DROP PARTITION` صامتًا على الوجهة (تُزال بيانات الوجهة ولا يحل محلها شيء)، وهو سبب شائع لفقدان البيانات غير المقصود (راجع [#23727](https://github.com/ClickHouse/ClickHouse/issues/23727)).

فعِّل هذا الإعداد لاستعادة السلوك السابق، على سبيل المثال عندما تستخدم عمدًا قسمًا فارغًا في المصدر لمسح البيانات من الوجهة. وإذا كنت تريد إسقاطًا غير مشروط، فالأفضل استخدام `ALTER TABLE ... DROP PARTITION ...` بدلًا من ذلك.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.11"},{"label": "0"},{"label": "عدم السماح افتراضيًا بالنوعين Variant/Dynamic في مفاتيح ORDER BY"}]}]} />
...
لا يكون له تأثير إلا في ClickHouse Cloud. يعرض العمودين `certificate` و`signature` في جدول `system.distributed_cache_registry`. تكون هذه الأعمدة فارغة افتراضيًا لإبقاء المخرجات مختصرة؛ فعِّل هذا الإعداد لفحصها.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "إعداد تجريبي جديد لتحديد عدد العاملين عديمي الحالة الذين سيُخصَّصون لاستعلام موزّع. تؤدي القيمة صفر إلى تعطيل هذا التخصيص."}]}]} />

عدد العاملين عديمي الحالة الذين سيُستخدمون لتنفيذ هذا الاستعلام. تؤدي القيمة صفر إلى تعطيل تخصيص العاملين عديمي الحالة للخطط الموزعة.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "اعتبارًا من الإصدار 26.7، تكون القيم الافتراضية في `EXPLAIN PLAN` هي `actions=1, compact=1, pretty=1`. اضبط هذا على `legacy` لاستعادة المخرجات التي كانت مستخدمة قبل 26.7."}]}]} />

التنسيق الافتراضي المستخدم في `EXPLAIN PLAN`.

القيم الممكنة:

* `pretty` (الافتراضي منذ 26.7) — تكون `actions` و`compact` و`pretty` مضبوطة افتراضيًا على `true`، مما ينتج خطة مدمجة ومنسقة ومشروحة بـ `actions`.
* `legacy` — المخرجات المستخدمة قبل 26.7.

يؤدي تحديد خيارات `actions` أو `compact` أو `pretty` صراحةً في تعليمة `EXPLAIN` (على سبيل المثال، `EXPLAIN actions = 0, compact = 0, pretty = 0 SELECT ...`) دائمًا إلى تجاوز هذا الإعداد.

يحتفظ `EXPLAIN PLAN` مع `json = 1` أو `distributed = 1` بالقيم الافتراضية القديمة (قبل 26.7) بغض النظر عن هذا الإعداد، ما لم يتم تعيين `actions` أو `compact` أو `pretty` صراحةً. ولا يمكن لمخرجات `pretty` تمثيل نتائج JSON أو الخطط الموزعة على مستوى كل shard، لذا لا تُعرَض هذه الأوضاع بشكل صحيح إلا بصيغة `legacy`.
...
بالنسبة إلى الجداول المكررة، افتراضيًا لا تُزال التكرارات إلا من أحدث 100 عملية إدراج لكل قسم (راجع [replicated&#95;deduplication&#95;window](/ar/reference/settings/merge-tree-settings#replicated_deduplication_window)، [replicated&#95;deduplication&#95;window&#95;seconds](/ar/reference/settings/merge-tree-settings#replicated_deduplication_window_seconds)).
...
<Note>
  يُتتبَّع `insert_deduplication_token` لكل قسم على حدة، لذلك يمكن لعدة أقسام مكتوبة بعملية إدراج واحدة أن تحمل الرمز نفسه. ومن دون رمز، يُحتسب checksum الافتراضي للمحتوى (`insert_deduplication_version = new_unified_hash`) على مستوى block المُدرَج بالكامل، لذلك لا تُزال تكرارات عملية إدراج إلا عندما تتطابق بياناتها بالكامل مع عملية إدراج سابقة (إعادة محاولة)، وليس عندما تتطابق صفوف قسم واحد مصادفةً مع عملية إدراج مختلفة.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "إعداد جديد لدفع `LIMIT` إلى التجميع بالترتيب لإنهاء التنفيذ مبكرًا عندما يكون `ORDER BY` بادئة لوصف الفرز في `GROUP BY`."}]}]} />

عند تفعيله، ومع كون التجميع بالترتيب نشطًا، يُدفَع LIMIT إلى خطوة التجميع لتمكين الإنهاء المبكر بعد إنتاج عدد كافٍ من المجموعات. يقلّل ذلك من كمية البيانات المقروءة عندما يطابق `ORDER BY` بادئة مفتاح `GROUP BY`. وقد يؤدي ذلك إلى تقليل القيمة المُبلَّغ عنها بواسطة `rows_before_limit_at_least`؛ استخدم `exact_rows_before_limit` إذا كنت بحاجة إلى أعداد دقيقة.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "إعداد جديد يحدّ من حجم العمل الذي ينفذه تحسين `optimize_and_compare_chain` (ويُقاس بعدد عُقَد شجرة الاستعلام التي جرى حساب hash لها) حتى لا يطغى على تحليل الاستعلامات التي تحتوي على عدد كبير جدًا من سلاسل `AND` الخاصة بالمقارنات أو سلاسل كبيرة جدًا منها. تعيد القيمة السابقة `0` (غير محدود) سلوك ما قبل 26.7 حيث لم يكن هذا التحسين مقيّدًا، لذا فإن ضبط `compatibility` على إصدار أقدم يُبقي اشتقاق الشروط الانتقالية من دون ميزانية. اضبطه على `0` لتعطيل الميزانية."}]}]} />

ميزانية العمل لتحسين `optimize_and_compare_chain` أثناء تحليل الاستعلام، وتُقاس بعدد عُقَد شجرة الاستعلام التي يُحسَب لها hash بواسطة `getTreeHash` (وهي التكلفة الأساسية لهذا التحسين). بمجرد أن يحسب الاستعلام hash لعدد من العُقَد يتجاوز هذا الحد أثناء تطبيق التحسين، يتوقف عن تطبيقه على بقية الاستعلام. يحدّ هذا من زمن التحليل للاستعلامات التي تحتوي على عدد كبير جدًا من سلاسل `AND` الخاصة بالمقارنات أو سلاسل كبيرة جدًا منها، حيث يمكن لهذا التحسين لولا ذلك أن يهيمن على التحليل من دون أن يحقق أي تبسيط. ويظل التوقف المبكر آمنًا دائمًا: فهو يعني فقط الاستغناء عن تحسين، ولا يغيّر النتائج مطلقًا. اضبطه على `0` لتعطيل الميزانية (غير محدود).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "إعداد جديد لتمكين تحسين على مستوى خطة الاستعلام يدفع `ORDER BY ... LIMIT n` عبر join LEFT/RIGHT عندما لا يشير مفتاح الفرز إلا إلى الجانب الذي يحتفظ به join."}]}]} />

يؤدي هذا إلى تفعيل تحسين على مستوى خطة الاستعلام يدفع `ORDER BY ... LIMIT n` إلى أسفل عبر عملية join عندما لا يشير مفتاح الفرز إلا إلى الأعمدة في الجانب الذي يحتفظ به الـ join ‏(LEFT/RIGHT). ويقيّد عدد الصفوف التي يجب أن ينتجها الإدخال الخاص بالجانب المُحتفَظ به قبل إجراء join.
لا يسري مفعوله إلا إذا كانت قيمة الإعداد [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) تساوي 1.
...
يُستخدم في جدولة أعباء العمل. وهو الحد الأدنى من RAM المحجوزة لاستخدامها في تشغيل استعلام على خادم واحد. يتم الحجز من خلال التسلسل الهرمي لـ WORKLOAD باستخدام قيمة إعداد الاستعلام `workload`.
إذا لم تكن هناك ذاكرة كافية متاحة لعبء العمل، فسيُمنع الاستعلام من البدء ويظل في حالة انتظار إلى أن يمكن استيفاء الحجز.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "100000"},{"label": "إعداد جديد لتقييد عدد الأدلة المقروءة عند توسيع أحرف البدل في دالة الجدول `url`."}]}]} />

الحد الأقصى لعدد الأدلة التي يمكن اجتيازها أثناء توسيع أحرف البدل في URL من صفحات الفهرس.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "استخدم دالة `toTime` الجديدة (لتحويل القيم إلى نوع البيانات `Time`) افتراضيًا بدلًا من الدالة القديمة `toTime` (التي لا تزال متاحة باسم `toTimeWithFixedDate`)."}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "إعداد جديد. يتيح للمستخدم استخدام المنطق القديم للدالة toTime، الذي يعمل مثل toTimeWithFixedDate."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "أصبح فهرس النص الآن GA"}]}, {"id": "row-2","items": [{"label": "26.1"},{"label": "0"},{"label": "مُمكّن افتراضيًا"}]}, {"id": "row-3","items": [{"label": "25.9"},{"label": "0"},{"label": "إعداد جديد"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "تم تمكين ذاكرة التخزين المؤقت لترويسة الفهرس النصي على مستوى النظام."}]}, {"id": "row-2","items": [{"label": "25.11"},{"label": "0"},{"label": "إعداد جديد"}]}]} />

ما إذا كان سيتم تخزين ترويسات الفهرس النصي التي خضعت لفك التسلسل مؤقتًا في الذاكرة.
...
ما إذا كان يجب تخزين قوائم النشر الخاصة بالفهرس النصي بعد فك تسلسلها مؤقتًا في الذاكرة.
...
ما إذا كان سيتم تخزين بيانات تعريف رموز الفهرس النصي بعد فك التسلسل مؤقتًا في الذاكرة.
