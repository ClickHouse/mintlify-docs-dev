...
<div id="allow_experimental_text_index_positions">
...
السماح بإنشاء فهارس نصية باستخدام الوسيطة التجريبية `positions`، التي
تخزن مواضع الرموز المميزة لدعم المطابقة الدقيقة للعبارات.
...
إعداد [replicated&#95;deduplication&#95;window](#replicated_deduplication_window))، بما في ذلك
مستوى granularity الخاص بـ `insert_deduplication_version` (الكتلة المُدرجة بالكامل ضمن
`new_unified_hash` الافتراضي، ولكل جزء مُنشأ ضمن إصدارات legacy). وتُكتب مجاميع hash إلى
ملف محلي على قرص بدلًا من ClickHouse Keeper.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "إعداد جديد. جمِّع أي substream لفهرس تخطٍّ لا يتجاوز حجمه المتسلسل على القرص هذا العدد من البايتات في أرشيف `skp_idx.packed` واحد لكل جزء؛ أما substreams الأكبر فتظل في تخطيط `skp_idx_<name>.idx2` / `.mrk2` المنفصل. يُتخذ القرار لكل substream عند وقت الكتابة."}]}]} />

الحدّ (بالبايتات المتسلسلة على القرص، أي بعد ضغط وسلسلة hash
الخاصة بالـ substream) الذي إذا كان أقل منه، يُجمَّع substream لفهرس التخطّي في أرشيف
`skp_idx.packed` واحد لكل جزء بدلًا من كتابته كملف `skp_idx_<name>.idx2` / `.mrk2`
منفصل. أما substreams الأكبر من ذلك فتظل في تخطيط legacy لكل ملف. ويُتخذ القرار
بشكل مستقل لكل substream عند وقت الكتابة، لذا يمكن أن يحتوي جزء واحد على
...
إن format على القرص ذاتي الوصف: يكتشف القارئون `skp_idx.packed` ويقدّمون
substreams المجمّعة من داخله بشفافية. يؤثر تغيير هذا الإعداد في الأجزاء المكتوبة حديثًا
فقط؛ أما الأجزاء الموجودة فتحتفظ بالتخطيط الذي كانت عليه وقت الكتابة.
...
بالنسبة إلى [إزالة التكرار](/ar/reference/engines/table-engines/mergetree-family/replication)،
عند الكتابة إلى الجداول المتماثلة، يكتب ClickHouse قيم تجزئة إزالة التكرار إلى
ClickHouse Keeper. ولا تُخزَّن قيم التجزئة إلا لأحدث
`replicated_deduplication_window` من الكتل. وتُزال أقدم قيم التجزئة من
ClickHouse Keeper.

في الإعداد الافتراضي `insert_deduplication_version = new_unified_hash`، تغطي قيمة التجزئة
الكتلة المُدرجة بأكملها، لذا لا تُزال ازدواجية عملية insert إلا عندما تتطابق جميع بياناتها مع
عملية insert سابقة (إعادة محاولة)، وليس لكل جزء على حدة. أما في `old_separate_hashes` /
`compatible_double_hashes` القديمة، فتُحسَب قيمة التجزئة بدلًا من ذلك لكل جزء مُنشأ، من
تكوين أسماء الحقول وأنواعها وبيانات ذلك الجزء (تدفّق من البايتات).

إن تعيين قيمة كبيرة لـ `replicated_deduplication_window` يبطّئ عمليات `Inserts` لأن عددًا أكبر من
الإدخالات يحتاج إلى المقارنة.
...
قِيَم التجزئة الخاصة بها للتحقق من التكرارات.
...
عند الكتابة إلى الجداول المتماثلة، يكتب ClickHouse قِيَم التجزئة لكل
insert في ClickHouse Keeper. ولا تُخزَّن قِيَم التجزئة إلا لأحدث
`replicated_deduplication_window_for_async_inserts` من الكتل. وتُزال أقدم قِيَم التجزئة
...
ينطبق هذا الإعداد فقط عند استخدام `insert_deduplication_version = old_separate_hashes` أو
`compatible_double_hashes`، حيث تحتفظ هذه القيم بتجزئات async-insert في directory
منفصل باسم `async_blocks`. أما عند استخدام القيمة الافتراضية `new_unified_hash`، فتشترك
async inserts في directory الموحّد `deduplication_hashes` مع sync inserts، ويخضع
سلوكها بدلًا من ذلك إلى `replicated_deduplication_window` / `replicated_deduplication_window_seconds`.
...
مثل `replicated_deduplication_window_for_async_inserts`، لا ينطبق هذا إلا عند
`insert_deduplication_version = old_separate_hashes` / `compatible_double_hashes`؛ أمّا في القيمة الافتراضية
`new_unified_hash`، فتستخدم عمليات async inserts القيمة `replicated_deduplication_window_seconds`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "تمكين استراتيجية الدمج المنسّق افتراضيًا"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "0"},{"label": "إعداد جديد"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "تمكين الدمج المنسق افتراضيًا"}]}, {"id": "row-2","items": [{"label": "25.3"},{"label": "0"},{"label": "إعداد جديد"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "إعداد جديد يتحكم في المدة التي تظل خلالها النسخة المتماثلة غير النشطة محسوبة ضمن التنظيف الخلفي (0 تعني مهلة جلستين لـ ZooKeeper)"}]}]} />

المدة التي تظل خلالها النسخة المتماثلة غير النشطة محسوبة ضمن التنظيف الخلفي
(`OutdatedPartsQuorumThread`): ما دامت النسخة المتماثلة غير نشطة لمدة تقل عن هذا العدد من الثواني،
فإنها تظل تؤخر إزالة الأجزاء القديمة واقتطاع عمليات `mutations` المكتملة. وتعني القيمة 0
استخدام القيمة الافتراضية، وهي مهلة جلستين لـ ZooKeeper.
فقط في ClickHouse Cloud
...
حجم النافذة (وفق أرقام الكتل) المستخدم لتوزيع إسناد الدمج عبر
النسخ المتماثلة. تُسنَد الأجزاء ضمن النافذة نفسها إلى النسخ المتماثلة نفسها.
متاح فقط في ClickHouse Cloud
...
إذا تم تعيينه على true، فسيكون الجدول في وضع القراءة فقط، ولن يُجري أي تعديلات على القرص.

تُرفَض جميع العمليات الأمامية التي من شأنها تعديل الجدول: عمليات الإدراج، والتعديلات، و`OPTIMIZE`، وأوامر الأقسام التي تُعدِّل البيانات
(`ATTACH`/`MOVE`/`DROP`/`DROP DETACHED`/`FETCH`/`REPLACE PARTITION`، وكذلك `MOVE PARTITION ... TO TABLE` عند استهداف هذا الجدول). أما العمليات
التي لا تعدّل بيانات الجدول، مثل `FREEZE`/`UNFREEZE` و`FORGET PARTITION`، فتبقى مسموحًا بها.

ولا تُجدول أي أعمال في الخلفية أيضًا: فعمليات الدمج العادية، وعمليات دمج TTL (`DELETE`/`MOVE`/إعادة الضغط)، وعمليات دمج إعادة الضغط، والتعديلات في الخلفية،
وعمليات نقل الأجزاء في الخلفية، جميعها تُوقَف. ونتيجة لذلك، فإن الجدول الذي يحتوي على TTL لن يتخلّص من بياناته منتهية الصلاحية أو ينقلها ما دام هذا الإعداد
مُمكّنًا.
