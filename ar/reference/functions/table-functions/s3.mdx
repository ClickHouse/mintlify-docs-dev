...
| `storage_class_name`                    | هذه المعلمة اختيارية. القيم المدعومة: `STANDARD` أو `INTELLIGENT_TIERING`. وتتيح تحديد [AWS S3 Intelligent Tiering](https://aws.amazon.com/s3/storage-classes/intelligent-tiering/). والقيمة الافتراضية هي `STANDARD`.                                                                                                                                                                                                                                  |
...
`wildcard`: يستبدل حرف البدل `{_partition_id}` في مسار الملف بمفتاح التقسيم الفعلي. يُحدَّد افتراضيًا فقط ضمن إعدادات `compatibility` الأقدم من `26.6`؛ وإلا تكون القيمة الافتراضية هي `hive` (راجع الإعداد `file_like_engine_default_partition_strategy`).
