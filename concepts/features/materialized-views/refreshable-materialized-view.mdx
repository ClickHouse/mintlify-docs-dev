...
<Image img="/static/images/materialized-view/refreshable-materialized-view-diagram.png" size="lg" alt="Refreshable materialized view diagram"/>
...
In the [dbt and ClickHouse integration guide](/integrations/connectors/data-ingestion/etl-tools/dbt) we populated an IMDb dataset with the following tables: `actors`, `directors`, `genres`, `movie_directors`, `movies`, and `roles`.
