...
## Using the query profiler in self-managed deployments <a id="self-managed-query-profiler"></a>

In self-managed deployments, to use the query profiler follow the steps below:

<Steps>
<Step>
### Install ClickHouse with debug info <a id="debug-info"></a>
...
<Step>
### Check server config <a id="server-config"></a>
...
<Step>
### Configure profile timers <a id="configure-profile-timers"></a>

Set up the [`query_profiler_cpu_time_period_ns`](/reference/settings/session-settings#query_profiler_cpu_time_period_ns) or [`query_profiler_real_time_period_ns`](/reference/settings/session-settings#query_profiler_real_time_period_ns) settings.
...
<Step>
### Analyze the `trace_log` system table <a id="analyze-trace-log-system-table"></a>

To analyze the `trace_log` system table allow introspection functions with the [`allow_introspection_functions`](/reference/settings/session-settings#allow_introspection_functions) setting:
...
Use the `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` and `demangle` [introspection functions](/reference/functions/regular-functions/introspection) to get function names and their positions in ClickHouse code.
...
## Building flame graphs with the `flameGraph` function <a id="flamegraph"></a>
...
### CPU flame graph <a id="cpu-flame-graph"></a>
...
### Memory flame graph — all allocations <a id="memory-flame-graph-all"></a>
...
### Memory flame graph — unfreed allocations <a id="memory-flame-graph-unfreed"></a>
...
### Memory flame graph — active allocations at a point in time <a id="memory-flame-graph-time-point"></a>
...
#### Find memory usage over time <a id="find-memory-usage-over-time"></a>
...
#### Find the time point with maximum memory usage <a id="find-time-point-maximum-memory-usage"></a>
...
#### Build a flame graph of active allocations at that time point <a id="build-flame-graph"></a>
...
#### Build a flame graph of deallocations after that time point (to understand what was freed later) <a id="build-flame-graph-deallocations"></a>
