...
- [Tableau Online](/integrations/connectors/data-visualization/tableau/tableau-online-and-clickhouse)
- [QuickSight](/integrations/connectors/data-visualization/quicksight-and-clickhouse)
...
If there is a native driver available (e.g., [DBeaver](/integrations/connectors/sql-clients/dbeaver)), it is always preferred to use it instead of the MySQL interface. Additionally, while most of the MySQL language clients should work fine, MySQL interface isn't guaranteed to be a drop-in replacement for a codebase with existing MySQL queries.
...
## Enabling the MySQL Interface On ClickHouse Cloud <a id="enabling-the-mysql-interface-on-clickhouse-cloud"></a>

1. After creating your ClickHouse Cloud Service, click the `Connect` button.

<br/>

<Image img="/images/interfaces/mysql0.png" alt="Credentials screen - Prompt" size="md"/>

2. Change the `Connect with` drop-down to `MySQL`. 

<br/>

<Image img="/images/interfaces/mysql1.png" alt="Credentials screen - MySQL selected" size="md" />
...
<Image img="/images/interfaces/mysql2.png" alt="Credentials screen - Enabled MySQL" size="md"/>

Copy the MySQL connection string shown.

<Image img="/images/interfaces/mysql3.png" alt="Credentials screen - Connection String" size="md"/>

## Creating multiple MySQL users in ClickHouse Cloud <a id="creating-multiple-mysql-users-in-clickhouse-cloud"></a>
...
2. [Create a user](/reference/statements/create/user) using the following format: `mysql4<subdomain>_<username>` ([see above](#creating-multiple-mysql-users-in-clickhouse-cloud)). The password must be in double SHA1 format. For example:
...
### Troubleshooting multiple MySQL users in ClickHouse Cloud <a id="troubleshooting-multiple-mysql-users-in-clickhouse-cloud"></a>
...
## Enabling the MySQL interface on self-managed ClickHouse <a id="enabling-the-mysql-interface-on-self-managed-clickhouse"></a>
...
## Connect MySQL to ClickHouse <a id="connect-mysql-to-clickhouse"></a>
