...
<Image img="/static/images/integrations/data-ingestion/dbms/postgres-replicas.png" size="md" alt="Eventual consistency"/>
...
1. **Read/Write to the same node** - If you're using native protocol, or a [session to do your write/read via HTTP](/integrations/connectors/data-integrations/drivers-and-interfaces/http#default-database), you should then be connected to the same replica: in this scenario you're reading directly from the node where you're writing, then your read will always be consistent.
...
Further details on optimizing and measuring compression can be found [here](/guides/cloud-oss/data-modelling/compression/compression-in-clickhouse).
