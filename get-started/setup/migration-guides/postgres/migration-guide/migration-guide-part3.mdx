...
We recommend users migrating from Postgres read [the guide for modeling data in ClickHouse](/guides/cloud-oss/data-modelling/schema-design). This guide uses the same Stack Overflow dataset and explores multiple approaches using ClickHouse features.
...
- Postgres primary keys are, by definition, unique per row. The use of [B-tree structures](/guides/cloud-oss/data-modelling/sparse-primary-indexes#an-index-design-for-massive-data-scales) allows the efficient lookup of single rows by this key. While ClickHouse can be optimized for the lookup of a single row value, analytics workloads will typically require the reading of a few columns but for many rows. Filters will more often need to identify **a subset of rows** on which an aggregation will be performed.
...
For more details, we recommend this [in-depth guide](/guides/cloud-oss/data-modelling/sparse-primary-indexes).

<Image img="/static/images/migrations/postgres-b-tree.png" size="lg" alt="PostgreSQL B-Tree Index"/>

<Image img="/static/images/migrations/postgres-sparse-index.png" size="lg" alt="PostgreSQL Sparse Index"/>
...
<Image img="/static/images/migrations/postgres-partitions.png" size="md" alt="PostgreSQL partitions to ClickHouse partitions"/>
...
In ClickHouse [data modeling docs](/guides/cloud-oss/data-modelling/schema-design), we explore how materialized views can be used in ClickHouse to pre-compute aggregations, transform rows, and optimize queries for different access patterns.
...
<Image img="/static/images/migrations/postgres-projections.png" size="md" alt="PostgreSQL projections in ClickHouse"/>
...
- Users are comfortable with the associated increase in storage footprint and 
  ...
  [evaluate the storage overhead](/guides/cloud-oss/data-modelling/compression/compression-in-clickhouse).
...
You can refer to this [guide](/guides/cloud-oss/data-modelling/denormalization) that shows the benefits of denormalizing the Stack Overflow dataset in ClickHouse.

This concludes our basic guide if you're migrating from Postgres to ClickHouse. We recommend reading the [guide for modeling data in ClickHouse](/guides/cloud-oss/data-modelling/schema-design) to learn more about advanced ClickHouse features.
