...
<Image img="/static/images/migrations/postgres-stackoverflow-schema.png" size="lg" alt="PostgreSQL Stack Overflow schema"/>
...
When loading data manually from PostgreSQL, you need to first create the tables in ClickHouse. Refer to this [Data Modeling documentation](/guides/cloud-oss/data-modelling/schema-design#establish-initial-schema) to that also uses the Stack Overflow dataset to optimize the table schema in ClickHouse. 
...
For an overview of data type mapping between PostgreSQL and ClickHouse, refer to the [appendix documentation](/get-started/setup/migration-guides/postgres/appendix#data-type-mappings).

The steps for optimizing the types for this schema are identical to if the data has been loaded from other sources e.g. Parquet on S3. Applying the process described in this [alternate guide using Parquet](/guides/cloud-oss/data-modelling/schema-design) results in the following schema:
...
[Click here for Part 2](/get-started/setup/migration-guides/postgres/migration-guide/migration-guide-part2)
