...
<Image img="/static/images/migrations/bigquery-2.png" size="md" alt="ClickHouse vs BigQuery"/>
...
<Image img="/static/images/migrations/bigquery-3.png" size="lg" alt="Schema"/>
...
<Image img="/static/images/migrations/bigquery-4.png" size="md" alt="Bulk loading"/>
...
Applying the process [described here](/guides/cloud-oss/data-modelling/schema-design) results in the following schema:
...
As described [here](/get-started/setup/migration-guides/bigquery), like in BigQuery, ClickHouse doesn't enforce uniqueness for a table's primary key column values.
...
In contrast to BigQuery, ClickHouse automatically creates [a (sparse) primary index](/guides/cloud-oss/data-modelling/sparse-primary-indexes) based on the primary key column values. This index is used to speed up all queries that contain filters on the primary key columns. Specifically:
...
For the considerations and steps in choosing an ordering key, using the posts table as an example, see [here](/guides/cloud-oss/data-modelling/schema-design#choosing-an-ordering-key).

## Data modeling techniques {#data-modeling-techniques}

We recommend users migrating from BigQuery read [the guide for modeling data in ClickHouse](/guides/cloud-oss/data-modelling/schema-design). This guide uses the same Stack Overflow dataset and explores multiple approaches using ClickHouse features.
...
<Image img="/static/images/migrations/bigquery-6.png" size="md" alt="Partitions"/>
...
In [ClickHouse data modeling](/guides/cloud-oss/data-modelling/schema-design), we explore how materialized views can be used
...
<Image img="/static/images/migrations/bigquery-7.png" size="md" alt="Projections"/>
...
- Users are comfortable with the associated increase in storage footprint and overhead of writing data twice. Test the impact on insertion speed and [evaluate the storage overhead](/guides/cloud-oss/data-modelling/compression/compression-in-clickhouse).
...
<Image img="/static/images/migrations/bigquery-8.png" size="sm" alt="Rewriting BigQuery queries" border/>
...
<Image img="/static/images/migrations/bigquery-9.png" size="sm" alt="BigQuery 1" border/>
...
Where possible, you should exploit ClickHouse aggregate functions. Below, we show the use of the [`argMax` function](/reference/functions/aggregate-functions/reference/argMax) to compute the most viewed question of each year.

_BigQuery_

<Image img="/static/images/migrations/bigquery-10.png" border size="sm" alt="Aggregate functions 1"/>

<Image img="/static/images/migrations/bigquery-11.png" border size="sm" alt="Aggregate functions 2"/>
...
<Image img="/static/images/migrations/bigquery-12.png" size="sm" border alt="Conditionals and Arrays"/>
...
This concludes our basic guide if you're migrating from BigQuery to ClickHouse. We recommend reading the guide for [modeling data in ClickHouse](/guides/cloud-oss/data-modelling/schema-design) to learn more about advanced ClickHouse features.
