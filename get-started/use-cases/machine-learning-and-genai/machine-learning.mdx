...
<Image img="/static/images/cloud/onboard/discover/use_cases/ml_data_layer.png" size="sm"/>
...
In contrast, data transformations are easily accomplished directly in ClickHouse through [materialized views](/concepts/features/materialized-views).
...
For those datasets that are more convenient to evaluate locally, [ClickHouse local](/concepts/features/tools-and-utilities/clickhouse-local) is a great alternative, along with [chDB](/products/chdb), that allow you to leverage ClickHouse with standard Python data libraries like Pandas.
...
On the other hand, it could be managed in the data layer itself. ClickHouse [User Defined Functions (UDFs)](/reference/functions/regular-functions/udf), give users the ability to invoke a model directly from ClickHouse at insert or query time.
