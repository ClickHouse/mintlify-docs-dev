...
<Steps>
<Step>
## Install Jan.ai <a id="install-janai"></a>
...
<Step>
## Add LLM to Jan.ai <a id="add-llm-to-janai"></a>

We can enabled models via the settings menu. 

To enable OpenAI, we need to provide an API key, as shown below:

<Image img="/images/use-cases/AI_ML/MCP/0_janai_openai.png" alt="Enable OpenAI models" size="md"/>
</Step>
<Step>
## Enable MCP servers <a id="enable-mcp-servers"></a>
...
<Image img="/images/use-cases/AI_ML/MCP/1_janai_mcp_servers.png" alt="Enable MCP servers" size="md"/>

Once that toggle is pressed, we'll see `MCP servers` on the left menu.
</Step>
<Step>
## Configure ClickHouse MCP server <a id="configure-clickhouse-mcp-server"></a>

If we click on the `MCP servers` menu, we'll see a list of MCP servers that we can connect to:

<Image img="/images/use-cases/AI_ML/MCP/2_janai_mcp_servers_list.png" alt="MCP servers list" size="md"/>
...
<Image img="/images/use-cases/AI_ML/MCP/3_janai_add_mcp_server.png" alt="Add MCP server" size="md"/>

Once we've done that, we'll need to toggle the ClickHouse Server if it's not already toggled:

<Image img="/images/use-cases/AI_ML/MCP/4_janai_toggle.png" alt="Enable MCP server" size="md"/>

The ClickHouse MCP server's tools will now be visible on the chat dialog:

<Image img="/images/use-cases/AI_ML/MCP/5_jani_tools.png" alt="ClickHouse MCP server tools" size="md"/>
</Step>
<Step>
## Chat to ClickHouse MCP server with Jan.ai <a id="chat-to-clickhouse-mcp-server"></a>

It's time to have a conversation about some data stored in ClickHouse!
Let's ask a question:

<Image img="/images/use-cases/AI_ML/MCP/6_janai_question.png" alt="Question" size="md"/>

Jan.ai will ask confirmation before calling a tool:

<Image img="/images/use-cases/AI_ML/MCP/7_janai_tool_confirmation.png" alt="Tool confirmation" size="md"/>

It will then show us the list of tool calls that were made:

<Image img="/images/use-cases/AI_ML/MCP/8_janai_tools_called.png" alt="Tools called" size="md"/>

If we click on the tool call, we can see the details of the call:

<Image img="/images/use-cases/AI_ML/MCP/9_janai_tools_called_expanded.png" alt="Tools called expanded" size="md"/>    

And then underneath, we have our result:

<Image img="/images/use-cases/AI_ML/MCP/10_janai_result.png" alt="Result" size="md"/>    
</Step>
</Steps>
