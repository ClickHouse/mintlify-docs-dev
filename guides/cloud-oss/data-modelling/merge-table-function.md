...
The [merge table function](/sql-reference/table-functions/merge) lets us query multiple tables in parallel.
It does this by creating a temporary [Merge](/engines/table-engines/special/merge) table and derives this table's structure by taking a union of their columns and by deriving common types.
...
We use the [`variantType`](/sql-reference/functions/other-functions#variantType) function to check the type of `winner_seed` for each row and then [`variantElement`](/sql-reference/functions/other-functions#variantElement) to extract the underlying value.
