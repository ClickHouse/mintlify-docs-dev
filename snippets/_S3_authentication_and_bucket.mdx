...
<Image size="md" img="/images/_snippets/s3/s3-1.png" alt="AWS IAM Management Console - Adding a new user" border force/>

3. Enter the username and set the credential type to `Access key - Programmatic access` and select `Next: Permissions`

<Image size="md" img="/images/_snippets/s3/s3-2.png" alt="Setting user name and access type for IAM user" border force/>

4. Don't add the user to any group; select `Next: Tags`

<Image size="md" img="/images/_snippets/s3/s3-3.png" alt="Skipping group assignment for IAM user" border force/>

5. Unless you need to add any tags, select `Next: Review`

<Image size="md" img="/images/_snippets/s3/s3-4.png" alt="Skipping tag assignment for IAM user" border force/>
...
<Image size="md" img="/images/_snippets/s3/s3-5.png" alt="Creating the IAM user with no permissions warning" border force/>
...
<Image size="md" img="/images/_snippets/s3/s3-6.png" alt="Viewing and copying the IAM user access keys" border force/>

8. Click close, then find the user in the users screen.

<Image size="md" img="/images/_snippets/s3/s3-7.png" alt="Finding the newly created IAM user in the users list" border force/>

9. Copy the ARN (Amazon Resource Name) and save it for use when configuring the access policy for the bucket.

<Image size="md" img="/images/_snippets/s3/s3-8.png" alt="Copying the ARN of the IAM user" border force/>

### Create an S3 bucket <a id="create-an-s3-bucket"></a>

1. In the S3 bucket section, select `Create bucket`

<Image size="md" img="/images/_snippets/s3/s3-9.png" alt="Starting the S3 bucket creation process" border force/>
...
<Image size="md" img="/images/_snippets/s3/s3-a.png" alt="Configuring the S3 bucket settings with public access blocked" border force/>

4. Select `Create Bucket` at the bottom of the page

<Image size="md" img="/images/_snippets/s3/s3-b.png" alt="Finalizing S3 bucket creation" border force/>
...
<Image size="md" img="/images/_snippets/s3/s3-c.png" alt="Finding the newly created S3 bucket in the buckets list" border force/>

7. Select `Create folder`

<Image size="md" img="/images/_snippets/s3/s3-d.png" alt="Creating a new folder in the S3 bucket" border force/>

8. Enter a folder name that will be the target for the ClickHouse S3 disk and select `Create folder`

<Image size="md" img="/images/_snippets/s3/s3-e.png" alt="Setting the folder name for ClickHouse S3 disk usage" border force/>

9. The folder should now be visible on the bucket list

<Image size="md" img="/images/_snippets/s3/s3-f.png" alt="Viewing the newly created folder in the S3 bucket" border force/>
...
<Image size="md" img="/images/_snippets/s3/s3-g.png" alt="Copying the S3 folder URL for ClickHouse configuration" border force/>

11. Select the `Permissions` tab and click on the `Edit` button in the `Bucket Policy` section

<Image size="md" img="/images/_snippets/s3/s3-h.png" alt="Accessing the S3 bucket policy configuration" border force/>
