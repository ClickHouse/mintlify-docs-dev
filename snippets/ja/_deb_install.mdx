...
<Steps>
  ...
  <Step>
    ...
    * 必要に応じて `stable` を `lts` に置き換え、[リリース種別](/ja/resources/support-center/knowledge-base/setup-installation/production)を選択できます。
  ...
  <Step>
    ...
    <Tip>
      ...
      テスト環境で ClickHouse Server と ClickHouse Keeper を同じサーバー上で実行する場合は、
      ClickHouse サーバー に ClickHouse Keeper が含まれているため、別途インストールする必要はありません。
    </Tip>
