...
sidebarTitle: 'Get started with Managed ClickStack'
...
<Note>
**Existing collector:** If you're using an existing OpenTelemetry collector, we assume it's already configured in a **gateway** role. We don't recommend using this process for reconfiguring collectors in the **agent** role.
</Note>
...
<Note>
**Production deployments:** For production, we recommend enabling TLS on the OTLP endpoint. See [Securing the collector](/clickstack/ingesting-data/collector#securing-the-collector).
</Note>
...
<Note>
**ClickHouse exporter required:** If you're using your own distribution, ensure it includes the ClickHouse exporter. The upstream [contrib image](https://github.com/open-telemetry/opentelemetry-collector-contrib) already does.
</Note>
...
<Image img="/images/use-cases/observability/clickstack-cloud-first-time.png" size="lg" alt="Launch ClickStack" border/>
...
{/* TODO: The following Docusaurus links point to Managed ClickStack onboarding pages that have not yet been ported to Mintlify:
  ...
  Add the links back once the corresponding pages are migrated. */}
