...
- A ClickStack collector reachable from this machine. If you haven't deployed one yet, see the [OpenTelemetry collector guide](/clickstack/ingesting-data/collector) first.
- The OTLP endpoint of that collector and the `OTLP_AUTH_TOKEN` you set on it.
- Node 18+ and npm.

<Steps>
<Step title="Clone and run the application">
...
<Step title="Get the connection details">
...
<Step title="Generate traffic and view telemetry">
...
- [OpenTelemetry collector](/clickstack/ingesting-data/collector) for configuring and deploying the collector.
