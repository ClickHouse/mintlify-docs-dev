...
ClickStack's [default schema](/clickstack/ingesting-data/schemas) stores resource, scope, log, and span attributes as `Map(LowCardinality(String), String)` columns. ClickHouse also supports a strongly typed [`JSON` type](/core/reference/formats/JSON/JSON), and ClickStack has beta support for using it in place of `Map`.
