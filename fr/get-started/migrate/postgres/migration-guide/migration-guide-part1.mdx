...
<Image img="/images/migrations/postgres-stackoverflow-schema.png" size="lg" alt="Schéma PostgreSQL de Stack Overflow" />
...
> Une méthode possible pour détecter les opérations UPDATE lors de l&#39;utilisation de la réplication des requêtes consiste à utiliser la [colonne système `XMIN`](https://www.postgresql.org/docs/9.1/ddl-system-columns.html) (identifiants de transaction) comme watermark - une modification de cette colonne indique qu&#39;un changement a eu lieu et peut donc être répercutée sur la table de destination. Les utilisateurs qui adoptent cette approche doivent savoir que les valeurs `XMIN` peuvent reboucler et que les comparaisons nécessitent un parcours complet de la table, ce qui complique le suivi des modifications.
