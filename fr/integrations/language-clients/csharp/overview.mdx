...
| AcceptEncoding        | `string`                      | Remplace, pour cette requête, l’en-tête `Accept-Encoding` (par ex. `"zstd"`, `"zstd, gzip;q=0.5"`) ; force également `enable_http_compression=1` dans l’URL. Voir [Compression de transport par requête](#per-query-accept-encoding). |
...
<Image img="/images/_snippets/connection-details-csharp.png" size="md" alt="Détails de connexion C# de ClickHouse Cloud" border />
