...
1. Dans Google Cloud Console, accédez à la page AlloyDB [Clusters](https://console.cloud.google.com/alloydb/clusters). Dans le menu **Actions** de votre instance primaire, cliquez sur **Edit**.
   ...
   <Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/alloydb/1_edit_instance.png" alt="Modifier la configuration de l’instance primaire" size="lg" border />
...
2. Faites défiler la page jusqu’à **Advanced configuration options** et développez la section. Sous **Flags**, cliquez sur **Add a database flag**.
   ...
   * Ajoutez le flag [`alloydb.logical_decoding`](https://cloud.google.com/alloydb/docs/reference/alloydb-flags#alloydb.logical_decoding) et définissez sa valeur sur `on`
   ...
   <Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/alloydb/2_set_flags.png" alt="Définir les flags allowdb.enable_pglogical et alloydb.logical_decoding sur on" size="lg" border />
...
4. Une fois que l’état de l’instance passe de `Updating` à `Ready`, exécutez la requête suivante sur votre instance primaire pour vérifier que la réplication logique est activée :
   ...
   <Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/alloydb/3_verify_logical_replication.png" alt="Vérifier que la réplication logique est activée" size="lg" border />
...
<Tabs>
  <Tab title="Autoriser les IP de ClickPipes">
    ...
    3. Cochez la case **Enable Public IP** pour autoriser les connexions à l’instance via l’internet public. Sous **Authorized external networks**, saisissez la [liste des adresses IP statiques de ClickPipes](/fr/integrations/clickpipes/home#list-of-static-ips) pour la région dans laquelle votre service est déployé.
       ...
       <Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/alloydb/4_configure_network_security.png" alt="Configurer le réseau pour un accès public avec une liste d’autorisation d’IP" size="lg" border />
