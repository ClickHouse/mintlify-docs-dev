...
2. Dans le cas particulier de ClickHouse, vous avez peut-être dû modifier les clés ORDER BY des tables cibles. Vous pouvez lancer un Resync pour remplir à nouveau les données dans la nouvelle table avec la bonne clé de tri.
...
8. Une fois le chargement initial terminé, le pipe échangera de manière atomique les tables `_resync` avec les tables d’origine. Pendant cet échange, le statut sera **Resync**.
