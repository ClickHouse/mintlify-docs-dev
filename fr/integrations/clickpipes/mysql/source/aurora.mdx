...
Le journal binaire est un ensemble de fichiers de log contenant des informations sur les modifications de données effectuées sur une instance de serveur MySQL, et ces fichiers sont nécessaires à la réplication. Pour configurer la rétention du journal binaire dans Aurora MySQL, vous devez [activer la journalisation binaire](#enable-binlog-logging) et [augmenter l’intervalle de rétention du binlog](#binlog-retention-interval).

<div id="enable-binlog-logging">
  ### 1. Activer la journalisation binaire via les sauvegardes automatiques
</div>

La fonctionnalité de sauvegardes automatiques détermine si la journalisation binaire est activée ou non pour MySQL. Vous pouvez configurer les sauvegardes automatiques pour votre instance dans la RDS Console en accédant à **Modify** &gt; **Additional configuration** &gt; **Backup**, puis en cochant la case **Enable automated backups** (si ce n’est pas déjà fait).

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/rds-backups.png" alt="Activation des sauvegardes automatiques dans Aurora" size="lg" border />

Nous vous recommandons de définir **Backup retention period** sur une durée suffisamment longue, selon votre cas d’usage de la réplication.
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/aurora_config.png" alt="Où trouver le groupe de paramètres dans Aurora" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/edit_button.png" alt="Modifier le groupe de paramètres" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_format.png" alt="Format du binlog sur ROW" size="lg" border />

2. `binlog_row_metadata` sur `FULL`.

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_metadata.png" alt="Métadonnées de ligne du binlog" size="lg" border />

3. `binlog_row_image` sur `FULL`.

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_image.png" alt="Image de ligne du binlog" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/enable_gtid.png" alt="GTID activé" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/security-group-in-rds-mysql.png" alt="Où trouver le groupe de sécurité dans Aurora MySQL ?" size="lg" border />

<Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/rds/edit_inbound_rules.png" alt="Modifier les Inbound rules pour le groupe de sécurité ci-dessus" size="lg" border />
