---
sidebarTitle: 'Luzmo'
slug: /integrations/luzmo
keywords: ['ClickHouse', 'Luzmo', 'connecter', 'intégrer', 'UI', 'embarqué']
description: 'Luzmo est une plateforme d’analytique embarquée dotée d’une intégration ClickHouse native, conçue spécifiquement pour les applications logicielles et SaaS.'
title: 'Intégration de Luzmo à ClickHouse'
sidebar: 'integrations'
doc_type: 'guide'
integration:
  - support_level: 'community'
  - category: 'data_visualization'
---

import { Image } from "/snippets/components/Image.jsx";

import CommunityMaintainedBadge from "/snippets/components/CommunityMaintainedBadge/CommunityMaintainedBadge.jsx";

<CommunityMaintainedBadge />

<div id="1-setup-a-clickhouse-connection">
  ## 1. Configurer une connexion à ClickHouse
</div>

Pour établir une connexion à ClickHouse, accédez à la **page Connections**, sélectionnez **New Connection**, puis choisissez ClickHouse dans la fenêtre modale **New Connection**.

<Image img="/images/integrations/data-visualization/luzmo_01.png" size="md" alt="Interface Luzmo affichant la boîte de dialogue Create a New Connection avec ClickHouse sélectionné" border />

Vous devrez renseigner un **host**, un **username** et un **password** :

<Image img="/images/integrations/data-visualization/luzmo_02.png" size="md" alt="Formulaire de configuration de connexion Luzmo affichant les champs pour le host, le username et le password de ClickHouse" border />

* **Host** : il s’agit du host sur lequel votre base de données ClickHouse est exposée. Notez que seul `https` est autorisé ici afin de transférer les données de façon sécurisée. La structure attendue pour l’URL du host est la suivante : `https://url-to-clickhouse-db:port/database`
  Par défaut, le plugin se connecte à la base de données &#39;default&#39; sur le port 443. En indiquant une base de données après le &#39;/&#39;, vous pouvez définir la base de données à laquelle vous connecter.
* **Username** : le username qui sera utilisé pour se connecter à votre cluster ClickHouse.
* **Password** : le password permettant de se connecter à votre cluster ClickHouse

Consultez les exemples de notre documentation développeur pour savoir comment [créer une connexion à ClickHouse](https://developer.luzmo.com/api/createAccount?exampleSection=AccountCreateClickhouseRequestBody) via notre API.

<div id="2-add-datasets">
  ## 2. Ajouter des jeux de données
</div>

Une fois votre instance ClickHouse connectée, vous pouvez ajouter des jeux de données comme expliqué [ici](https://academy.luzmo.com/article/ldx3iltg). Vous pouvez sélectionner un ou plusieurs jeux de données disponibles dans ClickHouse et les [lier](https://academy.luzmo.com/article/gkrx48x5) dans Luzmo afin de pouvoir les utiliser ensemble dans un tableau de bord. Pensez également à consulter cet article sur la [préparation de vos données pour l’analyse](https://academy.luzmo.com/article/u492qov0).

Pour savoir comment ajouter des jeux de données à l’aide de notre API, consultez [cet exemple dans notre documentation développeur](https://developer.luzmo.com/api/createDataprovider?exampleSection=DataproviderCreateClickhouseRequestBody).

Vous pouvez désormais utiliser vos jeux de données pour créer de superbes tableaux de bord (intégrés), ou même alimenter un analyste de données IA ([Luzmo IQ](https://luzmo.com/iq)) capable de répondre aux questions de vos clients.

<Image img="/images/integrations/data-visualization/luzmo_03.png" size="md" alt="Exemple de tableau de bord Luzmo montrant plusieurs visualisations de données issues de ClickHouse" border />

<div id="usage-notes">
  ## Notes d’utilisation
</div>

1. Le connecteur ClickHouse de Luzmo utilise l’interface HTTP de l’API (généralement en écoute sur le port 8123) pour se connecter.
2. Si vous utilisez des tables avec le moteur de table `Distributed`, certains graphiques Luzmo peuvent échouer lorsque `distributed_product_mode` est défini sur `deny`. Cela ne devrait toutefois se produire que si vous liez la table à une autre table et utilisez cette liaison dans un graphique. Dans ce cas, veillez à définir `distributed_product_mode` sur une autre option adaptée à votre cluster ClickHouse. Si vous utilisez ClickHouse Cloud, vous pouvez ignorer ce paramètre en toute sécurité.
3. Pour garantir, par exemple, que seule l’application Luzmo puisse accéder à votre instance ClickHouse, il est fortement recommandé d’**autoriser** la [plage d’adresses IP statiques de Luzmo](https://academy.luzmo.com/article/u9on8gbm). Nous recommandons également d’utiliser un utilisateur technique en lecture seule.
4. Le connecteur ClickHouse prend actuellement en charge les types de données suivants :

   | Type ClickHouse | Type Luzmo |
   | --------------- | ---------- |
   | UInt            | numeric    |
   | Int             | numeric    |
   | Float           | numeric    |
   | Decimal         | numeric    |
   | Date            | datetime   |
   | DateTime        | datetime   |
   | String          | hierarchy  |
   | Enum            | hierarchy  |
   | FixedString     | hierarchy  |
   | UUID            | hierarchy  |
   | Bool            | hierarchy  |