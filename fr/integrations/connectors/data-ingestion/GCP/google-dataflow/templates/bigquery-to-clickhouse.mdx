...
<Tabs>
  <Tab title="Google Cloud Console">
    ...
    1. Cliquez sur le bouton `CREATE JOB FROM TEMPLATE`
       <Image img="/images/integrations/data-ingestion/google-dataflow/create_job_from_template_button.png" border alt="Console DataFlow" />
    2. Une fois le formulaire du template ouvert, saisissez un nom de job et sélectionnez la région souhaitée.
       <Image img="/images/integrations/data-ingestion/google-dataflow/template_initial_form.png" border alt="Formulaire initial du template DataFlow" />
    3. Dans le champ `DataFlow Template`, saisissez `ClickHouse` ou `BigQuery`, puis sélectionnez le template `BigQuery to ClickHouse`
       <Image img="/images/integrations/data-ingestion/google-dataflow/template_clickhouse_search.png" border alt="Sélectionner le template BigQuery to ClickHouse" />
    ...
    <Image img="/images/integrations/data-ingestion/google-dataflow/extended_template_form.png" border alt="Formulaire étendu du template BigQuery vers ClickHouse" />
  ...
  <Tab title="Google Cloud CLI">
    ...
    * **Nom du job :** Le texte qui suit le keyword `run` correspond au nom unique du job.
    * **Fichier du Template :** Le fichier JSON spécifié par `--template-file-gcs-location` définit la structure du template et
      les détails des paramètres acceptés. Le file path mentionné est public et prêt à l’emploi.
...
<Image img="/images/integrations/data-ingestion/google-dataflow/dataflow-inqueue-job.png" size="lg" border alt="Console Dataflow affichant un job BigQuery vers ClickHouse en cours d’exécution" />
