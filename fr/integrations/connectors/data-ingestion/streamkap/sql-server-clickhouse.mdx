...
* Pour ce tutoriel, nous utilisons AWS RDS pour SQL Server, mais toute instance SQL Server moderne fera l’affaire.[Configurer AWS SQL Server à partir de zéro.](https://streamkap.com/blog/how-to-stream-data-from-rds-sql-server-to-clickhouse-cloud-using-streamkap%23setting-up-a-new-rds-sql-server-from-scratch)

* Une instance ClickHouse

* Auto-hébergée ou dans le cloud.[Configurer ClickHouse à partir de zéro.](https://streamkap.com/blog/how-to-stream-data-from-rds-sql-server-to-clickhouse-cloud-using-streamkap%23creating-a-new-clickhouse-account)
...
Commencez par configurer la connexion à la source. C’est ainsi que Streamkap sait où récupérer les modifications.
...
3. Renseignez les informations de connexion à SQL Server :
...
Vous vous demandez peut-être : faut-il faire un backfill des anciennes données ?

Dans de nombreux cas d’usage analytiques, il suffit de commencer à traiter les nouvelles modifications à partir de maintenant, quitte à revenir plus tard pour charger aussi les données plus anciennes.

Choisissez simplement « ne pas faire de backfill » pour l’instant, sauf si vous avez un besoin précis.
...
Tableau de bord en direct ou logs montrant le nombre de lignes augmenter dans ClickHouse et SQL Server en temps réel.
