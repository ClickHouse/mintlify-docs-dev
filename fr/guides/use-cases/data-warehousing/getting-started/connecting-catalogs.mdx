...
Databricks prend en charge plusieurs formats de données pour son lakehouse. Avec ClickHouse, vous pouvez interroger les tables de Unity Catalog au format Delta comme au format Iceberg.

<Note>
  L’intégration avec Unity Catalog fonctionne pour les tables gérées et les tables externes.
  Cette intégration est actuellement prise en charge uniquement sur AWS.
</Note>
