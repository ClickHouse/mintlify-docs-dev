...
<Image img="/images/use-cases/observability/observability-10.png" alt="Vue matérialisée" size="md" />
...
Les [dictionnaires](/fr/reference/statements/create/dictionary) sont une [fonctionnalité clé](https://clickhouse.com/blog/faster-queries-dictionaries-clickhouse) de ClickHouse : ils fournissent une représentation [clé-valeur](https://en.wikipedia.org/wiki/Key%E2%80%93value_database) en mémoire de données provenant de diverses [sources](/fr/reference/statements/create/dictionary/sources/overview#dictionary-sources), optimisée pour des requêtes de lookup à très faible latence.

<Image img="/images/use-cases/observability/observability-12.png" alt="Observabilité et dictionnaires" size="md" />

Cela s’avère utile dans divers scénarios, qu’il s’agisse d’enrichir à la volée les données ingérées sans ralentir le processus d’ingestion, ou d’améliorer les performances des requêtes de manière générale, en particulier pour les JOINs.
Même si les jointures sont rarement nécessaires dans les cas d’usage d’observabilité, les dictionnaires restent utiles pour l’enrichissement, aussi bien à l’insertion qu’au moment de l’exécution des requêtes. Nous donnons ci-dessous des exemples des deux approches.
...
<Image img="/images/use-cases/observability/observability-13.png" alt="Observability et projections" size="md" />
...
Outre la réduction considérable des coûts de stockage, moins de données sur le disque signifie moins d’entrées/sorties et des requêtes et insertions plus rapides. La réduction des E/S compense largement la surcharge de n’importe quel algorithme de compression du point de vue du CPU. Améliorer la compression des données doit donc être la priorité absolue si vous voulez garantir la rapidité des requêtes ClickHouse.

Vous trouverez plus de détails sur la mesure de la compression [ici](/fr/guides/clickhouse/data-modelling/compression/compression-in-clickhouse).