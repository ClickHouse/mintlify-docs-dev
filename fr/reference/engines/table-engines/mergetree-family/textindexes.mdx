...
Si vous ajoutez un index à une table existante, nous vous recommandons de matérialiser l’index pour les parts de la table existantes (sinon, la recherche sur les parts sans index reviendra à des scans exhaustifs lents).
...
**Argument `postprocessor` (facultatif)**. Le postprocesseur désigne une expression appliquée à chaque token de sortie après la tokenisation.
...
3. **Racinisation**. Ramener chaque token à sa racine améliore le rappel de recherche en faisant correspondre des variantes morphologiques qui partagent la même racine.
   ...
   ClickHouse fournit une fonction [stem](/fr/reference/functions/regular-functions/string-functions#stem) intégrée pour plusieurs langues.
   Exemple : `stem(str, 'en')`
...
Lorsque la colonne est de type `Array(String)`, le postprocesseur continue d’opérer sur des tokens individuels en tant que valeurs `String` simples.
...
¹ `LIKE` et `match` utilisent la lecture directe comme indice pour les tokenizers listés ; sinon, ils reviennent à un balayage en force brute.
...
Le paramètre expérimental `positions` (par défaut : `0`) contrôle si l’index stocke les positions des tokens.
...
La création d’un index avec `positions = 1` nécessite donc que le paramètre MergeTree [`allow_experimental_text_index_positions`](/fr/reference/settings/merge-tree-settings#allow_experimental_text_index_positions) soit activé.
Définissez `positions = 0` (la valeur par défaut) pour conserver un stockage reposant uniquement sur des posting lists ; les index de texte créés sans cet argument restent sans positions.

<Warning>
  Cet argument est expérimental et ne doit être utilisé que pour des tests.
  Activez le paramètre MergeTree [`allow_experimental_text_index_positions`](/fr/reference/settings/merge-tree-settings#allow_experimental_text_index_positions) pour autoriser le stockage des positions.
</Warning>
