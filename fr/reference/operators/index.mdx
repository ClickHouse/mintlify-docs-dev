...
<Note>
  Contrairement à PostgreSQL, où `timestamp without time zone AT TIME ZONE zone` réinterprète l’heure affichée comme appartenant au fuseau donné avant la conversion, ClickHouse conserve toujours le même instant absolu et modifie uniquement l’étiquette de fuseau horaire utilisée pour l’affichage. Les deux formes sont équivalentes à `toTimeZone` et ne modifient pas le timestamp sous-jacent.
</Note>

`AT TIME ZONE` a une priorité d’opérateur de 13 (au-dessus de `*`/`/`/`%` à 12, et au-dessus de `+`/`-` à 11), comme dans PostgreSQL. Cela signifie que `a * ts AT TIME ZONE 'tz'` s’associe comme `a * (ts AT TIME ZONE 'tz')`, et que `ts + interval AT TIME ZONE 'tz'` s’associe comme `ts + (interval AT TIME ZONE 'tz')`. Pour appliquer la conversion de fuseau horaire après l’opération arithmétique, utilisez des parenthèses explicites :
