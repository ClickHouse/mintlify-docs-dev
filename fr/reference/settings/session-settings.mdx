...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Les écritures Delta Lake sont passées en bêta."}]}, {"id": "row-2","items": [{"label": "25.9"},{"label": "0"},{"label": "Nouveau paramètre."}]}]} />

Active la fonctionnalité d’écriture avec delta-kernel.
...
Autorise l’expansion expérimentale des caractères génériques pour `url()` et `ENGINE = URL` à partir de pages d’index HTTP.
...
Autorise les arguments de fuseau horaire non constants dans certaines fonctions liées à l’heure, comme toTimeZone(), fromUnixTimestamp*() et snowflakeIDToDateTime*().
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Nouveau contrôle de sécurité : `ALTER TABLE ... REPLACE PARTITION ... FROM ...` lève désormais une exception lorsque la table source ne contient aucune part dans la partition demandée (corrige la perte silencieuse de données dans [#23727](https:\/\/github.com\/ClickHouse\/ClickHouse\/issues\/23727)). L’ancien comportement, qui supprimait silencieusement la partition de destination, est conservé si `allow_replace_partition_from_empty_source = 1`."}]}]} />

Autorise `ALTER TABLE ... REPLACE PARTITION ... FROM ...` à supprimer silencieusement la partition de destination lorsque la source ne contient aucune part dans cette partition.

Par défaut, cela n’est pas autorisé : `REPLACE PARTITION` à partir d’une source qui ne contient aucune donnée dans la partition demandée lève une exception, car dans ce cas l’opération revient en pratique à exécuter un `DROP PARTITION` silencieux sur la destination (les données de destination sont supprimées et rien ne les remplace), ce qui est une cause fréquente de perte de données accidentelle (voir [#23727](https://github.com/ClickHouse/ClickHouse/issues/23727)).

Activez ce paramètre pour rétablir l’ancien comportement, par exemple si vous utilisez volontairement une partition source vide pour effacer les données de destination. Pour une suppression inconditionnelle, préférez `ALTER TABLE ... DROP PARTITION ...`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.3"},{"label": "0"},{"label": "Interdire les PRIMARY KEY/ORDER BY suspects pour MergeTree (c.-à-d. SimpleAggregateFunction)"}]}]} />

Autoriser les `PRIMARY KEY`/`ORDER BY` suspects pour MergeTree (c.-à-d. SimpleAggregateFunction).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.7"},{"label": "1000"},{"label": "Paramètre pour Aws::Client::RetryStrategy, Aws::Client gère lui-même les tentatives, 0 signifie aucune tentative. S’applique uniquement à la sauvegarde\/restauration."}]}]} />

Paramètre pour Aws::Client::RetryStrategy, Aws::Client gère lui-même les tentatives, 0 signifie aucune tentative. S’applique uniquement à la sauvegarde/restauration.
...
N’a d’effet que dans ClickHouse Cloud. Affiche les colonnes `certificate` et `signature` dans la table `system.distributed_cache_registry`. Par défaut, ces colonnes sont vides afin de conserver une sortie compacte ; activez ce paramètre pour les examiner.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Nouveau paramètre expérimental définissant le nombre de workers sans état à réserver pour une requête distribuée. Zéro désactive cette réservation."}]}]} />

Nombre de workers sans état qui seront utilisés pour exécuter cette requête. Zéro désactive la réservation de workers sans état pour les plans distribués.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "À partir de la version 26.7, `EXPLAIN PLAN` utilise par défaut `actions=1, compact=1, pretty=1`. Définissez cette valeur sur `legacy` pour restaurer la sortie antérieure à la version 26.7."}]}]} />

Format par défaut utilisé par `EXPLAIN PLAN`.

Valeurs possibles :

* `pretty` (par défaut depuis la version 26.7) — `actions`, `compact` et `pretty` prennent par défaut la valeur `true`, ce qui produit un plan compact, lisible et annoté avec les actions.
* `legacy` — sortie antérieure à la version 26.7.

Le fait de spécifier explicitement les options `actions`, `compact` ou `pretty` dans l’instruction `EXPLAIN` (par exemple, `EXPLAIN actions = 0, compact = 0, pretty = 0 SELECT ...`) l’emporte toujours sur ce paramètre.

`EXPLAIN PLAN` avec `json = 1` ou `distributed = 1` conserve les valeurs par défaut de `legacy` (antérieures à la version 26.7), quel que soit ce paramètre, sauf si `actions`, `compact` ou `pretty` sont définis explicitement. La sortie `pretty` ne peut pas représenter les résultats JSON ni les plans distribués par shard ; ces modes ne sont donc correctement rendus que sous la forme `legacy`.
...
<Note>
  `insert_deduplication_token` est suivi par partition, de sorte que plusieurs partitions écrites par une seule insertion peuvent porter le même jeton. Sans jeton, la somme de contrôle de contenu par défaut (`insert_deduplication_version = new_unified_hash`) est calculée sur l&#39;ensemble du bloc inséré, de sorte qu&#39;une insertion n&#39;est dédupliquée que lorsque l&#39;intégralité de ses données correspond à une insertion précédente (une nouvelle tentative), et non lorsqu&#39;il se trouve que les lignes d&#39;une seule partition coïncident avec une insertion différente.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Nouveau paramètre permettant d'appliquer `LIMIT` à l'aggregation-in-order afin d'autoriser un arrêt anticipé lorsque `ORDER BY` est un préfixe de la description de tri de `GROUP BY`."}]}]} />

Lorsqu&#39;il est activé et que l&#39;aggregation-in-order est active, ce paramètre applique `LIMIT` à l&#39;étape d&#39;agrégation afin de permettre un arrêt anticipé une fois qu&#39;un nombre suffisant de groupes a été produit. Cela réduit la quantité de données lues lorsque `ORDER BY` correspond au préfixe de la clé `GROUP BY`. La valeur renvoyée par `rows_before_limit_at_least` peut être réduite ; utilisez `exact_rows_before_limit` si des décomptes exacts sont nécessaires.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "Nouveau paramètre qui limite le travail de l’optimisation `optimize_and_compare_chain` (mesuré en nœuds de l’arbre de requête hachés) afin qu’elle ne monopolise pas l’analyse des requêtes comportant un très grand nombre de chaînes `AND` de comparaisons, ou des chaînes `AND` très longues. La valeur précédente `0` (illimitée) reproduit le comportement antérieur à la version 26.7, où l’optimisation n’était pas plafonnée. Ainsi, lorsque `compatibility` est défini sur une version antérieure, des prédicats transitifs continuent d’être dérivés sans budget. Définissez la valeur sur `0` pour désactiver le budget."}]}]} />

Budget de travail de l’optimisation `optimize_and_compare_chain` lors de l’analyse des requêtes, mesuré par le nombre de nœuds de l’arbre de requête hachés par `getTreeHash` (le coût dominant de cette optimisation). Une fois qu’une requête a haché plus de ce nombre de nœuds pendant l’application de l’optimisation, celle-ci cesse d’être appliquée pour le reste de la requête. Cela limite le temps d’analyse des requêtes comportant un très grand nombre de chaînes `AND` de comparaisons, ou des chaînes `AND` très longues, pour lesquelles l’optimisation pourrait sinon monopoliser l’analyse sans rien simplifier. Cet arrêt anticipé est toujours sûr : il renonce seulement à une optimisation et ne modifie jamais les résultats. Définissez la valeur sur `0` pour désactiver le budget (illimité).
...
Cela peut accélérer la phase des lignes non jointes lors de l&#39;utilisation de l&#39;algorithme de jointure `parallel_hash` avec de grandes tables.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "Nouveau paramètre permettant d’activer une optimisation au niveau du plan de requête, qui pousse ORDER BY ... LIMIT n à travers une jointure LEFT\/RIGHT lorsque la clé de tri ne référence que le côté préservé."}]}]} />

Active ou désactive une optimisation au niveau du plan de requête qui pousse `ORDER BY ... LIMIT n` à travers une jointure lorsque la clé de tri ne référence que les colonnes du côté préservé par la jointure (LEFT/RIGHT). Limite le nombre de lignes que l’entrée du côté préservé doit produire avant la jointure.
Ne prend effet que si le paramètre [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) vaut 1.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Nouveau paramètre permettant de réserver de la mémoire pour un workload spécifique avant le démarrage d'une requête."}]}]} />

Utilisé dans l’ordonnancement des workloads. Quantité minimale de RAM réservée à l’exécution d’une requête sur un seul serveur. La réservation est effectuée dans la hiérarchie WORKLOAD à l’aide de la valeur du paramètre de requête `workload`.
Si le workload ne dispose pas d’assez de mémoire, une requête ne démarre pas et reste en attente jusqu’à ce que la réservation puisse être honorée.
Une valeur de `0` signifie qu’aucune réservation n’est effectuée.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "100000"},{"label": "Nouveau paramètre limitant le nombre de répertoires lus lors de l’expansion des caractères génériques dans la fonction de table `url`."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Utilise par défaut la nouvelle fonction `toTime` (qui convertit les valeurs en type de données `Time`) au lieu de la fonction legacy `toTime` (qui reste disponible sous le nom `toTimeWithFixedDate`)."}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "Nouveau paramètre. Permet à l'utilisateur d'utiliser l'ancienne logique de la fonction toTime, qui se comporte comme toTimeWithFixedDate."}]}]} />

Lorsqu&#39;il est activé, ce paramètre permet d&#39;utiliser la fonction legacy toTime, qui convertit une date avec heure en une date fixe donnée, tout en conservant l&#39;heure.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "L’index de texte est désormais GA"}]}, {"id": "row-2","items": [{"label": "26.1"},{"label": "0"},{"label": "Activé par défaut"}]}, {"id": "row-3","items": [{"label": "25.9"},{"label": "0"},{"label": "Nouveau paramètre"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Cache de l’en-tête de l’index de texte activé globalement."}]}, {"id": "row-2","items": [{"label": "25.11"},{"label": "0"},{"label": "Nouveau paramètre"}]}]} />

Indique s’il faut mettre en cache en mémoire les en-têtes désérialisés de l’index de texte.
...
Indique s’il faut mettre en cache en mémoire les listes de postings désérialisées de l’index de texte.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "A activé globalement le cache des jetons de l’index de texte."}]}, {"id": "row-2","items": [{"label": "26.3"},{"label": "0"},{"label": "Nouveau paramètre"}]}]} />

Indique s’il faut mettre en cache en mémoire des informations désérialisées sur les jetons de l’index de texte.
