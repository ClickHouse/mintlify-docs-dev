...
Autorise la création d’index textuels avec l’argument expérimental `positions`, qui
stocke la position des tokens pour permettre une correspondance exacte des phrases.
...
paramètre [replicated&#95;deduplication&#95;window](#replicated_deduplication_window)), est utilisé, y compris la
granularité `insert_deduplication_version` (bloc entier inséré avec la valeur par défaut
`new_unified_hash`, par part créée avec les versions `legacy`). Les sommes de hachage sont écrites dans
un fichier local sur disque plutôt que dans ClickHouse Keeper.
...
ClickHouse Keeper. Les sommes de hachage ne sont conservées que pour les
`replicated_deduplication_window` blocs les plus récents. Les plus anciennes sont supprimées de
ClickHouse Keeper.

Avec la valeur par défaut `insert_deduplication_version = new_unified_hash`, la somme de hachage couvre
l&#39;intégralité du bloc inséré, de sorte qu&#39;une insertion n&#39;est dédupliquée que lorsque toutes ses données correspondent à une
insertion précédente (une nouvelle tentative), et non part par part. Avec l&#39;option legacy `old_separate_hashes` /
`compatible_double_hashes`, la somme de hachage est au contraire calculée pour chaque part créée, à partir de la
composition des noms et types de champs et des données de cette part (flux d&#39;octets).
...
Ce paramètre s’applique uniquement avec `insert_deduplication_version = old_separate_hashes` ou
`compatible_double_hashes`, qui conservent les hachages des insertions asynchrones dans un répertoire
`async_blocks` distinct. Avec la valeur par défaut `new_unified_hash`, les insertions asynchrones partagent le répertoire unifié
`deduplication_hashes` avec les insertions synchrones et sont régies par
`replicated_deduplication_window` / `replicated_deduplication_window_seconds` à la place.
...
Comme `replicated_deduplication_window_for_async_inserts`, cela s&#39;applique uniquement avec
`insert_deduplication_version = old_separate_hashes` / `compatible_double_hashes` ; avec la valeur
par défaut `new_unified_hash`, les insertions asynchrones utilisent `replicated_deduplication_window_seconds`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "Active les fusions coordonnées par défaut"}]}, {"id": "row-2","items": [{"label": "25.3"},{"label": "0"},{"label": "Nouveau paramètre"}]}]} />

Permet d&#39;écrire des attributs dans les parts virtuelles et de valider les blocs dans Keeper
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Nouveau paramètre qui détermine combien de temps une réplique inactive est prise en compte par le nettoyage en arrière-plan (0 signifie deux expirations de session ZooKeeper)"}]}]} />

Durée pendant laquelle une réplique inactive est encore prise en compte par le nettoyage en arrière-plan
(`OutdatedPartsQuorumThread`) : tant qu&#39;une réplique est inactive depuis moins de ce nombre de secondes,
elle continue d&#39;empêcher la suppression des parts obsolètes et la troncature des mutations terminées. Une valeur de 0
signifie que la valeur par défaut de deux expirations de session ZooKeeper est utilisée.
Uniquement dans ClickHouse Cloud
...
Taille de la fenêtre (en numéros de bloc) utilisée pour répartir l’affectation des fusions entre les
répliques. Les parts situées dans la même fenêtre sont affectées aux mêmes répliques.
Disponible uniquement dans ClickHouse Cloud
...
Si ce paramètre est défini sur true, la table est en mode lecture seule et n’effectue aucune modification sur le disque.
...
(`ATTACH`/`MOVE`/`DROP`/`DROP DETACHED`/`FETCH`/`REPLACE PARTITION`, ainsi que `MOVE PARTITION ... TO TABLE` lorsqu’elle cible cette table). Les opérations
qui ne modifient pas les données de la table, telles que `FREEZE`/`UNFREEZE` et `FORGET PARTITION`, restent autorisées.

Aucun travail en arrière-plan n’est non plus planifié : les fusions ordinaires, les fusions TTL (`DELETE`/`MOVE`/recompression), les fusions de recompression, les mutations en arrière-plan
et les déplacements de parts en arrière-plan sont tous inhibés. Par conséquent, une table avec un TTL ne supprime ni ne déplace plus ses données expirées tant que ce paramètre
est activé.

Ce paramètre peut toujours être désactivé à nouveau avec `ALTER TABLE ... MODIFY SETTING table_readonly = 0` (ou `RESET SETTING`). Il n’est pas pris en charge pour `ReplicatedMergeTree`.
