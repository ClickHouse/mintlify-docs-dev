...
<Image img="/images/cloud/reference/architecture.png" size="lg" alt="Architecture de Cloud" />
...
Il n’existe aucune limite au nombre de requêtes par seconde (QPS) dans votre service ClickHouse Cloud. Il existe toutefois une limite de 1 000 requêtes concurrentes par réplique. Le QPS dépend au final du temps d’exécution moyen de vos requêtes et du nombre de répliques dans votre service.

L’un des principaux avantages de ClickHouse Cloud par rapport à une instance ClickHouse autogérée ou à d’autres bases de données/entrepôts de données est que vous pouvez facilement augmenter la concurrence en [ajoutant plus de répliques (mise à l’échelle horizontale)](/fr/products/cloud/features/autoscaling/horizontal#manual-horizontal-scaling).