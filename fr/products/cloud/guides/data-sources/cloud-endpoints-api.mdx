...
Si vous devez récupérer la liste des adresses IP statiques, vous pouvez utiliser le point de terminaison suivant de l’API ClickHouse Cloud : [`https://api.clickhouse.cloud/static-ips.json`](https://api.clickhouse.cloud/static-ips.json). Cette API fournit les endpoints des services ClickHouse Cloud, comme les adresses IP d’entrée/de sortie et les endpoints S3 par région et par cloud.
...
<Image img="/images/_snippets/aws-rds-mysql.png" size="lg" alt="Règles du groupe de sécurité AWS" border />
...
<Image img="/images/_snippets/gcp-authorized-network.png" size="md" alt="Réseaux autorisés GCP" border />