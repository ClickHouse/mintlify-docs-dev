...
<Image img="/images/managed-postgres/clickhouse-integration-intro.png" alt="Page d’accueil de ClickHouse integration montrant l’option d’intégration dans la barre latérale" size="md" border />
...
<Steps>
  <Step>
    ...
    * **Méthode de réplication** : Choisissez l’une des options suivantes :
    ...
    <Image img="/images/managed-postgres/replication-service-step.png" alt="Configuration du service de réplication montrant le nom de l’intégration, le service de destination et les options de méthode de réplication" size="md" border />
  ...
  <Step>
    ...
    * **Nombre de tables du snapshot en parallèle** : Nombre de tables snapshotées simultanément (par défaut : 1)
    ...
    <Image img="/images/managed-postgres/integration-replication-settings.png" alt="Étape des paramètres de réplication montrant l’intervalle de synchronisation, les threads parallèles, la taille du lot de récupération et les options de configuration du snapshot" size="md" border />
  ...
  <Step>
    ...
    * **Préserver les valeurs NULL de la source** : Conserve les valeurs NULL au lieu de les convertir en valeurs par défaut
    * **Supprimer les lignes supprimées pendant les fusions** : Pour les tables [ReplacingMergeTree](/fr/reference/engines/table-engines/mergetree-family/replacingmergetree), supprime physiquement les lignes supprimées lors des merges en arrière-plan
    ...
    <Image img="/images/managed-postgres/select-tables-step.png" alt="Étape de sélection des tables montrant la sélection de la base de données, les options de réplication et le sélecteur de tables regroupé par schéma" size="md" border />

    Cliquez sur **Répliquer les données vers ClickHouse** pour démarrer la réplication.
  </Step>

  <Step>
    ## Suivre votre ClickPipe
    ...
    <Image img="/images/managed-postgres/integration-running.png" alt="Liste des ClickHouse integration montrant un ClickPipe en cours d’exécution avec son service de destination et son statut" size="md" border />
