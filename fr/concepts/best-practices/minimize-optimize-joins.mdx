...
<Note>
  La prise en charge des types de JOIN varie selon l’algorithme. La liste complète des types de jointure pris en charge par chaque algorithme est disponible [ici](/fr/concepts/features/operations/select/joining-tables#choosing-a-join-algorithm).
</Note>

Vous pouvez laisser ClickHouse choisir le meilleur algorithme en définissant `join_algorithm = 'auto'` (par défaut), ou le contrôler explicitement selon votre charge de travail. Si vous devez sélectionner un algorithme de JOIN pour optimiser les performances ou réduire le surcoût mémoire, nous vous recommandons [ce guide](/fr/concepts/features/operations/select/joining-tables#choosing-a-join-algorithm).
