...
<Image img="/images/_snippets/cloud-connect-button.png" size="md" alt="Bouton Connect du service ClickHouse Cloud" />
...
<Image img="/images/_snippets/connection-details-native.png" size="md" alt="Détails de connexion Native TCP du service ClickHouse Cloud" />
...
| `--hints <bool>`                  | Affiche pendant la saisie des indications d’autocomplétion (texte « fantôme » intégré) correspondant à la meilleure suggestion lorsque le curseur se trouve à la fin de l’entrée. Parcourez les indications avec Haut/Bas (ou Ctrl-Haut/Ctrl-Bas) ; acceptez l’indication intégrée avec Tab ou Droite ; `Enter` n’accepte une indication qu’après sa sélection explicite et, sinon, exécute la requête ; `Tab` ouvre également la liste classique de complétion. Nécessite `--highlight` (les indications ont besoin de couleur) et le mécanisme de suggestion (ainsi, `--disable_suggestion` les désactive aussi). | `true`                                                            |
