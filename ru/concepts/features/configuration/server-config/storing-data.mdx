...
<Note>
  Google Cloud Storage (GCS) также поддерживается с типом `s3`. См. [MergeTree с хранилищем GCS](/ru/integrations/connectors/data-ingestion/GCP).
</Note>
...
[Мутации](/ru/reference/statements/alter#mutations) и репликация таблиц не поддерживаются.
...
[`ALTER TABLE`](/ru/reference/statements/alter), [`RENAME TABLE`](/ru/reference/statements/rename#rename-table),
...
<Tip>
  Хранилище также можно временно настроить в рамках запроса, если веб-датасет
