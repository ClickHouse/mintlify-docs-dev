...
<Image img="/images/materialized-view/materialized-view-diagram.png" size="md" alt="Схема materialized view" />
...
* **Для простых поисков предпочитайте словари**. Используйте [Dictionaries](/ru/concepts/features/dictionaries) для поиска по модели ключ-значение (например, сопоставления ID пользователя с именем), чтобы избежать дорогостоящих операций JOIN.
...
В этом случае CTE `users` пересчитывается при каждой вставке в `posts`, а materialized view не будет обновляться при добавлении новых пользователей — только при вставке в `posts`.
