...
ClickHouse определяет по расширению имени файла, что в нём используется формат с табуляцией в качестве разделителя. Если вам нужно явно указать format, просто добавьте один из [многих input format в ClickHouse](/ru/reference/formats):
