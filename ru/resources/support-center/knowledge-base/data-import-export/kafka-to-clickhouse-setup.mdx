...
В этой статье рассматривается процесс отправки данных из топика Kafka в таблицу ClickHouse. Мы будем использовать ленту последних изменений Wiki, которая представляет собой [поток событий](https://stream.wikimedia.org/v2/stream/recentchange), отражающих изменения в различных проектах Wikimedia. Шаги включают:

1. Как настроить Kafka на Ubuntu
2. Организовать приём потока данных в топик Kafka
3. Создать таблицу ClickHouse, подписанную на этот топик

<div id="1-setup-kafka-on-ubuntu">
  # 1. Настройте Kafka на Ubuntu
</div>
