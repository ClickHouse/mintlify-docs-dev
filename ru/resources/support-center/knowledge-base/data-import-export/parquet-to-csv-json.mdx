...
Вы можете использовать `clickhouse-local` для преобразования файлов между любыми [входными и выходными форматами](/ru/reference/formats), которые поддерживает ClickHouse (а это более 70 различных форматов!). В этой статье мы преобразуем файл Parquet из S3 в CSV- и JSON‑файл.

Начнём с самого начала. В ClickHouse есть набор [табличных функций](/ru/reference/functions/table-functions), которые читают данные из файлов, баз данных и других ресурсов и представляют их в виде таблицы. Для примера предположим, что у нас есть файл Parquet в S3. Мы будем использовать табличную функцию `s3`, чтобы прочитать его (ClickHouse определяет, что это файл Parquet, по имени файла).
...
Как уже упоминалось выше, вы можете использовать любой из [форматов ввода и вывода ClickHouse](/ru/reference/formats) вместе с `clickhouse local`, чтобы легко преобразовывать файлы в разные форматы.