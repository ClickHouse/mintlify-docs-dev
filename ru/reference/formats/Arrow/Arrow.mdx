...
[Apache Arrow](https://arrow.apache.org/) включает два встроенных формата столбцового хранения. ClickHouse поддерживает чтение и запись в этих форматах.
`Arrow` — это формат Apache Arrow в «файловом режиме». Он предназначен для произвольного доступа к данным в памяти.
...
| `TIMESTAMP`, `TIME32`, `TIME64`         | [DateTime64](/ru/reference/data-types/datetime64)                                                            | `TIMESTAMP`                 |
...
Типы данных столбцов таблицы ClickHouse не обязательно должны совпадать с соответствующими полями данных Arrow. При вставке данных ClickHouse интерпретирует типы данных в соответствии с приведённой выше таблицей, а затем [преобразует](/ru/reference/functions/regular-functions/type-conversion-functions#CAST) данные в тип данных, заданный для столбца таблицы ClickHouse.
...
Вы можете выполнить вставку данных Arrow из файла в таблицу ClickHouse с помощью следующей команды:
...
Вы можете выбрать данные из таблицы ClickHouse и сохранить их в файл в формате Arrow с помощью следующей команды:
