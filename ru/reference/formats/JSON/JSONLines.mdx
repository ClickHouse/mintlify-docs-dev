...
Этот формат также известен как `JSONEachRow`, `NDJSON` (JSON с разделением по строкам) или `JSONL` (`JSONLines`). Все эти названия являются псевдонимами одного и того же формата и могут использоваться как взаимозаменяемые.
