...
Функция `url` создает таблицу на основе `URL` с указанными `format` и `structure`.

Функцию `url` можно использовать в запросах `SELECT` и `INSERT` к данным в таблицах [URL](/ru/reference/engines/table-engines/special/url).
