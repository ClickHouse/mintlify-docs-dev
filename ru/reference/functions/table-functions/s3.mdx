...
`wildcard`: заменяет подстановочный шаблон `{_partition_id}` в пути к файлу на фактическое значение ключа партиционирования. Выбирается по умолчанию только при значении `compatibility` ниже `26.6`; в противном случае значением по умолчанию является `hive` (см. настройку `file_like_engine_default_partition_strategy`).
