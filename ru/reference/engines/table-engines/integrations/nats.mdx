...
* `nats_format` – Формат сообщений. Используется та же нотация, что и в SQL-функции `FORMAT`, например `JSONEachRow`. Подробнее см. в разделе [Форматы](/ru/reference/formats).
...
Движок NATS поддерживает все [форматы](/ru/reference/formats), доступные в ClickHouse.
