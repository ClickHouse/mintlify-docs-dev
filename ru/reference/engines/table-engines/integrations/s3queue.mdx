...
Повторять попытку загрузки файла указанное количество раз. По умолчанию повторные попытки не выполняются.
Возможные значения:
...
В случае нештатного завершения работы сервера, если включён параметр `use_persistent_processing_nodes`, может оказаться, что узлы обработки не были удалены. Этот параметр задаёт период времени, по истечении которого такие узлы обработки можно безопасно удалить.
