...
* `rabbitmq_format` – формат сообщения. Используется та же нотация, что и для SQL-функции `FORMAT`, например `JSONEachRow`. Дополнительные сведения см. в разделе [Форматы](/ru/reference/formats).
...
Движок RabbitMQ поддерживает все [форматы](/ru/reference/formats), доступные в ClickHouse.
