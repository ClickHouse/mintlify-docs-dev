...
<Note>
  ...
  Подробности см. на странице [&quot;Полнотекстовый поиск с текстовыми индексами&quot;](/ru/reference/engines/table-engines/mergetree-family/textindexes).
</Note>
...
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `n`                             | размер n-граммы                                                                                                                  |
| `size_of_bloom_filter_in_bytes` | Размер bloom filter в байтах. Здесь можно использовать большое значение, например `256` или `512`, так как он хорошо сжимается). |
| `number_of_hash_functions`      | Количество хеш-функций, используемых в bloom filter.                                                                             |
| `random_seed`                   | seed для хеш-функций bloom filter.                                                                                               |
...
Приведённые выше функции отсылают к калькулятору bloom filter [здесь](https://hur.st/bloomfilter).
...
Чтобы задать `interval`, используйте операторы [для работы с временными интервалами](/ru/reference/operators#operators-for-working-with-dates-and-times), например:
...
* [`gcs` для GCS](/ru/reference/functions/table-functions/gcs#creating-a-disk)
...
* Во время фоновых слияний и [мутаций](/ru/reference/statements/alter#mutations).
...
В настоящее время отсечение частей поддерживается статистикой `MinMax` и `NullCount`. Когда для столбца определена статистика `MinMax`, ClickHouse отслеживает минимальное и максимальное значения этого столбца в каждой части. Когда для столбца `Nullable` определена статистика `NullCount`, ClickHouse отслеживает количество значений NULL в каждой части, что позволяет выполнять отсечение на основе предикатов `IS NULL` / `IS NOT NULL` и повышает точность отсечения по фильтру диапазона для столбцов со значениями NULL.
...
```sql
-- Создаём таблицу со статистикой MinMax для столбца 'value'
...
-- Вставляем данные отдельными операциями для создания нескольких частей
INSERT INTO test_stats SELECT number, number FROM numbers(1000); -- Часть 1: диапазон значений [0, 999]
INSERT INTO test_stats SELECT number, number + 10000 FROM numbers(1000); -- Часть 2: диапазон значений [10000, 10999]

SET use_statistics_for_part_pruning = 1;

-- Этот запрос полностью пропустит часть 1, поскольку её максимальное значение (999) < 5000
SELECT count() FROM test_stats WHERE value > 5000;

-- Используем EXPLAIN для просмотра эффекта отсечения частей
EXPLAIN indexes = 1 SELECT count() FROM test_stats WHERE value > 5000;
-- В выводе отобразится "Parts: 1/2", что означает: одна часть была отсечена
```
...
* `MinMax`
  ...
  Минимальное и максимальное значения столбца, что позволяет оценивать селективность фильтров диапазона для числовых столбцов.
...
* `Uniq`
  ...
  Скетчи [HyperLogLog](https://en.wikipedia.org/wiki/HyperLogLog), которые позволяют оценить количество различных значений в столбце.

  Синтаксис: `uniq`

* `NullCount`

  Отслеживает количество значений `NULL` в столбцах `Nullable`. Используется для точной оценки селективности предикатов `IS NULL`/`IS NOT NULL` при оптимизации PREWHERE, а также для отсечения частей на основе наличия NULL.
...
|           | Фильтры по равенству (==) | Фильтры диапазона (`>, >=, <, <=`) | `IS NULL` / `IS NOT NULL` |
| --------- | ------------------------- | ---------------------------------- | ------------------------- |
| CountMin  | ✔                         | ✗                                  | ✗                         |
| MinMax    | ✗                         | ✔                                  | ✗                         |
| NullCount | ✗                         | ✗                                  | ✔                         |
