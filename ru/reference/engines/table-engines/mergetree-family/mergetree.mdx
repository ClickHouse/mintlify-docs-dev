...
В настоящее время отсечение частей поддерживается статистикой `MinMax` и `Basic`. Когда такая статистика определена для столбца, ClickHouse отслеживает минимальное и максимальное значения этого столбца в каждой части.
