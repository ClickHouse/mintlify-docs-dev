...
Этот движок отличается от [MergeTree](/ru/reference/engines/table-engines/mergetree-family/versionedcollapsingmergetree) тем, что удаляет повторяющиеся записи с одинаковым значением [ключа сортировки](/ru/reference/engines/table-engines/mergetree-family/mergetree) (раздел таблицы `ORDER BY`, а не `PRIMARY KEY`).
