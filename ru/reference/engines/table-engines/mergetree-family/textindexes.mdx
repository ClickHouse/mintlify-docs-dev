...
Чтобы определить препроцессор в текстовом индексе, построенном на столбцах типа [Map](/ru/reference/data-types/map), пользователям нужно решить, будет ли индекс строиться
по ключам Map или по её значениям.
...
Оператор доступа [operator[]](/ru/reference/operators#access-operators) можно использовать с текстовым индексом для фильтрации по ключам и значениям. Текстовый индекс используется только в том случае, если он создан для выражений `mapKeys(map)` или `mapValues(map)`, либо для обоих.
