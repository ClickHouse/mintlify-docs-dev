...
Движок `Join` позволяет указать настройку [join&#95;use&#95;nulls](/ru/reference/settings/session-settings#join_use_nulls) в операторе `CREATE TABLE`. В запросе [SELECT](/ru/reference/statements/select) должно использоваться то же значение `join_use_nulls`.
