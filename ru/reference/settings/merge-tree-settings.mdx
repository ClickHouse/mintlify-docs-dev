...
Разрешает создавать текстовые индексы с экспериментальным аргументом `positions`, который
сохраняет позиции токенов, что позволяет выполнять точный поиск по фразам.
...
настройку [replicated&#95;deduplication&#95;window](#replicated_deduplication_window)), включая
гранулярность `insert_deduplication_version` (весь вставленный блок при значении по умолчанию
`new_unified_hash`, или каждая созданная part в legacy-версиях). Хеш-суммы записываются в
...
Для [дедупликации вставок](/ru/reference/engines/table-engines/mergetree-family/replication)
при записи в реплицируемые таблицы ClickHouse записывает хеш-суммы
дедупликации в ClickHouse Keeper. Хеш-суммы хранятся только для последних
`replicated_deduplication_window` блоков. Самые старые хеш-суммы удаляются из
ClickHouse Keeper.

При значении по умолчанию `insert_deduplication_version = new_unified_hash` хеш-сумма охватывает
весь вставленный блок, поэтому вставка дедуплицируется только тогда, когда все её данные совпадают с
данными предыдущей вставки (повторной попытки), а не для каждой отдельной части. В устаревших `old_separate_hashes` /
`compatible_double_hashes` хеш-сумма, напротив, вычисляется для каждой созданной части на
основе состава имён и типов полей, а также данных этой части (потока байтов).

Большое значение `replicated_deduplication_window` замедляет `Inserts`, поскольку приходится
сравнивать больше записей.
...
Этот параметр применяется только при `insert_deduplication_version = old_separate_hashes` или
`compatible_double_hashes`, которые хранят хеши асинхронных вставок в отдельном каталоге `async_blocks`.
При значении по умолчанию `new_unified_hash` асинхронные вставки используют общий
каталог `deduplication_hashes` вместе с синхронными вставками и регулируются
параметрами `replicated_deduplication_window` / `replicated_deduplication_window_seconds`.
...
Как и `replicated_deduplication_window_for_async_inserts`, этот параметр применяется только при
`insert_deduplication_version = old_separate_hashes` / `compatible_double_hashes`; при значении
по умолчанию `new_unified_hash` асинхронные вставки используют `replicated_deduplication_window_seconds`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "Координируемые слияния включены по умолчанию"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "0"},{"label": "новая настройка"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "Координируемые слияния включены по умолчанию"}]}, {"id": "row-2","items": [{"label": "25.3"},{"label": "0"},{"label": "Новая настройка"}]}]} />
...
(`OutdatedPartsQuorumThread`): пока реплика неактивна меньше указанного числа секунд,
она по-прежнему задерживает удаление устаревших частей и усечение завершённых мутаций. Значение 0
...
Размер окна (в номерах блоков), используемый для распределения слияний между
репликами. Части в пределах одного окна закрепляются за одними и теми же репликами.
Доступно только в ClickHouse Cloud
...
Отклоняются все операции переднего плана, которые изменяли бы таблицу: вставки, мутации, `OPTIMIZE` и команды с партициями, изменяющие данные
...
которые не изменяют данные таблицы, такие как `FREEZE`/`UNFREEZE` и `FORGET PARTITION`, остаются разрешенными.

Фоновые задачи также не планируются: обычные слияния, TTL-слияния (`DELETE`/`MOVE`/повторное сжатие), слияния повторного сжатия, фоновые мутации
и фоновые перемещения частей — все они подавляются. Как следствие, таблица с TTL больше не удаляет и не перемещает устаревшие данные, пока эта настройка
включена.

Настройку всегда можно переключить обратно с помощью `ALTER TABLE ... MODIFY SETTING table_readonly = 0` (или `RESET SETTING`). Она не поддерживается для `ReplicatedMergeTree`.
