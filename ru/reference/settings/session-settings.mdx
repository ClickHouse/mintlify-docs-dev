...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Новая настройка для включения раскрытия подстановочных шаблонов в табличной функции `url` путём перечисления HTTP-страниц индекса."}]}]} />
...
Разрешить неконстантные аргументы часового пояса в некоторых функциях, связанных со временем, таких как toTimeZone(), fromUnixTimestamp*(), snowflakeIDToDateTime*().
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Новая проверка безопасности: `ALTER TABLE ... REPLACE PARTITION ... FROM ...` теперь генерирует исключение, если в исходной таблице нет частей в запрошенной партиции (это исправляет незаметную потерю данных в [#23727](https:\/\/github.com\/ClickHouse\/ClickHouse\/issues\/23727)). Предыдущее поведение — незаметное удаление целевой партиции — сохраняется при установке `allow_replace_partition_from_empty_source = 1`."}]}]} />

Разрешает `ALTER TABLE ... REPLACE PARTITION ... FROM ...` незаметно удалять целевую партицию, если в источнике нет частей в этой партиции.

По умолчанию это запрещено: `REPLACE PARTITION` из источника, в котором нет данных в запрошенной партиции, вызывает исключение, потому что в этом случае операция фактически превращается в незаметный `DROP PARTITION` целевой таблицы (её данные удаляются, но ничем не заменяются), что часто приводит к случайной потере данных (см. [#23727](https://github.com/ClickHouse/ClickHouse/issues/23727)).

Включите этот параметр, чтобы восстановить прежнее поведение, например если вы намеренно используете пустую исходную партицию для очистки данных в целевой таблице. Для безусловного удаления вместо этого предпочтительнее использовать `ALTER TABLE ... DROP PARTITION ...`.
...
Действует только в ClickHouse Cloud. Показывает столбцы `certificate` и `signature` в таблице `system.distributed_cache_registry`. По умолчанию эти столбцы пусты, чтобы вывод оставался компактным; включите эту настройку, чтобы просмотреть их.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Новая экспериментальная настройка, определяющая, сколько stateless-воркеров выделять для распределённого запроса. Ноль отключает выделение."}]}]} />

Сколько stateless-воркеров будет использоваться для выполнения этого запроса. Ноль отключает выделение stateless-воркеров для распределённых планов.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "Начиная с 26.7, для `EXPLAIN PLAN` по умолчанию используются `actions=1, compact=1, pretty=1`. Установите `legacy`, чтобы вернуть вывод, использовавшийся до 26.7."}]}]} />

Формат, используемый по умолчанию в `EXPLAIN PLAN`.

Возможные значения:

* `pretty` (по умолчанию с 26.7) — для `actions`, `compact` и `pretty` значение `true` используется по умолчанию, что даёт компактный, наглядно отформатированный план с аннотациями действий.
* `legacy` — вывод в формате до 26.7.

Явное указание параметров `actions`, `compact` или `pretty` в операторе `EXPLAIN` (например, `EXPLAIN actions = 0, compact = 0, pretty = 0 SELECT ...`) всегда переопределяет эту настройку.

`EXPLAIN PLAN` с `json = 1` или `distributed = 1` использует прежние значения по умолчанию (до 26.7) независимо от этой настройки, если только `actions`, `compact` или `pretty` не заданы явно. Формат `pretty` не может представлять результаты JSON или распределённые планы по сегментам, поэтому эти режимы корректно отображаются только в устаревшем формате.
...
<div id="formatdatetime_f_prints_single_zero">
  ## formatdatetime_f_prints_один_ноль
</div>
...
<Note>
  `insert_deduplication_token` отслеживается отдельно для каждой партиции, поэтому несколько партиций, записанных одной вставкой, могут иметь один и тот же токен. Без токена контрольная сумма содержимого по умолчанию (`insert_deduplication_version = new_unified_hash`) вычисляется для всего вставленного блока целиком, поэтому вставка дедуплицируется только тогда, когда все ее данные полностью совпадают с предыдущей вставкой (при повторной попытке), а не когда строки одной партиции случайно совпадают со строками из другой вставки.
</Note>
...
Например, если нужное число записей находится в каждом блоке и max&#95;threads = 8, то будут считаны 8 блоков, хотя достаточно было бы прочитать только один.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Новая настройка, которая позволяет протолкнуть `LIMIT` в aggregation-in-order для раннего завершения, когда `ORDER BY` является префиксом описания сортировки `GROUP BY`."}]}]} />

Если настройка включена и активна aggregation in order, `LIMIT` проталкивается на этап aggregation, что позволяет завершить выполнение раньше после формирования достаточного количества групп. Это уменьшает объем считываемых данных, когда `ORDER BY` совпадает с префиксом ключа `GROUP BY`. Может уменьшить значение, сообщаемое в `rows_before_limit_at_least`; используйте `exact_rows_before_limit`, если требуется точное количество.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "Новая настройка, ограничивающая объём работы оптимизации `optimize_and_compare_chain` (измеряемый числом хешируемых узлов дерева запроса), чтобы она не занимала основную часть времени анализа запросов с очень большим количеством или очень длинными цепочками сравнений `AND`. Прежнее значение `0` (без ограничений) воспроизводит поведение до версии 26.7, когда оптимизация не была ограничена, поэтому при `compatibility`, установленной на более раннюю версию, транзитивные предикаты по-прежнему выводятся без какого-либо бюджета. Установите `0`, чтобы отключить бюджет."}]}]} />

Бюджет работы для оптимизации `optimize_and_compare_chain` при анализе запроса, измеряемый количеством узлов дерева запроса, хешируемых с помощью `getTreeHash` (это основная стоимость данной оптимизации). Как только в ходе применения оптимизации запрос хеширует больше этого количества узлов, её применение прекращается для оставшейся части запроса. Это ограничивает время анализа для запросов с очень большим количеством или очень длинными цепочками сравнений `AND`, где в противном случае оптимизация может занимать основную часть анализа, ничего при этом не сворачивая. Досрочная остановка всегда безопасна: она лишь отказывается от оптимизации и никогда не меняет результаты. Установите `0`, чтобы отключить бюджет (без ограничений).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Новая настройка для резервирования памяти для определённой рабочей нагрузки перед запуском запроса."}]}]} />

Используется при планировании рабочих нагрузок. Минимальный объём оперативной памяти, резервируемый для выполнения запроса на одном сервере. Резервирование выполняется через иерархию WORKLOAD с использованием значения настройки запроса `workload`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "100000"},{"label": "Новая настройка для ограничения количества каталогов, просматриваемых при разворачивании подстановочных шаблонов в табличной функции `url`."}]}]} />

Максимальное количество каталогов, которые можно обойти при разворачивании подстановочных шаблонов URL на страницах индекса.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "По умолчанию использовать новую функцию `toTime` (преобразующую значения в тип данных `Time`) вместо устаревшей `toTime` (которая по-прежнему доступна как `toTimeWithFixedDate`)."}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "Новая настройка. Позволяет использовать старую логику функции toTime, которая работает как toTimeWithFixedDate."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Глобально включен кэш заголовка текстового индекса."}]}, {"id": "row-2","items": [{"label": "25.11"},{"label": "0"},{"label": "новая настройка"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Кеш токенов текстового индекса включен глобально."}]}, {"id": "row-2","items": [{"label": "26.3"},{"label": "0"},{"label": "новая настройка"}]}]} />

Определяет, следует ли кешировать в памяти десериализованную информацию о токенах текстового индекса.
