...
Запрос [легковесного `DELETE`](/ru/snippets/delete) можно выполнить как легковесный `UPDATE`, а не как мутацию `ALTER UPDATE`. Реализация легковесного `DELETE` управляется настройкой [`lightweight_delete_mode`](/ru/reference/settings/session-settings#lightweight_delete_mode).
...
* [Легковесный `DELETE`](/ru/snippets/delete) — операции легковесного `DELETE`
