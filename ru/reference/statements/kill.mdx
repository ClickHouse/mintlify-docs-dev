...
Пытается отменить и удалить [мутации](/ru/reference/statements/alter#mutations), которые выполняются в данный момент. Мутации для отмены выбираются из таблицы [`system.mutations`](/ru/reference/system-tables/mutations) с помощью фильтра, заданного в условии `WHERE` запроса `KILL`.
