...
Создаёт таблицу с тем же результатом, что и указанная [табличная функция](/ru/reference/functions/table-functions). Созданная таблица также будет работать так же, как соответствующая табличная функция.
...
В запросе после предложения `ENGINE` могут следовать и другие предложения. Подробную документацию о том, как создавать таблицы, см. в описаниях [движков таблиц](/ru/reference/engines/table-engines).
...
* Семейство [Log](/ru/reference/engines/table-engines/log-family). По умолчанию использует метод сжатия `lz4` и поддерживает кодеки сжатия для столбцов.
