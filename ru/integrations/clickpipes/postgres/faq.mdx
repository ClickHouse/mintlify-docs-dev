...
1. **SSH-туннелирование**
   ...
   * Инструкции по настройке смотрите [здесь](/ru/integrations/clickpipes/postgres#adding-your-source-postgres-database-connection)
...
Подробную информацию о ценах см. в [разделе с тарифами ClickPipes для Postgres CDC на нашей основной странице обзора billing](/ru/products/cloud/reference/billing/clickpipes).
...
Столбцы JSON и JSONB реплицируются в ClickHouse как тип String. Поскольку ClickHouse поддерживает [тип JSON](/ru/reference/data-types/newjson) нативно, при необходимости вы можете создать materialized view поверх таблиц ClickPipes, чтобы выполнить это преобразование. Либо можно напрямую использовать [функции JSON](/ru/reference/functions/regular-functions/json-functions) для столбцов String. Мы активно работаем над возможностью, которая позволит реплицировать столбцы JSON и JSONB напрямую в тип JSON в ClickHouse. Ожидается, что эта возможность станет доступна через несколько месяцев.
...
ClickPipe для Postgres также можно создавать и управлять им через конечные точки [OpenAPI](/ru/products/cloud/features/admin-features/api/openapi). Эта возможность находится в стадии бета, а справочник API доступен [здесь](/ru/api-reference/organization/get-list-of-available-organizations#tag/beta). Мы также активно работаем над поддержкой Terraform для создания Postgres ClickPipes.
...
Затем при [настройке](/ru/integrations/clickpipes/postgres#configuring-the-replication-settings) вашего ClickPipe для Postgres убедитесь, что выбрано имя этой публикации.
