...
<View title="v0.8+">
  ...
  * Artifactory со старыми ночными сборками (ссылка на репозиторий): https://s01.oss.sonatype.org/content/repositories/snapshots/
  ...
  Перечисление [поддерживаемых форматов](/ru/reference/formats). Включает все форматы, поддерживаемые ClickHouse.
  ...
  **Примеры**
  ...
  try (InsertResponse response = client.insert(TABLE_NAME, events).get()) {
      // обработать ответ; после этого он будет закрыт, а соединение, обслуживавшее запрос, — освобождено.
  }
  ```
  ...
  **Примеры**
  ...
  // Формат по умолчанию — RowBinaryWithNamesAndTypesFormatReader, поэтому reader содержит всю информацию о столбцах
  try (QueryResponse response = client.query(sql).get(3, TimeUnit.SECONDS);) {
      ...
      while (reader.hasNext()) {
          reader.next(); // Читаем следующую запись из потока и разбираем её
  ...
  // Размещайте бизнес-логику за пределами блока чтения, чтобы как можно скорее освободить HTTP-соединение.
  ```
  ...
  **Возвращаемое значение**
  ...
  Future типа `QueryResponse` — результирующий набор данных и дополнительная информация, например метрики на стороне сервера. Объект Response необходимо закрыть после обработки набора данных.
  ...
  **Примеры**
  ...
  try {
      ...
      // Считать весь результирующий набор и обработать каждую запись
  ...
  **Методы конфигурации**
  ...
  | `serverSetting(String name, String value)`        | Устанавливает отдельные настройки сервера для операции.                                                                                              |
  | `serverSetting(String name, Collection values)`   | Устанавливает отдельные настройки сервера с несколькими значениями для операции. Элементы коллекции должны иметь тип `String`.                       |
  | `setDBRoles(Collection dbRoles)`                  | Задает роли DB, которые будут установлены перед выполнением операции. Элементы коллекции должны иметь тип `String`.                                  |
  ...
  <Note>
    Этот объект следует закрыть как можно скорее, чтобы освободить соединение, поскольку его нельзя использовать повторно, пока все данные предыдущего ответа не будут полностью прочитаны.
  </Note>
  ...
  * `getObjectArray(...)` — универсальный вариант для `Array(...)` с любым типом элементов, включая вложенные массивы. Используйте для чтения массивов с Nullable-значениями и вложенных массивов.
  ...
  try (QueryResponse response = client.query("SELECT * FROM my_table").get()) {
      ...
      while (reader.next() != null) {
          ...
          // вложенные массивы возвращаются как вложенные Object[]:
  ...
  * `getList(...)` — считывает любой `Array(...)` как `List<T>`. Хороший вариант по умолчанию для чтения с гибкой типизацией. Поддерживает вложенные массивы.
  * `getByteArray(...)`, `getShortArray(...)`, `getIntArray(...)`, `getLongArray(...)`, `getFloatArray(...)`, `getDoubleArray(...)`, `getBooleanArray(...)` — лучше всего подходят для одномерных массивов значений, совместимых с примитивными типами.
  * `getStringArray(...)` — для `Array(String)` (и значений enum, представленных в виде имён).
  * `getObjectArray(...)` — универсальная опция для любого типа элементов `Array(...)`, включая вложенные массивы. Используется для чтения массивов со значениями Nullable и вложенными массивами.
  ...
  **V1: вставка данных в формате TSV.**
  ...
  try (ClickHousePipedOutputStream requestBody = ClickHouseDataStreamFactory.getInstance()
      ...
      // запускаем воркер-поток, который передаёт данные из входного потока в ClickHouse
      future = request.data(requestBody.getInputStream()).execute();

      // Копируем данные из потока inData в поток requestBody

      // Необходимо закрыть поток перед получением ответа
      requestBody.close();
  ...
  **V2 Вставка данных в формате TSV.**
  ...
  ```java
  InputStream inData = getInData();
  InsertSettings settings = new InsertSettings().setInputStreamCopyBufferSize(8198 * 2); // установить размер буфера копирования
  try (InsertResponse response = client.insert(tableName, inData, ClickHouseFormat.TSV, settings).get(30, TimeUnit.SECONDS)) {

    // Вставка завершена на этом этапе
