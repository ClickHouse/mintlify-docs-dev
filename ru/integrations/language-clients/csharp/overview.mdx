...
| ReadBufferSize          | `int`                    | `8192` (8 KiB) | `ReadBufferSize`          | Размер буфера в байтах, используемого для чтения HTTP-ответов на запросы. Увеличьте его, чтобы сократить число повторных заполнений буфера для больших результирующих наборов. Значения от 85 000 байт и выше выделяются в Large Object Heap, что может вызывать фрагментацию LOH и более длительные паузы GC. |
...
| AcceptEncoding        | `string`                      | Переопределяет заголовок `Accept-Encoding` для отдельного запроса (например, `"zstd"`, `"zstd, gzip;q=0.5"`); также принудительно задаёт `enable_http_compression=1` в URL. См. [Сжатие передачи для отдельного запроса](#per-query-accept-encoding). |
...
<Image img="/images/_snippets/connection-details-csharp.png" size="md" alt="Сведения о подключении C# для ClickHouse Cloud" border />
...
Сопоставление столбцов чувствительно к регистру. Если в результате отсутствуют столбцы, свойства сохраняют значения по умолчанию; лишние столбцы в результате игнорируются.

Автоматические преобразования отсутствуют, а несоответствие типов вызывает `InvalidOperationException`.
...
По умолчанию клиент согласовывает `gzip, deflate`, когда `Compression=true` (значение по умолчанию в строке подключения), а HTTP-клиент прозрачно распаковывает поток.
...
`HttpClient`, который драйвер создает по умолчанию, имеет `AutomaticDecompression = GZip | Deflate`, поэтому данные, сжатые этими алгоритмами, распаковываются автоматически, а `Content-Encoding` удаляется из ответа. Для обычных запросов это именно то, что нужно, но не если вы хотите сами обрабатывать исходные сжатые данные.

В таком случае передайте в ClickHouseClient `HttpClient` или `HttpClientFactory` с `AutomaticDecompression = DecompressionMethods.None`

<Warning>
  Если `AcceptEncoding` запрашивает кодек, который настроенный `HttpClient` не может распаковать автоматически, безопасно использовать только `ExecuteRawResultAsync`. Вызов `ExecuteReaderAsync`, `ExecuteScalarAsync` или `ExecuteNonQueryAsync` по тому же пути запроса попытается разобрать сжатые байты как обычный формат результата и вернет некорректные данные.
</Warning>
...
Когда сервер отвечает кодом 4xx/5xx и задан параметр `enable_http_compression=1`, он сжимает тело ошибки тем же кодеком, который использовал бы для успешного ответа. Драйвер распаковывает такие ответы для кодеков, входящих в BCL (`gzip`, `deflate`, `br`/`brotli`), поэтому сообщение, передаваемое в `ClickHouseServerException`, остаётся читаемым. Для кодеков, которые он не может декодировать (`zstd`, `lz4`, …), драйвер возвращает сообщение-заполнитель, в котором указан кодек и дана ссылка на `system.query_log`, где можно найти исходный текст ошибки.
