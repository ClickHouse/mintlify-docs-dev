...
<Steps>
  <Step>
    ...
    3. Как только в терминале появится баннер Airbyte, вы сможете перейти по адресу <a href="http://localhost:8000" target="_blank">localhost:8000</a>
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_01.png" size="lg" border alt="Баннер Airbyte" />
  ...
  <Step>
    ## Добавление ClickHouse в качестве пункта назначения
    ...
    2. В Airbyte откройте страницу &quot;Destinations&quot; и добавьте новый пункт назначения:
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_02.png" size="lg" border alt="Добавление пункта назначения в Airbyte" />
    ...
    3. Выберите ClickHouse в раскрывающемся списке &quot;Destination type&quot; и заполните форму &quot;Set up the destination&quot;: укажите hostname и порты ClickHouse, имя базы данных, имя пользователя и пароль, а также выберите, будет ли использоваться SSL-соединение (эквивалент флага `--secure` в `clickhouse-client`):
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_03.png" size="lg" border alt="Создание пункта назначения ClickHouse в Airbyte" />
  ...
  <Step>
    ## Добавьте набор данных как источник
    ...
    1. В Airbyte откройте страницу &quot;Sources&quot; и добавьте новый источник типа файла.
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_04.png" size="lg" border alt="Добавление источника в Airbyte" />
    ...
    2. Заполните форму &quot;Set up the source&quot;: укажите имя источника и URL файла NYC Taxi за январь 2022 года (см. ниже). Убедитесь, что в качестве формата файла выбран `parquet`, в качестве Storage Provider — `HTTPS Public Web`, а в качестве Dataset Name — `nyc_taxi_2022`.
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_05.png" size="lg" border alt="Создание источника ClickHouse в Airbyte" />

    3. Готово! Теперь вы добавили в Airbyte файловый источник.
  </Step>

  <Step>
    ## Создание подключения и загрузка набора данных в ClickHouse

    1. В Airbyte перейдите на страницу &quot;Connections&quot; и добавьте новое подключение

    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_06.png" size="lg" border alt="Добавить подключение в Airbyte" />
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_07.png" size="lg" border alt="Создание подключения в Airbyte" />
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_08.png" size="lg" border alt="Запустите синхронизацию в Airbyte" />
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_09.png" size="lg" border alt="Успешно завершено" />
