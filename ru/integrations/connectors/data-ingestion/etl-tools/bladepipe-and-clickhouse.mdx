...
<Steps>
  ...
  <Step>
    ## Добавьте ClickHouse в качестве целевого назначения
    ...
    2. Выберите `ClickHouse`, затем заполните настройки: укажите хост и порт ClickHouse, имя пользователя и пароль, после чего нажмите &quot;Проверить соединение&quot;.
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_1.png" size="lg" border alt="Добавьте ClickHouse в качестве целевого назначения" />
  ...
  <Step>
    ...
    2. Выберите `MySQL`, заполните настройки, указав хост и порт MySQL, имя пользователя и пароль, затем нажмите &quot;Test Connection&quot;.
       ...
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_2.png" size="lg" border alt="Добавление MySQL в качестве источника" />
  ...
  <Step>
    ...
    2. Выберите добавленные экземпляры MySQL и ClickHouse и нажмите &quot;Test Connection&quot;, чтобы убедиться, что BladePipe подключён к ним. Затем выберите базы данных, которые нужно перенести.
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_3.png" size="lg" border alt="Выберите источник и назначение" />

    3. В качестве типа DataJob выберите &quot;Incremental&quot; вместе с опцией &quot;Full Data&quot;.
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_4.png" size="lg" border alt="Выберите тип синхронизации" />

    4. Выберите таблицы для репликации.
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_5.png" size="lg" border alt="Выберите таблицы" />

    5. Выберите столбцы для репликации.
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_6.png" size="lg" border alt="Выберите столбцы" />

    6. Подтвердите создание DataJob — он запустится автоматически.
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_8.png" size="lg" border alt="DataJob запущен" />
  </Step>
  ...
  <Step>
    ...
    2. Создайте <a href="https://doc.bladepipe.com/operation/job_manage/create_job/create_period_verification_correction_job" target="_blank">Verification DataJob</a>. После завершения Verification DataJob проверьте результаты, чтобы убедиться, что данные в ClickHouse совпадают с данными в MySQL.
       <Image img="/images/integrations/data-ingestion/etl-tools/bp_ck_9.png" size="lg" border alt="Проверка данных" />
  </Step>
</Steps>