...
Эти руководства предназначены для использования вместе с остальной [документацией](/ru/integrations/connectors/data-ingestion/etl-tools/dbt), разделом [возможностей и конфигураций](/ru/integrations/connectors/data-ingestion/etl-tools/dbt/features-and-configurations) и [справочником по материализациям](/ru/integrations/connectors/data-ingestion/etl-tools/dbt/materializations).

<div id="setup">
  ## Настройка
</div>

Следуйте инструкциям в разделе [Настройка dbt и адаптера ClickHouse](/ru/integrations/connectors/data-ingestion/etl-tools/dbt), чтобы подготовить окружение.
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_01.png" size="lg" alt="Схема таблиц IMDB" />
...
3. На этом этапе вам понадобится любой текстовый редактор. В примерах ниже мы используем популярный VS Code. Открыв каталог IMDB, вы должны увидеть набор файлов yml и sql:
   ...
   <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_02.png" size="lg" alt="Новый проект dbt" />

4. Обновите файл `dbt_project.yml`, чтобы указать нашу первую модель — `actor_summary`, и задайте профиль `clickhouse_imdb`.

   <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_03.png" size="lg" alt="Профиль dbt" />

   <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_04.png" size="lg" alt="Профиль dbt" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_05.png" size="lg" alt="инкрементальные обновления dbt" />

Эта стратегия может вызывать трудности при работе с очень большими моделями. Подробнее см. в разделе [Ограничения](/ru/integrations/connectors/data-ingestion/etl-tools/dbt#limitations).
...
Изначально в ClickHouse была лишь ограниченная поддержка обновлений и удалений в виде асинхронных [Мутаций](/ru/reference/statements/alter). Они могут быть чрезвычайно затратными по I/O, поэтому их обычно следует избегать.
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_06.png" size="lg" alt="инкрементальное легковесное удаление" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_07.png" size="lg" alt="инкрементальный insert overwrite" />
