...
1. Предположим, у нас есть файл в формате TSV с именем `comments.tsv`, содержащий комментарии Hacker News, а в строке заголовка указаны имена столбцов. При вставке данных нужно указать [входной формат](/ru/reference/formats), который в нашем случае — `TabSeparatedWithNames`:
...
Посетите [страницу документации о `clickhouse-client`](/ru/concepts/features/interfaces/cli), чтобы узнать, как установить `clickhouse-client` в вашей операционной системе.