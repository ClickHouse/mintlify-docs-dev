---
sidebarTitle: 'Looker'
slug: /integrations/looker
keywords: ['ClickHouse', 'looker', 'подключение', 'интеграция', 'интерфейс']
description: 'Looker — это корпоративная платформа для BI, приложений для работы с данными и встроенной аналитики, которая помогает анализировать данные и делиться выводами в реальном времени.'
title: 'Looker'
doc_type: 'guide'
integration:
  - support_level: 'partner'
  - category: 'data_visualization'
---

import { Image } from "/snippets/components/Image.jsx";
import ConnectionDetails from '/snippets/ru/_gather_your_details_http.mdx';

import PartnerBadge from "/snippets/components/PartnerBadge/PartnerBadge.jsx";

<PartnerBadge />

Looker может подключаться к ClickHouse Cloud или к развертыванию в собственной инфраструктуре через официальный источник данных ClickHouse.

<div id="1-gather-your-connection-details">
  ## 1. Подготовьте сведения о подключении
</div>

<ConnectionDetails />

<div id="2-create-a-clickhouse-data-source">
  ## 2. Создайте источник данных ClickHouse
</div>

Перейдите в Admin -&gt; Database -&gt; Connections и нажмите кнопку &quot;Add Connection&quot; в правом верхнем углу.

<Image size="md" img="/images/integrations/data-visualization/looker_01.png" alt="Добавление нового соединения в интерфейсе управления базой данных Looker" border />

<br />

Укажите имя источника данных, выберите `ClickHouse` в раскрывающемся списке dialect и введите свои учетные данные в соответствующие поля формы.

<Image size="md" img="/images/integrations/data-visualization/looker_02.png" alt="Указание учетных данных ClickHouse в форме подключения Looker" border />

<br />

Если вы используете ClickHouse Cloud или для вашего развертывания требуется SSL, убедитесь, что SSL включен в дополнительных настройках.

<Image size="md" img="/images/integrations/data-visualization/looker_03.png" alt="Включение SSL для подключения ClickHouse в настройках Looker" border />

<br />

Сначала проверьте подключение, а затем, когда проверка завершится, подключитесь к новому источнику данных ClickHouse.

<Image size="md" img="/images/integrations/data-visualization/looker_04.png" alt="Проверка подключения и подключение к источнику данных ClickHouse" border />

<br />

Теперь вы сможете подключить источник данных ClickHouse к своему проекту Looker.

<div id="3-known-limitations">
  ## 3. Известные ограничения
</div>

1. Следующие типы данных по умолчанию обрабатываются как строки:
   * Array — из-за ограничений JDBC driver сериализация работает не так, как ожидается
   * Decimal* — в модели можно изменить на number
   * LowCardinality(...) — в модели можно изменить на подходящий тип
   * Enum8, Enum16
   * UUID
   * Tuple
   * Map
   * JSON
   * Nested
   * FixedString
   * Гео-типы
     * MultiPolygon
     * Polygon
     * Point
     * Ring
2. [Функция symmetric aggregate](https://cloud.google.com/looker/docs/reference/param-explore-symmetric-aggregates) не поддерживается
3. [Full outer join](https://cloud.google.com/looker/docs/reference/param-explore-join-type#full_outer) пока не реализован в драйвере