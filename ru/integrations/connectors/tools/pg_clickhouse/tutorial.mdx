...
В этом руководстве все запросы из [руководства по ClickHouse] выполняются через
pg&#95;clickhouse.
...
Сначала создайте базу данных ClickHouse, если у вас её ещё нет. Быстрее всего
начать с Docker-образа:
...
Давайте воспользуемся [руководством по ClickHouse], чтобы создать простую базу данных с датасетом
о такси Нью-Йорка:
...
Убедитесь, что запросы выполняются, затем выйдите из клиента:
...
Docker-контейнер с образом [pg&#95;clickhouse image], который просто добавляет
pg&#95;clickhouse в Docker-образ [Postgres image]:
...
Создайте foreign server, указав имя хоста, порт и базу данных вашей
...
Здесь мы выбрали бинарный драйвер, использующий бинарный
протокол ClickHouse. Также можно использовать драйвер &quot;http&quot;, который работает через HTTP-интерфейс.

Далее сопоставьте пользователя Postgres с пользователем ClickHouse. Проще всего
для этого сопоставить текущего пользователя Postgres с удалённым пользователем внешнего
сервера:
...
Теперь таблица уже должна быть импортирована: в [psql] используйте `\det+`, чтобы увидеть её:
...
Обратите внимание, как быстро выполнился запрос. pg&#95;clickhouse полностью
передаёт выполнение всего запроса, включая агрегат `COUNT()`, в ClickHouse, поэтому в
...
Обратите внимание, что &quot;Foreign Scan&quot; находится в корне плана, а это означает, что
...
Выполните несколько запросов, чтобы проанализировать данные. Ознакомьтесь со следующими примерами или попробуйте
...
Создайте словарь, связанный с таблицей в вашем сервисе ClickHouse. Таблица
и словарь основаны на CSV-файле, в котором содержится по одной строке для каждого
района Нью-Йорка.

Районы сопоставлены с названиями пяти боро Нью-Йорка
(Bronx, Brooklyn, Manhattan, Queens и Staten Island),
а также Newark Airport (EWR).

Ниже приведён фрагмент используемого CSV-файла в виде таблицы. Столбец
...
<Note>
  Значение `LIFETIME`, равное 0, отключает автоматические обновления, чтобы избежать лишнего
...
3. Убедитесь, что к ней можно выполнить запрос:
...
4. Отлично. Теперь используйте функцию `dictGet`, чтобы получить
   название боро в запросе. Этот запрос суммирует количество поездок
   на такси по каждому боро, которые заканчиваются либо в аэропорту LaGuardia, либо в JFK:
...
Напишите несколько запросов, в которых `taxi_zone_dictionary` объединяется с
