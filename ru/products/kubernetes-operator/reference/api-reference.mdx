...
| `upgradeChannel`       | string                                                                                                                              | Канал релизов задает канал выпуска для проверок обновлений мажорных версий.<br />Если значение пустое, будут предлагаться только минорные обновления. Допустимые значения: stable, lts или конкретная версия major.minor (например, 25.8).                          | false       |               |
...
| `additionalPorts`      | [AdditionalPort](#additionalport) array                                                                                             | AdditionalPorts объявляет дополнительные TCP-порты, которые нужно открыть в поде ClickHouse и headless Service, управляемом оператором.<br />Оператор только добавляет порты в ресурсы Kubernetes, но не настраивает сервер ClickHouse на прослушивание этих портов. | false       |               |
...
| `versionProbeRevision`  | string                                                                                                     | VersionProbeRevision — хэш образа последней успешной проверки версии.<br />Когда он совпадает с текущим хэшом образа, кэшированное значение Version используется напрямую. | false        |              |
