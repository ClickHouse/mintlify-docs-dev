...
| `upgradeChannel` | string | канал релизов указывает канал выпуска для проверки обновлений мажорной версии.<br />Если не задано, будут предлагаться только минорные обновления. Допустимые значения: stable, lts или конкретная версия major.minor (например, 25.8). | false |  |
| `versionProbeTemplate` | [VersionProbeTemplate](#versionprobetemplate) | VersionProbeTemplate — переопределения для задачи обнаружения версии. | false |  |
