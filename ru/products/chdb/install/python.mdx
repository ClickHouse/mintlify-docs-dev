---
title: DataFrame со смешанными JSON-объектами
sidebarTitle: 'Python'
...
```python
# Сеанс с пользовательскими настройками
sess = session.Session(
...
class DatabaseReader(chdb.PyReader):
    """Пользовательский ридер для источников данных, подобных базам данных"""
...
# Управление выводом типов JSON с помощью настроек
result = chdb.query("""
    SELECT 
        user_id,
    ...
    SETTINGS pandas_analyze_sample = 1000  -- Анализ всех строк для определения типов JSON
""", "Pretty")
print(result)
