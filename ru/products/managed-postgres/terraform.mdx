...
См. [Управление ключами API](/ru/cloud/manage/openapi), чтобы узнать, как создать ключ API для работы с провайдером.
...
`ha_type` можно изменять после создания; его изменение вызывает переход HA. Подробности см. в разделе [Высокая доступность](/ru/cloud/managed-postgres/high-availability).
...
Подробнее см. в разделе [реплики для чтения](/ru/cloud/managed-postgres/read-replicas).
...
Весь этот блок используется только на этапе создания: изменение `source_id` или `restore_target`, а также удаление блока приводят к удалению и повторному созданию сервиса. Подробности см. в разделе [Резервное копирование и восстановление](/ru/cloud/managed-postgres/backup-and-restore).
