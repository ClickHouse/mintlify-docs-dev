...
[Динамический выбор столбцов](/ru/reference/statements/select#dynamic-column-selection) — это мощная, но недооценённая возможность ClickHouse, которая позволяет выбирать столбцы с помощью регулярных выражений вместо того, чтобы перечислять каждый столбец отдельно. Вы также можете применять функции к совпадающим столбцам с помощью модификатора `APPLY`, что делает эту возможность особенно полезной для задач анализа и преобразования данных.

Мы разберём, как использовать эту возможность, на примере [датасета New York taxis](/ru/get-started/sample-datasets/nyc-taxi), который также доступен в [Песочнице ClickHouse](https://sql.clickhouse.com?query=LS0gRGF0YXNldCBjb250YWluaW5nIHRheGkgcmlkZSBkYXRhIGluIE5ZQyBmcm9tIDIwMDkuIE1vcmUgaW5mbyBoZXJlOiBodHRwczovL2NsaWNraG91c2UuY29tL2RvY3MvZW4vZ2V0dGluZy1zdGFydGVkL2V4YW1wbGUtZGF0YXNldHMvbnljLXRheGkKU0VMRUNUICogRlJPTSBueWNfdGF4aS50cmlwcyBMSU1JVCAxMDA).
...
Мы также можем использовать модификатор [`APPLY`](/ru/reference/statements/select), чтобы применять функции сразу ко всем столбцам.
...
Пока всё хорошо. Но предположим, что мы хотим изменить одно из значений, оставив остальные как есть. Например, мы хотим удвоить общую сумму и разделить налог MTA на 1.1. Это можно сделать с помощью модификатора [`REPLACE`](/ru/reference/statements/select), который заменяет один столбец, не затрагивая остальные.
...
Мы также можем исключить столбец с помощью модификатора [`EXCEPT`](/ru/reference/statements/select). Например, чтобы убрать столбец `tolls_amount`, нужно написать следующий запрос:
