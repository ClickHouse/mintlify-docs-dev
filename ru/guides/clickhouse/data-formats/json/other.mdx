...
Мы можем объявить ключ `request` как `Nested`. Как и в случае с `Tuple`, необходимо указать вложенные столбцы.
