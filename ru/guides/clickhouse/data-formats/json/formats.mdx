...
Один из самых распространённых форматов JSON-данных — список объектов JSON в JSON-массиве, как в [этом примере](/ru/assets/list.json):
...
Чтобы импортировать список объектов JSON, можно использовать формат [`JSONEachRow`](/ru/reference/formats/JSON/JSONEachRow) (для вставки данных из файла [list.json](/ru/assets/list.json)):
...
В некоторых случаях список объектов JSON может быть представлен как свойства объекта, а не как элементы массива (см. пример в [objects.json](/ru/assets/objects.json)):
...
Иногда для экономии места JSON‑файлы кодируют в виде массивов, а не объектов. В этом случае мы имеем дело со [списком JSON-массивов](/ru/assets/arrays.jsonl){/* MIGRATE: unresolved relative link */}:
...
В некоторых случаях данные могут быть представлены по столбцам, а не по строкам. В этом случае родительский объект JSON содержит столбцы со значениями. См. [следующий файл](/ru/assets/columns.json):
...
Также поддерживается более компактный формат для работы с [массивом столбцов](/ru/assets/columns-array.json) вместо объекта с использованием формата [`JSONCompactColumns`](/ru/reference/formats/JSON/JSONCompactColumns):
...
Когда мы имеем дело с [вложенными JSON-объектами](/ru/assets/list-nested.json), можно дополнительно определить явную схему и использовать сложные типы ([`Array`](/ru/reference/data-types/array), [`JSON`](/ru/guides/clickhouse/data-formats/json/intro) или [`Tuple`](/ru/reference/data-types/tuple)) для загрузки данных:
...
Мы можем обращаться к [ключам вложенных JSON-объектов](/ru/assets/list-nested.json), включив [следующий параметр настройки](/ru/reference/settings/formats#input_format_import_nested_json):
...
Мы по-прежнему можем вставить в эту таблицу [исходные данные в формате JSON](/ru/assets/list.json) с 3 столбцами:
...
Для импорта данных BSON используется формат [BSONEachRow](/ru/reference/formats/BSONEachRow). Давайте импортируем данные из [этого BSON-файла](/ru/assets/data.bson):
