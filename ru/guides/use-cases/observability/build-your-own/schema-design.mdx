...
[Materialized views](/ru/concepts/features/materialized-views) дают более широкие возможности для применения SQL-фильтрации и преобразований к журналам и трассировкам.
...
<Image img="/images/use-cases/observability/observability-10.png" alt="Materialized view" size="md" />
...
<Image img="/images/use-cases/observability/observability-12.png" alt="Обсервабилити и словари" size="md" />
...
Хотя JOIN в сценариях обсервабилити требуются редко, словари всё равно могут быть полезны для обогащения — как во время вставки, так и во время выполнения запроса. Ниже мы приводим примеры обоих вариантов.

<Info>
  **Ускорение JOIN**

  Пользователи, которым интересно ускорить JOIN с помощью словарей, могут найти дополнительные сведения [здесь](/ru/concepts/features/dictionaries).
</Info>
...
Примеры простого обогащения смотрите в руководстве по словарям [здесь](/ru/concepts/features/dictionaries). Ниже мы сосредоточимся на распространённых задачах обогащения в обсервабилити.
...
* [Продвинутые темы словарей](/ru/concepts/features/dictionaries#advanced-dictionary-topics)
...
<Image img="/images/use-cases/observability/observability-13.png" alt="Обсервабилити и проекции" size="md" />
...
1 строка в наборе. Elapsed: 0.008 sec.
```
...
Одна из основных причин, по которой ClickHouse используют для обсервабилити, — эффективное сжатие.

Помимо существенного снижения затрат на хранение, меньший объем данных на диске означает меньше операций ввода-вывода, а также более быстрые запросы и вставки. Снижение объема операций ввода-вывода с лихвой компенсирует нагрузку на CPU, создаваемую любым алгоритмом сжатия. Поэтому, если вы хотите ускорить запросы ClickHouse, в первую очередь следует сосредоточиться на улучшении сжатия данных.
