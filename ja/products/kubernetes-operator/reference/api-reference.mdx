...
| フィールド | 型 | 説明 | 必須 | デフォルト |
|-------|------|-------------|----------|---------|
| `replicas` | integer | 単一シャード内のレプリカ数。 | false | 3 |
| `shards` | integer | クラスター内のシャード数。 | false | 1 |
| `keeperClusterRef` | [KeeperClusterReference](#keeperclusterreference) | ClickHouse の調整に使用される KeeperCluster への参照。<br />名前空間が省略された場合、ClickHouseCluster の名前空間が使用されます。 | true |  |
| `podTemplate` | [PodTemplateSpec](#podtemplatespec) | ClickHouse ポッドスペックに渡されるパラメーター。 | false |  |
| `containerTemplate` | [ContainerTemplateSpec](#containertemplatespec) | ClickHouse コンテナスペックに渡されるパラメーター。 | false |  |
...
| `additionalVolumeClaimTemplates` | [PersistentVolumeClaimTemplate](#persistentvolumeclaimtemplate) array | JBOD / マルチディスクストレージ用の追加のポッドごとの PVC テンプレート。<br />各エントリは StatefulSet の volumeClaimTemplate に反映され、/var/lib/clickhouse/disks/<name> にマウントされ、<br />生成された JBOD ストレージポリシーに追加されます。<br />ディスクのセットは作成時に固定されます。 | false |  |
| `labels` | object (keys:string, values:string) | リソースに追加される追加ラベル。 | false |  |
| `annotations` | object (keys:string, values:string) | リソースに追加される追加アノテーション。 | false |  |
| `podDisruptionBudget` | [PodDisruptionBudgetSpec](#poddisruptionbudgetspec) | PodDisruptionBudget は各シャードに作成される PDB を設定します。<br />未設定の場合、オペレーターはシングルレプリカシャードに対して maxUnavailable=1、<br />マルチレプリカシャードに対して minAvailable=1 をデフォルトとします。 | false |  |
| `settings` | [ClickHouseSettings](#clickhousesettings) | ClickHouse サーバーの設定パラメーター。 | false |  |
| `clusterDomain` | string | ClusterDomain は DNS 解決に使用される Kubernetes クラスタードメインサフィックスです。 | false | cluster.local |
| `upgradeChannel` | string | UpgradeChannel はメジャーバージョンアップグレードチェックのリリースチャンネルを指定します。<br />空の場合、マイナーアップデートのみが提案されます。使用可能な値は stable、lts、または特定の major.minor バージョン（例: 25.8）です。 | false |  |
| `versionProbeTemplate` | [VersionProbeTemplate](#versionprobetemplate) | VersionProbeTemplate はバージョン検出ジョブのオーバーライドです。 | false |  |
| `externalSecret` | [ExternalSecret](#externalsecret) | ExternalSecret はクラスターシークレットを含む外部管理Secretへのオプションの参照です。<br />シークレットはクラスターと同じ名前空間に存在する必要があります。 | false |  |
| `additionalPorts` | [AdditionalPort](#additionalport) array | AdditionalPorts は ClickHouse ポッドおよびオペレーター管理のヘッドレス Serviceに公開する追加の TCP ポートを宣言します。<br />オペレーターは Kubernetes リソースにポートを追加するだけで、ClickHouse サーバーがそれらをリッスンするように設定しません。 | false |  |
...
ClickHouseSettings は、ClickHouse サーバー の設定オプションを定義します。
...
| `logger`              | [LoggerConfig](#loggerconfig)                                                                                 | ClickHouse サーバー のログ設定です。                                                                                                  | false    |         |
| `tls`                 | [ClusterTLSSpec](#clustertlsspec)                                                                             | TLS の設定です。ClickHouse サーバー のセキュアなエンドポイントや証明書検証を構成できます。                                                                     | false    |         |
| `enableDatabaseSync`  | boolean                                                                                                       | 新しく作成されたレプリカへの ClickHouse データベースの同期と、スケールダウン後の古くなったレプリカのクリーンアップを有効にします。<br />Replicated およびインテグレーションデータベースのみをサポートします。<br /> | false    | true    |
