...
<Steps>
  <Step>
    ...
    <Image img="/images/cloud/agent-builder/launch-ch-agents.png" alt="ClickHouse agents（ベータ）が選択された Cloud サービスのナビゲーションと、Launch ClickHouse agents ボタンが表示されたランチパッド" size="lg" />
  </Step>
  ...
  <Step>
    ...
    * **Model** - ドロップダウンからモデルを選択します。temperature などの生成設定は [model parameters](/ja/products/cloud/features/ai-ml/agents/builder/model-parameters) で調整します。

    <Image img="/images/cloud/agent-builder/agent-builder.png" alt="Create New Agent ドロップダウン、フォームフィールド（Name、Description、Category、Instructions、Model）、および Capabilities セクションが表示されたエージェントビルダーパネル" size="lg" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/cloud/agent-builder/capabilities.png" alt="Run Code、ウェブ検索、File Context、Artifacts、MCPサーバー、スキル の切り替えが表示されたエージェントビルダーパネルの Capabilities セクション" size="sm" />
    ...
    <Image img="/images/cloud/agent-builder/tools-button.png" alt="Add Tools ボタンが強調表示されたエージェントビルダーパネル下部" size="sm" />

    **Add Tools** をクリックしてカタログを参照します。

    <Image img="/images/cloud/agent-builder/tools-modal.png" alt="Google、OpenAI Image Tools、Wolfram、DALL-E-3、Tavily Search、Calculator、Stable Diffusion などのサードパーティインテグレーションがグリッド表示された Agent Tools モーダル" size="lg" />
  ...
  <Step>
    ...
    <Image img="/images/cloud/agent-builder/chat-query.png" alt="「What are my top 10 tables by row count this week?」という質問と、それに対するエージェントの応答を示すチャット。行数でサービス全体の上位 10 テーブルを順位付けした Markdown テーブルと、その下に Key Observations が表示されている" size="lg" />
  </Step>
</Steps>
