...
ClickHouse Cloud の[プライベート ネットワーキング](/ja/products/cloud/guides/security/connectivity/private-networking)を使用すると、ClickHouse のサービスをクラウドの仮想ネットワークに直接接続でき、データがパブリック インターネットを経由しないようにできます。これは、厳格なセキュリティ要件やコンプライアンス要件がある組織、またはプライベート サブネットでアプリケーションを実行している組織にとって不可欠です。

ClickHouse Cloud は、以下の方式でプライベート ネットワーキングをサポートしています。

* [AWS PrivateLink](/ja/products/cloud/guides/security/connectivity/private-networking/aws-privatelink): VPC と ClickHouse Cloud 間で、トラフィックをパブリック インターネットに公開することなく安全に接続できます。クロスリージョン接続をサポートしており、Scale および Enterprise プランで利用できます。Setup では、プライベートリンクの エンドポイント を作成し、これを ClickHouse Cloud の organization と service の許可リストに追加します。詳細と手順については、こちらのドキュメントを参照してください。
...
ClickHouse Cloud は現在、SCIM や IdP を介した自動プロビジョニング／デプロビジョニングをサポートしていません。ユーザーは、IdP から削除された後に ClickHouse Cloud コンソールから手動で削除する必要があります。これらの機能が利用可能になるまでは、手動でユーザーを管理する運用を想定してください。

[クラウドアクセス管理](/ja/products/cloud/guides/security/cloud-access-management) および [SAML SSO セットアップ](/ja/products/cloud/guides/security/cloud-access-management/saml-sso-setup) について詳しくは、こちらをご覧ください。
...
現在の [サービスティアの制限](/ja/products/cloud/features/cloud-tiers) が本番環境での運用に影響している場合は、[プライベートネットワーキング](/ja/products/cloud/guides/security/connectivity/private-networking)、[TDE/CMEK](/ja/products/cloud/guides/security/cmek) (顧客管理暗号鍵による透過的データ暗号化) 、[高度なバックアップオプション](/ja/products/cloud/guides/backups/configurable-backups) など、より高度な機能を利用するためのアップグレードを検討してください。