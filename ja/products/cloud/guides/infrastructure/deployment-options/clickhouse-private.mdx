...
<Image img="/images/cloud/reference/private-gov-architecture.png" size="md" alt="ClickHouse Private アーキテクチャ" background="black" />
...
お客様は、[お問い合わせ](https://clickhouse.com/company/contact?loc=nav)から、ClickHouse Private がお客様のユースケースに適しているかを確認するための打ち合わせを依頼できます。最小規模要件を満たし、サポート対象の構成にデプロイされるユースケースが審査対象となります。オンボーディングの対応枠には限りがあります。インストールは、AWS ECR からダウンロードしたイメージと Helm チャートを使用し、ClickHouse をデプロイする対象環境向けのインストールガイドに従って行います。