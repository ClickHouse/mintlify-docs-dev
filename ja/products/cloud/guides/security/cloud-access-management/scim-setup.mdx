...
<Steps>
  <Step>
    ...
    ![組織設定で SCIM 設定タブに移動する](/images/cloud/security/scim-okta/scim-okta-01.png)

    `Enable SCIM` をクリックします。SCIM は SAML SSO の接続後に有効になります。オプションがグレーアウトしている場合は、先に SAML の設定を完了してください。

    ![Enable SCIM を切り替える](/images/cloud/security/scim-okta/scim-okta-02.png)
  ...
  <Step>
    ...
    ![新しい SCIM API key を生成する](/images/cloud/security/scim-okta/scim-okta-07.png)
  </Step>
...
<Steps>
  <Step>
    ...
    **General** タブで **App Settings** セクションを見つけて `Edit` をクリックします。**Provisioning** で `SCIM` を選択し、`Save` をクリックします。
    ...
    ![Okta アプリケーション設定でプロビジョニングモードを SCIM に設定する](/images/cloud/security/scim-okta/scim-okta-03.png)

    これで、アプリケーションに **Provisioning** タブが表示されます。

    ![アプリケーションに Provisioning タブが表示された状態](/images/cloud/security/scim-okta/scim-okta-05.png)
  </Step>
  ...
  <Step>
    ...
    * **Authentication Mode** — `Basic Auth`。
    ...
    ![SCIM コネクタ URL を入力し、一意識別子を userName に設定する](/images/cloud/security/scim-okta/scim-okta-06.png)

    ![SCIM 認証用の API 認証情報を入力する](/images/cloud/security/scim-okta/scim-okta-08.png)

    `Test Connector Configuration` をクリックします。緑色の確認メッセージが表示されるはずです。失敗した場合は、[Troubleshooting](#troubleshooting) に進んでください。

    ![SCIM 接続をテストする](/images/cloud/security/scim-okta/scim-okta-09.png)
  ...
  <Step>
    ...
    ![ユーザー向けの SCIM Provisioning アクションを有効にする](/images/cloud/security/scim-okta/scim-okta-10.png)

    `Save` をクリックし、アプリケーションの **Sign On** / **Provisioning** タブに戻って、設定が反映されていることを確認します。

    ![Provisioning の設定を保存して Sign On タブに戻る](/images/cloud/security/scim-okta/scim-okta-10b.png)
  </Step>
  ...
  <Step>
    ...
    **Okta でグループを作成します。** 先ほど設定した各ロールマッピングに対して、**表示名が完全に一致する** Okta グループを作成するか、既存のグループを特定します。たとえば、マッピングが `ClickHouse-Admins → Admin` であれば、Okta で `ClickHouse-Admins` という名前のグループを作成します。
    ...
    ![Okta で新しいグループを作成する](/images/cloud/security/scim-okta/scim-okta-13.png)

    作成したグループを開き、`Assign people` をクリックしてメンバーを追加します。

    ![グループの Assign people をクリックする](/images/cloud/security/scim-okta/scim-okta-14.png)

    ![ユーザーをグループに割り当てる](/images/cloud/security/scim-okta/scim-okta-15.png)

    次に、ロールのメンバーシップとアプリへのアクセスを同期した状態に保つため、同じグループに SCIM アプリケーションも割り当てます。

    ![アプリケーションをグループに割り当てる](/images/cloud/security/scim-okta/scim-okta-16.png)
    ...
    **グループをプッシュします。** アプリケーションの **Provisioning** タブで `Push Groups → Find groups by name` をクリックし、グループを検索して `Save` をクリックします。これを各ロールグループに対して繰り返します。プロビジョニングが完了すると、それぞれの **Push Status** は **Active (Pushed)** と表示されるはずです。
    ...
    ![アプリケーションの Push Groups タブで名前による Group Push を設定する](/images/cloud/security/scim-okta/scim-okta-17.png)
...
![Users and roles でユーザー同期を確認](/images/cloud/security/scim-okta/scim-okta-18.png)
