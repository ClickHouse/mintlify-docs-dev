...
<Info>
  **PrivateLink 外からの接続にのみ適用されます**
  ...
  IP Access List は、[PrivateLink](/ja/products/cloud/guides/security/connectivity/private-networking) の外側にあるパブリックインターネットからの接続にのみ適用されます。
  PrivateLink からのトラフィックのみを許可する場合は、IP Allow list で `DenyAll` を設定してください。
</Info>

<Accordion title="ClickHouse service の IP Access List">
  ClickHouse service を作成すると、IP allow list のデフォルト設定は「すべての場所から許可」です。

  ClickHouse Cloud services の一覧から service を選択し、**Settings** を選択します。**Security** セクションに IP Access List があります。Add IPs ボタンをクリックします。

  設定オプションを選べるサイドバーが表示されます。

  * すべての場所から service への受信トラフィックを許可する
  * 特定の場所から service へのアクセスを許可する
  * service へのすべてのアクセスを拒否する
</Accordion>

<Accordion title="API key の IP Access List">
  API key を作成すると、IP allow list のデフォルト設定は「すべての場所から許可」です。

  API key の一覧で、**Actions** カラムの API key の横にある 3 点メニューをクリックし、**Edit** を選択します。画面下部に IP Access List と、その設定オプションがあります。
...
このスクリーンショットは、「NY Office range」と記載された IP アドレス範囲からのトラフィックを許可する access list を示しています。

<Image img="/images/cloud/security/ip-filtering-after-provisioning.png" size="md" alt="ClickHouse Cloud の既存の access list" border />
...
<Image img="/images/cloud/security/ip-filter-add-single-ip.png" size="md" alt="ClickHouse Cloud のアクセスリストに 1 つの IP を追加する" border />
