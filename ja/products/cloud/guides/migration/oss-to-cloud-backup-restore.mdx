...
<div id="migrating-from-self-managed-clickhouse-to-clickhouse-cloud-using-backup-commands">
  # バックアップコマンドを使用して、セルフマネージドのClickHouseからClickHouse Cloudに移行する
</div>
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/cloud/onboard/migrate/oss_to_cloud_via_backup/create_service.png" size="md" alt="新しいサービスを作成する" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/cloud/onboard/migrate/oss_to_cloud_via_backup/service_details.png" size="md" alt="サービスの設定を行う" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/cloud/onboard/migrate/oss_to_cloud_via_backup/open_console.png" size="md" alt="サービスの設定を行う" />

    ### S3 アクセスを設定する

    S3 からバックアップを復元するには、ClickHouse Cloud と S3 バケットの間で安全にアクセスできるよう設定する必要があります。

    1. [&quot;Accessing S3 data securely&quot;](/ja/products/cloud/guides/data-sources/accessing-s3-data-securely) の手順に従ってアクセスロールを作成し、ロール ARN を取得します。

    2. [&quot;How to create an S3 bucket and IAM role&quot;](/ja/integrations/connectors/data-ingestion/AWS/creating-an-s3-iam-role-and-bucket) で作成した S3 バケットポリシーを更新し、前の手順で取得したロール ARN を追加します。
    ...
    * **IAM ユーザー** (`docs-s3-user`): セルフマネージドの ClickHouse クラスターが S3 にバックアップできるようにします
    * **ClickHouse Cloud ロール** (`ClickHouseAccess-001`): Cloud サービスが S3 から復元できるようにします
  </Step>
</Steps>
...
<Image img="/images/cloud/onboard/migrate/oss_to_cloud_via_backup/backup_in_s3_bucket.png" size="md" alt="バックアップ、データ、メタデータ" />
