...
<Image img="/images/_snippets/aws-rds-mysql.png" size="lg" alt="AWS セキュリティグループルール" border />

同じ ClickHouse Cloud サービスが `us-east-2` で稼働していて、今度は GCP 上の MySQL に接続する場合、`Authorized networks` は次のようになります。

<Image img="/images/_snippets/gcp-authorized-network.png" size="md" alt="GCP 承認済みネットワーク" border />