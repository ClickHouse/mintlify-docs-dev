...
<Steps>
  ...
  <Step title="モデルを選択する">
    ...
    <Image img="/images/agentic-data-stack/select-model.png" alt="左上隅にある LibreChat のモデル selector。デフォルトのモデルと「Select a model」のツールチップが表示されている" size="md" />
    ...
    <Accordion title="UI で provider の key を設定する">
      ...
      <Image img="/images/agentic-data-stack/set-api-key.png" alt="Anthropic provider の横にある Set API Key ボタンが表示された LibreChat のモデル selector" size="md" />

      ダイアログに key を貼り付けて **Submit** をクリックします。有効期限を設定することも、期限切れにしないこともできます。

      <Image img="/images/agentic-data-stack/set-api-key-modal.png" alt="Anthropic 用の Set API Key ダイアログ。expiration のドロップダウン、Key フィールド、Submit ボタンと Revoke ボタンが表示されている" size="md" />
    </Accordion>
  </Step>
  ...
  <Step title="MCPサーバー を選択する">
    ...
    <Image img="/images/agentic-data-stack/select-clickhouse-local.png" alt="LibreChat の composer で MCP Servers メニューが開かれ、緑の接続済みドット付きの ClickHouse-Local と ClickHouse-Cloud が表示されている" size="md" />
  </Step>
  ...
  <Step title="最初の質問をする">
    ...
    <Image img="/images/agentic-data-stack/prompt-chat.png" alt="ClickHouse-Local の MCP ツールを呼び出して利用可能な database と table を一覧表示し、例の質問に回答する LibreChat" size="md" />
  </Step>
</Steps>
