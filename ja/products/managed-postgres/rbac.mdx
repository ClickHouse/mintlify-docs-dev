...
<Image img="/images/managed-postgres/rbac/usersandroles.png" alt="「Users and roles」メニュー" size="md" border />

2. **Roles** タブに切り替え、**Create role** をクリックします。
3. ロール名を入力し、**+ Allow** をクリックして、エンティティ一覧から **Postgres Service** を選択します。

<Image img="/images/managed-postgres/rbac/postgresentity.png" alt="Postgres Service エンティティの選択" size="md" border />

4. ロールの適用対象とする Postgres サービスを選択し、付与する権限を選びます。

<Image img="/images/managed-postgres/rbac/newpostgresperms.png" alt="ロールの Postgres 権限の設定" size="md" border />
