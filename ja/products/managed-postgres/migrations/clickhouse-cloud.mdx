...
title: 'ClickHouse Cloud の Data sources を使用して PostgreSQL データを移行する'
...
<div id="migrate-managed-postgres">
  # ClickHouse Cloud を使用した Managed Postgres への移行
</div>

<BetaBadge />

ClickHouse Cloud には、外部 PostgreSQL データベースを Managed Postgres サービスに移行するための組み込みインポートウィザードが用意されています。このウィザードでは、ソース接続、スキーマのエクスポートとインポート、レプリケーション設定、テーブルの選択を、ガイド付きの 5 つの手順で行えます。
...
<Image img="/images/managed-postgres/pgpg/servicecard.png" alt="ClickHouse Cloud のサービス一覧にある Managed Postgres サービスカード" size="lg" border />

左側のサイドバーで **Data sources** をクリックします。

<Image img="/images/managed-postgres/pgpg/overview.png" alt="Managed Postgres サービスのサイドバーにある Data sources エントリ" size="lg" border />

**Start import** をクリックします。

<Image img="/images/managed-postgres/pgpg/startimport.png" alt="Start import ボタンがある Data sources ページ" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/migrationform.png" alt="ステップ 1: インジェスト方法のオプションを含むソースデータベース接続フォーム" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/nextexport.png" alt="ステップ 2: スキーマのエクスポート用 `pg_dump` コマンド" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/psqlexport.png" alt="pg_dump 実行後のターミナル出力" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/nextimport.png" alt="ステップ 3: スキーマのインポート用 `psql` コマンド" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/psqlimport.png" alt="psql でスキーマをインポートした後のターミナル出力" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/advancedsettings.png" alt="Step 4: publication と高度なレプリケーションオプションを含むインジェスト設定フォーム" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/tablepicker.png" alt="ステップ 5: スキーマごとにまとめられたテーブル選択画面と移行を作成ボタン" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/migrationlist.png" alt="実行中の移行が表示された Data sources の一覧" size="lg" border />
...
<Image img="/images/managed-postgres/pgpg/initialload.png" alt="テーブルごとの初期ロード統計が表示された移行の詳細ビュー" size="lg" border />
