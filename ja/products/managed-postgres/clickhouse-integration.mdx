...
<Image img="/images/managed-postgres/clickhouse-integration-intro.png" alt="サイドバーのインテグレーションオプションを示す ClickHouse 連携のランディングページ" size="md" border />
...
<Steps>
  <Step>
    ...
    * **Replication method**: 次のいずれかを選択します。
    ...
    <Image img="/images/managed-postgres/replication-service-step.png" alt="インテグレーション名、宛先サービス、レプリケーション方式のオプションを示すレプリケーションサービス設定" size="md" border />
  ...
  <Step>
    ...
    * **Sync interval (seconds)**: Postgres から変更を取得する間隔 (デフォルト: 60)
    ...
    * **Snapshot number of tables in parallel**: 同時にスナップショットを取得するテーブル数 (デフォルト: 1)
    ...
    <Image img="/images/managed-postgres/integration-replication-settings.png" alt="同期間隔、並列スレッド、Pull バッチサイズ、スナップショット構成オプションを示すレプリケーション設定ステップ" size="md" border />
  ...
  <Step>
    ...
    * **Remove deleted rows during merges**: [ReplacingMergeTree](/ja/reference/engines/table-engines/mergetree-family/replacingmergetree) テーブルで、バックグラウンドマージ中に削除済みの行を物理的に削除します
    ...
    <Image img="/images/managed-postgres/select-tables-step.png" alt="データベース選択、レプリケーションオプション、スキーマごとにグループ化されたテーブルピッカーを示すテーブル選択ステップ" size="md" border />
  ...
  <Step>
    ...
    <Image img="/images/managed-postgres/integration-running.png" alt="宛先サービスとステータスを含む Running 状態の ClickPipe を示す ClickHouse 連携の一覧" size="md" border />
