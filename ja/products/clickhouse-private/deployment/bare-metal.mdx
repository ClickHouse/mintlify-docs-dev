---
hidden: true
title: "ベアメタルでの ClickHouse Private"
slug: "docs/products/clickhouse-private/bare-metal"
sidebarTitle: "ベアメタル"
---

以下は、AWS のベアメタルノードでのインストール手順の例です
...
<div id="acquire-aws-iam-credentials-and-create-a-credentials-file">
  #### AWS IAM 認証情報を取得し、認証情報ファイルを作成する
</div>

AWS IAM コンソールにアクセスし、アクセスキーとシークレットアクセスキーを作成して、
それぞれの値を保存します。

**NOTE**
`<Your AWS ACCESS KEY ID>` と `<Your AWS SECRET ACCESS KEY>` は、AWS IAM コンソールで
取得した値に置き換えてください。
...
**注: state をローカルから S3 に移行する場合は、`-migrate-state` を追加してください**

<div id="initialize-the-rke2-terraform-module">
  #### rke2 の Terraform モジュールを初期化する
</div>
...
<div id="issue-a-terraform-apply-command-to-bring-the-stack-up">
  #### スタックを起動するために Terraform の apply コマンドを実行する
</div>

Git リポジトリ内にある `terraform` ディレクトリから
...
<div id="run-ansible-to-provision-rke2-software">
  #### Ansible を実行して RKE2 ソフトウェアをプロビジョニングする
</div>
...
<div id="install-ansible-galaxy-role-lablabsrke2">
  ##### Ansible Galaxy のロール [lablabs.rke2](https://galaxy.ansible.com/ui/standalone/roles/lablabs/rke2/) をインストールする
</div>
...
<div id="install-ansible-galaxy-collection-kubernetes-core">
  ##### Ansible Galaxy の [kubernetes-core](https://galaxy.ansible.com/ui/repo/published/kubernetes/core/) コレクションをインストールする
</div>
...
<div id="run-rke2-playbook-ansiblerke2_playbook">
  ##### RKE2 プレイブック *../ansible/rke2_playbook* を実行
</div>
...
<div id="prepare-cert-manager-and-cas">
  #### cert-manager と CA を準備する
</div>

cert-manager をインストールして設定し、証明書を作成します
...
[ローカルストレージのドライバー](https://raw.githubusercontent.com/rancher/local-path-provisioner/v0.0.32/deploy/local-path-storage.yaml) をインストールします
...
```
  # 変更を適用するには、このファイルを local-nvme-sc.yaml として保存し、
  # 'kubectl apply -f local-nvme-sc.yaml' または 'kubectl patch sc local-nvme-sc --patch "$(cat local-nvme-sc.yaml)"' を使用してください
...
<div id="install-operator">
  ##### [operator](https://github.com/minio/operator)をインストールする
</div>
...
<div id="install-directpv-plugin">
  ##### [directpvプラグイン](https://www.min.io/directpv) をインストールする
</div>
...
* 応答が &quot;No drives are available to initialize&quot; の場合は、初期化しないでください！ *
...
*注記: minio-values.yaml は [https://docs.min.io/community/minio-object-store/reference/tenant-chart-values.html](https://docs.min.io/community/minio-object-store/reference/tenant-chart-values.html) をもとに修正したバージョンです*
...
# これは無視してください - error: the path "operator-ca-ts-certificate.yml" does not exist
...
* ローカルホストからUIにアクセスできるように、ssh でもポートフォワーディングを設定していることを確認してください。

* プロビジョニングホストの環境変数にあるユーザー名とパスワードを使用してログインしてください:
  * MINIO&#95;ACCESS&#95;KEY
  * MINIO&#95;SECRET&#95;KEY

* バケットを作成します ([https://localhost:9443/browser/add-bucket](https://localhost:9443/browser/add-bucket))
  環境変数 S3&#95;BUCKET&#95;NAME の値を使用してください

* アクセスキーを作成します ([https://localhost:9443/access-keys/new-account](https://localhost:9443/access-keys/new-account))
  次の環境変数の値を使用してください:
  * S3&#95;ACCESS&#95;KEY&#95;ID
  * S3&#95;SECRET&#95;ACCESS&#95;KEY

<div id="label-nodes-in-prep-for-clickhouse-keeper-and-server-pods">
  ##### ClickHouse Keeper と Server のポッド用にノードへラベルを付ける
</div>
...
<div id="clickhouse-installation-notes-on-bare-metal">
  ### ベアメタル環境での ClickHouse インストールに関する注意事項
</div>

このドキュメントでは、ベアメタル環境への ClickHouse のインストールに必要な手順とコマンドについて説明します。あらかじめ Kubernetes クラスターが構築されており、コンテナレジストリにアクセスできることを前提としています。 

<div id="helm-chart-configuration-clickhouse-valuesyaml">
  #### Helm チャートの設定 (clickhouse-values.yaml)
</div>

このセクションでは、Helm で使用する ClickHouse の values ファイルの構成と主要なパラメータについて説明します。
...
<div id="1-helm-variables-script-1_set-helm-varssh">
  ##### 1. Helm 変数スクリプト (1_set-helm-vars.sh)
</div>

このスクリプトでは、Helm でのデプロイに必要なすべての環境変数を定義します。
...
# VPCサブネットで決定されたAZを設定します。該当しない場合はk8sホスト名を使用してください。
...
#export SERVER_CPU="7"
export SERVER_MEMORY="128Gi" # 406948151296 のディスク容量が必要
...
<div id="2-install-clickhouse-operator-2_helm_install_operatorsh">
  ##### 2. ClickHouse Operator をインストールする (2_helm_install_operator.sh)
</div>

このスクリプトは、必要なイメージリポジトリ、機能フラグ、可用性ゾーンを設定し、Helm を使用して ClickHouse Operator をインストールまたはアップグレードします。

```bash
# 必要に応じてECR_HOSTを更新する
source set_helm_vars.sh

# ユーザーがhelmレジストリにログインしていることを確認する
aws ecr get-login-password  | helm registry login --username AWS --password-stdin 359774249275.dkr.ecr.us-east-2.amazonaws.com

# operatorイメージをプルするためのシークレットが存在することを確認する
kubectl delete secret all-icr-io -n clickhouse-operator-system  # 古くなっている場合に備えて以前のシークレットを削除する
...
# operator本体ではなく、operatorのHelmチャートのバージョンを使用すること（例：main.1.11917.1ではなく1.11917.1）
OPERATOR_VERSION=1.13500.1
# IMAGE_TAGは省略するか、以下のようにデフォルト値を上書きすることができる。
...
# 変数が設定されており空でない場合のみ、IMAGE_TAG引数を追加する
if [[ -n "$IMAGE_TAG" ]]; then
...
<div id="if-error-such-as-this-then-you-need-to-log-into-the-helm-registry">
  ###### このようなエラーが表示された場合は、Helmレジストリにログインする必要があります：
</div>
...
<div id="3-install-clickhouse-cluster-3_helm_install_clickhousesh">
  ##### 3. ClickHouseクラスターのインストール (3_helm_install_clickhouse.sh)
</div>
...
<div id="optional-if-you-dont-have-a-local-storage-driver-and-plan-to-use-one-here-is-an-example-of-how-to-set-one-up-with-nvme">
  ###### *任意: ローカルストレージドライバーがなく、これから使用する予定がある場合は、nvme を使って設定する方法の例を以下に示します。*
</div>

* ローカルストレージドライバーをインストール
...
<div id="run-3_helm_install_clickhousesh">
  ###### 3_helm_install_clickhouse.sh を実行
</div>

このスクリプトは、メインの ClickHouse クラスターに対する Helm でのインストール／アップグレードを自動化し、変数を読み込んで values ファイルを準備します。

```bash
  #!/bin/bash
  source ./set_helm_vars.sh
  
  # my-dynamic-values.yaml の変数を展開して一時的な values ファイルを準備する。
  echo "Expanding YAML values from my-dynamic-values.yaml using envsubst..."
  TEMP_VALUES_FILE=$(mktemp) # 一意の一時ファイルを作成する
  ...
  # docker registry シークレットが最新であることを確認する。
  ...
  # 5. 一時的な values ファイルを削除する。
  echo "Cleaning up temporary values file..."
  rm "$TEMP_VALUES_FILE"
  
  # 6. アップグレードの効果を確認するためにポッドを監視する。
...
<div id="4-fix-keeper-statefulset-4_fix_keeper_stssh">
  ##### 4. Keeper StatefulSet を修正する (4_fix_keeper_sts.sh)
</div>

このスクリプトは、Kubernetes シークレットと ClickHouse Keeper の StatefulSet にパッチを適用するためのものです。イメージプルシークレットや環境変数に関連する特定のアクセス問題を解消するうえで重要になることがあります。これにより、Keeper のポッドが必要なリソースに正しくアクセスできるようになります。
...
この YAML テンプレートは、5&#95;fix&#95;keeper&#95;sts.sh スクリプトで ClickHouse Keeper の StatefulSet にパッチを適用するために使用されます。imagePullSecrets を追加し、AWS アクセスキーとシークレットキーを環境変数として keeper コンテナーに注入します。
...
<div id="5-fix-server-pvs-5_fix_server_pvssh">
  ##### 5. Server PV の修正 (5_fix_server_pvs.sh)
</div>

このスクリプトは、Persistent Volume (PV) のトポロジーラベルの不一致が原因で ClickHouse server のポッドが 0/1 ready の状態のままになる問題に対処します。非推奨の failure-domain.beta.kubernetes.io/ ラベルを topology.kubernetes.io/region および topology.kubernetes.io/zone に更新することで、適切にスケジューリングされ、正常に動作するようにします。また、image pull secrets の適用も試みたうえで、正しい構成で再作成されるようサーバーのポッドを削除します。
...
このスクリプトは、ClickHouse server ポッドのStatefulSetにパッチを適用するためのものです。IBM Cloud Container Registry のようなプライベートレジストリからコンテナーイメージを取得できるよう、必要な image pull secret が server の StatefulSet に適用されるようにします。これは重要な設定です。また、エラーハンドリングも含まれており、変更を反映するにはポッドの再起動が必要であることをユーザーに通知します。
...
# 'set -e' により、シークレットが見つからない場合はスクリプトが終了する。
...
# より信頼性の高い jsonpath クエリを使用して名前を直接取得する。
# '|| true' により、StatefulSet が見つからない場合でもスクリプトが失敗しないようにする。
...
for STS_NAME in $STS_LIST; do
  ...
  # StatefulSet にパッチを適用する。ログを見やすくするため、成功時の出力を /dev/null にリダイレクトする。
...
ポッドが起動し正常な状態になったら、ポートフォワーディングで ClickHouse への接続を確認できます。本番環境では、ロードバランサーを使用してください。

```
kubectl port-forward svc/c-$CLUSTER_NAME-server-any 9440:9440 -n ns-$CLUSTER_NAME
```

`clickhouse-client.yml` ファイルで、CA 証明書やその他の TLS 設定を構成できます
...
<div id="missing-all-icr-io-secret-in-clickhouse-operator-system-namespace">
  ##### `clickhouse-operator-system` ネームスペースに `all-icr-io` シークレットが存在しません。
</div>
...
<div id="missing-local-path-storage-crds">
  ##### local path storage CRD が不足しています
</div>

`Install local storage driver` をスキップすると、Keeper のプロビジョニング時に、`describe` で次のようなメッセージが表示されます。
...
<div id="keeper-images-wont-install-due-to-operator-not-populating-needed-sts-configuration">
  ##### オペレーターが必要な STS 設定を反映しないため、Keeper イメージをインストールできない
</div>
...
Keeper を describe すると、以下のように表示されます:
...
4&#95;fix&#95;keeper&#95;sts.sh で STS にパッチを適用します。主なポイントは、`clickhouse-operator-system` と `ns-$CLUSTER_NAME` の各ネームスペースにシークレットを作成することです。
...
* kubectl create secret docker-registry all-icr-io --docker-server=359774249275.dkr.ecr.us-east-2.amazonaws.com --docker-username=AWS --docker-password=$(aws ecr get-login-password) -n ns-$CLUSTER&#95;NAME
  *注: 以前にこれを実行してから時間が経過している場合は、トークンの有効期限が切れている可能性があります。シークレットはその場で更新できないため、削除してから再実行する必要があります。*

<div id="keeper-pods-crashloopbackoffing">
  ##### Keeper ポッドが CrashLoopBackOff に陥る
</div>
...
ポッドのログを確認してください。おそらく、MinIO のバケットとキーが設定されていない可能性があります。
...
<div id="keeper-pods-launch-and-are-runninghealthy-but-server-pods-havent-spawned">
  ##### Keeper ポッドは起動して正常に稼働しているが、サーバーポッドは起動していない
</div>
...
<div id="issue-using-operator-1135381-causes-this-error-rolling-back-to-1135001-works-fine">
  ##### 問題 - operator 1.13538.1 を使用すると、このエラーが発生します。1.13500.1 にロールバックすると問題なく動作します。
</div>
...
<div id="if-server-pods-are-pending-for-a-few-minutes-then-check-the-operator-logs-it-may-be-a-problem-with-your-sc-local-nvme-sc-configuration">
  ##### サーバーのポッドが数分たっても `Pending` のままの場合は、オペレーターのログを確認してください。`sc local-nvme-sc` の設定に問題がある可能性があります。
</div>
...
<div id="debugging-crashloopbackoff-errors">
  ##### CrashLoopBackOff エラーのトラブルシューティング
</div>

ポッドが CrashLoopBackOff 状態になっている場合は、ポッドを describe してエラーメッセージとイベントを確認します。

```bash
kubectl describe pod c-default-xx-01-keeper-0 -n ns-default-xx-01 
```

問題があることを示す describe の出力例:
...
エラーが表示された場合は、問題のあるポッドのログを確認してください。
...
<div id="s3-access-denied-errors">
  ##### S3 Access Denied エラー
</div>

コンテナーのログに S3 関連の Access Denied エラー (例：keeper 用) が表示される場合は、S3 の認証情報をポッドの環境変数に設定する必要があるかもしれません。kubectl edit の代わりに、YAML ファイルを使って kubectl patch を実行できます。
...
次の内容で sts&#95;keeper&#95;patch.yaml という名前のファイルを作成し、YOUR&#95;KEY と YOUR&#95;SECRET は実際の AWS 認証情報に置き換えてください。
    ...
    spec: 
      containers: 
        - name: c-default-xx-01-keeper 
          env: 
          - name: AWS_ACCESS_KEY_ID 
            value: YOUR_KEY # AWS Access Key IDを入力してください 
          - name: AWS_SECRET_ACCESS_KEY 
            value: YOUR_SECRET # AWS Secret Access Keyを入力してください 
```
...
パッチ適用後は、ポッドの状態を確認してください。新しい設定で再作成させるために、問題のあるポッドを削除する必要がある場合があります。

```bash
kubectl delete pod c-default-xx-01-keeper-0 -n ns-default-xx-01 
```

kubectl Secretを別のネームスペースにコピーする (必要な場合) 

kubectl Secret を別のネームスペース (例: `default` から `clickhouse-operator-system`) にコピーする必要がある場合は、次のコマンドを使用してください。通常、これはスクリプトによってすでに処理されているはずです。
...
imagePullSecrets が正しく機能していることを確認するため、ノード上のキャッシュ済みコンテナーイメージを削除してください。そうしないと、そのノードでは最初の実行時にしか確認できません。 

<div id="run-script-remove_cached_imagessh">
  ##### remove_cached_images.sh スクリプトを実行
</div>
...
kubectl get nodes -o name | while read -r node_ref; do 
  ...
  # ステップ1: デバッグコマンドを実行し、すべての出力を変数にキャプチャする。 
  # '2>&1' により、標準出力とエラー出力の両方がキャプチャされる。 
  CMD_OUTPUT=$(kubectl debug $node_ref  \ 
    -n default -it --image=busybox --profile=general \ 
    -- sh -c 'chroot /host crictl rmi --all' 2>&1) 
 
  # ステップ2: "Creating" または "Deleted" で始まる行のみを出力する。 
  # '-E' フラグは '|'（OR）演算子の拡張正規表現を有効にする。 
  # '^' は行頭へのマッチをアンカーする。 
  echo "$CMD_OUTPUT" | grep -E '^Creating|^Deleted' 
 
  # ステップ3: 出力全体を解析して、削除対象のポッド名を取得する。 
