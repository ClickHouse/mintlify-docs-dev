...
<style>
  {`
        ...
        /* ホームページでサイドバーをただちに非表示にする（ちらつき防止） */
        #sidebar { display: none !important; }
        /* サイドバーを非表示にしているため、ナビゲーションバーを全幅にする */
...
<PageWrapper>
  ...
  { label: "高いパフォーマンスを維持しながらコストを抑えるためのヒント", href: "/concepts/best-practices/index" },
          ...
          <div className="flex flex-col gap-4">
            <HeroCard logo="logo-ch.svg" logoInvert title="ClickStack" description="ClickHouse を基盤とするオールインワンのオブザーバビリティスタックで、ログ、メトリクス、トレース、セッションリプレイ、エラーに対して超高速のクエリと強力な集計を実現します。" href="/ja/clickstack/overview" extraClassName="flex-1" showArrow arrowBottom />

            <HeroCard logo="logo-postgres.svg" title="Managed Postgres" description="高速でスケーラブルなエンタープライズ向け Postgres サービス。ClickHouse とネイティブに統合されています。トランザクションには Postgres、分析には ClickHouse を簡単に組み合わせられます。" href="/ja/products/managed-postgres/overview" extraClassName="flex-1" showArrow arrowBottom />

            <HeroCard logo="logo-integration.svg" logoInvert title="インテグレーション" description="100 以上のインテグレーションで、お気に入りのツールやサービスを ClickHouse と接続できます。" href="/ja/integrations/home" extraClassName="flex-1" showArrow arrowBottom />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <HeroCard filename="thumbnail-langfuse-light.png" title="LangFuse" description="オープンソースの LLM エンジニアリングプラットフォーム。オブザーバビリティ、メトリクス、評価、プロンプト管理に対応しています。" href="https://langfuse.com/docs" showArrow external />
      ...
      <div id="mcp-setup" className="mt-16 scroll-mt-20">
        ...
        <div className="mt-6 p-4 rounded-lg bg-[#f9f9f9] dark:bg-white/[0.03] border border-[#e5e7eb] dark:border-white/10">
          <p className="text-sm font-semibold text-gray-900 dark:text-zinc-50 mb-1">コーディングエージェントに ClickHouse スキルを追加</p>
          <p className="text-sm text-gray-500 dark:text-zinc-500 mb-3">ClickHouse スキルをインストールして、エージェントに ClickHouse の専門知識とベストプラクティスを持たせます。</p>
