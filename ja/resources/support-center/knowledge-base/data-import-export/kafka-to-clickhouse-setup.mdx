...
この記事では、KafkaトピックからClickHouseテーブルへデータを送信する手順を説明します。ここでは、Wikimedia の各種サービスで行われた変更を表す[イベントストリーム](https://stream.wikimedia.org/v2/stream/recentchange)を提供する Wiki recent changes feed を使用します。手順は次のとおりです。
