...
* [`postgresql` table function](/ja/reference/functions/table-functions/postgresql) を使用してデータを読み取ります
* `INTO OUTFILE _filename_ FORMAT` 句を使用し、必要な出力フォーマットを指定します

出力フォーマットには、ClickHouse でサポートされている任意の [ouput formats](/ja/reference/formats) を使用できます。いくつか例を見てみましょう...
...
<Note>
  ここで終わりではありません。`clickhouse-local` を使えば、PostgreSQL からデータを取得して、[さまざまな出力フォーマット](/ja/reference/formats) に送信できます。
