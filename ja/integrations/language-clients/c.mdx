...
`clickhouse-c` は、ClickHouse の[ネイティブプロトコル](/ja/interfaces/tcp)向けのヘッダーオンリーCクライアントです。
ソースコードと各ヘッダーのリファレンスは、[GitHubリポジトリ](https://github.com/ClickHouse/clickhouse-c)にあります。

上位レベルのクライアントとは異なり、このライブラリは意図的に多くのことを行いません。コアヘッダーは、ユーザーが提供する I/O コールバックを介して [Native](/ja/interfaces/formats/Native) フォーマットの ブロック をデコードおよび
...
* HTTP プロトコル。[HTTP インターフェイス](/ja/interfaces/http) については、libcurl を直接ラップしてください。
...
[ClickHouse Cloud](/ja/cloud/overview) やその他の TLS 対応デプロイでは、
