...
このステップバイステップガイドでは、[MySQL ClickPipe](/ja/integrations/clickpipes/mysql) を使用して、Amazon Aurora MySQL から ClickHouse Cloud にデータをレプリケートするよう設定する方法を説明します。MySQL CDC (変更データキャプチャ) に関するよくある質問については、[MySQL よくある質問ページ](/ja/integrations/clickpipes/mysql/faq) を参照してください。
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/rds-backups.png" alt="Aurora で自動バックアップを有効にする" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/aurora_config.png" alt="Aurora でパラメータグループを確認する場所" size="lg" border />

<br />

パラメータグループのリンクをクリックすると、その詳細ページに移動します。右上に **Edit** ボタンが表示されるはずです。

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/edit_button.png" alt="パラメータグループの編集" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_format.png" alt="Binlog フォーマットを ROW に設定" size="lg" border />

2. `binlog_row_metadata` を `FULL` に設定します。

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_metadata.png" alt="Binlog 行メタデータ" size="lg" border />

3. `binlog_row_image` を `FULL` に設定します。

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_image.png" alt="Binlog 行イメージ" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/enable_gtid.png" alt="GTID が有効" size="lg" border />
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/security-group-in-rds-mysql.png" alt="Aurora MySQL でセキュリティグループはどこで確認できますか？" size="lg" border />

<Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/rds/edit_inbound_rules.png" alt="上記のセキュリティグループのインバウンドルールを編集" size="lg" border />
...
これで、Amazon Aurora MySQL インスタンスは binlog レプリケーション向けに設定され、ClickHouse Cloud への安全な接続も完了しました。[最初の MySQL ClickPipe を作成](/ja/integrations/clickpipes/mysql#create-your-clickpipe)できます。MySQL CDC (変更データキャプチャ) に関するよくある質問については、[MySQL よくある質問ページ](/ja/integrations/clickpipes/mysql/faq)を参照してください。