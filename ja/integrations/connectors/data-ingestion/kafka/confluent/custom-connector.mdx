...
'Connector type' - Sink
'Sensitive properties' - 'password'. これにより、設定時にClickHouseのパスワードのエントリがマスクされます。
```

例:

<Image img="/images/integrations/data-ingestion/kafka/confluent/AddCustomConnectorPlugin.png" size="md" alt="カスタム ClickHouse コネクタを追加するための設定が表示された Confluent Platform UI" border />
