...
Kafka Connect (フレームワーク) は、コネクタとは独立して、バックグラウンドで Kafka トピックからメッセージをフェッチします。

* **`fetch.min.bytes`**: フレームワークがデータをコネクタに渡す前に必要な最小データ量 (デフォルト: 1 byte)
* **`fetch.max.bytes`**: 1 回のリクエストでフェッチできるデータ量の上限 (デフォルト: 52428800 / 50 MB)
* **`fetch.max.wait.ms`**: `fetch.min.bytes` に達しない場合に、データを返すまでの最大待機時間 (デフォルト: 500 ms)

<Info>
  ***

  Confluent Cloud では、これらの設定を変更するには Confluent Cloud のサポートケースを起票する必要があります。
</Info>
