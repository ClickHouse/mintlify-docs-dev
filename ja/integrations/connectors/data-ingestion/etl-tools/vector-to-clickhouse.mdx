...
このガイドでは、一般的なデータパイプラインである [Vector](https://vector.dev/docs/about/what-is-vector/) を使用して、Nginx のログファイルを tail し、ClickHouse に送信する方法を紹介します。
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/vector_01.png" size="lg" border alt="ClickHouseのログをテーブル形式で表示" />
  </Step>
  ...
  <Step>
    ...
    **materialized view** は、SQLのinsert triggerと同様の機能を持ちます。ソーステーブルにデータの行が挿入されると、materialized viewはそれらの行に変換処理を適用し、結果をターゲットテーブルに挿入します。
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/vector_02.png" size="lg" border alt="解析済みのClickHouseログをテーブル形式で表示" />
