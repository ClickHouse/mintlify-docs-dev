...
your_profile_name:
  ...
  outputs:
    dev:
      ...
      driver: [http] # httpまたはnative。設定しない場合、ポート設定に基づいて自動判定されます
      host: [localhost] 
      port: [8123]  # 設定しない場合、secureおよびdriverの設定に応じて8123、8443、9000、9440のいずれかがデフォルトになります 
      user: [default] # すべてのデータベース操作に使用するユーザー
      password: [<empty string>] # ユーザーのパスワード
      cluster: [<empty string>] # 設定した場合、特定のDDL/テーブル操作がこのクラスターを使用して`ON CLUSTER`句付きで実行されます。分散マテリアライゼーションにはこの設定が必要です。詳細は後述のClickHouse Clusterセクションを参照してください。
      verify: [True] # TLS/SSL使用時にTLS証明書を検証する
      secure: [False] # TLS（ネイティブプロトコル）またはHTTPS（httpプロトコル）を使用する
      ...
      send_receive_timeout: [300] # ClickHouseサーバーからデータを受信する際のタイムアウト（秒）
      cluster_mode: [False] # レプリケートデータベースでの動作を改善するための専用設定を使用する（ClickHouse Cloud推奨）
      use_lw_deletes: [False] # デフォルトのインクリメンタル戦略として`delete+insert`戦略を使用する
      check_exchange: [True] # ClickHouseがアトミックなEXCHANGE TABLESコマンドをサポートしているか検証する（ほとんどのClickHouseバージョンでは不要）
      local_suffix: [_local] # 分散マテリアライゼーション用の分片上のローカルテーブルに付けるテーブル接尾辞
      local_db_prefix: [<empty string>] # 分散マテリアライゼーション用の分片上のローカルテーブルのデータベースプレフィックス。空の場合、分散テーブルと同じデータベースを使用します。
      allow_automatic_deduplication: [False] # レプリケートテーブルに対するClickHouseの自動重複排除を有効にする
      tcp_keepalive: [False] # ネイティブクライアント専用。TCPキープアライブ設定を指定します。カスタムキープアライブ設定は[idle_time_sec, interval_sec, probes]の形式で指定してください。
      custom_settings: [{}] # 接続用のカスタムClickHouse設定の辞書/マッピング。デフォルトは空です。
      database_engine: '' # 新しいClickHouseスキーマ（データベース）を作成する際に使用するデータベースエンジン。設定しない場合（デフォルト）、新しいデータベースはデフォルトのClickHouseデータベースエンジン（通常はAtomic）を使用します。
      threads: [1] # クエリ実行時に使用するスレッド数。1より大きい値に設定する前に、[書き込み後の読み取り整合性](#read-after-write-consistency)セクションを必ずお読みください。
