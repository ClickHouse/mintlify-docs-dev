...
<Steps>
  ...
  <Step>
    ...
    dataset_table_separator = "___"          # dataset から生成される dataset テーブル名の区切り文字。
    ```
    ...
    <Info>
      **HTTP&#95;PORT**
      ...
      外部 staging を使用しない場合 (つまり、パイプラインで staging パラメーターを設定しない場合) は、`http_port` を設定する必要があります。これは、組み込みの ClickHouse ローカルストレージ staging で、HTTP 経由で ClickHouse と通信する <a href="https://github.com/ClickHouse/clickhouse-connect">clickhouse-connect</a> ライブラリが使用されるためです。

      ClickHouse サーバーが、`http_port` で指定したポートで HTTP 接続を受け付けるよう設定されていることを確認してください。たとえば、`http_port = 8443` を設定した場合、ClickHouse は 8443 番ポートで HTTP リクエストを待ち受けている必要があります。外部 staging を使用している場合は、このケースでは clickhouse-connect は使用されないため、`http_port` パラメーターは省略できます。
    </Info>
