...
<div id="integrating-mysql-with-clickhouse">
  # MySQL と ClickHouse の連携
</div>

このページでは、MySQL テーブルからデータを読み取るための `MySQL` テーブルエンジンの使用方法を説明します。

<Note>
  ClickHouse Cloud では、[MySQL ClickPipe](/ja/integrations/clickpipes/mysql) (現在パブリックベータ提供中) を使用して、MySQL テーブルから ClickHouse にデータを簡単に取り込むこともできます。
</Note>
