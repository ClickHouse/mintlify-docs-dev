...
[SELECT](/ja/reference/statements/select) クエリの実行を許可します。
...
次の権限階層に従って、[ALTER](/ja/reference/statements/alter)クエリの実行を許可します。
...
次の権限階層に応じて、[CREATE](/ja/reference/statements/create) および [ATTACH](/ja/reference/statements/attach) DDLクエリの実行を許可します。
...
外部データソースの使用を許可します。[テーブルエンジン](/ja/reference/engines/table-engines)および[テーブル関数](/ja/reference/functions/table-functions)に適用されます。
...
テーブル作成時に、指定したテーブルエンジンを使用できるようにします。[テーブルエンジン](/ja/reference/engines/table-engines) に適用されます。
