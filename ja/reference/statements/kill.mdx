...
現在実行中の[ミューテーション](/ja/reference/statements/alter#mutations)のキャンセルと削除を試みます。キャンセルするミューテーションは、`KILL` クエリの `WHERE` 句で指定したフィルタを使用して [`system.mutations`](/ja/reference/system-tables/mutations) テーブルから選択されます。
