...
`(c1, c2, c3)` を使って、挿入するカラムのリストを指定できます。また、`*` のようなカラム[マッチャー](/ja/reference/statements/select#asterisk)を使った式や、[APPLY](/ja/reference/statements/select/apply_modifier)、[EXCEPT](/ja/reference/statements/select/except_modifier)、[REPLACE](/ja/reference/statements/select/replace_modifier) のような[修飾子](/ja/reference/statements/select#select-modifiers)も使用できます。
...
データは、ClickHouse がサポートする任意の[フォーマット](/ja/reference/formats)で INSERT に渡せます。フォーマットはクエリ内で明示的に指定する必要があります。
...
上記の構文を使用すると、**クライアント**側に保存されているファイル、または複数のファイルからデータを挿入できます。`file_name` と `type` は文字列リテラルです。入力ファイルの[フォーマット](/ja/reference/formats)は、`FORMAT` 句で設定する必要があります。
...
データは、[テーブル関数](/ja/reference/functions/table-functions)で参照されるテーブルに挿入できます。
