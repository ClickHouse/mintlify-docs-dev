...
* [SELECT](/ja/reference/statements/select)、[HAVING](/ja/reference/statements/select/having)、および [ORDER BY](/ja/reference/statements/select/order-by) 句内のすべての式は、キー式に基づいて計算されるか、非キー式 (通常のカラムを含む) に対する [集約関数](/ja/reference/functions/aggregate-functions) に基づいて計算される**必要があります**。言い換えると、テーブルから選択される各カラムは、キー式で使用するか、集約関数の内部で使用するかのいずれかでなければならず、両方で使用することはできません。
