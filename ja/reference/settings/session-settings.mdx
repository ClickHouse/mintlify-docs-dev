...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "HTTP インデックスページを一覧して `url` テーブル関数でワイルドカードを展開できるようにする新しい設定"}]}]} />

HTTP インデックスページからの `url()` および `ENGINE = URL` の実験的なワイルドカード展開を許可します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "新しい安全性チェック: `ALTER TABLE ... REPLACE PARTITION ... FROM ...` は、ソーステーブルの対象パーティションにパーツが存在しない場合、例外をスローするようになりました（[#23727](https:\/\/github.com\/ClickHouse\/ClickHouse\/issues\/23727) の無言のデータ損失を修正）。以前の動作である、宛先パーティションを黙って削除する挙動は、`allow_replace_partition_from_empty_source = 1` を設定することで維持できます。"}]}]} />

ソースのそのパーティションにパーツが存在しない場合でも、`ALTER TABLE ... REPLACE PARTITION ... FROM ...` が宛先パーティションを黙って削除できるようにします。

デフォルトでは、これは許可されていません。対象パーティションにデータを持たないソースからの `REPLACE PARTITION` は例外を発生させます。これは、その場合この操作が実質的に宛先に対する暗黙の `DROP PARTITION` になるためです (宛先のデータは削除されますが、それを置き換えるデータはありません) 。これは、偶発的なデータ損失の一般的な原因です ([#23727](https://github.com/ClickHouse/ClickHouse/issues/23727) を参照) 。

この設定を有効にすると、以前の動作に戻せます。たとえば、意図的に空のソースパーティションを使って宛先のデータを消去したい場合です。無条件に削除したい場合は、代わりに `ALTER TABLE ... DROP PARTITION ...` を使用してください。
...
ClickHouse Cloud でのみ有効です。`system.distributed_cache_registry` テーブルで `certificate` カラムと `signature` カラムを表示します。既定では、出力をコンパクトに保つため、これらのカラムは空になっています。内容を確認するには、この設定を有効にしてください。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "分散クエリ用にリースする stateless worker の数に関する新しい実験的な設定。0 にするとリースは無効になります。"}]}]} />

このクエリの実行に使用する stateless worker の数です。0 にすると、分散プランでの stateless worker のリースは無効になります。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "26.7 以降、`EXPLAIN PLAN` のデフォルトは `actions=1, compact=1, pretty=1` です。26.7 より前の出力に戻すには、`legacy` に設定してください。"}]}]} />

`EXPLAIN PLAN` で使用されるデフォルトのフォーマットです。

設定可能な値:

* `pretty` (26.7 以降のデフォルト) — `actions`、`compact`、`pretty` はデフォルトで `true` となり、compact で Pretty な、actions 注釈付きの plan を生成します。
* `legacy` — 26.7 より前の出力。

`EXPLAIN` ステートメントで `actions`、`compact`、または `pretty` オプションを明示的に指定した場合 (たとえば `EXPLAIN actions = 0, compact = 0, pretty = 0 SELECT ...`) 、その指定が常にこの設定より優先されます。

`json = 1` または `distributed = 1` を指定した `EXPLAIN PLAN` では、`actions`、`compact`、または `pretty` が明示的に設定されていない限り、この設定に関係なく legacy (26.7 より前) のデフォルトが維持されます。Pretty 出力では JSON の結果や分片ごとの distributed plan を表現できないため、これらのモードは legacy 形式でのみ正しくレンダリングされます。
...
この設定を使用すると、MergeTree/ReplicatedMergeTree で独自の 重複排除 の動作を指定できます。
たとえば、各 INSERT ステートメントでこの設定に一意の値を指定することで、
同じデータが 重複排除 によって排除されるのを防げます。

設定可能な値:

* 任意の文字列

`insert_deduplication_token` は、空でない場合に*のみ* 重複排除 に使用されます。

レプリケートテーブルでは、デフォルトで各パーティションごとに直近 100 件の insert のみが 重複排除 の対象になります ([replicated&#95;deduplication&#95;window](/ja/reference/settings/merge-tree-settings#replicated_deduplication_window)、[replicated&#95;deduplication&#95;window&#95;seconds](/ja/reference/settings/merge-tree-settings#replicated_deduplication_window_seconds) を参照) 。
...
<Note>
  `insert_deduplication_token` はパーティションごとに追跡されるため、1 回の insert で書き込まれる複数のパーティションが同じトークンを持つことがあります。トークンがない場合、デフォルトのコンテンツ checksum (`insert_deduplication_version = new_unified_hash`) は挿入された block 全体に対して計算されるため、insert が 重複排除 されるのは、データ全体が以前の insert (再試行) と一致する場合のみであり、単一のパーティションの行がたまたま別の insert と一致した場合は対象になりません。
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "`ORDER BY` が `GROUP BY` のソート記述のプレフィックスである場合に、十分な数のグループが生成された時点で早期終了できるよう、`LIMIT` を順序付き集約にプッシュする新しい設定。"}]}]} />

有効で、かつ順序付き集約が有効になっている場合、十分な数のグループが生成された時点で早期終了できるよう、`LIMIT` を集約ステップにプッシュします。これにより、`ORDER BY` が `GROUP BY` キーのプレフィックスに一致する場合に、読み取るデータ量を削減できます。`rows_before_limit_at_least` で報告される値が少なくなる可能性があります。正確な件数が必要な場合は、`exact_rows_before_limit` を使用してください。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "`optimize_and_compare_chain` 最適化の処理量に上限を設ける新しい設定です（`hash` 化されるクエリツリーのノード数で測定）。これにより、比較の `AND` 連鎖が非常に多い、または非常に大きいクエリの解析で、この最適化が支配的になってしまうのを防ぎます。以前の値 `0`（無制限）を指定すると、26.7 より前の、最適化に上限がなかった動作が再現されるため、`compatibility` をそれ以前のバージョンに設定すると、予算の制約なしで推移的な述語の導出が継続されます。予算を無効にするには `0` に設定してください。"}]}]} />

クエリ解析中に `optimize_and_compare_chain` 最適化で使用する処理予算です。これは `getTreeHash` によってハッシュ化されるクエリツリーノード数で測定され、この最適化における主要なコストとなります。この最適化の適用中に、クエリでハッシュ化されたノード数がこの値を超えると、そのクエリの残りの部分にはこの最適化を適用しなくなります。これにより、比較の `AND` 連鎖が非常に多い、または非常に大きいクエリで、何も畳み込めないままこの最適化が解析時間の大半を占めてしまう状況でも、解析時間を抑えられます。途中で打ち切っても安全です。最適化を見送るだけで、結果が変わることはありません。予算を無効にするには `0` に設定してください (無制限) 。
...
関数 `parseDateTime` のフォーマッタ `%e` は、1 桁の日は空白で埋められていることを想定しています。たとえば、`' 2'` は受け付けられますが、`'2'` はエラーになります。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "ソートキーが保持側のみを参照している場合に、ORDER BY ... LIMIT n を LEFT/RIGHT 結合 を通じてプッシュダウンするクエリプランレベルの最適化を有効にする新しい設定。"}]}]} />

ソートキーが 結合 の保持側 (LEFT/RIGHT) のカラムのみを参照している場合に、`ORDER BY ... LIMIT n` を 結合 の下にプッシュダウンするクエリプランレベルの最適化を切り替えます。これにより、結合 前に保持側の入力が生成する必要のある行数が制限されます。
この設定は、[query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) が 1 の場合にのみ有効です。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "クエリの開始前に、特定のワークロード向けのメモリを予約するための新しい設定。"}]}]} />

Workload scheduling で使用されます。単一サーバー上でクエリを実行するために予約される RAM の最小量です。予約は、`workload` クエリ設定の値を使用して、WORKLOAD 階層を通じて行われます。
ワークロードで使用可能なメモリが不足している場合、クエリは開始されず、予約を確保できるようになるまで pending 状態で待機します。
`0` は予約なしを意味します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "100000"},{"label": "新しい設定。`url` テーブル関数でワイルドカードを展開する際に読み取るディレクトリ数を制限します。"}]}]} />

インデックスページから URL ワイルドカードを展開する際にたどることができるディレクトリの最大数。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "従来の `toTime`（`toTimeWithFixedDate` として引き続き利用可能）の代わりに、新しい `toTime` 関数（値を `Time` データ型に変換する）をデフォルトで使用します。"}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "新しい設定。toTime の従来の関数ロジックを使用できるようにします。これは toTimeWithFixedDate と同様に動作します。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "テキスト索引ヘッダーキャッシュをグローバルに有効化しました。"}]}, {"id": "row-2","items": [{"label": "25.11"},{"label": "0"},{"label": "新しい設定"}]}]} />

デシリアライズされたテキスト索引ヘッダーをメモリにcacheするかどうかを指定します。
...
デシリアライズ済みのテキスト索引 posting list をメモリ内で cache するかどうか。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "テキスト索引トークンキャッシュをグローバルに有効化しました。"}]}, {"id": "row-2","items": [{"label": "26.3"},{"label": "0"},{"label": "新しい設定"}]}]} />

デシリアライズされたテキスト索引トークン情報をメモリにキャッシュするかどうか。
