...
トークンの位置を保存して厳密なフレーズ一致を可能にする、実験的な `positions` 引数付きのテキスト索引を作成できるようにします。
...
[replicated&#95;deduplication&#95;window](#replicated_deduplication_window) 設定を参照) 。これには
`insert_deduplication_version` の粒度 (デフォルトの
`new_unified_hash` では挿入されたブロック全体、legacy バージョンでは作成された各パーツごと) も含まれます。ハッシュサムは
...
[挿入の重複排除](/ja/reference/engines/table-engines/mergetree-family/replication)
では、レプリケートテーブルへの書き込み時に、ClickHouse は重複排除用のハッシュサムを
ClickHouse Keeper に書き込みます。ハッシュサムが保存されるのは、直近の
`replicated_deduplication_window` ブロックのみです。最も古いハッシュサムは
ClickHouse Keeper から削除されます。

デフォルトの `insert_deduplication_version = new_unified_hash` では、ハッシュサムは
挿入されたブロック全体を対象に計算されるため、重複排除されるのは、挿入データ全体が
以前の挿入 (再試行) と一致する場合のみであり、個々のパーツ単位ではありません。従来の `old_separate_hashes` /
`compatible_double_hashes` では、代わりにハッシュサムは作成された各パーツごとに、
フィールド名と型の構成、およびそのパーツのデータ (バイトストリーム) から
計算されます。
...
この設定が適用されるのは、非同期 insert のハッシュを別個の `async_blocks`
ディレクトリに保持する `insert_deduplication_version = old_separate_hashes` または
`compatible_double_hashes` の場合のみです。デフォルトの `new_unified_hash` では、非同期 insert は
同期 insert と共通の `deduplication_hashes` ディレクトリを使用し、代わりに
`replicated_deduplication_window` / `replicated_deduplication_window_seconds` が
適用されます。
...
`replicated_deduplication_window_for_async_inserts` と同様に、これは
`insert_deduplication_version = old_separate_hashes` / `compatible_double_hashes` の場合にのみ適用されます。デフォルトの
`new_unified_hash` では、非同期 INSERT は `replicated_deduplication_window_seconds` を使用します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "既定で協調マージを有効にする"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "0"},{"label": "新しい設定"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "デフォルトで協調マージを有効にする"}]}, {"id": "row-2","items": [{"label": "25.3"},{"label": "0"},{"label": "新しい設定"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "バックグラウンドクリーンアップで非アクティブなレプリカをどのくらいの期間考慮するかを制御する新しい設定（0 は ZooKeeper のセッションタイムアウト 2 回分を意味します）"}]}]} />

バックグラウンドクリーンアップ
(`OutdatedPartsQuorumThread`) が非アクティブなレプリカをどのくらいの期間考慮し続けるかを指定します。
レプリカの非アクティブ期間がこの秒数未満である間は、
そのレプリカが古いパーツの削除と完了済みミューテーションの切り捨てを引き続き妨げます。値を 0 にすると、
デフォルト値である ZooKeeper のセッションタイムアウト 2 回分が使用されます。
ClickHouse Cloud でのみ利用できます
...
レプリカ間でマージ割り当てを分散するために使用される、ウィンドウサイズ (block番号単位) です。
同じウィンドウ内のパーツは、同じレプリカに割り当てられます。
ClickHouse Cloud でのみ利用できます
...
true に設定すると、テーブルは読み取り専用モードになり、ディスク上では一切変更が行われません。

テーブルを変更するすべてのフォアグラウンド操作は拒否されます。具体的には、insert、ミューテーション、`OPTIMIZE`、およびデータを変更するパーティションコマンド
(`ATTACH`/`MOVE`/`DROP`/`DROP DETACHED`/`FETCH`/`REPLACE PARTITION`、ならびにこのテーブルを対象とする `MOVE PARTITION ... TO TABLE`) です。`FREEZE`/`UNFREEZE` や `FORGET PARTITION` のような、
テーブルのデータを変更しない操作は引き続き許可されます。

バックグラウンド処理も一切スケジュールされません。通常の マージ、TTL マージ (`DELETE`/`MOVE`/再圧縮) 、再圧縮 マージ、バックグラウンド ミューテーション、
およびバックグラウンドでの パーツ の move はすべて抑止されます。その結果、有効期限 (TTL) を持つテーブルでは、この設定が
有効になっている間、期限切れデータの削除や移動は行われなくなります。
