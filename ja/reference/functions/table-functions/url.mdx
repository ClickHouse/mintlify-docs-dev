...
`url` 関数は、指定した `フォーマット` と `structure` で `URL` からテーブルを作成します。

`url` 関数は、[URL](/ja/reference/engines/table-engines/special/url) テーブルのデータに対する `SELECT` および `INSERT` クエリで使用できます。
