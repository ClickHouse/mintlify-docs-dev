---
description: 's3 テーブル関数と同様に、ファイルに対して SELECT や INSERT を行うためのテーブル形式インターフェイスを提供するテーブルエンジンです。ローカルファイルを扱う場合は `file()` を、S3、GCS、MinIO などのオブジェクトストレージ上のバケットを扱う場合は `s3()` を使用します。'
sidebarTitle: 'file'
