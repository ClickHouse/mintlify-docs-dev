...
2. [`user_defined_executable_function_drivers_config`](/ja/reference/settings/server-settings/settings#user_defined_executable_function_drivers_config) に 1 つ以上の ドライバー configuration file を指定します (glob を使用可能) 。必要に応じて、生成された実行可能 UDF の configuration が保存されるディレクトリである [`dynamic_user_defined_executable_functions_path`](/ja/reference/settings/server-settings/settings#dynamic_user_defined_executable_functions_path) も設定します。
