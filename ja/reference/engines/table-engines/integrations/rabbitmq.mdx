...
* `rabbitmq_format` – メッセージのフォーマット。`JSONEachRow` など、SQL `FORMAT` 関数と同じ記法を使用します。詳細は、[フォーマット](/ja/reference/formats) セクションを参照してください。
...
RabbitMQエンジンは、ClickHouseでサポートされているすべての[フォーマット](/ja/reference/formats)に対応しています。
