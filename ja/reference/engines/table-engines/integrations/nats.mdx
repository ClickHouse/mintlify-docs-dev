...
* `nats_format` – メッセージのフォーマット。`JSONEachRow` など、SQL の `FORMAT` 関数と同じ記法を使用します。詳細は [フォーマット](/ja/reference/formats) セクションを参照してください。
...
NATS エンジンは、ClickHouse でサポートされているすべての[フォーマット](/ja/reference/formats)に対応しています。
