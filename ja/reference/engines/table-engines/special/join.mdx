...
`Join` エンジンでは、`CREATE TABLE` ステートメントで [join&#95;use&#95;nulls](/ja/reference/settings/session-settings#join_use_nulls) 設定を指定できます。[SELECT](/ja/reference/statements/select) クエリでも、`join_use_nulls` の値を同じにする必要があります。
