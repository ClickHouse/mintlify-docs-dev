...
* `Format` には、ClickHouse が `SELECT` クエリで使用でき、必要に応じて `INSERT` でも使用できるものを指定する必要があります。対応フォーマットの一覧については、[Formats](/ja/reference/formats#formats-overview) を参照してください。
...
圧縮を有効にするには、まず `URL` パラメーターで指定したリモート HTTP エンドポイントが対応する圧縮アルゴリズムをサポートしていることを確認してください。
