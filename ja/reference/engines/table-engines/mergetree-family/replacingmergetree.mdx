...
このエンジンは、同じ[ソートキー](/ja/reference/engines/table-engines/mergetree-family/mergetree)の値 (`PRIMARY KEY` ではなく、テーブル定義の `ORDER BY` セクション) を持つ重複エントリを削除する点で、[MergeTree](/ja/reference/engines/table-engines/mergetree-family/versionedcollapsingmergetree)と異なります。
