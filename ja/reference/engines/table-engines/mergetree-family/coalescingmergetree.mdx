...
このエンジンは [MergeTree](/ja/reference/engines/table-engines/mergetree-family/mergetree) を継承しています。主な違いは、データパーツのマージ方法にあります。`CoalescingMergeTree` テーブルでは、ClickHouse は同じ主キー (より正確には同じ[ソートキー](/ja/reference/engines/table-engines/mergetree-family/mergetree)) を持つすべての行を、各カラムの最新の非 NULL 値を含む 1 行に置き換えます。
