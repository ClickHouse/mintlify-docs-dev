...
関数 [mapContains](/ja/reference/functions/regular-functions/tuple-map-functions#mapcontains) (`mapContainsKey` のエイリアス) は、マップのキー内の単一のトークンにマッチします。
...
アクセス演算子[operator[]](/ja/reference/operators#access-operators)は、テキスト索引と組み合わせて使用し、キーと値で絞り込むことができます。

例:

```sql
SELECT count() FROM tab WHERE map['engine'] = 'clickhouse'; -- テキスト索引が定義されている場合に使用される
```
