...
バックアップの宛先には、`Disk`、`S3`、`File` など、有効な任意のバックアップ[destination](/ja/concepts/features/backup-restore/local-disk#configure-backup-destinations-for-disk)を指定できます。

`Disk` をバックアップの宛先として使用する場合、バックアップからデータベースを作成するクエリは次のようになります。
