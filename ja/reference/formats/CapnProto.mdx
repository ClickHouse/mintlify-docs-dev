...
関連項目: [フォーマットスキーマ](/ja/reference/formats#formatschema)。

<div id="data_types-matching-capnproto">
  ## データ型の対応
</div>

以下の表は、サポートされているデータ型と、`INSERT` および `SELECT` クエリにおいてそれらが ClickHouse の[データ型](/ja/reference/data-types)にどのように対応するかを示しています。
...
設定 [`format_capn_proto_use_autogenerated_schema`](/ja/reference/settings/formats#format_capn_proto_use_autogenerated_schema) はデフォルトで有効になっており、[`format_schema`](/ja/reference/formats#formatschema) が設定されていない場合に適用されます。
