---
alias: []
description: 'JSONStringsEachRowフォーマットに関するドキュメント'
input_format: false
...
[`JSONEachRow`](/ja/reference/formats/JSON/JSONEachRow) との違いは、データフィールドが型付きの JSON 値ではなく、文字列として出力される点だけです。
...
`football.json` という名前の、次のデータを含むJSONファイルを使用します。
...
`JSONStringsEachRow`フォーマットを使用してデータを読み込みます。
