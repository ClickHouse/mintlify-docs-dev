...
ネストされたメッセージは、[ネストされたデータ構造](/ja/reference/data-types/nested-data-structures) への入力や、そこからの出力に適しています。
...
ClickHouse は protobuf メッセージを `length-delimited` フォーマットで入出力します。
