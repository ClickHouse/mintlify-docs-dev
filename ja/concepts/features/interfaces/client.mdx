...
| `--hints <bool>`                  | カーソルが入力の末尾にあるとき、最もよく一致する候補について、入力中にオートコンプリートのヒント (インラインの「ゴースト」テキスト) を表示します。ヒントの移動には Up/Down (または Ctrl-Up/Ctrl-Down) を使用し、インラインヒントの確定には Tab または Right を使用します。`Enter` は、ヒントが明示的に選択されている場合にのみそのヒントを確定し、それ以外の場合はクエリを実行します。`Tab` では従来の補完リストも開きます。`--highlight` (ヒントの表示には色が必要) と候補提示の仕組みが必要なため、`--disable_suggestion` を指定するとこれも無効になります。 | `true`                                   |
