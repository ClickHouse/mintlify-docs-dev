...
* `--proto-caps` — データ転送でのチャンク化の有効/無効を切り替えます。指定可能な値 (カンマ区切りで複数指定可) : `chunked_optional`, `notchunked`, `notchunked_optional`, `send_chunked`, `send_chunked_optional`, `send_notchunked`, `send_notchunked_optional`, `recv_chunked`, `recv_chunked_optional`, `recv_notchunked`, `recv_notchunked_optional`。デフォルト値: `notchunked`。
* `--help` — ヘルプメッセージを表示します。
