...
このドキュメントファイルでは、必須の位置引数はすべて `<parameter>`、名前付き引数は `[--parameter value]` と表記します。すべての位置パラメーターは、対応する名前の名前付きパラメーターとして指定することもできます。
...
* `copy (cp) [--disk-from disk_1] [--disk-to disk_2] <path-from> <path-to>`.
  ...
  ディスク `disk_2` 上の `path-to` にコピーします (どちらもデフォルト値は現在のディスクで、非対話型モードではパラメーター `disk` になります) 。
