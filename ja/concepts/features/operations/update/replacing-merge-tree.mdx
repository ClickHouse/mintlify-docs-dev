...
<Image img="/images/migrations/postgres-replacingmergetree.png" size="md" alt="ReplacingMergeTree のプロセス" />
...
マージ時に ReplacingMergeTree は、`ORDER BY` カラムの値を一意の識別子として使って重複した行を特定し、最も大きいバージョンだけを残すか、最新バージョンが削除を示している場合は重複した行をすべて削除します。ただし、これで保証されるのは最終的な整合性だけであり、行が必ず重複排除されるわけではないため、これに依存すべきではありません。したがって、クエリでは更新行や削除行も対象に含まれるため、誤った結果が返される可能性があります。

正しい結果を得るには、background merges をクエリ時の重複排除と削除行の除去で補う必要があります。これは `FINAL` 演算子を使うことで実現できます。
