...
<!-- グローバルレポートデザイナー用。 -->
<web_global>
    <!-- keyed – quota_key の「key」はクエリパラメータとして渡され、
            クォータはキーの値ごとに個別に追跡されます。
        たとえば、ユーザー名をキーとして渡すことで、
            ユーザー名ごとにクォータが個別にカウントされます。
        キーを使用する意味があるのは、quota_key がユーザーではなく
            プログラムによって送信される場合のみです。

        <keyed_by_ip /> と記述することもでき、その場合は IP アドレスがクォータキーとして使用されます。
        （ただし、ユーザーは IPv6 アドレスを比較的容易に変更できる点に注意してください。）
    -->
    <keyed />
```

DDL構文を使用する場合は、正規化されたクエリハッシュをキーとしてQUOTAを設定することもできます。これにより、異なる各クエリパターンごとに独立したQUOTAバケットが割り当てられます。

```sql
CREATE QUOTA my_quota KEYED BY normalized_query_hash FOR INTERVAL 1 hour MAX queries = 100 TO my_user;
```

この例では、ユーザーは1時間あたり、個々の正規化済みクエリごとに最大100回まで実行できます。`SELECT number FROM numbers(1)` と `SELECT number FROM numbers(2)` は同じバケットを共有します (正規化形式が同じであるため) が、`SELECT number, number FROM numbers(1)` は別のバケットを使用します。
