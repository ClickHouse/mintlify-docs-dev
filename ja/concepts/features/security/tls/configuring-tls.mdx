...
<Steps>
  ...
  <Step>
    ...
    3. ClickHouse Keeper の正常性を確認する
       一般的な [4 letter word (4lW)](/ja/guides/oss/deployment-and-scaling/keeper#four-letter-word-commands) コマンドは、TLS なしで `echo` を使っても動作しません。以下では、`openssl` を使ってこれらのコマンドを実行する方法を示します。
    ...
    * OpenSSL セッションで 4LW コマンドを送信する
    ...
    5. `https://chnode1.marsnet.local:8443/play` の `https` インターフェイス経由で Play UI にログインします。
       ...
       <Image img="/images/guides/sre/configuring-ssl_01.png" alt="TLS の設定" size="md" border />
