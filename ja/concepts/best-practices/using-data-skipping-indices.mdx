...
次のアニメーションは、例のテーブルに対して minmax スキッピング索引がどのように構築されるかを示したもので、テーブル内の各行ブロック (granule) ごとに `ViewCount` の最小値と最大値を追跡しています。

<Image img="/images/bestpractices/building_skipping_indices.gif" size="lg" alt="スキッピング索引の構築" />
...
<Image img="/images/bestpractices/using_skipping_indices.gif" size="lg" alt="スキッピング索引の使用" />
