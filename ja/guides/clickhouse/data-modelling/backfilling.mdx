...
履歴データはオブジェクトストレージからバックフィルすることを推奨します。可能であれば、最適な読み取り性能と圧縮 (ネットワーク転送量の削減) のため、データはParquetにエクスポートしてください。通常は約150MBのファイルサイズが推奨されますが、ClickHouseは[70を超えるファイルフォーマット](/ja/reference/formats)をサポートしており、あらゆるサイズのファイルを処理できます。
...
<Image img="/images/data-modeling/null_table_mv.png" size="md" alt="ClickHouse における非正規化" />
