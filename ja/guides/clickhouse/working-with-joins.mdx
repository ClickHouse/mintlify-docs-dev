...
<Image img="/images/starter_guides/joins/imdb_schema.png" alt="IMDB スキーマ" />
...
<Image img="/images/starter_guides/joins/cross_join.png" alt="クロス結合" />
...
<Image img="/images/starter_guides/joins/semi_join.png" alt="セミ結合" />
...
`LEFT ANY JOIN` は `LEFT OUTER JOIN` と `LEFT SEMI JOIN` を組み合わせたもので、ClickHouse は左側のテーブルの各行に対して、右側のテーブルに一致する行があればその行のカラム値を結合して返し、一致する行がなければ右側のテーブルのデフォルトのカラム値を結合して返します。
左側のテーブルのある行に対して右側のテーブルに複数の一致がある場合、ClickHouse は最初に見つかった一致との結合結果のカラム値だけを返します (デカルト積は無効化されています) 。
...
次の例では、[values](https://github.com/ClickHouse/ClickHouse/blob/23.2/src/TableFunctions/TableFunctionValues.h) [table function](/ja/reference/functions/table-functions) を使って構築した 2 つの一時テーブル (`left_table` と `right_table`) による抽象的な例で、`LEFT ANY JOIN` を示します。
...
このガイドでは、ClickHouse が標準的な SQL JOIN の型をすべてサポートしているほか、分析クエリに対応するための特殊な JOIN もサポートしていることを説明します。
