...
Polarisカタログに接続するには、次のものが必要です。

* Snowflake Open Catalog (ホスト型の Polaris) またはセルフホストの Polaris Catalog
* Polarisカタログの URI (例: `https://<account-id>.<region>.aws.snowflakecomputing.com/polaris/api/catalog/v1` または `http://polaris:8181/api/catalog/v1/oauth/tokens`) 
...
<Steps>
  <Step>
    ...
    ClickHouse を Polarisカタログに接続するデータベースを作成します。
  ...
  <Step>
    ## ClickHouse を使って Polarisカタログをクエリする
