...
<Info>
  **Server、local、または chDB**
  ...
  このガイドの手順は、既存の ClickHouse server インストールを使って実行できます。アドホックなクエリでは、代わりに [clickhouse-local](/ja/concepts/features/tools-and-utilities/clickhouse-local) を使用して、server を起動せずに同じワークフローを完了することもできます。少し調整すれば、ClickHouse のインプロセス版である [chDB](/ja/products/chdb/index) を使って同じ手順を実行することも可能です。
</Info>
