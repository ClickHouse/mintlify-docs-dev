...
sidebarTitle: 'Claude Desktop と連携する'
title: 'Claude Desktop で ClickHouse MCPサーバーを設定する'
description: 'このガイドでは、Claude Desktop で ClickHouse MCPサーバーを設定する方法を説明します。'
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/use-cases/AI_ML/MCP/claude-desktop-config.png" alt="Claude Desktopの設定" size="md" />
  ...
  <Step>
    ## ClickHouse MCPサーバーを使う

    Claude Desktop を再起動したら、`Search and tools` アイコンをクリックして ClickHouse MCPサーバーを見つけられます。

    <Image img="/images/use-cases/AI_ML/MCP/find-mcp-servers.gif" alt="MCPサーバーを見つける" size="md" />
    ...
    <Image img="/images/use-cases/AI_ML/MCP/mcp-permission.png" alt="list_databasesツールの使用を許可する" size="md" />

    以下では、ClickHouse MCPサーバーへのいくつかのツール呼び出しを含む会話の一部を確認できます。

    <Image img="/images/use-cases/AI_ML/MCP/claude-conversation.png" alt="Claudeとの会話" size="md" />
  </Step>
</Steps>