...
<Image img="/images/deployment-guides/replication-sharding-examples/both.png" size="md" alt="2 つの分片と 1 つのレプリカのアーキテクチャ図" />
  ...
  <Step>
    ...
    <remote_servers>
      ...
      <cluster_2S_2R>
          ...
          <shard>
              <!-- 省略可能。データをレプリカの1つだけに書き込むかどうか。デフォルト: false（すべてのレプリカにデータを書き込む）。 -->
              <internal_replication>true</internal_replication>
              <replica>
                  <host>clickhouse-01</host>
                  <port>9000</port>
...
2 つの分片と 2 つのレプリカから成るこのクラスター構成の利点は、スケーラビリティと耐障害性の両方を備えていることです。
データは個別のホストに分散されるため、各ノードで必要なストレージ容量と I/O を抑えられる一方、クエリは両方の分片にまたがって並列に処理されるため、パフォーマンスとメモリ効率が向上します。
特に重要なのは、各分片について別のノードに予備のレプリカを用意できるため、1 台のノードが失われてもクラスターは停止することなくクエリ処理を継続できる点です。

このクラスター構成の主な欠点は、ストレージのオーバーヘッドが増えることです。各分片が複製されるため、レプリカのない構成に比べて 2 倍のストレージ容量が必要になります。
さらに、クラスターは単一ノードの障害には耐えられるものの、どのノードで障害が発生するかや分片の配置によっては、2 台のノードを同時に失うとクラスターが動作不能になる可能性があります。
この構成は可用性とコストのバランスが取れており、より高いレプリケーション係数に伴うコストをかけずに、一定水準の耐障害性が求められる本番環境に適しています。

スケーラビリティと耐障害性の両方を実現する ClickHouse Cloud のクエリ処理については、[&quot;並列レプリカ&quot;](/ja/products/cloud/features/infrastructure/parallel-replicas) のセクションを参照してください。