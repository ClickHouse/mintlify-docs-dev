...
<div id="2-create-a-table-backed-by-s3">
  ## 2. S3 をバックエンドとするテーブルを作成する
</div>

S3 ディスクが適切に設定されていることを確認するため、テーブルを作成してクエリを実行してみます。
...
エンジンとして `S3BackedMergeTree` を指定する必要がなかった点に注意してください。ClickHouse は、テーブルがストレージに S3 を使用していることを検出すると、内部的にエンジンの種類を自動的に変換します。

テーブルが正しいポリシーで作成されていることを確認します。
...
AWS コンソールで、データの S3 への書き込みが正常に完了していれば、指定したバケット内に ClickHouse が新しいファイルを作成していることを確認できるはずです。

すべて正常に動作していれば、これでストレージとコンピュートを分離した ClickHouse を利用できるようになります！

<Image img="/images/guides/s3_bucket_example.png" size="md" alt="コンピュートとストレージの分離を使用した S3 バケットの例" border />

<div id="3-implementing-replication-for-fault-tolerance-optional">
  ## 3. 耐障害性のためのレプリケーションの実装 (任意)
</div>

<Warning>
  AWS/GCS のライフサイクルポリシーは設定しないでください。これはサポートされておらず、テーブルの破損を引き起こす可能性があります。
</Warning>

耐障害性を高めるには、複数の AWS リージョンに分散した複数の ClickHouse server ノードと、各ノードに対応する S3 バケットを使用できます。

S3 ディスクでのレプリケーションは、`ReplicatedMergeTree` テーブルエンジンを使用することで実現できます。詳細については、次のガイドを参照してください。
