...
ClickHouse は、[ClickHouse Keeper](/ja/guides/oss/deployment-and-scaling/keeper) へのリクエスト (ZooKeeper 互換の協調サービス) に対する OpenTelemetry トレーシングをサポートしています。この機能により、クライアントからのリクエスト送信からサーバー側での処理まで、Keeper 操作のライフサイクルを詳細に可視化できます。
