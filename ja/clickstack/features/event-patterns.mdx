...
<Image img="/images/use-cases/observability/event_patterns.png" alt="イベントパターン" size="lg" />
...
<Image img="/images/use-cases/observability/event_patterns_highlight.png" alt="イベントパターン" size="lg" />
...
イベントパターンは、**絞り込んだデータの一部**に適用したときに最も効果を発揮します。たとえば、イベントパターンを有効にする前に 1 つのサービスに絞り込むと、通常は数千ものサービス全体に一度にパターンを適用するよりも、関連性が高く興味深いメッセージを見つけやすくなります。

また、エラーメッセージの要約にも特に有効で、ID やペイロードが異なる繰り返し発生するエラーを、簡潔なクラスターとしてグループ化できます。

実際の例については、[Remote デモデータセット](/ja/clickstack/example-datasets/remote-demo-data#identify-error-patterns)でイベントパターンがどのように使われているかを参照してください。