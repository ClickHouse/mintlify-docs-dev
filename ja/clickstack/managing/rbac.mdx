...
<Tabs>
  <Tab title="Cloud Users and roles">
    <Image img="/images/clickstack/rbac/team-page-cloud.png" alt="ClickHouse Cloud Users and roles ページ" size="lg" />
  </Tab>

  <Tab title="ClickStack Team Settings">
    <Image img="/images/clickstack/rbac/team-page-clickstack.png" alt="チームメンバーとそのロールが表示された ClickStack Team Settings ページ" size="lg" />
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/rbac/add-role-modal.png" alt="Add Role モーダル" size="md" />
  </Step>
</Steps>
...
<Image img="/images/clickstack/rbac/default-vs-fine-grained.png" alt="ロールエディタでのデフォルトアクセスと詳細な制御のモード" size="md" />
...
<Image img="/images/clickstack/rbac/condition-tip.png" alt="条件のツールチップ: Name または Tag（タイトルで確認）や ID（URL で確認）でリソースを照合" size="md" />
...
<Image img="/images/clickstack/rbac/dashboard-id-and-tag-example.png" alt="URL バーにリソース ID が表示され、右上にタグがあるダッシュボード" size="lg" />

リソースタイプごとに複数のルールを追加できます。各ルールは OR ロジックで個別に評価されるため、**いずれか** のルールに一致すれば、そのリソースにアクセスできます。どのルールにも一致しないリソースにはアクセスできません。

<Image img="/images/clickstack/rbac/access-rules-tip.png" alt="OR ロジックを使用したアクセスルールのツールチップ" size="md" />
...
<Image img="/images/clickstack/rbac/dashboard-fine-grained-example.png" alt="OR で結合された 2 つのきめ細かなアクセスルール: Name contains testing で Read アクセス、Tag is testing で Read アクセス" size="md" />
...
<Image img="/images/clickstack/rbac/security-policies.png" alt="セキュリティポリシー" size="lg" />