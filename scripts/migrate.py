...
Usage:
    python scripts/migrate.py <path>                 # one file or directory
    python scripts/migrate.py --all                  # every page in this repo
    python scripts/migrate.py <path> --dry-run       # show what would change
...
def _override_java_client_versions(text: str) -> str:
    ...
    with Mintlify <View view="..."> blocks, one per version.

    The `versions={[ {version: 'v0.8+'}, {version: 'v0.7.x'} ]}` array on the
    opening tag drives the `view=` attribute on each <View>, in order.
    """
    ...
    # Replace each <Version>/</Version> in order with <View view="vX">/</View>.
    idx = [0]
    def open_repl(_):
        v = versions[idx[0]] if idx[0] < len(versions) else f"v{idx[0]}"
        idx[0] += 1
        return f'<View view="{v}">'
