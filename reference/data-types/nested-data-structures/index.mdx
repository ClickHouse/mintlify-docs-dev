...
When the [`flatten_nested`](/reference/settings/session-settings#flatten_nested) setting is set to `0` (`flatten_nested=1` by default) arbitrary levels of nesting are supported.
...
### Filtering Nested columns in WHERE <a id="filtering-nested-columns-in-where"></a>

Because each column of a `Nested` structure is stored as an `Array`, referencing it in a `WHERE` clause gives you the whole array for every row, not an individual element. You cannot compare a nested column directly to a scalar value so you must use [array functions](/reference/functions/regular-functions/array-functions) instead.
...
Use [`has`](/reference/functions/regular-functions/array-functions#has) to check whether an array contains a specific value:
...
Use [`arrayExists`](/reference/functions/regular-functions/array-functions#arrayExists) when the condition is more complex:
...
For more information, see [`ARRAY JOIN` clause](/reference/statements/select/array-join). Example:
...
### ALTER limitations <a id="alter-limitations"></a>
