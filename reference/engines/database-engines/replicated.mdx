...
If `zoo_path` contains macro `{uuid}`, it is required to specify explicit UUID or add [ON CLUSTER](/snippets/distributed-ddl) to create statement to ensure all replicas use the same UUID for this database.
...
DDL queries with `Replicated` database work in a similar way to [ON CLUSTER](/snippets/distributed-ddl) queries, but with minor differences.
