...
<Note>
This is an experimental feature that may change in backward-incompatible ways in future releases. Enable it with the [`allow_experimental_executable_udf_drivers`](/reference/settings/server-settings/settings#allow_experimental_executable_udf_drivers) server setting.
</Note>
...
2. Point [`user_defined_executable_function_drivers_config`](/reference/settings/server-settings/settings#user_defined_executable_function_drivers_config) at one or more driver configuration files (a glob is supported), and optionally set [`dynamic_user_defined_executable_functions_path`](/reference/settings/server-settings/settings#dynamic_user_defined_executable_functions_path), the directory where the generated executable UDF configurations are stored:
