...
old-slug: /sql-reference/aggregate-functions/reference/quantilesExactInclusive
...
- `level` — Levels of quantiles. Constant floating-point numbers from 0 to 1 (inclusive). We recommend using `level` values in the range of `[0.01, 0.99]`. [`Float*`](/sql-reference/data-types/float)
