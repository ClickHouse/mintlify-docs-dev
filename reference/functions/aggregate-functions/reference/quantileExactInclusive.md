...
old-slug: /sql-reference/aggregate-functions/reference/quantileExactInclusive
...
- `level` — Level of quantile. Constant floating-point number from 0 to 1 (inclusive). We recommend using a `level` value in the range of `[0.01, 0.99]`. [`Float*`](/sql-reference/data-types/float)
