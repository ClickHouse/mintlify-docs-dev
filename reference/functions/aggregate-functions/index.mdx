...
There is an exception to this rule, which are the functions [`first_value`](/reference/functions/aggregate-functions/first_value), [`last_value`](/reference/functions/aggregate-functions/last_value) and their aliases (`any` and `anyLast` respectively) when followed by the modifier `RESPECT NULLS`. For example, `FIRST_VALUE(b) RESPECT NULLS`.
