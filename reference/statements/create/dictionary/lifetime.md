...
If a dictionary update is successful, the old version of the dictionary is replaced [atomically](/concepts/glossary#atomicity).
...
When updating the dictionaries, the ClickHouse server applies different logic depending on the type of [source](./sources/):
