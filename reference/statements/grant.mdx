...
## Granting Privilege Syntax <a id="granting-privilege-syntax"></a>
...
## Assigning Role Syntax <a id="assigning-role-syntax"></a>
...
## Grant Current Grants Syntax <a id="grant-current-grants-syntax"></a>
```sql
...
## Wildcard grants <a id="wildcard-grants"></a>
...
Allows executing [SELECT](/reference/statements/select/index) queries.
...
Allows executing [ALTER](/reference/statements/alter/index) queries according to the following hierarchy of privileges:
...
Allows executing [CREATE](/reference/statements/create/index) and [ATTACH](/reference/statements/attach) DDL-queries according to the following hierarchy of privileges:
...
### ACCESS MANAGEMENT <a id="access-management"></a>
...
### Source Filter Grants <a id="source-filter-grants"></a>
...
### displaySecretsInShowAndSelect <a id="displaysecretsinshowandselect"></a>
...
### NAMED COLLECTION ADMIN <a id="named-collection-admin"></a>
...
### TABLE ENGINE <a id="table-engine"></a>

Allows using a specified table engine when creating a table. Applies to [table engines](/reference/engines/table-engines/index).
...
<Note>
The privilege `ALL` is not supported in ClickHouse Cloud, where the `default` user has limited permissions. Users can grant the maximum permissions to a user by granting the `default_role`. See [here](/products/cloud/guides/security/cloud-access-management/manage-cloud-users) for further details.
...
### ADMIN OPTION <a id="admin-option"></a>
