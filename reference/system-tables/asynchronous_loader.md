...
<!--AUTOGENERATED_START-->
- `job` ([String](../../sql-reference/data-types/)) — Job name (may be not unique).
- `job_id` ([UInt64](../../sql-reference/data-types/)) — Unique ID of the job.
- `dependencies` ([Array(UInt64)](../../sql-reference/data-types/)) — List of IDs of jobs that should be done before this job.
- `dependencies_left` ([UInt64](../../sql-reference/data-types/)) — Current number of dependencies left to be done.
- `status` ([Enum8('PENDING' = 0, 'OK' = 1, 'FAILED' = 2, 'CANCELED' = 3)](../../sql-reference/data-types/)) — Current load status of a job: PENDING: Load job is not started yet. OK: Load job executed and was successful. FAILED: Load job executed and failed. CANCELED: Load job is not going to be executed due to removal or dependency failure.
- `is_executing` ([UInt8](../../sql-reference/data-types/)) — The job is currently being executed by a worker.
- `is_blocked` ([UInt8](../../sql-reference/data-types/)) — The job waits for its dependencies to be done.
- `is_ready` ([UInt8](../../sql-reference/data-types/)) — The job is ready to be executed and waits for a worker.
- `elapsed` ([Float64](../../sql-reference/data-types/)) — Seconds elapsed since start of execution. Zero if job is not started. Total execution time if job finished.
- `pool_id` ([UInt64](../../sql-reference/data-types/)) — ID of a pool currently assigned to the job.
- `pool` ([String](../../sql-reference/data-types/)) — Name of `pool_id` pool.
- `priority` ([Int64](../../sql-reference/data-types/)) — Priority of `pool_id` pool.
- `execution_pool_id` ([UInt64](../../sql-reference/data-types/)) — ID of a pool the job is executed in. Equals initially assigned pool before execution starts.
- `execution_pool` ([String](../../sql-reference/data-types/)) — Name of `execution_pool_id` pool.
- `execution_priority` ([Int64](../../sql-reference/data-types/)) — Priority of execution_pool_id pool.
- `ready_seqno` ([Nullable(UInt64)](../../sql-reference/data-types/)) — Not null for ready jobs. Worker pulls the next job to be executed from a ready queue of its pool. If there are multiple ready jobs, then job with the lowest value of `ready_seqno` is picked.
- `waiters` ([UInt64](../../sql-reference/data-types/)) — The number of threads waiting on this job.
- `exception` ([Nullable(String)](../../sql-reference/data-types/)) — Not null for failed and canceled jobs. Holds error message raised during query execution or error leading to cancelling of this job along with dependency failure chain of job names.
- `schedule_time` ([DateTime64(6)](../../sql-reference/data-types/)) — Time when job was created and scheduled to be executed (usually with all its dependencies).
- `enqueue_time` ([Nullable(DateTime64(6))](../../sql-reference/data-types/)) — Time when job became ready and was enqueued into a ready queue of it's pool. Null if the job is not ready yet.
- `start_time` ([Nullable(DateTime64(6))](../../sql-reference/data-types/)) — Time when worker dequeues the job from ready queue and start its execution. Null if the job is not started yet.
- `finish_time` ([Nullable(DateTime64(6))](../../sql-reference/data-types/)) — Time when job execution is finished. Null if the job is not finished yet.
<!--AUTOGENERATED_END-->
