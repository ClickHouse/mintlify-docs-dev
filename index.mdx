...
export const QuickstartLink = () => {
    const assetBase = typeof window !== 'undefined' && window.location.pathname.startsWith('/docs') ? '/docs' : '';
    const href = `${assetBase}/get-started/quickstarts/create-your-first-service-on-cloud`;
    return (
        <a className="inline-flex items-center gap-1.5 text-gray-500 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-[#fdff75] transition-colors font-normal no-underline" href={href} onClick={(e) => { e.preventDefault(); window.location.href = href; }}>
