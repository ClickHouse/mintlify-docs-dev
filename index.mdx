...
export const McpSetup = () => {
  ...
  "mcpServers": {
    ...
    const codeStyle = { background: 'rgba(0,0,0,0.18)', color: '#e5e7eb', fontSize: '11px', lineHeight: '1.7', borderRadius: '8px', padding: '12px 14px', overflowX: 'auto', textAlign: 'left', display: 'block' };

    return (
        <div className="text-xs font-normal" style={{ padding: '12px 0 4px' }}>
            <button
                onClick={() => setOpen(o => !o)}
                className="cursor-pointer hover:text-gray-900 dark:hover:text-[#fdff75] transition-colors select-none bg-transparent border-none p-0 text-xs text-gray-500 dark:text-zinc-500 inline-flex items-center gap-1"
                style={{ font: 'inherit' }}
            >
            ...
            {open && (
                <div className="mt-3 text-left" style={{ maxWidth: '480px', margin: '12px auto 0' }}>
                    {/* Tab bar */}
                    <div className="flex gap-0 border-b" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
                        {tabs.map(t => (
                            <button
                                ...
                                style={{
                                    padding: '6px 12px',
                                    color: tab === t.id ? '#fdff75' : '#6b7280',
                                    borderBottom: tab === t.id ? '2px solid #fdff75' : '2px solid transparent',
                                    marginBottom: '-1px',
                                    font: 'inherit',
                    ...
                    <div style={{ padding: '12px 0 0' }}>
                        {tab === 'claude-code' && (
                            <>
                                <p className="text-gray-400 dark:text-zinc-400 mb-2">Run in your terminal:</p>
                                <pre style={codeStyle}>{`claude mcp add clickhouse-docs -- npx -y mintlify-mcp --doc-url https://clickhouse.com/docs`}</pre>
                            </>
                        )}
                        {tab === 'claude-desktop' && (
                            <>
                                <p className="text-gray-400 dark:text-zinc-400 mb-2">Add to <code style={{ fontSize: '10px', background: 'rgba(255,255,255,0.08)', padding: '1px 4px', borderRadius: '3px' }}>claude_desktop_config.json</code>:</p>
                                <pre style={codeStyle}>{jsonConfig}</pre>
                            </>
                        )}
                        {tab === 'cursor' && (
                            <>
                                <p className="text-gray-400 dark:text-zinc-400 mb-2">Add to <code style={{ fontSize: '10px', background: 'rgba(255,255,255,0.08)', padding: '1px 4px', borderRadius: '3px' }}>.cursor/mcp.json</code>:</p>
                                <pre style={codeStyle}>{jsonConfig}</pre>
                            </>
                        )}
                        {tab === 'other' && (
                            <>
                                <p className="text-gray-400 dark:text-zinc-400 mb-2">Add to your MCP client config:</p>
                                <pre style={codeStyle}>{jsonConfig}</pre>
                            </>
                        )}
                    </div>
...
<div className="relative">
    ...
    <div className="relative z-10 px-0 py-16 lg:py-12 lg:pb-24 max-w-6xl mx-auto">
        ...
        <div className="text-center my-6">
            <a className="inline-block text-gray-900 dark:text-[#fdff75] hover:underline text-xs font-normal" href="/get-started/quickstarts/create-your-first-service-on-cloud">Get started with ClickHouse Cloud →</a>
            <McpSetup />
        </div>
        <ContentSwitcher />
    </div>
