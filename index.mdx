...
export const HeroCard = ({ filename, darkFilename: darkFilenameProp, title, description, href, links, icon }) => {
    ...
    if (links && links.length > 0) {
        return <div className={cardClassName} style={{ color: 'inherit' }}>{content}</div>;
    }

    return <a className={cardClassName} href={href} style={{ color: 'inherit' }} onClick={(e) => { e.preventDefault(); window.location.href = href; }}>{content}</a>;
};
    ...
    const tabs = [
        {
            ...
            bullets: ['Sub-second queries on billions of rows', 'Live dashboards that update as data arrives', 'Scales from gigabytes to petabytes', 'Built for high-concurrency workloads'],
        ...
        },
        {
            ...
            bullets: ['Ingest millions of log events per second', 'Query traces and metrics with SQL', 'ClickStack: open source observability stack', 'Cost-effective long-term retention'],
            href: '/get-started/use-cases/observability',
        },
        {
            label: 'Data warehousing',
            title: 'Data warehousing',
            description: 'Analyze and explore data instantly for insights and apps. Scale faster by offloading heavy workloads.',
            bullets: ['Instant results on large analytical queries', 'Columnar compression reduces storage costs', 'Seamless integration with existing data stacks', 'Offload heavy workloads from your primary database'],
            href: '/get-started/use-cases/data-warehousing',
        },
        {
            label: 'Agentic analytics',
            title: 'Agentic analytics',
            description: 'Let AI agents query your real-time data directly to deliver instant, data-driven answers and actions.',
            bullets: ['Native MCP server for AI integrations', 'SQL-native interface for LLM agents', 'Real-time data for AI-powered applications', 'Connect any agent framework to ClickHouse'],
            href: '/get-started/use-cases/agentic-analytics',
        },
    ];
    return (
        <div className="overflow-hidden rounded-lg border border-[#e5e7eb] dark:border-[#2B2B2B] lg:flex">
            <div className="overflow-x-auto p-4 lg:w-52 lg:overflow-x-visible lg:py-8" style={{ flexShrink: 0, background: 'rgba(0,0,0,0.03)' }}>
                <ul style={{ display: 'flex', gap: '8px', whiteSpace: 'nowrap', width: 'max-content', margin: 0, padding: 0, listStyle: 'none' }} className="lg:block lg:space-y-2 lg:whitespace-normal lg:w-full">
                    {tabs.map((tab, i) => (
                        <li key={i}>
                            <button
                                onClick={() => setActive(i)}
                                style={{
                                    background: active === i ? '#FAFF69' : 'transparent',
                                ...
                                className={active !== i ? 'text-gray-600 dark:text-gray-300 hover:bg-black/[0.06] dark:hover:bg-white/[0.08] hover:text-gray-900 dark:hover:text-white' : ''}
                            >
                                {tab.label}
                            </button>
                        </li>
            ...
            <div style={{ flex: 1, padding: '32px' }} className="lg:p-12">
                {tabs.map((tab, i) => (
                    <div key={i} style={{ display: active === i ? 'block' : 'none' }}>
                        <div style={{ borderLeft: '4px solid #FAFF69', paddingLeft: '24px' }}>
                            <h3 className="text-gray-900 dark:text-white" style={{ fontSize: '18px', fontWeight: '600', margin: '0 0 12px' }}>{tab.title}</h3>
                            <p className="text-gray-600 dark:text-gray-300" style={{ margin: '0 0 20px', fontSize: '15px', lineHeight: '1.6' }}>{tab.description}</p>
                            <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {tab.bullets.map((bullet, j) => (
                                    <li key={j} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="15" fill="none" viewBox="0 0 25 18" style={{ flexShrink: 0, marginTop: '3px' }}>
                                            <path stroke="#FAFF69" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M23.32 1.67 8.65 16.33 2 9.67"/>
                                        </svg>
                                        <span className="text-gray-700 dark:text-gray-200" style={{ fontSize: '14px' }}>{bullet}</span>
                                    </li>
                                ))}
                            </ul>
                            <a
                                href={tab.href}
                                className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-900 dark:text-[#FAFF69] hover:underline"
                                style={{ textDecoration: 'none' }}
                                onClick={(e) => { e.preventDefault(); window.location.href = tab.href; }}
                            >
                                Learn more
...
export const McpLink = () => {
    ...
    return (
        <a
            href="#mcp-setup"
            onClick={handleClick}
            className="inline-flex items-center gap-1.5 text-xs text-gray-500 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-[#fdff75] transition-colors"
            style={{ textDecoration: 'none' }}
        >
            <svg fill="currentColor" fillRule="evenodd" height="13" width="13" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ flexShrink: 0 }}>
    ...
    return (
        <Tabs>
            <Tab title="Claude Code" icon="/images/logo-claudecode-color.svg">
                <p style={{ marginBottom: '8px', color: '#9ca3af', fontSize: '13px' }}>Run in your terminal:</p>
            ...
            <Tab title="Claude Desktop" icon="/images/logo-claude.svg">
                <p style={{ marginBottom: '8px', color: '#9ca3af', fontSize: '13px' }}>Add to <code>claude_desktop_config.json</code>:</p>
                <CodeBlock language="json">{jsonConfig}</CodeBlock>
            </Tab>
            <Tab title="Cursor" icon="/images/logo-cursor.png">
                <p style={{ marginBottom: '8px', color: '#9ca3af', fontSize: '13px' }}>Add to <code>.cursor/mcp.json</code>:</p>
                <CodeBlock language="json">{jsonConfig}</CodeBlock>
            </Tab>
            <Tab title="Windsurf" icon="/images/logo-windsurf.svg">
                <p style={{ marginBottom: '8px', color: '#9ca3af', fontSize: '13px' }}>Add to <code>~/.codeium/windsurf/mcp_config.json</code>:</p>
                <CodeBlock language="json">{jsonConfig}</CodeBlock>
            </Tab>
        </Tabs>
    );
    ...
    return (
        <>
            <div className="flex items-center justify-center gap-2 my-6 text-xs">
                ...
                <a className="inline-flex items-center gap-1.5 text-gray-500 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-[#fdff75] transition-colors font-normal" style={{ textDecoration: 'none' }} href="/get-started/quickstarts/create-your-first-service-on-cloud">
                    <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="currentColor" viewBox="0 0 256 256" aria-hidden="true" style={{ flexShrink: 0 }}><path d="M215.79,118.17a8,8,0,0,0-5-5.66L153.18,90.9l14.66-73.33a8,8,0,0,0-13.69-7l-112,120a8,8,0,0,0,3,13l57.63,21.61L88.16,238.43a8,8,0,0,0,13.69,7l112-120A8,8,0,0,0,215.79,118.17ZM109.37,214l10.47-52.38a8,8,0,0,0-5-9.06L62,132.71l84.62-90.66L136.16,94.43a8,8,0,0,0,5,9.06l52.8,19.8Z"></path></svg>
                    Read the quickstart
                </a>
            </div>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4" style={{ marginTop: '96px' }}>
                        <HeroCard
                            ...
                            links={[
                                ...
                                { label: "Migration guides", href: "/get-started/setup/migration-guides/overview" },
            ...
            {showProduct && (
                <>
                    ...
                    <div className="mt-6 grid sm:grid-cols-2 min-[1300px]:grid-cols-3 gap-x-6 gap-y-4">
                        <HeroCard
                        ...
                        />
                        <HeroCard
                            filename="product-postgres-light.png"
                            darkFilename="product-postgres-dark.png"
                        ...
                        />
                        <HeroCard
                            filename="product-langfuse-light.png"
                            title="LangFuse"
            ...
            <div style={{ marginTop: '64px' }}>
                <h2 className="text-2xl font-semibold text-gray-900 dark:text-zinc-50 mb-6">What ClickHouse is built for</h2>
                <UseCaseTabs />
            </div>

            <div id="mcp-setup" style={{ marginTop: '64px', paddingTop: '40px', borderTop: '1px solid rgba(255,255,255,0.08)', scrollMarginTop: '80px' }}>
                <h2 className="text-2xl font-semibold text-gray-900 dark:text-zinc-50 mb-2">Set up docs MCP server</h2>
                <p className="text-sm text-gray-500 dark:text-zinc-500 mb-6">Query the ClickHouse documentation directly from your AI tools using the Model Context Protocol.</p>
                <McpSetup />
            </div>
        </>
    );
...
<div className="relative">
    ...
    <div className="relative z-10 px-0 py-16 lg:py-12 lg:pb-40 max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-3 mx-auto mt-8">
            ...
            <button type="button" className="flex-none hidden lg:flex items-center justify-center gap-1.5 pl-3 pr-3.5 h-9 shadow-none bg-gray-950/[0.03] dark:bg-white/[0.03] hover:bg-gray-950/10 dark:hover:bg-white/10 border border-white/10" style={{ borderRadius: '4px' }} id="home-assistant-entry" aria-label="Open Ask AI" onClick={() => {
