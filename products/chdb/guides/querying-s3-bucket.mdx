...
Let's start by listing all the files in [an S3 bucket that contains Amazon reviews](/core/get-started/sample-datasets/amazon-reviews).
To do this, we can use the [`s3` table function](/core/reference/functions/table-functions/s3) and pass in the path to a file or a wildcard to a set of files.

<Tip>
If you pass just the bucket name it will throw an exception.
</Tip>

We're also going to use the [`One`](/core/reference/formats/One) input format so that the file isn't parsed, instead a single row is returned per file and we can access the file via the `_file` virtual column and the path via the `_path` virtual column.
...
An alternative way is to used [named collections](/core/concepts/features/configuration/server-config/named-collections), but this approach isn't yet supported by chDB.
