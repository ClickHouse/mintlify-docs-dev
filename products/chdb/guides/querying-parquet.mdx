...
We're going to explore a Parquet file from the [Amazon reviews](/core/get-started/sample-datasets/amazon-reviews) dataset.
...
When querying Parquet files, we can use the [`ParquetMetadata`](/core/reference/formats/Parquet/ParquetMetadata) input format to have it return Parquet metadata rather than the content of the file.
