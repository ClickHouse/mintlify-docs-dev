...
<Image img="/images/cloud/agent-builder/launch-ch-agents.png" alt="Cloud service navigation with ClickHouse agents (Beta) selected, showing the launchpad with the Launch ClickHouse agents button" size="lg"/>
</Step>
<Step>
...
- **Model** - pick a model from the dropdown. Tune temperature and other generation settings in [model parameters](/products/cloud/features/ai-ml/agents/builder/model-parameters).

<Image img="/images/cloud/agent-builder/agent-builder.png" alt="Agent Builder panel showing the Create New Agent dropdown, the form fields (Name, Description, Category, Instructions, Model), and the Capabilities section" size="lg"/>
</Step>
<Step>
...
<Image img="/images/cloud/agent-builder/capabilities.png" alt="Capabilities section of the Agent Builder panel showing Run Code, Web Search, File Context, Artifacts, MCP Servers, and Skills toggles" size="sm"/>
...
<Image img="/images/cloud/agent-builder/tools-button.png" alt="Bottom of the Agent Builder panel with the Add Tools button highlighted" size="sm"/>

Click **Add Tools** to browse the catalog:

<Image img="/images/cloud/agent-builder/tools-modal.png" alt="Agent Tools modal showing a grid of third-party integrations including Google, OpenAI Image Tools, Wolfram, DALL-E-3, Tavily Search, Calculator, and Stable Diffusion" size="lg"/>
...
<Image img="/images/cloud/agent-builder/chat-query.png" alt="Chat exchange showing the question 'What are my top 10 tables by row count this week?' and the agent's response — a Markdown table ranking the top 10 tables across services by row count, with Key Observations below" size="lg"/>
</Step>
</Steps>
