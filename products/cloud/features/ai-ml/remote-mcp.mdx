...
For self-hosted ClickHouse instances, see the [open-source MCP server guides](/guides/ai-ml/MCP).
...
For detailed steps with screenshots, see the [setup guide](/use-cases/ai/mcp/remote-mcp#enable-remote-mcp-server).
...
See the [setup guide](/use-cases/ai/mcp/remote-mcp) for step-by-step instructions on enabling the remote MCP server and connecting it to an MCP client.
