...
will use a small portion of the table, called [granules](/guides/best-practices/sparse-primary-indexes#data-is-organized-into-granules-for-parallel-data-processing),
as the unit of work.
...
[`system.query_log`](/operations/system-tables/query_log) table. You can
also look at the [`system.events`](/operations/system-tables/events)
table to see all the events that have occured on the server, and you can use the
[`clusterAllReplicas`](/sql-reference/table-functions/cluster) table function to see the tables on all the replicas
...
The [`system.text_log`](/operations/system-tables/text_log) table also 
