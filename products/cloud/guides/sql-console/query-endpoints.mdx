...
Open a new query tab. For demonstration purposes, we'll use the [youtube dataset](/core/get-started/sample-datasets/youtube-dislikes), which contains approximately 4.5 billion records.
Follow the steps in section ["Create table"](/core/get-started/sample-datasets/youtube-dislikes#create-the-table) to create the table on your Cloud service and insert data to it.
...
| `:clickhouse_setting` | No | Any supported [ClickHouse setting](/core/reference/settings/session-settings) | `?max_threads=8` |
