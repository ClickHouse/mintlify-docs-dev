...
<Note>
Any usage where backups are being exported to a different region in the same cloud provider will incur [data transfer](/products/cloud/reference/billing/network-data-transfer) charges.  Currently we don't support cross cloud backups.
</Note>
...
See: [Configuring BACKUP/RESTORE to use an S3 Endpoint](/core/concepts/features/backup-restore/s3-endpoint) for more details.
...
See: [Configuring BACKUP/RESTORE to use an AzureBlobStorage Endpoint](/core/concepts/features/backup-restore/azure-blob-storage#configuring-backuprestore-to-use-an-azureblobstorage-endpoint) for more details.
