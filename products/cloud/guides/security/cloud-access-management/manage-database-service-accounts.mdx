...
[SET ROLE](/core/reference/statements/set-role) can be used to dynamically scope permissions for a service account during a session. This works by limiting a user's effective permissions to only those granted by the activated role(s). This approach has several advantages:
