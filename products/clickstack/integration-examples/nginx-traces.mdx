...
If you would like to test the integration before configuring your own existing setup, you can test with our preconfigured setup and sample data in the [following section](/products/clickstack/integration-examples/nginx-traces#demo-dataset).
...
<Image img="/static/images/clickstack/nginx-traces-search-view.png" alt="View Traces"/>
...
#### <TrackedLink href={'/examples/nginx-traces-dashboard.json'} download="nginx-traces-dashboard.json" eventName="docs.nginx_traces_monitoring.dashboard_download">Download</TrackedLink> the dashboard configuration {#download}
...
<Image img="/static/images/clickstack/finish-nginx-traces-dashboard.png" alt="Finish Import"/>
...
<Image img="/static/images/clickstack/nginx-traces-dashboard.png" alt="Example Dashboard"/>
...
This guide sends traces directly from the Nginx OpenTelemetry module to ClickStack's OTLP endpoint. For production deployments, we recommend running your own OTel Collector as a gateway to provide batching and resilience. See [Sending OpenTelemetry data](/products/clickstack/ingesting-data/opentelemetry) for production configuration.
