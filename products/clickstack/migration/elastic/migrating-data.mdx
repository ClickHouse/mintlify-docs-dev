...
Configure Elastic's TTL settings to match your desired retention period. Set up the ClickStack [TTL](/products/clickstack/managing#configure-ttl) to maintain data for the same duration.
</Step>
<Step>
...
ClickHouse uses [TTL clauses](/products/clickstack/managing#configure-ttl) on MergeTree tables to manage data expiration. TTL policies can:
...
We recommend aligning your ClickHouse TTL configuration with your existing Elastic retention policies to maintain a consistent data lifecycle during the migration. For examples, see [ClickStack production TTL setup](/products/clickstack/managing#configure-ttl).
