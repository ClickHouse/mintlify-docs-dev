...
How this is achieved depends on your current agents for collection - see ["Migrating Agents"](/products/clickstack/migration/elastic/migrating-agents).
</Step>
<Step>
#### Adjust retention periods {#adjust-retention-period}

Configure Elastic's TTL settings to match your desired retention period. Set up the ClickStack [TTL](/products/clickstack/managing/production#configure-ttl) to maintain data for the same duration.
</Step>
<Step>
...
- ClickStack [tiered storage](/core/reference/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-multiple-volumes) capabilities can help manage long-term data efficiently.
- Consider using [materialized views](/core/concepts/features/materialized-views/incremental-materialized-view) to maintain aggregated or filtered historical data while allowing raw data to expire.
...
- **ClickHouse OSS**: In self-managed deployments, we recommend:
  ...
  - Using [tiered storage](/core/guides/use-cases/observability/build-your-own/managing-data#storage-tiers) to extend local disk with S3-compatible object storage
  - Using [`ReplicatedMergeTree`](/core/reference/engines/table-engines/mergetree-family/replication) if high availability is required
  - For fault tolerance, [1 replica of your shard](/core/reference/engines/table-engines/mergetree-family/replication) is typically sufficient in Observability workloads.
...
If you do need to shard, refer to [Horizontal scaling](/core/guides/oss/deployment-and-scaling/examples/2-shards-1-replica) for guidance on shard keys and distributed table setup.

### Retention and TTL {#retention-and-ttl}

ClickHouse uses [TTL clauses](/products/clickstack/managing/production#configure-ttl) on MergeTree tables to manage data expiration. TTL policies can:
...
We recommend aligning your ClickHouse TTL configuration with your existing Elastic retention policies to maintain a consistent data lifecycle during the migration. For examples, see [ClickStack production TTL setup](/products/clickstack/managing/production#configure-ttl).
...
Create a table in ClickHouse for the index being migrated from Elasticsearch. You can map [Elasticsearch types to their ClickHouse](/products/clickstack/migration/elastic/types) equivalent. Alternatively, you can simply rely on the JSON data type in ClickHouse, which will dynamically create columns of the appropriate type as data is inserted.
...
- Used appropriate ClickHouse types based on the mapping:
  ...
  - `boolean` → `UInt8`
  - `long` → `Int64`
  - `ip` → `Array(Variant(IPv4, IPv6))`. We use a [`Variant(IPv4, IPv6)`](/core/reference/data-types/variant) here as the field contains a mixture of [`IPv4`](/core/reference/data-types/ipv4) and [`IPv6`](/core/reference/data-types/ipv6).
...
**This approach of statically defining the schema and using the JSON type selectively where required [is recommended](/core/guides/clickhouse/data-formats/json/schema#handling-semi-structured-dynamic-structures).**
...
For more details on using the JSON type in schemas, and how to efficiently apply it, we recommend the guide ["Designing your schema"](/core/guides/clickhouse/data-formats/json/schema).
</Step>
<Step>
...
Ensure ClickHouse is [installed on the server](/core/get-started/setup/install) on which `elasticdump` is located. **Don't start a ClickHouse server** - these steps only require the client.
</Step>
<Step>
...
- `query="INSERT INTO logs_system_syslog FORMAT JSONAsRow"` - Inserting the data as [JSONEachRow format](/core/guides/clickhouse/data-formats/json/formats). This is appropriate if sending to a well-defined schema such as `logs_system_syslog.`
<br/>
...
See ["Reading JSON as an object"](/core/guides/clickhouse/data-formats/json/formats#reading-json-as-an-object) for further details.
</Info>
</Step>
<Step>
...
This can be achieved using the [`input`](/core/reference/functions/table-functions/input) table function, which allows us to execute any `SELECT` query on the stdout.
