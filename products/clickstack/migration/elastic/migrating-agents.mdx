...
Where possible we recommend migrating to the [OpenTelemetry (OTel) Collector](https://opentelemetry.io/docs/collector/) for all log, metric, and trace collection, deploying the collector at the [edge in an agent role](/products/clickstack/ingesting-data/collector#collector-roles). This represents the most efficient means of sending data and avoids architectural complexity and data transformation.
...
All data is ingested into ClickStack via an **OpenTelemetry (OTel) collector** instance, which acts as the primary entry point for logs, metrics, traces, and session data. We recommend using the official [ClickStack distribution](/products/clickstack/ingesting-data/opentelemetry#installing-otel-collector) of the collector for this instance, if not [already bundled in your ClickStack deployment model](/products/clickstack/deployment).

Users send data to this collector from [language SDKs](/products/clickstack/ingesting-data/sdks) or through data collection agents collecting infrastructure metrics and logs (such OTel collectors in an [agent](/products/clickstack/ingesting-data/collector#collector-roles) role or other technologies e.g. [Fluentd](https://www.fluentd.org/) or [Vector](https://vector.dev/)). For teams that want a managed OpenTelemetry pipeline, [Bindplane](/products/clickstack/integration-partners/bindplane)offers an OpenTelemetry-native solution with a native ClickStack destination, simplifying telemetry collection, processing, and routing.
...
<Image img="/static/images/use-cases/observability/clickstack-migrating-agents.png" alt="Migrating agents" size="lg" background/>
...
<Image img="/static/images/use-cases/observability/ingestion-keys.png" alt="Ingestion keys" size="lg"/>
...
<Image img="/static/images/use-cases/observability/add-logstash-output.png" alt="Add Logstash output" size="md"/>
...
<Image img="/static/images/use-cases/observability/agent-output-settings.png" alt="Agent settings" size="md"/>
...
By default, an API ingestion key isn't required if running an OpenTelemetry Collector standalone for Managed ClickStack. Ingestion can be secured however, by specifying an OTLP auth token. See ["Securing the collector"](/products/clickstack/ingesting-data/collector#securing-the-collector).
</Info>
