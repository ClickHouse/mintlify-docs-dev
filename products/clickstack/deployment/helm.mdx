...
This page documents the **v2.x** subchart-based Helm chart. If you are still using the v1.x inline-template chart, see the [v1.x Helm guide](/products/clickstack/deployment/helm-v1). For migration steps, see the [Upgrade guide](/products/clickstack/deployment/helm-upgrade).
</Warning>
...
- [Additional manifests](/products/clickstack/deployment/helm-additional-manifests) for deploying arbitrary Kubernetes objects (NetworkPolicy, HPA, ALB Ingress, etc.) alongside the chart
...
For production deployments, configure ingress with TLS instead of port forwarding. See the [Ingress Configuration guide](/products/clickstack/deployment/helm-configuration#ingress-setup) for detailed setup instructions.
</Tip>
...
<Image img="/static/images/use-cases/observability/hyperdx-login.png" alt="HyperDX UI" size="lg"/>
...
For detailed API key setup instructions including multiple configuration methods and pod restart procedures, see the [API Key Setup guide](/products/clickstack/deployment/helm-configuration#api-key-setup).
</Tip>
...
For production deployments with secret-based configuration, external OTEL collectors, or minimal setups, see the [Deployment Options guide](/products/clickstack/deployment/helm-deployment-options).
</Tip>
...
- [Configuration Guide](/products/clickstack/deployment/helm-configuration) - Ingress, TLS, and secrets management
- [Cloud Deployments](/products/clickstack/deployment/helm-cloud) - Cloud-specific settings and production checklist
</Tip>
...
If you are upgrading from the v1.x inline-template chart, see the [Upgrade guide](/products/clickstack/deployment/helm-upgrade) for migration instructions. This is a breaking change — an in-place `helm upgrade` is not supported.
</Info>
...
- [Ingress Troubleshooting](/products/clickstack/deployment/helm-configuration#troubleshooting-ingress) - Asset serving, path rewrites, browser issues
- [Cloud Deployments](/products/clickstack/deployment/helm-cloud#loadbalancer-dns-resolution-issue) - GKE OpAMP issues and cloud-specific problems
</Tip>
...
- [Deployment options](/products/clickstack/deployment/helm-deployment-options) - External ClickHouse, OTEL collector, and minimal deployments
- [Configuration guide](/products/clickstack/deployment/helm-configuration) - API keys, secrets, and ingress setup
- [Cloud deployments](/products/clickstack/deployment/helm-cloud) - GKE, EKS, AKS configurations and production best practices
- [Upgrade guide](/products/clickstack/deployment/helm-upgrade) - Migrating from v1.x to v2.x
- [Additional manifests](/products/clickstack/deployment/helm-additional-manifests) - Deploying custom Kubernetes objects alongside the chart

### v1.x documentation {#v1x-documentation}
- [Helm (v1.x)](/products/clickstack/deployment/helm-v1) - v1.x deployment guide
- [Configuration (v1.x)](/products/clickstack/deployment/helm-configuration-v1) - v1.x configuration
- [Deployment options (v1.x)](/products/clickstack/deployment/helm-deployment-options-v1) - v1.x deployment options
- [Cloud deployments (v1.x)](/products/clickstack/deployment/helm-cloud-v1) - v1.x cloud configurations

### Additional resources {#additional-resources}
- [ClickStack getting started guide](/products/clickstack/getting-started) - Introduction to ClickStack
