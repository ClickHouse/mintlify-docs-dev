...
<Note>
The `tracePropagationTargets` option is key to connecting session replays with backend traces — set it to your API domain to enable full frontend-to-backend distributed tracing. For a complete list of SDK options including privacy controls, custom actions, React error boundaries, and source maps, see the [Browser SDK reference](/products/clickstack/ingesting-data/sdks/browser).
</Note>

The Browser SDK also supports [masking inputs and text](/products/clickstack/ingesting-data/sdks/browser#options) for privacy-sensitive applications, and [attaching user information](/products/clickstack/ingesting-data/sdks/browser#attach-user-information-or-metadata) so you can search and filter sessions by user in the ClickStack UI.
...
<Image img="/static/images/clickstack/session-replay/replay-search-view.png" alt="Session replay search view" size="lg"/>
...
<Image img="/static/images/clickstack/session-replay/session-replay.png" alt="Session replay playback" size="lg"/>
...
<Image img="/static/images/clickstack/session-replay/trace-to-replay.png" alt="Session replay trace view" size="lg"/>

## How session data is stored {#data-storage}

Session replay data is stored in a dedicated [`hyperdx_sessions`](/products/clickstack/ingesting-data/schemas#sessions) table in ClickHouse, separate from logs and traces. Each session event is a row with a `Body` field containing the event payload and a `LogAttributes` map storing the event metadata. The `Body` and `LogAttributes` columns together hold the details of the actual session events that are used to reconstruct the replay.

For the full table schema information, see [Tables and schemas used by ClickStack](/products/clickstack/ingesting-data/schemas).
...
- **Local demo** — the [Session replay demo](/products/clickstack/example-datasets/session-replay) walks through instrumenting a demo application step by step, including running ClickStack locally and viewing your replays.

## Learn more {#learn-more}

- [Session replay demo](/products/clickstack/example-datasets/session-replay) — interactive local demo application with step-by-step instructions
- [Browser SDK Reference](/products/clickstack/ingesting-data/sdks/browser) — full SDK options, source maps, custom actions, and advanced configuration
- [Search](/products/clickstack/features/search) — search syntax for filtering sessions and events
- [Dashboards](/products/clickstack/features/dashboards/overview) — build visualizations and dashboards from session and trace data
- [Alerts](/products/clickstack/features/alerts) — set up alerts on errors, latency, and other signals
- [ClickStack Architecture](/products/clickstack/architecture) — how ClickHouse, HyperDX, and the OTel collector fit together
