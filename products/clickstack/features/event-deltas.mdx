...
<Image img="/static/images/use-cases/observability/hyperdx-demo/step_17.png" alt="Event deltas overview" size="lg"/>
...
<Image img="/static/images/clickstack/event-deltas/distribution-mode.png" alt="Distribution mode showing per-attribute value distributions across all spans" size="lg"/>
...
<Image img="/static/images/use-cases/observability/event_deltas_separation.png" alt="Heatmap selection over a slow band starting at a specific time, with comparison bars below" size="lg"/>
...
<Image img="/static/images/use-cases/observability/event_deltas_issue.png" alt="Bar popover showing filter, exclude and copy actions over a value isolated to one population" size="lg"/>
...
<Note>
Aggregated **Other (N)** buckets that collapse low-frequency values aren't clickable. To filter for a specific value within that bucket, use the [search bar](/products/clickstack/features/search) directly.
</Note>
...
<Image img="/static/images/clickstack/event-deltas/settings-drawer.png" alt="Heatmap Settings drawer with Scale, Value and Count fields" size="lg"/>
...
The same heatmap is also available as a [dashboard tile](/products/clickstack/features/dashboards/overview#create-a-tile-heatmap), which is useful when you want to monitor the distribution shape over time outside the Event Deltas drill-down flow.
</Tip>
