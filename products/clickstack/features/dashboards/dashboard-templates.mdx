...
<Image img="/static/images/use-cases/observability/browse-dashboard-template.png" alt="Browse Dashboard Templates Button" size="lg"/>
...
<Image img="/static/images/use-cases/observability/dashboard-template-gallery.png" alt="Dashboard Template Gallery" size="lg"/>
...
<Image img="/static/images/use-cases/observability/import-dashboard-template.png" alt="Dashboard Template Import" size="lg"/>
...
- Each template is configured with a [custom filter](/products/clickstack/features/dashboards/overview#custom-filters) for services which have the [`telemetry.sdk.language`](https://opentelemetry.io/docs/specs/semconv/registry/attributes/telemetry/#telemetry-sdk-language) resource attribute matching the dashboard's runtime.
  ...
  - For high-volume environments, filter load times can be reduced by [materializing](/products/clickstack/managing/performance-tuning#materialize-frequently-queried-attributes) the `ResourceAttributes['telemetry.sdk.language']` column.
- Templates reference up-to-date OTel Semantic Conventions when published, and are updated periodically as the OTel Spec is updated. For services instrumented with older OTel SDKs, the visualizations may need to be [edited](/products/clickstack/features/dashboards/overview#dashboards-editing-visualizations) to reference older metric names.
