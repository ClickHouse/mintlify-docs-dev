...
### pg_clickhouse: unified query layer
...
Configure up to two standby replicas across different availability zones using quorum-based replication. These standbys are dedicated to high availability and automatic failover, ensuring your database recovers quickly from failures. For read scaling, you can provision separate [read replicas](/docs/managed-postgres/read-replicas). See the [High availability](/docs/managed-postgres/high-availability) page for configuration details.
