...
## Open query insights <a id="open"></a>
...
<Image img="/images/managed-postgres/monitoring/query-insights-overview.png" alt="Query Insights overview showing the six stats cards: queries per second, query latency percentiles, operations breakdown donut, rows returned area chart, buffer hit ratio donut at 95.2 percent, and an errors column chart" size="lg" border/>

## Slow patterns <a id="slow-patterns"></a>
...
<Image img="/images/managed-postgres/monitoring/query-insights-patterns.png" alt="Slow query patterns table showing one row per normalized query with Database, User, Operation, Calls, Errors, Avg latency, P95, Max latency, Total runtime, Rows returned, and Cache hit columns" size="lg" border/>

### Sort by what you suspect <a id="sort"></a>
...
### Narrow the table <a id="filters"></a>
...
## Recent queries <a id="recent-queries"></a>
...
<Image img="/images/managed-postgres/monitoring/query-insights-recent-queries.png" alt="Recent Queries table with Database, User, Operation, and Application filter dropdowns and columns for Time, Operation, Query, Duration, Rows, Database, User, and Blks read" size="lg" border/>
...
<Image img="/images/managed-postgres/monitoring/query-insights-detail-aggregate.png" alt="Query detail flyout showing the Query pattern code block and the Aggregate resource usage grid with thirteen stat cards including total calls, latency percentiles, total runtime, rows returned, cache hit ratio, blocks read, blocks hit, CPU time, WAL bytes, and errors" size="md" border/>

<Image img="/images/managed-postgres/monitoring/query-insights-detail-recent.png" alt="Query detail flyout continued, showing the Query context section with database, user, operation, and application, and a Recent executions card with timestamp, OK status, server role, host id, and per-execution counters for duration, rows, cache hit, CPU, shared blocks read, and shared blocks hit" size="md" border/>

### Per-execution counters <a id="counters"></a>
...
### Normalized in Postgres, before the wire <a id="how-normalized"></a>
...
### Out of the way of the database <a id="how-overhead"></a>
...
### Raw events, not aggregates <a id="how-raw-events"></a>
...
### Same engine our customers use <a id="how-engine"></a>
...
### Open source <a id="how-open-source"></a>
