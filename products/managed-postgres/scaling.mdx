...
For read-heavy workloads, consider using [read replicas](/managed-postgres/read-replicas) to scale read capacity horizontally:
...
If you're replicating data to ClickHouse using [ClickPipes](/managed-postgres/clickhouse-integration), you can independently scale the CDC (Change Data Capture) pipeline:
...
- [Settings and configuration](/managed-postgres/settings)
- [Read replicas](/managed-postgres/read-replicas)
- [High availability](/managed-postgres/high-availability)
- [Performance benchmarks](/managed-postgres/benchmarks)
