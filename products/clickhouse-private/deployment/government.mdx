...
* The [ClickHouse Private AWS Technical Setup Guide](/products/clickhouse-private/deployment/aws) has been followed for setting up the AWS resources (do not do the Kubernetes Resources section) with the following considerations for FIPS:
...
* [Install VolumeSnapshot CRDs](/products/clickhouse-private/deployment/aws#install-volumesnapshot-crds)
* [Install StorageClass via Helm Chart](/products/clickhouse-private/deployment/aws#install-storageclass-via-helm) if you do not wish to use a custom or existing StorageClass
