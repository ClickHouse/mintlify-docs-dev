...
输出格式可以是 ClickHouse 支持的任意一种 [ouput formats](/zh/reference/formats)。我们来看几个示例……
...
<Note>
  你不必止步于此——还可以使用 `clickhouse-local` 从 PostgreSQL 拉取数据，并将其输出为[各种输出格式](/zh/reference/formats)。
