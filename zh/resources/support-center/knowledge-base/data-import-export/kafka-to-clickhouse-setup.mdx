...
本文将介绍如何将数据从 Kafka topic 发送到 ClickHouse 表。我们将使用 Wiki 的最近更改 feed，它提供了一个[事件 stream](https://stream.wikimedia.org/v2/stream/recentchange)，用于表示 Wikimedia 各项内容所发生的更改。步骤包括：

1. 如何在 Ubuntu 上设置 Kafka
2. 将数据 stream 摄取到 Kakfa topic
