---
keywords: ['ClickHouse', 'Python', '客户端', '连接', '集成']
slug: /integrations/python
description: '用于将 Python 连接到 ClickHouse 的 ClickHouse Connect 项目套件'
title: '通过 ClickHouse Connect 将 Python 集成到 ClickHouse'
sidebarTitle: '简介'
doc_type: 'guide'
integration:
  - support_level: 'core'
...
<Note>
  官方 ClickHouse Connect Python 驱动使用 HTTP protocol 与 ClickHouse 服务器通信。因此，它支持 HTTP 负载均衡器，并且在使用防火墙和代理的企业环境中也能良好运行；但与基于 native TCP 的 protocol 相比，其压缩率和性能略低，并且不支持某些高级功能，例如取消查询。对于某些用例，你也可以考虑使用采用基于 native TCP 的 protocol 的 [Community Python drivers](/zh/integrations/language-clients/third-party/client-libraries)。
</Note>
