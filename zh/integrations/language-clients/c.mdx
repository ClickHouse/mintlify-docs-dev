...
`clickhouse-c` 是一个面向 ClickHouse [原生协议](/zh/interfaces/tcp)的仅头文件 C 客户端。
其源代码以及各头文件的参考文档位于 [GitHub 仓库](https://github.com/ClickHouse/clickhouse-c)。

与更高层的客户端不同，它是有意只提供尽可能少的封装。核心头文件会通过你提供的 I/O 回调，对 [Native](/zh/interfaces/formats/Native) 格式的块进行解码和编码。套接字、TLS 上下文、内存分配器、重试机制以及连接池都由你自行管理。因此它足够小，便于嵌入：仅包含 `clickhouse.h` 即可，除 libc 外不会引入任何链接时依赖项。
...
* HTTP 协议。直接封装 libcurl 以使用 [HTTP interface](/zh/interfaces/http)。
...
[ClickHouse Cloud](/zh/cloud/overview) 和其他启用 TLS 的部署使用 9440
