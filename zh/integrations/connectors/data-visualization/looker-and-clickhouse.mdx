---
sidebarTitle: 'Looker'
slug: /integrations/looker
keywords: ['ClickHouse', 'Looker', '连接', '集成', 'UI']
description: 'Looker 是一个面向企业的 BI、数据应用和嵌入式分析平台，可帮助您实时探索并分享见解。'
title: 'Looker'
doc_type: 'guide'
integration:
  - support_level: 'partner'
  - category: 'data_visualization'
---

import { Image } from "/snippets/components/Image.jsx";
import ConnectionDetails from '/snippets/zh/_gather_your_details_http.mdx';

import PartnerBadge from "/snippets/components/PartnerBadge/PartnerBadge.jsx";

<PartnerBadge />

Looker 可通过官方 ClickHouse 数据源连接到 ClickHouse Cloud 或本地部署。

<div id="1-gather-your-connection-details">
  ## 1. 准备连接信息
</div>

<ConnectionDetails />

<div id="2-create-a-clickhouse-data-source">
  ## 2. 创建 ClickHouse 数据源
</div>

前往 Admin -&gt; Database -&gt; Connections，然后点击右上角的 “Add Connection” 按钮。

<Image size="md" img="/images/integrations/data-visualization/looker_01.png" alt="在 Looker 的数据库管理界面中添加新连接" border />

<br />

为数据源命名，并在 dialect 下拉菜单中选择 `ClickHouse`。然后在表单中输入您的凭据。

<Image size="md" img="/images/integrations/data-visualization/looker_02.png" alt="在 Looker 连接表单中指定您的 ClickHouse 凭据" border />

<br />

如果您使用的是 ClickHouse Cloud，或者您的部署需要 SSL，请确保已在附加设置中启用 SSL。

<Image size="md" img="/images/integrations/data-visualization/looker_03.png" alt="在 Looker 设置中为 ClickHouse 连接启用 SSL" border />

<br />

请先测试连接；完成后，再连接到新的 ClickHouse 数据源。

<Image size="md" img="/images/integrations/data-visualization/looker_04.png" alt="测试并连接到 ClickHouse 数据源" border />

<br />

现在，您应该已经可以将 ClickHouse 数据源关联到您的 Looker 项目。

<div id="3-known-limitations">
  ## 3. 已知限制
</div>

1. 默认情况下，以下数据类型会被作为字符串处理：
   * Array - 由于 JDBC 驱动的限制，serialization 无法按预期运行
   * Decimal* - 可在模型中改为数值
   * LowCardinality(...) - 可在模型中改为适当的类型
   * Enum8, Enum16
   * UUID
   * Tuple
   * Map
   * JSON
   * Nested
   * FixedString
   * Geo 类型
     * MultiPolygon
     * Polygon
     * Point
     * Ring
2. 不支持[对称聚合功能](https://cloud.google.com/looker/docs/reference/param-explore-symmetric-aggregates)
3. 驱动尚未实现[完全外连接](https://cloud.google.com/looker/docs/reference/param-explore-join-type#full_outer)