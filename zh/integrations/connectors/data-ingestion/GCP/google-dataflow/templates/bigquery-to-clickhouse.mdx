...
<Tabs>
  <Tab title="Google Cloud Console">
    ...
    1. 点击 `CREATE JOB FROM TEMPLATE` 按钮
       <Image img="/images/integrations/data-ingestion/google-dataflow/create_job_from_template_button.png" border alt="DataFlow 控制台" />
    2. 打开Template表单后，输入作业名称并选择所需区域。
       <Image img="/images/integrations/data-ingestion/google-dataflow/template_initial_form.png" border alt="DataFlow Template初始表单" />
    3. 在 `DataFlow Template` 输入框中，输入 `ClickHouse` 或 `BigQuery`，然后选择 `BigQuery to ClickHouse` Template
       <Image img="/images/integrations/data-ingestion/google-dataflow/template_clickhouse_search.png" border alt="选择 BigQuery 到 ClickHouse Template" />
    ...
    <Image img="/images/integrations/data-ingestion/google-dataflow/extended_template_form.png" border alt="BigQuery 到 ClickHouse 扩展Template表单" />
...
<Image img="/images/integrations/data-ingestion/google-dataflow/dataflow-inqueue-job.png" size="lg" border alt="DataFlow 控制台显示一个正在运行的从 BigQuery 到 ClickHouse 的作业" />
