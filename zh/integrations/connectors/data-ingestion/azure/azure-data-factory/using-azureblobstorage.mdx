...
它提供了类似表的接口，使您能够直接从源中选择、插入和过滤数据。该函数经过高度优化，支持许多常用文件格式，包括 `CSV`、`JSON`、`Parquet`、`Arrow`、`TSV`、`ORC`、`Avro` 等。完整列表请参见[&quot;数据格式&quot;](/zh/reference/formats)。
...
2. 在左侧菜单中，选择 **安全 +
   网络** 部分下的 **访问密钥**。
   <Image img="/images/integrations/data-ingestion/azure-data-factory/azure-data-store-settings.png" size="lg" alt="Azure 数据存储设置" border />

3. 选择 **key1** 或 **key2**，然后点击 **连接字符串** 字段旁边的
   **显示** 按钮。
   <Image img="/images/integrations/data-ingestion/azure-data-factory/azure-data-store-access-keys.png" size="lg" alt="Azure 数据存储访问密钥" border />
...
* [输入和输出数据格式](/zh/reference/formats)
