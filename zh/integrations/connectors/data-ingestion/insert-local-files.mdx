...
1. 假设我们有一个名为 `comments.tsv` 的 TSV 文件，其中包含一些 Hacker News 评论，且表头行包含列名。插入数据时，你需要指定一种[输入格式](/zh/reference/formats)，在本例中是 `TabSeparatedWithNames`：
...
如需了解如何在本地操作系统上安装 `clickhouse-client` 的详细信息，请访问 [`clickhouse-client` 文档页面](/zh/concepts/features/interfaces/cli)。