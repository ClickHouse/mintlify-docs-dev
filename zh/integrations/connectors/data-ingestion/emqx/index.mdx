...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_1.png" size="sm" border alt="显示 AWS 区域选择的 ClickHouse Cloud 服务部署界面" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_2.png" size="sm" border alt="显示 IP 访问配置的 ClickHouse Cloud 连接设置" />

然后，你需要保存用户名和密码，以备后续使用。

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_3.png" size="sm" border alt="显示用户名和密码的 ClickHouse Cloud 凭据界面" />

完成后，你将获得一个正在运行的 ClickHouse 实例。点击 &quot;Connect&quot; 获取 ClickHouse Cloud 的实例连接地址。

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_4.png" size="lg" border alt="带有连接选项的 ClickHouse Cloud 运行中实例仪表板" />

点击 &quot;Connect to SQL Console&quot;，为与 EMQX Cloud 集成创建数据库和表。

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_5.png" size="lg" border alt="ClickHouse Cloud SQL 控制台界面" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_6.png" size="lg" border alt="ClickHouse Cloud 中创建数据库和表的 SQL 查询执行界面" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_sign_up.png" size="lg" border alt="带有注册表单的 EMQX Cloud 注册页面" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_create_1.png" size="lg" border alt="EMQX Cloud 创建部署第 1 步，显示部署选项" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_overview.png" size="lg" border alt="EMQX Cloud 概览仪表板，显示 broker 指标" />
...
点击左侧菜单中的“Authentication &amp; ACL”，然后点击子菜单中的“Authentication”。点击右侧的“Add”按钮，并为稍后建立 MQTT 连接设置用户名和密码。这里我们将使用 `emqx` 和 `xxxxxx` 作为用户名和密码。

<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_auth.png" size="lg" border alt="用于添加凭据的 EMQX Cloud 身份验证设置界面" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_nat_gateway.png" size="lg" border alt="EMQX Cloud NAT Gateway 配置面板" />
...
[EMQX Cloud 数据集成](https://docs.emqx.com/en/cloud/latest/rule_engine/introduction.html#general-flow) 用于配置处理和响应 EMQX 消息流及设备事件的规则。数据集成不仅提供了清晰、灵活且“可配置”的架构方案，还简化了开发流程，提升了易用性，并降低了业务系统与 EMQX Cloud 之间的耦合度。它还为定制 EMQX Cloud 的专有能力提供了更完善的基础设施。

<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_data_integration.png" size="lg" border alt="显示可用连接器的 EMQX Cloud 数据集成 选项" />

EMQX Cloud 为常见数据系统提供了 30 多种原生集成，ClickHouse 就是其中之一。

<Image img="/images/integrations/data-ingestion/emqx/data_integration_clickhouse.png" size="lg" border alt="EMQX Cloud ClickHouse 数据集成 连接器详情" />

<div id="create-clickhouse-resource">
  ### 创建 ClickHouse 资源
</div>

点击左侧菜单中的“Data Integrations”，然后点击“View All Resources”。你可以在 Data Persistence 部分找到 ClickHouse，也可以直接搜索 ClickHouse。

点击 ClickHouse 卡片以创建新资源。

* 注意：为该资源添加备注。
* 服务器地址：这是你的 ClickHouse Cloud 服务地址，记得填写端口。
* 数据库名称：我们在上述步骤中创建的 `emqx`。
* 用户：用于连接到你的 ClickHouse Cloud 服务的用户名。
* 密钥：连接密码。

<Image img="/images/integrations/data-ingestion/emqx/data_integration_resource.png" size="lg" border alt="EMQX Cloud ClickHouse 资源设置表单，包含连接详情" />
...
EMQX 提供了强大的[规则引擎](https://docs.emqx.com/en/cloud/latest/rule_engine/rules.html)，可在将原始 MQTT 消息发送到第三方系统之前，对其进行转换和增强。
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_1.png" size="md" border alt="EMQX Cloud 数据集成规则创建第 1 步，显示 SQL 查询" />

你可以使用 SQL 测试功能来查看结果。

<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_2.png" size="md" border alt="EMQX Cloud 数据集成规则创建第 2 步，显示测试结果" />
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_action.png" size="md" border alt="使用 SQL 模板的 EMQX Cloud 数据集成规则操作设置" />
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_details.png" size="md" border alt="显示配置摘要的 EMQX Cloud 数据集成规则详情" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-overview.png" size="lg" border alt="显示客户端界面的 MQTTX 概览" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-new.png" size="lg" border alt="包含连接详情的 MQTTX 新建连接设置表单" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-publish.png" size="lg" border alt="显示消息编写界面的 MQTTX 发布 MQTT 消息界面" />
...
<Image img="/images/integrations/data-ingestion/emqx/rule_monitor.png" size="lg" border alt="显示消息处理指标的 EMQX Cloud 规则监控仪表板" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_result.png" size="lg" border alt="展示已持久化 IoT 数据的 ClickHouse 查询结果" />
