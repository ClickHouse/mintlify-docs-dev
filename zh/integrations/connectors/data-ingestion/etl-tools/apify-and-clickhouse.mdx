...
<Steps>
  ...
  <Step>
    ...
    <Note>
      Apify 还提供 [Python client](https://docs.apify.com/api/client/python)。如果你更习惯使用 Python，请通过 pip 安装 `apify-client`，并使用 [clickhouse-connect](/zh/integrations/language-clients/python) 连接 ClickHouse。
    </Note>
  </Step>
...
为便于说明，本页中的示例使用了 `default` 用户和数据库。在生产环境中，请创建一个专用用户，并仅授予其向目标表执行 insert 所需的最小权限；同时妥善保管凭证 (例如存放在环境变量或 secrets 管理器中，而不要提交到源代码仓库) 。有关说明，请参阅 [云访问管理](/zh/products/cloud/guides/security/cloud-access-management)。
