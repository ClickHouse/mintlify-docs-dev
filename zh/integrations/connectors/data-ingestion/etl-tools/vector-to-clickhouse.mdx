...
本指南将向你展示如何使用流行的数据管道 [Vector](https://vector.dev/docs/about/what-is-vector/)，对 Nginx 日志文件进行 tail 并将其发送到 ClickHouse。
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/vector_01.png" size="lg" border alt="以表格形式查看 ClickHouse 日志" />
  </Step>
  ...
  <Step>
    ...
    **materialized view** 的功能类似于 SQL 中的插入触发器。当数据行插入源表时，materialized view 会对这些行进行转换，并将结果插入目标表。
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/vector_02.png" size="lg" border alt="以表格形式查看解析后的 ClickHouse 日志" />
