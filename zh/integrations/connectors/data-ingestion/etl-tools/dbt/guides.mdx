...
这些指南应结合其余[文档](/zh/integrations/connectors/data-ingestion/etl-tools/dbt)、[功能和配置](/zh/integrations/connectors/data-ingestion/etl-tools/dbt/features-and-configurations)以及[物化类型参考](/zh/integrations/connectors/data-ingestion/etl-tools/dbt/materializations)一并使用。

<div id="setup">
  ## 设置
</div>

请按照 [dbt 和 ClickHouse 适配器 的设置](/zh/integrations/connectors/data-ingestion/etl-tools/dbt) 部分中的说明准备环境。
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_01.png" size="lg" alt="IMDB 表 schema" />
...
3. 此时，你需要使用自己选择的文本编辑器。在下面的示例中，我们使用常见的 VS Code。打开 IMDB 目录后，你应该会看到一组 yml 和 sql 文件：
   ...
   <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_02.png" size="lg" alt="新的 dbt 项目" />
...
前面的示例创建了一张用于物化模型的表。每次执行 dbt 时，这张表都会被重新构建。对于较大的结果集或复杂的转换，这种做法可能既不现实，成本也极其高昂。为了解决这一问题并缩短构建时间，dbt 提供了增量物化。这使 dbt 能够将自上次执行以来的记录插入或更新到表中，因此非常适合事件型数据。在底层实现上，系统会先创建一张包含所有已更新记录的临时表，然后将所有未变更的记录以及已更新的记录一并插入到新的目标表中。因此，对于大型结果集，它与表模型一样存在类似的[限制](/zh/integrations/connectors/data-ingestion/etl-tools/dbt#limitations)。
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_05.png" size="lg" alt="dbt 增量更新" />

这种策略在非常大的模型上可能会遇到一些挑战。更多细节请参见 [限制](/zh/integrations/connectors/data-ingestion/etl-tools/dbt#limitations)。
...
一直以来，ClickHouse 对更新和删除的支持都比较有限，主要通过异步的[变更](/zh/reference/statements/alter)实现。这类操作可能会产生极高的 IO 开销，因此通常应尽量避免。
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_06.png" size="lg" alt="轻量级删除增量" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_07.png" size="lg" alt="insert overwrite 增量" />
...
dbt 支持从 CSV 文件加载数据。不过，这一功能并不适合加载数据库的大型导出数据，更适用于通常作为代码表和[字典](/zh/concepts/features/dictionaries)的小型文件，例如将国家代码映射为国家名称。下面通过一个简单示例，使用 seed 功能生成并上传一份类型代码列表。
