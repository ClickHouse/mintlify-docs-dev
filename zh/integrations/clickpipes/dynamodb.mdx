...
首先，你需要为 DynamoDB 表启用 Kinesis 数据流，以实时捕获变更。我们希望在创建快照之前先完成这一步，以避免遗漏任何数据。
可在[此处](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/kds.html)查看 AWS 指南。

<Image img="/images/integrations/data-ingestion/dbms/dynamodb/dynamodb-kinesis-stream.png" size="lg" alt="DynamoDB Kinesis Stream" border />

<div id="2-create-the-snapshot">
  ## 2. 创建快照
</div>

接下来，我们将创建该 DynamoDB 表的快照。这可以通过 AWS 导出到 S3 来实现。请参阅[此处](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/S3DataExport.HowItWorks.html)的 AWS 指南。
**你需要选择以 DynamoDB JSON 格式进行“完整导出”。**

<Image img="/images/integrations/data-ingestion/dbms/dynamodb/dynamodb-s3-export.png" size="md" alt="DynamoDB S3 导出" border />
...
现在，您可以创建一个 ClickPipe，将快照数据从 S3 加载到 ClickHouse。请参考[这里](/zh/integrations/clickpipes/object-storage/amazon-s3/overview)的 S3 ClickPipe 指南，但请使用以下设置：
...
* **格式**: JSONEachRow
* **表**: 你的快照表 (例如上述示例中的 `default.snapshot`)
...
现在我们可以设置 Kinesis ClickPipe，以捕获 Kinesis 数据流中的实时变更。请参考[此处](/zh/integrations/clickpipes/kinesis/overview)的 Kinesis ClickPipe 指南，并使用以下设置：
...
<Image img="/images/integrations/data-ingestion/dbms/dynamodb/dynamodb-map-columns.png" size="md" alt="DynamoDB 列映射" border />
