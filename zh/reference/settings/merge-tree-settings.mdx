...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "新设置"}]}]} />

允许使用实验性的 `positions` 参数创建文本索引，该参数会存储标记位置，以支持精确短语匹配。
...
[replicated&#95;deduplication&#95;window](#replicated_deduplication_window) 设置)，包括
`insert_deduplication_version` 粒度 (默认
`new_unified_hash` 下针对整个插入块，legacy 版本下针对每个创建的 part) 。哈希值会写入
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "新设置。将任一序列化后磁盘大小不超过该字节数的跳过索引子流，为每个 parts 打包到单个 skp_idx.packed 归档中；更大的子流则保留独立的 skp_idx_<name>.idx2 \/ .mrk2 布局。该决定会在写入时针对每个子流分别作出。"}]}]} />

低于此阈值的跳过索引子流 (按序列化后的磁盘字节数计算，即经过子流压缩和哈希
链之后) 会为每个 parts 打包到单个 `skp_idx.packed`
归档中，而不是写成单独的 `skp_idx_<name>.idx2` / `.mrk2`
文件。大于此值的子流会保留旧版的按文件布局。该决定会在写入时针对每个子流
独立作出，因此单个 parts 可以同时包含已打包的小型索引
...
ClickHouse Keeper 为最近插入的这些块保存哈希值，
以检查重复项；该设置指定这类块的数量。
...
在向复制表写入时，ClickHouse 会将去重哈希值写入
ClickHouse Keeper。哈希值只会为最近的
`replicated_deduplication_window` 个块保存。最旧的哈希值会从
ClickHouse Keeper 中移除。

在默认的 `insert_deduplication_version = new_unified_hash` 下，哈希值覆盖整个
已插入的块，因此只有当一次 insert 的全部数据与之前的一次
insert (重试) 完全一致时，才会被去重，而不是按单个分片去重。在 legacy 的 `old_separate_hashes` /
`compatible_double_hashes` 下，哈希值则是按创建出的每个分片分别计算，依据是
字段名称与类型的组合，以及该分片中的数据 (字节流) 。

较大的 `replicated_deduplication_window` 值会减慢 `Inserts`，因为需要比较更多
条目。
...
ClickHouse Keeper 为检查重复项而存储哈希值时，所保留的最近异步插入块数量。
...
[异步插入](/zh/reference/settings/session-settings#async_insert) 命令会被缓存为一个或多个块 (parts)。对于[插入去重](/zh/reference/engines/table-engines/mergetree-family/replication)，在写入复制表时，ClickHouse 会将每次插入的哈希值写入 ClickHouse Keeper。哈希值只会为最近的 `replicated_deduplication_window_for_async_inserts` 个块保存，最旧的哈希值会从 ClickHouse Keeper 中移除。
`replicated_deduplication_window_for_async_inserts` 的值越大，`Async Inserts` 就会越慢，因为需要比较更多条目。
哈希值是根据字段名称与类型的组合以及插入的数据 (字节流) 计算得出的。

此设置仅在 `insert_deduplication_version = old_separate_hashes` 或
`compatible_double_hashes` 下适用；在这两种模式下，异步插入的哈希会保存在单独的 `async_blocks`
目录中。而在默认的 `new_unified_hash` 下，异步插入会与同步插入共享统一的
`deduplication_hashes` 目录，此时改由
`replicated_deduplication_window` / `replicated_deduplication_window_seconds` 控制。
...
`replicated_deduplication_window_seconds_for_async_inserts` 指定用于异步插入去重的块哈希值
应保留多长时间。早于 `replicated_deduplication_window_seconds_for_async_inserts` 的哈希值
...
与 `replicated_deduplication_window_for_async_inserts` 一样，这仅在
`insert_deduplication_version = old_separate_hashes` / `compatible_double_hashes` 下适用；在默认的
`new_unified_hash` 下，异步插入使用 `replicated_deduplication_window_seconds`。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "默认启用协调合并"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "0"},{"label": "新增设置"}]}]} />
...
启用在 Keeper 中向 virtual parts 写入属性并提交块
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "用于控制后台清理会在多长时间内仍将非活动副本纳入考虑的新设置（0 表示两个 ZooKeeper 会话超时）"}]}]} />

后台清理
(`OutdatedPartsQuorumThread`) 会在多长时间内仍将非活动副本纳入考虑：如果某个副本处于非活动状态的时间少于该秒数，
它仍会延缓删除过期分区片段以及截断已完成的变更。值为 0
...
用于在各副本之间分配 merge 任务的窗口大小 (按块编号计) 。
同一窗口内的 parts 会被分配给相同的副本。
仅在 ClickHouse Cloud 中可用
...
如果设置为 true，则该表处于只读模式，并且不会对磁盘执行任何修改。

所有会修改表的前台操作都会被拒绝：插入、变更、`OPTIMIZE`，以及会更改数据的分区命令
(`ATTACH`/`MOVE`/`DROP`/`DROP DETACHED`/`FETCH`/`REPLACE PARTITION`，以及以该表为目标的 `MOVE PARTITION ... TO TABLE`) 。对于
不会修改表数据的操作，例如 `FREEZE`/`UNFREEZE` 和 `FORGET PARTITION`，仍然允许执行。

同样也不会调度任何后台工作：常规合并、生存时间 (TTL) 合并 (`DELETE`/`MOVE`/重新压缩) 、重新压缩合并、后台变更，
以及后台分片移动都会被抑制。因此，带有 TTL 的表在启用此设置时将不再回收或移动其过期数据。

始终可以使用 `ALTER TABLE ... MODIFY SETTING table_readonly = 0` (或 `RESET SETTING`) 将此设置切换回去。`ReplicatedMergeTree` 不支持此设置。
