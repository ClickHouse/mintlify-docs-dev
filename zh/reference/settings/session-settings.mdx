...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Delta Lake 写入功能已转为 Beta。"}]}, {"id": "row-2","items": [{"label": "25.9"},{"label": "0"},{"label": "新增设置。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "新增设置，允许将 Iceberg geometry/geography 字段解析为 Geometry 类型。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "新增设置：通过列出 HTTP 索引页，在 `url` 表函数中启用通配符展开。"}]}]} />

允许根据 HTTP 索引页为 `url()` 和 `ENGINE = URL` 进行 Experimental 通配符展开。
...
允许在某些与时间相关的函数中使用非常量时区参数，例如 toTimeZone()、fromUnixTimestamp*()、snowflakeIDToDateTime*()。
此设置仅为兼容性而保留。在 ClickHouse 中，时区是数据类型的属性，更准确地说，也是列的属性。
启用此设置会造成一种错误印象，仿佛同一列中的不同值可以具有不同的时区。
因此，请不要启用此设置。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "新增安全检查：当源表在请求的分区中没有 parts 时，`ALTER TABLE ... REPLACE PARTITION ... FROM ...` 现在会抛出异常（修复了 [#23727](https:\/\/github.com\/ClickHouse\/ClickHouse\/issues\/23727) 中静默数据丢失的问题）。将 `allow_replace_partition_from_empty_source = 1` 后，可保留之前的行为，即静默删除目标分区。"}]}]} />

当源表在该分区中没有 parts 时，允许 `ALTER TABLE ... REPLACE PARTITION ... FROM ...` 静默删除目标分区。

默认情况下不允许这样做：如果源表在请求的分区中没有数据，`REPLACE PARTITION` 会抛出异常，因为在这种情况下，该操作实际上等同于对目标分区执行静默的 `DROP PARTITION` (目标分区的数据会被删除，但不会有任何内容替换它) ，这是意外数据丢失的常见原因 (参见 [#23727](https://github.com/ClickHouse/ClickHouse/issues/23727)) 。

启用此设置可恢复之前的行为，例如当你有意使用空的源分区来清空目标分区中的数据时。若要无条件删除，建议直接改用 `ALTER TABLE ... DROP PARTITION ...`。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "新增设置，用于显示 `system.distributed_cache_registry` 中的 `certificate` 和 `signature` 列。"}]}]} />

仅在 ClickHouse Cloud 中生效。显示 `system.distributed_cache_registry` 表中的 `certificate` 和 `signature` 列。默认情况下，这些列为空，以保持输出简洁；启用此设置即可查看它们。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "新增 Experimental 设置，用于指定为分布式查询租用多少个无状态工作线程。值为 0 时禁用租用。"}]}]} />

用于执行此查询的无状态工作线程数量。值为 0 时，将禁用分布式执行计划的无状态工作线程租用。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "从 26.7 起，`EXPLAIN PLAN` 默认采用 `actions=1, compact=1, pretty=1`。将其设置为 `legacy` 可恢复 26.7 之前的输出。"}]}]} />

`EXPLAIN PLAN` 使用的默认格式。

可能的值：

* `pretty` (自 26.7 起为默认值) — `actions`、`compact` 和 `pretty` 默认均为 `true`，生成紧凑、美观且带有 action 注释的执行计划。
* `legacy` — 26.7 之前的输出。

在 `EXPLAIN` 语句中显式指定 `actions`、`compact` 或 `pretty` 选项 (例如，`EXPLAIN actions = 0, compact = 0, pretty = 0 SELECT ...`) 始终会覆盖此设置。

无论此设置如何，带有 `json = 1` 或 `distributed = 1` 的 `EXPLAIN PLAN` 都会保留 legacy (26.7 之前) 的默认值，除非显式设置了 `actions`、`compact` 或 `pretty`。pretty 输出无法表示 JSON 结果或按分片划分的分布式执行计划，因此这些模式只有以 legacy 形式呈现时才是正确的。
...
<Note>
  `insert_deduplication_token` 按分区跟踪，因此一次插入写入的多个分区可以携带相同的标记。没有标记时，默认的内容校验和 (`insert_deduplication_version = new_unified_hash`) 会基于整个插入块计算，因此只有当某次插入的全部数据与之前某次插入完全匹配时 (即重试) 才会被去重，而不会因为某个单独分区中的行恰好与另一次不同的插入一致而被去重。
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "新增设置：当 `ORDER BY` 是 `GROUP BY` 排序描述的前缀时，将 `LIMIT` 下推到按顺序聚合中，以便在生成足够多的分组后提前终止。"}]}]} />

启用后，当按顺序聚合生效时，会将 `LIMIT` 下推到聚合步骤中，以便在生成足够多的分组后提前结束。当 `ORDER BY` 与 `GROUP BY` 键前缀匹配时，这可以减少读取的数据量。`rows_before_limit_at_least` 报告的值可能会变小；如果需要精确计数，请使用 `exact_rows_before_limit`。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "新增设置，用于限制 `optimize_and_compare_chain` 优化的工作量（以哈希的查询树节点数衡量），避免它在包含大量或超长比较 `AND` 事件链的查询中占据分析过程的主导。此前的值 `0`（无限制）会复现 26.7 之前的行为，即该优化不设上限，因此当 `compatibility` 设为更早版本时，仍会在没有预算限制的情况下推导传递谓词。设为 `0` 可禁用该预算。"}]}]} />

在查询分析期间，为 `optimize_and_compare_chain` 优化设置的工作预算，以 `getTreeHash` 哈希的查询树节点数来衡量 (这是该优化的主要开销) 。一旦某个查询在应用该优化时哈希的节点数超过此值，后续在该查询的剩余部分中将停止应用此优化。这可以限制包含大量或超长比较 `AND` 事件链的查询的分析时间，否则该优化可能会在没有任何折叠收益的情况下主导分析过程。提前停止始终是安全的：这只会放弃一次优化，不会改变结果。设为 `0` 可禁用该预算 (无限制) 。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "23.5"},{"label": "1"},{"label": "允许在执行读取自 file\/url\/s3 等的数据查询时并行处理。这可能会改变行顺序。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "用于启用一项查询计划级别的优化的新设置：当排序键仅引用JOIN中保留侧时，将 ORDER BY ... LIMIT n 下推到 LEFT\/RIGHT JOIN 之前。"}]}]} />

用于切换一项查询计划级别的优化：当排序键仅引用JOIN (LEFT/RIGHT) 中保留侧的列时，将 `ORDER BY ... LIMIT n` 下推到JOIN之前。这会限制保留侧输入在执行 JOIN 前需要产生的行数。
仅当设置 [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) 为 1 时才生效。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "在查询开始前为特定工作负载预留内存的新设置。"}]}]} />

用于工作负载调度。该设置指定在单台 server 上运行查询时预留的最小 RAM 量。预留通过 WORKLOAD 层级结构进行，使用的是查询设置 `workload` 的值。
如果该工作负载没有足够的可用内存，查询将无法启动，并会保持在待处理状态，直到能够满足预留要求。
值为 `0` 表示不预留内存。
仅当已创建 MEMORY RESERVATION resource 时，此设置才会生效。
...
如果不存在已附加的 materialized view，启用此设置后即可读取数据。请注意，读取到的数据通常会从队列中移除。为避免移除已读取的数据，应正确配置相关引擎设置。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "100000"},{"label": "新增设置，用于限制在 `url` 表函数中展开通配符时可读取的目录数量。"}]}]} />

从索引页展开 URL 通配符时可遍历的最大目录数。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "默认使用新的 `toTime` 函数（将值转换为 `Time` 数据类型），而不是旧版 `toTime`（后者仍可通过 `toTimeWithFixedDate` 使用）。"}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "新设置。允许用户使用旧版 toTime 函数逻辑，其行为与 toTimeWithFixedDate 相同。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "文本索引现已正式可用"}]}, {"id": "row-2","items": [{"label": "26.1"},{"label": "0"},{"label": "默认启用"}]}, {"id": "row-3","items": [{"label": "25.9"},{"label": "0"},{"label": "新增设置"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "已全局启用文本索引标记缓存。"}]}, {"id": "row-2","items": [{"label": "26.3"},{"label": "0"},{"label": "新增设置"}]}]} />
