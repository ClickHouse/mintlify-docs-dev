...
针对 `Join` 引擎表的 `ALTER DELETE` 查询是以 [变更](/zh/reference/statements/alter#mutations) 的形式实现的。`DELETE` 变更会读取过滤后的数据，并覆盖内存和磁盘中的数据。
...
`Join` 引擎允许在 `CREATE TABLE` 语句中指定 [join&#95;use&#95;nulls](/zh/reference/settings/session-settings#join_use_nulls) 设置。[SELECT](/zh/reference/statements/select) 查询应使用相同的 `join_use_nulls` 值。
