...
* `rabbitmq_format` – 消息格式。使用与 SQL `FORMAT` 函数相同的记法，例如 `JSONEachRow`。更多信息，请参见 [Formats](/zh/reference/formats) 章节。
...
* `rabbitmq_queue_consume` - 使用用户定义的队列，且不执行任何 RabbitMQ 设置：声明 exchanges、queues、bindings。默认值：`false`。
...
RabbitMQ engine 支持 ClickHouse 支持的所有[格式](/zh/reference/formats)。
