...
`SELECT` 对流式导入并不是特别有用 (调试除外) ，因为每个文件只能导入一次。更实用的做法是使用 [materialized view](/zh/reference/statements/create/view) 创建实时处理链路。为此：
...
有关虚拟列的更多信息，见[此处](/zh/reference/engines/table-engines#table_engines-virtual_columns)。
