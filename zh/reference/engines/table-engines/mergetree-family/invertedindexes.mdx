...
函数 [mapContains](/zh/reference/functions/regular-functions/tuple-map-functions#mapcontains) (`mapContainsKey` 的别名) 用于匹配 map 键中的单个标记。
...
访问 [operator[]](/zh/reference/operators#access-operators) 可与文本索引配合使用，用于过滤键和值。

示例：

```sql
SELECT count() FROM tab WHERE map['engine'] = 'clickhouse'; -- 如果已定义，将使用文本索引
```
