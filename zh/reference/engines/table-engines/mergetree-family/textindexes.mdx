...
**后处理器参数 (可选)**。后处理器是指在分词后应用于每个输出标记的表达式。
...
3. **词干提取**。将每个标记映射为其词干，可以通过匹配共享相同词根的形态变体来提升搜索召回率。
   ...
   ClickHouse 为多种语言提供了内置的 [stem](/zh/reference/functions/regular-functions/string-functions#stem) 函数。
   示例：`stem(str, 'en')`
...
后处理器表达式会将 [String](/zh/reference/data-types/string) 类型的标记转换为相同类型的标记。
此外，后处理器表达式只能引用文本索引所定义的列或表达式本身。
当列的类型为 `Array(String)` 时，后处理器仍然会将单个标记作为普通的 `String` 值进行处理。
