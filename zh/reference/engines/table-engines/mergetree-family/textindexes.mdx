...
函数 [hasToken](/zh/reference/functions/regular-functions/string-search-functions#hasToken)、[hasAllTokens](/zh/reference/functions/regular-functions/string-search-functions#hasAllTokens) 和 [hasAnyTokens](/zh/reference/functions/regular-functions/string-search-functions#hasAnyTokens) 会先使用预处理器转换搜索词，再对其进行分词。
...
要为构建在 [Map](/zh/reference/data-types/map) 类型列上的文本索引定义预处理器，用户需要确定该索引是
基于 map 的键还是值构建的。
...
│↳    `k` UInt64,                                                       ↴│
│↳    `s` String,                                                       ↴│
│↳    INDEX idx s TYPE text(tokenizer = ngrams(2)) GRANULARITY 100000000↴│ <-- 此处
│↳)                                                                     ↴│
│↳ENGINE = MergeTree                                                    ↴│
│↳ORDER BY k                                                            ↴│
...
访问 [operator[]](/zh/reference/operators#access-operators) 可与文本索引配合使用，以过滤键和值。只有在 `mapKeys(map)` 或 `mapValues(map)` 表达式上创建了文本索引，或同时在两者上创建时，才会使用该文本索引。
