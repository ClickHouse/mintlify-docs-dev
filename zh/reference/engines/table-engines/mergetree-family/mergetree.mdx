...
对排序键执行 [ALTER](/zh/reference/statements/alter) 是一种轻量级操作，因为当新列同时添加到表和排序键中时，现有的数据 parts 不需要修改。由于旧排序键是新排序键的前缀，并且新添加的列中还没有数据，因此在修改表的那一刻，数据同时满足按旧排序键和新排序键排序。
...
要定义 `interval`，请使用[时间间隔](/zh/reference/operators#operators-for-working-with-dates-and-times)运算符，例如：
...
* 作为插入 (`INSERT` 查询) 的结果。
* 在后台合并和[变更](/zh/reference/statements/alter#mutations)期间。
* 从另一个副本下载时。
* 作为分区冻结 [ALTER TABLE ... FREEZE PARTITION](/zh/reference/statements/alter/partition#freeze-partition) 的结果。

除变更和分区冻结外，在上述所有情况下，分片 都会根据给定的存储策略存储到某个卷和磁盘上：

1. 选择第一个 (按定义顺序) 有足够磁盘空间存储 分片 (`unreserved_space > current_part_size`) ，并且允许存储该大小 分片 (`max_data_part_size_bytes > current_part_size`) 的卷。
2. 在该卷内，选择紧随上一个用于存储前一个 chunk 数据的磁盘之后的那个磁盘，并且该磁盘的可用空间大于 分片 的大小 (`unreserved_space - keep_free_space_bytes > current_part_size`) 。

在底层实现中，变更和分区冻结会使用[硬链接](https://en.wikipedia.org/wiki/Hard_link)。不同磁盘之间不支持硬链接，因此在这种情况下，生成的 分片 会存储在与原始 分片 相同的磁盘上。

在后台，分片 会根据空闲空间量 (`move_factor` parameter) ，按照各卷在配置文件中声明的顺序，在卷之间移动。
...
用户可以使用查询 [ALTER TABLE ... MOVE PART|PARTITION ... TO VOLUME|DISK ...](/zh/reference/statements/alter/partition) 强制将某个 分片 或分区从一个卷移动到另一个卷或磁盘，同时会考虑后台操作的所有限制条件。该查询会自行发起移动，不会等待后台操作完成。如果可用空间不足，或者任何必需条件未满足，用户将收到错误消息。

移动数据不会干扰数据复制。因此，对于同一张表，可以在不同副本上指定不同的存储策略。

后台合并和变更完成后，旧 分片 只有在经过一定时间 (`old_parts_lifetime`) 之后才会被删除。
在此期间，它们不会被移动到其他卷或磁盘。因此，在这些 分片 最终被删除之前，它们仍会被计入已占用磁盘空间的评估中。

用户可以使用 [min&#95;bytes&#95;to&#95;rebalance&#95;partition&#95;over&#95;jbod](/zh/reference/settings/merge-tree-settings#min_bytes_to_rebalance_partition_over_jbod) setting，将新的大 分片 以均衡方式分配到某个 [JBOD](https://en.wikipedia.org/wiki/Non-RAID_drive_architectures) 卷的不同磁盘上。
...
启用 `use_statistics_for_part_pruning` 后，即可使用统计信息进行数据分片裁剪。
目前，`MinMax` 和 `NullCount` 统计信息支持数据分片裁剪。为某一列定义 MinMax 统计信息后，ClickHouse 会跟踪该列在每个数据分片中的最小值和最大值。为 `Nullable` 列定义 NullCount 统计信息后，ClickHouse 会跟踪每个数据分片中的 NULL 值数量，从而支持基于 `IS NULL` / `IS NOT NULL` 谓词进行裁剪，并提高包含 NULL 值的列在范围过滤裁剪中的准确性。
数据分片裁剪可以在查询过滤条件不可能匹配该分片中的任何行时，跳过读取整个数据分片。

**示例：**

```sql
-- 在 'value' 列上创建带有 MinMax 统计信息的表
...
-- 分批插入数据以创建多个 parts
INSERT INTO test_stats SELECT number, number FROM numbers(1000); -- Part 1：value 范围 [0, 999]
INSERT INTO test_stats SELECT number, number + 10000 FROM numbers(1000); -- Part 2：value 范围 [10000, 10999]

SET use_statistics_for_part_pruning = 1;

-- 此查询将完全跳过 Part 1，因为其最大值 (999) < 5000
SELECT count() FROM test_stats WHERE value > 5000;

-- 使用 EXPLAIN 查看裁剪效果
EXPLAIN indexes = 1 SELECT count() FROM test_stats WHERE value > 5000;
-- 输出将显示 "Parts: 1/2"，表示有一个 分片 被裁剪掉了
```
...
* `NullCount`
  ...
  跟踪 `Nullable` 列中 `NULL` 值的数量。用于在 PREWHERE 优化中准确估算 `IS NULL`/`IS NOT NULL` 谓词的选择性，并可根据是否存在 NULL 进行分片裁剪。
...
|           | 等值过滤 (==) | 范围过滤器 (`>, >=, <, <=`) | `IS NULL` / `IS NOT NULL` |
| --------- | --------- | ---------------------- | ------------------------- |
| CountMin  | ✔         | ✗                      | ✗                         |
| MinMax    | ✗         | ✔                      | ✗                         |
| NullCount | ✗         | ✗                      | ✔                         |
