...
该引擎与 [MergeTree](/zh/reference/engines/table-engines/mergetree-family/versionedcollapsingmergetree) 的不同之处在于，它会删除具有相同[排序键](/zh/reference/engines/table-engines/mergetree-family/mergetree)值 (`ORDER BY` 表子句，而不是 `PRIMARY KEY`) 的重复条目。
