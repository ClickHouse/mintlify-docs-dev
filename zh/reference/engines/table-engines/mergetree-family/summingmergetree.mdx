...
该引擎继承自 [MergeTree](/zh/reference/engines/table-engines/mergetree-family/versionedcollapsingmergetree)。不同之处在于，在合并 `SummingMergeTree` 表的数据分区片段时，ClickHouse 会将所有具有相同主键 (更准确地说，具有相同[排序键](/zh/reference/engines/table-engines/mergetree-family/mergetree)) 的行合并为一行，其中包含数值数据类型列的求和值。如果排序键的组成方式使一个键值对应大量行，这将显著减少存储占用并加快数据查询。
