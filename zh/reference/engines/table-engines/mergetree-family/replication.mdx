...
`INSERT` 和 `ALTER` 查询中的压缩数据会被复制 (更多信息请参见 [ALTER](/zh/reference/statements/alter) 文档) 。
...
ClickHouse 使用 [ClickHouse Keeper](/zh/guides/oss/deployment-and-scaling/keeper) 存储副本元信息。也可以使用 3.4.5 或更高版本的 ZooKeeper，但推荐使用 ClickHouse Keeper。
...
ClickHouse 还支持将副本元信息存储在辅助 ZooKeeper 集群中。为此，请将 ZooKeeper 集群名称和 path 作为 engine 参数传入。
