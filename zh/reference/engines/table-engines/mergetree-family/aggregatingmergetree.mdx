...
该引擎继承自 [MergeTree](/zh/reference/engines/table-engines/mergetree-family/versionedcollapsingmergetree)，并修改了数据分区片段合并的逻辑。ClickHouse 会将所有具有相同主键 (更准确地说，具有相同[排序键](/zh/reference/engines/table-engines/mergetree-family/mergetree)) 的行合并为一行 (在单个数据分区片段内) ，该行存储聚合函数状态的组合。
