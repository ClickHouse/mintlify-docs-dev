...
你可以使用 `(c1, c2, c3)` 指定要插入的列列表。你也可以使用带有列[匹配器](/zh/reference/statements/select#asterisk) (如 `*`) 和/或[修饰符](/zh/reference/statements/select#select-modifiers) (如 [APPLY](/zh/reference/statements/select/apply_modifier)、[EXCEPT](/zh/reference/statements/select/except_modifier)、[REPLACE](/zh/reference/statements/select/replace_modifier)) 的表达式。
...
数据可以通过 ClickHouse 支持的任意一种 [格式](/zh/reference/formats) 传递给 INSERT。必须在查询中显式指定格式：
...
使用上述语法可从存储在**客户端**侧的一个或多个文件中插入数据。`file_name` 和 `type` 是字符串字面量。输入文件的[格式](/zh/reference/formats)必须在 `FORMAT` 子句中设置。
...
可以向由[表函数](/zh/reference/functions/table-functions)引用的表中插入数据。
