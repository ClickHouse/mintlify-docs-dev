...
其中有些计算与聚合函数所能完成的计算类似，但窗口函数不会将多行分组成单个输出，仍会返回各个行。
...
| `percent_rank()`                                                     | ✅ 高效计算某个值在数据集分区中的相对排名。该函数可有效替代更冗长且计算开销更高的手动 SQL 写法，即 `ifNull((rank() OVER(PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER(PARTITION BY x) - 1, 0), 0)` <br />别名：`percentRank()`                                                                                                |
| `cume_dist()`                                                        | ✅ 计算某个值在一组值中的累积分布。返回值小于或等于当前行值的行所占的百分比。                                                                                                                                                                                                                                               |
| `lag/lead(value, offset)`                                            | ✅ <br /> 你也可以使用以下任一替代方案：<br /> 1) `any(value) over (.... rows between <offset> preceding and <offset> preceding)`，或者对 `lead` 使用 `following` <br /> 2) `lagInFrame/leadInFrame`，它们与之类似，但会遵循窗口帧。若要获得与 `lag/lead` 完全相同的行为，请使用 `rows between unbounded preceding and unbounded following` |
| ntile(buckets)                                                       | ✅ <br /> 例如，可按如下方式指定窗口：`(partition by x order by y rows between unbounded preceding and unbounded following)`。                                                                                                                                                                        |



## ClickHouse 特有的窗口函数 {#clickhouse-specific-window-functions}
...
* [`row_number()`](/reference/functions/window-functions/row_number) - 对当前行在其分区内从 1 开始编号。
* [`first_value(x)`](/reference/functions/window-functions/first_value) - 返回其有序窗口帧内计算得到的第一个值。
* [`last_value(x)`](/reference/functions/window-functions/last_value) -    返回其有序窗口帧内计算得到的最后一个值。
* [`nth_value(x, offset)`](/reference/functions/window-functions/nth_value) - 返回其有序窗口帧中第 n 行 (offset) 计算得到的第一个非 NULL 值。
* [`rank()`](/reference/functions/window-functions/rank) - 对当前行在其分区内进行有间隔排名。
* [`dense_rank()`](/reference/functions/window-functions/dense_rank) - 对当前行在其分区内进行无间隔排名。
* [`lagInFrame(x)`](/reference/functions/window-functions/lagInFrame) - 返回其有序窗口帧内当前行之前按指定物理偏移量对应行上计算得到的值。
* [`leadInFrame(x)`](/reference/functions/window-functions/leadInFrame) - 返回其有序窗口帧内当前行之后偏移若干行处计算得到的值。
...
温度以秒级精度存储，但通过使用 `Range` 和 `ORDER BY toDate(ts)`，我们构造了一个大小为 10 个单位的窗口帧；而由于使用了 `toDate(ts)`，这里的单位就是天。
