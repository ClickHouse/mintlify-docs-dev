...
* `x1[, x2, ...]` — 一个或多个待比较的值。所有参数都必须是可比较的类型。[`Any`](/zh/reference/data-types)

**返回值**

返回参数中的最大值，结果会提升为最大的兼容类型。[`Any`](/zh/reference/data-types)
...
* `x1[, x2, ...]` — 要比较的单个值或多个值。所有参数都必须属于可比较的类型。[`Any`](/zh/reference/data-types)

**返回值**

返回参数中的最小值，结果类型提升为最大的兼容类型。[`Any`](/zh/reference/data-types)
...
允许在查询中以更紧凑的方式编写 [`CASE`](/zh/reference/operators#conditional-expression) 运算符。
