...
2. 将 [`user_defined_executable_function_drivers_config`](/zh/reference/settings/server-settings/settings#user_defined_executable_function_drivers_config) 配置为指向一个或多个驱动配置文件 (支持 glob) ，并可选择设置 [`dynamic_user_defined_executable_functions_path`](/zh/reference/settings/server-settings/settings#dynamic_user_defined_executable_functions_path)，即用于存储生成的可执行 UDF 配置的目录：
