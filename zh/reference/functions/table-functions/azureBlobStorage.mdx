...
| `partition_strategy`             | 可选。支持的值：`WILDCARD` 或 `HIVE`。`WILDCARD` 要求路径中包含 `{_partition_id}`，该占位符会被替换为分区键。`HIVE` 不允许使用通配符，假定该路径是表根路径，并生成 Hive 风格的分区目录，以 Snowflake IDs 作为文件名、以文件格式作为扩展名。默认值取决于 `file_like_engine_default_partition_strategy` 设置 (在早于 `26.6` 的 `compatibility` 设置下为 `WILDCARD`，否则为 `HIVE`)。                                                                                 |
...
`WILDCARD`：将文件路径中的 `{_partition_id}` 通配符替换为实际分区键。仅在早于 `26.6` 的 `compatibility` 设置下才默认选用；否则默认值为 `HIVE` (参见 `file_like_engine_default_partition_strategy` 设置) 。
