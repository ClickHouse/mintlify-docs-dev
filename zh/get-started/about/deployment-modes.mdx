...
[chDB](/zh/products/chdb) 是以进程内数据库引擎形式嵌入的 ClickHouse，目前以 Python 作为主要实现，同时也提供 Go、Rust、NodeJS 和 Bun 版本。这种部署方式可将 ClickHouse 强大的 OLAP 能力直接带入你的应用进程内，无需单独安装数据库。

<Image img="/images/deployment-modes/chdb.png" alt="chDB - 嵌入式 ClickHouse" size="sm" />
