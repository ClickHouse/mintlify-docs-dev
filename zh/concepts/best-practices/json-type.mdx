...
尽管这里的 JSON 很复杂，包含嵌套结构，但其结构是可预测的。字段的数量和类型都不会变化。虽然在这个示例中我们可以使用 JSON 类型，但也可以直接用 [Tuples](/zh/reference/data-types/tuple) 和 [Nested](/zh/reference/data-types/nested-data-structures) 类型显式定义其结构：
