...
分区会将 [MergeTree 引擎家族](/zh/reference/engines/table-engines/mergetree-family)中表的[数据分区片段](/zh/concepts/core-concepts/parts)组织成有序的逻辑单元。这种数据组织方式在概念上清晰明确，并可根据时间范围、类别或其他关键属性等特定标准进行划分。这些逻辑单元使数据更易于管理、查询和优化。
...
<Image img="/images/managing-data/core-concepts/merges_with_partitions.png" size="lg" alt="分片合并" />
...
由于该表按 `toStartOfMonth(date)` 进行分区，因此，满足生存时间 (TTL) 条件的整个分区 (即一组[表 parts](/zh/concepts/core-concepts/parts)) 都会被直接删除，从而使清理操作更高效，[无需重写 parts](/zh/reference/statements/alter#mutations)。
...
<Image img="/images/managing-data/core-concepts/partition-pruning.png" size="lg" alt="分片合并 2" />
