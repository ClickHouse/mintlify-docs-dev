...
<Steps>
  ...
  <Step>
    ...
    3. 检查 ClickHouse Keeper 的健康状态
       常见的 [4 字母命令 (4lW) ](/zh/guides/oss/deployment-and-scaling/keeper#four-letter-word-commands) 在未启用 TLS 时无法直接通过 `echo` 使用，以下说明如何用 `openssl` 执行这些命令。
    ...
    * 在 OpenSSL 会话中执行 4LW 命令
    ...
    5. 使用 `https` 接口登录位于 `https://chnode1.marsnet.local:8443/play` 的 Play UI。
       ...
       <Image img="/images/guides/sre/configuring-ssl_01.png" alt="配置 TLS" size="md" border />
