...
实际上，Projection 可以视为原始表中一个额外的隐藏表。Projection 可以采用与原始表不同的行顺序，因此也会拥有
不同于原始表的主索引，还能够自动
以增量方式预先计算聚合值。因此，使用 Projections
可以通过两个“调优旋钮”来加速查询执行：

* **合理使用主索引**
* **预先计算聚合**

在某些方面，Projections 与 [Materialized Views](/zh/concepts/features/materialized-views)
类似，后者同样允许使用多种行顺序，并在
写入时预先计算聚合。
不过，Projections 会自动更新，
...
<Image img="/images/data-modeling/projections_1.png" size="md" alt="ClickHouse 中的 Projections" />
...
<Image img="/images/data-modeling/projections_2.png" size="md" alt="主表 uk_price_paid_with_projections 及其两个投影的可视化" />
...
* [Materialized Views](/zh/concepts/features/materialized-views)
