...
<Note>
  也支持通过 `s3` 类型使用 Google Cloud Storage (GCS)。请参阅[基于 GCS 的 MergeTree](/zh/integrations/connectors/data-ingestion/GCP)。
</Note>
...
并支持 `INSERT` 操作。
不支持[变更](/zh/reference/statements/alter#mutations)和表的复制。
...
[`ALTER TABLE`](/zh/reference/statements/alter)、[`RENAME TABLE`](/zh/reference/statements/rename#rename-table)、
