...
| `settings`             | [ClickHouseSettings](#clickhousesettings)                                                                                           | ClickHouse 服务器 的配置参数。                                                                                                                               | false |               |
...
| `additionalPorts`      | [AdditionalPort](#additionalport) array                                                                                             | AdditionalPorts 声明了要在 ClickHouse Pod（容器组） 和由 Operator 管理的无头 Service 上暴露的额外 TCP 端口。<br />Operator 仅会将这些端口添加到 Kubernetes 资源中，不会配置 ClickHouse 服务器 在这些端口上监听。 | false |               |
