...
使用 [ClickPipes 中的 Postgres CDC 连接器](/zh/integrations/clickpipes/postgres)将你的 Postgres 数据复制到 ClickHouse。该连接器既支持初始加载，也支持持续增量同步，并已在数百家 Enterprise 客户中得到充分验证；这些客户每月迁移的数据量达数百 TB。
