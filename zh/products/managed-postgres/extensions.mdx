...
| 扩展                             | 版本     | 描述                                                    |
| ------------------------------ | ------ | ----------------------------------------------------- |
| `address_standardizer`         | 3.6.1  | 用于将地址解析为各组成部分                                         |
...
| `pg_partman`                   | 5.4.0  | 按时间或 ID 管理分区表的扩展                                      |
| `pg_prewarm`                   | 1.2    | 预热关系数据                                                |
| `pg_re2`                       |        | 基于 Google&#39;s RE2 的、与 ClickHouse 兼容的正则表达式函数         |
...
| `postgis_tiger_geocoder`       | 3.6.1  | PostGIS tiger 地理编码器和反向地理编码器                           |
| `postgis_topology`             | 3.6.1  | PostGIS 拓扑空间类型和函数                                     |
...
| `vector`                       | 0.8.1  | 向量数据类型，以及 ivfflat 和 hnsw 访问方法                         |
...
有关设置说明和使用详情，请参阅 [pg&#95;clickhouse 文档](/zh/integrations/connectors/tools/pg_clickhouse/introduction)。
