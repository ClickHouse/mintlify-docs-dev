...
<Image img="/images/managed-postgres/clickhouse-integration-intro.png" alt="显示侧边栏中集成选项的 ClickHouse 集成页面" size="md" border />
...
<Steps>
  <Step>
    ...
    * **复制方法**：选择以下之一：
    ...
    <Image img="/images/managed-postgres/replication-service-step.png" alt="复制服务配置，显示集成名称、目标服务和复制方法选项" size="md" border />
  ...
  <Step>
    ...
    * **并行快照的表数量**：并发执行快照的表数量 (默认：1)
    ...
    <Image img="/images/managed-postgres/integration-replication-settings.png" alt="复制设置步骤，显示同步间隔、并行线程、拉取批次大小和快照配置选项" size="md" border />
  ...
  <Step>
    ...
    * **在合并期间移除已删除的行**：对于 [ReplacingMergeTree](/zh/reference/engines/table-engines/mergetree-family/replacingmergetree) 表，会在后台合并期间物理删除已删除的行
    ...
    <Image img="/images/managed-postgres/select-tables-step.png" alt="选择表步骤，显示数据库选择、复制选项，以及按 schema 分组的表选择器" size="md" border />
  ...
  <Step>
    ...
    <Image img="/images/managed-postgres/integration-running.png" alt="ClickHouse 集成列表，显示一个处于运行中的 ClickPipe 及其目标服务和状态" size="md" border />
