...
<Image img="/images/cloud/sqlconsole/table-list-and-schema.png" size="md" alt="表列表和 schema" />

还可以展开列表中的表，查看其列和类型。

<Image img="/images/cloud/sqlconsole/view-columns.png" size="md" alt="查看列" />
...
<Image img="/images/cloud/sqlconsole/inspecting-cell-content.png" size="md" alt="检查单元格内容" />
...
<Image img="/images/cloud/sqlconsole/sort-descending-on-column.png" size="md" alt="按列降序排序" />
...
<Image img="/images/cloud/sqlconsole/filter-on-radio-column-equal-gsm.png" size="md" alt="在 radio 列上按等于 GSM 进行过滤" />

设置好过滤器后，你可以选择“Apply”来过滤数据。你还可以像下图所示添加更多过滤器。

<Image img="/images/cloud/sqlconsole/add-more-filters.png" size="md" alt="添加一个大于 2000 的范围过滤器" />
...
<Image img="/images/cloud/sqlconsole/filtering-and-sorting-together.png" size="md" alt="为 range 添加一个大于 2000 的过滤器" />
...
<Image img="/images/cloud/sqlconsole/create-a-query-from-sorts-and-filters.png" size="md" alt="根据排序和过滤器创建查询" />
...
<Image img="/images/cloud/sqlconsole/creating-a-query.png" size="md" alt="创建查询" />
...
<Image img="/images/cloud/sqlconsole/run-selected-query.png" size="md" alt="运行选中的查询" />

运行当前光标位置的命令可以通过以下两种方式实现：

* 在扩展运行选项菜单中选择“在光标处” (或使用对应的 `cmd / ctrl + shift + enter` 键盘快捷键

<Image img="/images/cloud/sqlconsole/run-at-cursor-2.png" size="md" alt="在光标处运行" />

* 在 SQL 编辑器的上下文菜单中选择“在光标处运行”

<Image img="/images/cloud/sqlconsole/run-at-cursor.png" size="md" alt="在光标处运行" />
...
<Image img="/images/cloud/sqlconsole/cancel-a-query.png" size="md" alt="取消查询" />
...
<Image img="/images/cloud/sqlconsole/sql-console-save-query.png" size="md" alt="保存查询" />

或者，你也可以点击工具栏中的“Untitled Query”，修改名称后按 Enter，同时完成查询命名和保存：

<Image img="/images/cloud/sqlconsole/sql-console-rename.png" size="md" alt="重命名查询" />
...
<Image img="/images/cloud/sqlconsole/sql-console-share.png" size="md" alt="共享查询" />

如需为组织中所有有权访问该服务的成员调整查询访问权限，只需调整顶部一行中的访问级别选择器：

<Image img="/images/cloud/sqlconsole/sql-console-edit-access.png" size="md" alt="编辑访问权限" />

应用上述设置后，所有有权访问该服务的 SQL 控制台的团队成员都可以查看 (并执行) 该查询。

如需为特定成员调整查询访问权限，请在“添加团队成员”选择器中选择所需的团队成员：

<Image img="/images/cloud/sqlconsole/sql-console-add-team.png" size="md" alt="添加团队成员" />

选择团队成员后，应会出现一个新的条目，其中包含访问级别选择器：

<Image img="/images/cloud/sqlconsole/sql-console-edit-member.png" size="md" alt="编辑团队成员访问权限" />
...
<Image img="/images/cloud/sqlconsole/sql-console-access-queries.png" size="md" alt="访问查询" />
...
<Image img="/images/cloud/sqlconsole/pagination.png" size="md" alt="分页选项" />

选择页面大小后，结果集会立即启用分页，导航选项会显示在结果面板页脚的中间位置

<Image img="/images/cloud/sqlconsole/pagination-nav.png" size="md" alt="分页导航" />
...
<Image img="/images/cloud/sqlconsole/download-as-csv.png" size="md" alt="下载为 CSV" />
...
<Image img="/images/cloud/sqlconsole/tabular-query-results.png" size="md" alt="表格查询结果" />
...
<Image img="/images/cloud/sqlconsole/switch-from-query-to-chart.png" size="md" alt="从查询切换到图表" />

我们先创建一个简单的条形图，按 `week` 展示 `trip_total`。为此，将 `week` 字段拖到 x 轴上，再将 `trip_total` 字段拖到 y 轴上：

<Image img="/images/cloud/sqlconsole/trip-total-by-week.png" size="md" alt="按周统计的总行程数" />

大多数图表类型都支持在数值轴上放置多个字段。为了演示这一点，我们将 `fare_total` 字段拖到 y 轴上：

<Image img="/images/cloud/sqlconsole/bar-chart.png" size="md" alt="条形图" />
...
<Image img="/images/cloud/sqlconsole/change-from-bar-to-area.png" size="md" alt="将图表从柱状图切换为面积图" />

图表标题与提供数据的查询名称保持一致。更新查询名称后，图表标题也会随之更新：

<Image img="/images/cloud/sqlconsole/update-query-name.png" size="md" alt="更新查询名称" />
...
<Image img="/images/cloud/sqlconsole/update-subtitle-etc.png" size="md" alt="更新副标题等" />
...
<Image img="/images/cloud/sqlconsole/adjust-axis-scale.png" size="md" alt="调整坐标轴刻度" />