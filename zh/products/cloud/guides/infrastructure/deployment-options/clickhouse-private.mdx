...
ClickHouse Private 完全部署在您的环境中，提供我们云原生的计算与存储分离架构。

<br />

<Image img="/images/cloud/reference/private-gov-architecture.png" size="md" alt="ClickHouse Private 架构" background="black" />
...
| 裸机  | Kubernetes                       | AIStor (需要 NVMe)            | 预览版 |

<div id="onboarding-process">
  ## 引导流程
</div>

客户可[联系我们](https://clickhouse.com/company/contact?loc=nav)，申请安排沟通，以评估 ClickHouse Private 是否适合其用例。我们会审核满足最低规模要求且部署在受支持配置上的用例。可提供的引导支持名额有限。安装过程需要遵循对应部署环境的安装指南，并使用从 AWS ECR 下载的镜像和 Helm 图表部署 ClickHouse。