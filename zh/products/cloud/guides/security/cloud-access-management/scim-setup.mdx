...
<Steps>
  <Step>
    ...
    ![在组织设置中进入 SCIM 配置选项卡](/images/cloud/security/scim-okta/scim-okta-01.png)

    点击 `Enable SCIM`。连接 SAML 单点登录后，SCIM 才会解锁；如果该选项显示为灰色，请先完成 SAML 设置。

    ![切换 Enable SCIM](/images/cloud/security/scim-okta/scim-okta-02.png)
  ...
  <Step>
    ...
    ![生成新的 SCIM API 密钥](/images/cloud/security/scim-okta/scim-okta-07.png)
  </Step>
...
<Steps>
  <Step>
    ...
    ![在 Okta 应用设置中将预配模式设为 SCIM](/images/cloud/security/scim-okta/scim-okta-03.png)

    现在，该应用会显示一个 **Provisioning** 选项卡。

    ![应用中现已显示 Provisioning 选项卡](/images/cloud/security/scim-okta/scim-okta-05.png)
  </Step>
  ...
  <Step>
    ...
    * **Authentication Mode** — `Basic Auth`。
    ...
    ![输入 SCIM connector URL，并将唯一标识字段设置为 userName](/images/cloud/security/scim-okta/scim-okta-06.png)

    ![输入用于 SCIM 身份验证的 API 凭据](/images/cloud/security/scim-okta/scim-okta-08.png)

    点击 `Test Connector Configuration`。你应会看到绿色的确认提示。如果失败，请跳转到[故障排查](#troubleshooting)。

    ![测试 SCIM 连接](/images/cloud/security/scim-okta/scim-okta-09.png)
  ...
  <Step>
    ...
    ![为用户启用 SCIM 预配操作](/images/cloud/security/scim-okta/scim-okta-10.png)

    点击 `Save`，然后返回该应用的 **Sign On** / **Provisioning** 选项卡，确认这些设置已生效。

    ![保存预配设置并返回 Sign On 选项卡](/images/cloud/security/scim-okta/scim-okta-10b.png)
  </Step>
  ...
  <Step>
    ...
    **在 Okta 中创建组。** 对于你之前设置的每个角色映射，在 Okta 中创建或确认一个**显示名称完全一致**的组。例如，如果你的映射是 `ClickHouse-Admins → Admin`，就在 Okta 中创建一个名为 `ClickHouse-Admins` 的组。
    ...
    ![在 Okta 中创建一个新组](/images/cloud/security/scim-okta/scim-okta-13.png)

    打开你刚创建的组，然后点击 `Assign people`，将成员添加到该组。

    ![点击为该组分配人员](/images/cloud/security/scim-okta/scim-okta-14.png)

    ![将用户分配到该组](/images/cloud/security/scim-okta/scim-okta-15.png)

    然后将 SCIM 应用也分配给同一个组，这样角色成员关系和应用访问权限就能保持同步。

    ![将应用分配给该组](/images/cloud/security/scim-okta/scim-okta-16.png)
    ...
    **推送这些组。** 在应用的 **Provisioning** 选项卡中，点击 `Push Groups → Find groups by name`，搜索你的组，然后点击 `Save`。对每个角色组重复此操作。预配完成后，每个组的 **Push Status** 都应显示为 **Active (Pushed)**。
    ...
    ![在应用的 Push Groups 选项卡上按名称设置 Group Push](/images/cloud/security/scim-okta/scim-okta-17.png)
...
![在用户和角色中验证用户同步](/images/cloud/security/scim-okta/scim-okta-18.png)
