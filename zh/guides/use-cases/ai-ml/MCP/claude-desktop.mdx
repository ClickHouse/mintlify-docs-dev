...
title: '为 Claude Desktop 配置 ClickHouse MCP 服务器'
description: '本指南介绍如何为 Claude Desktop 配置 ClickHouse MCP 服务器。'
...
<Steps>
  ...
  <Step>
    ...
    要找到该文件，请先进入设置页面 (Mac 上按 `Cmd+,`) ，然后点击左侧菜单中的 `Developer` 选项卡。
    随后你会看到如下界面，需要点击 `Edit config` 按钮：

    <Image img="/images/use-cases/AI_ML/MCP/claude-desktop-config.png" alt="Claude Desktop 配置" size="md" />
  ...
  <Step>
    ...
    <Image img="/images/use-cases/AI_ML/MCP/find-mcp-servers.gif" alt="查找 MCP 服务器" size="md" />
    ...
    <Image img="/images/use-cases/AI_ML/MCP/mcp-permission.png" alt="授予使用 list_databases 工具的权限" size="md" />

    下面是部分对话内容，其中包含一些对 ClickHouse MCP 服务器的工具调用：

    <Image img="/images/use-cases/AI_ML/MCP/claude-conversation.png" alt="Claude 对话" size="md" />
  </Step>
</Steps>