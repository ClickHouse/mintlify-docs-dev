...
<Info>
  **Server、local 或 chDB**
  ...
  本指南中的步骤可以通过现有的 ClickHouse server 安装来执行。对于即席查询，你也可以改用 [clickhouse-local](/zh/concepts/features/tools-and-utilities/clickhouse-local)，无需运行 server 即可完成相同的工作流。稍作调整后，也可以使用 ClickHouse 的进程内发行版 [chDB](/zh/products/chdb) 完成这一过程。
</Info>
