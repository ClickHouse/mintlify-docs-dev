...
[Materialized views](/zh/concepts/features/materialized-views) 提供了对日志和链路追踪应用 SQL 过滤与转换的更强大方式。
...
<Image img="/images/use-cases/observability/observability-12.png" alt="可观测性与字典" size="md" />

这在多种场景下都很实用，例如在不拖慢摄取过程的情况下实时富化已摄取的数据，以及整体提升查询性能，其中对 JOIN 的加速尤为明显。
虽然在可观测性场景中很少需要使用 JOIN，但字典在富集方面仍然很有用——无论是在插入时还是查询时。下面我们会分别给出这两种示例。

<Info>
  **加速 JOIN**

  如果你希望使用字典来加速 JOIN，可以在[这里](/zh/concepts/features/dictionaries)了解更多详情。
</Info>
...
如需查看简单的富集示例，请参阅[此处](/zh/concepts/features/dictionaries)的字典指南。下面我们将重点介绍常见的可观测性富集任务。
...
* [高级字典主题](/zh/concepts/features/dictionaries#advanced-dictionary-topics)
...
<Image img="/images/use-cases/observability/observability-13.png" alt="可观测性与投影" size="md" />
