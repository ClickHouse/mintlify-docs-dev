...
[Nested 类型](/zh/reference/data-types/nested-data-structures)可用于对很少变动的静态对象进行建模，作为 `Tuple` 和 `Array(Tuple)` 的替代方案。我们通常建议避免将这种类型用于 JSON，因为它的行为往往容易令人困惑。`Nested` 的主要优势在于，子列可用于排序键。
