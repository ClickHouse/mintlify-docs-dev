...
[动态列选择](/zh/reference/statements/select#dynamic-column-selection) 是 ClickHouse 一项强大但尚未得到充分利用的功能。它允许你使用正则表达式选择列，而无需逐一指定每个列名。你还可以使用 `APPLY` 修饰符对匹配的列应用函数，这使它在数据分析和转换任务中特别实用。
...
我们也可以使用 [`APPLY`](/zh/reference/statements/select) 修饰符，对每一列应用函数。
...
到目前为止都没问题。不过，假设我们想调整其中一个值，同时保持其他值不变。例如，我们可能想将总金额翻倍，并将 MTA 税除以 1.1。我们可以使用 [`REPLACE`](/zh/reference/statements/select) 修饰符来实现：它会替换某一列，同时保持其他列不变。
...
我们也可以使用 [`EXCEPT`](/zh/reference/statements/select) 修饰符来排除某一列。例如，要去掉 `tolls_amount` 列，可以编写如下查询：
