...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/use-cases/observability/clickstack-cloud-v2.png" size="lg" alt="启动 ClickStack" border />

    在 **搜索** 视图中，将数据源切换为 `Logs`，并将时间范围设置为 **最近 15 分钟**。CloudWatch 事件应会在几个轮询间隔内出现。

    <Image img="/images/clickstack/cloudwatch/log-search-view-clickstack.png" size="lg" alt="显示 CloudWatch 日志的 ClickStack 搜索视图" />
    ...
    * `Body`：原始日志行
    ...
    <Image img="/images/clickstack/cloudwatch/log-search-attributes-clickstack.png" size="lg" alt="带有 CloudWatch 日志和属性的 ClickStack 搜索视图" />

    选择一条日志条目以查看其元数据：

    <Image img="/images/clickstack/cloudwatch/error-log-column-values-clickstack.png" size="lg" alt="日志详细视图中的 CloudWatch 属性" />
  ...
  <Step>
    ...
    <TrackedLink href={'/zh/examples/cloudwatch-logs-dashboard.json'} download="cloudwatch-logs-dashboard.json" eventName="docs.cloudwatch_logs_monitoring.dashboard_download">下载 `cloudwatch-logs-dashboard.json`</TrackedLink>，然后在 ClickStack UI 中进入 **仪表盘**，点击 **导入**。

    <Image img="/images/clickstack/clickstack-import-dashboard.png" size="lg" alt="导入仪表盘按钮" />

    上传 JSON 文件，然后点击 **完成导入**。

    <Image img="/images/clickstack/cloudwatch/finish-clickstack-import.png" size="lg" alt="完成导入对话框" />
  </Step>
