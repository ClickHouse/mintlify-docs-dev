...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/getting-started/hackernews_main.png" alt="在本地运行的 HackerNews Analyzer 应用" />
  ...
  <Step>
    ## 启动 ClickStack 并探索你的遥测数据

    在 [ClickHouse Cloud 控制台](https://console.clickhouse.cloud) 中打开你的服务，从左侧菜单选择 **ClickStack**，然后点击 **开始摄取**。

    <Image img="/images/use-cases/observability/clickstack-cloud-first-time.png" size="lg" alt="启动 ClickStack" border />
    ...
    <Image img="/images/use-cases/observability/clickstack-start-ingestion.png" size="lg" alt="ClickStack 开始摄取" border />

    ClickStack 应该会自动检测你的表和遥测数据，让你可以继续。选择 **开始探索**，开始查看你的数据。

    <Image img="/images/use-cases/observability/clickstack-start-exploring.png" size="lg" alt="ClickStack 开始探索" border />

    现在来查看应用程序正在生成的数据：

    1. 前往 **搜索**，将时间范围过滤为最近 5 分钟。你会看到 `hn-analyzer-api` 的日志持续流入。

    <Image img="/images/clickstack/getting-started/instrument_app_clickstack_logs.png" alt="ClickStack 日志" />
    ...
    <Image img="/images/clickstack/getting-started/instrument_app_clickstack_traces.png" alt="ClickStack 链路追踪" />

    3. 打开 **Session Replay**，回放浏览器会话视频，并可拖动时间轴查看；它会与 trace 时间线保持同步。

    <Image img="/images/clickstack/getting-started/instrument_app_clickstack_sessions.png" alt="ClickStack 会话" />
...
* [监控 Kubernetes](/zh/clickstack/managed-onboarding/monitoring-kubernetes)：从集群收集日志、基础设施指标和 Kubernetes 事件。
...
* [调优托管 ClickStack](/zh/clickstack/managed-onboarding/tuning-clickstack-schema)：优化 schema，以提升查询性能和存储效率。
* [保护 collector](/zh/clickstack/ingesting-data/collector#securing-the-collector)：为 OTLP 端点启用 TLS，并使用最小权限的摄取用户。
* [进入生产环境](/zh/clickstack/managing/production)：查看生产环境部署建议。