...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/use-cases/observability/clickstack-search-with-k8-logs.png" size="lg" alt="带有 Kubernetes 日志的 ClickStack 搜索视图" />

    若要结合上下文查看基础设施指标和事件，请通过 **仪表盘** -&gt; **Kubernetes** 打开内置的 **Kubernetes** 仪表盘。`Pods`、`Nodes` 和 `Namespaces` 选项卡中都应显示数据。

    <Image img="/images/use-cases/observability/clickstack-dashboard-kubernetes.png" size="lg" alt="ClickStack Kubernetes 仪表盘" border />
