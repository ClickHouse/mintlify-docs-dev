...
<Tabs>
  <Tab title="Cloud 用户和角色">
    <Image img="/images/clickstack/rbac/team-page-cloud.png" alt="ClickHouse Cloud 用户和角色页面" size="lg" />
  </Tab>

  <Tab title="ClickStack 团队设置">
    <Image img="/images/clickstack/rbac/team-page-clickstack.png" alt="显示团队成员及其角色的 ClickStack 团队设置页面" size="lg" />
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/rbac/add-role-modal.png" alt="添加角色模态框" size="md" />
  </Step>
</Steps>
...
<Image img="/images/clickstack/rbac/default-vs-fine-grained.png" alt="角色编辑器中的默认访问与细粒度控制模式" size="md" />
...
<Image img="/images/clickstack/rbac/condition-tip.png" alt="条件提示：可按名称或标签（显示在标题中），或按 ID（可在 URL 中找到）匹配资源" size="md" />
...
<Image img="/images/clickstack/rbac/dashboard-id-and-tag-example.png" alt="仪表盘中显示了 URL 栏里的资源 ID，以及右上角的一个标签" size="lg" />

你可以为每种资源类型添加多条规则。系统会基于 OR 逻辑分别检查每条规则——只要资源匹配**任意**一条规则，即可访问。不匹配任何规则的资源则无法访问。

<Image img="/images/clickstack/rbac/access-rules-tip.png" alt="使用 OR 逻辑的访问规则提示" size="md" />
...
<Image img="/images/clickstack/rbac/dashboard-fine-grained-example.png" alt="两条通过 OR 连接的细粒度访问规则：名称包含 testing 且访问级别为 Read，以及标签为 testing 且访问级别为 Read" size="md" />
...
<Image img="/images/clickstack/rbac/security-policies.png" alt="安全策略" size="lg" />