...
<Steps>
  ...
  <Step>
    ...
    在下面的示例中，我们绘制按服务名称区分的平均请求耗时随时间变化的图表。这要求用户指定一个指标、一个列 (可以是 SQL 表达式) 以及一个聚合字段。
...
<Steps>
  <Step>
    ...
    <Image img="/images/use-cases/observability/hyperdx-dashboard-1.png" alt="创建仪表盘" size="lg" />
  </Step>
  ...
  <Step>
    ...
    * 别名：`Average Time`
    ...
    <Image img="/images/use-cases/observability/hyperdx-dashboard-2.png" alt="创建仪表板可视化" size="lg" />

    调整可视化大小，使其占满仪表板的整个宽度。

    <Image img="/images/use-cases/observability/hyperdx-dashboard-3.png" alt="包含可视化内容的仪表板" size="lg" />
  </Step>
  ...
  <Step>
    ...
    * 别名：`Count of events`
    ...
    <Image img="/images/use-cases/observability/hyperdx-dashboard-4.png" alt="仪表板可视化 2" size="lg" />

    调整可视化大小，使其占满整个仪表板宽度。

    <Image img="/images/use-cases/observability/hyperdx-dashboard-5.png" alt="带可视化内容的仪表板 2" size="lg" />
  </Step>
  ...
  <Step>
    ...
    * **Count**：`count()`
    ...
    <Image img="/images/clickstack/dashboards/heatmap-tile-editor.png" alt="已预填 span 耗时默认值、ServiceName payment 过滤条件和 Display Settings 按钮的热力图卡片编辑器" size="lg" />

    保存后的卡片会在仪表盘上渲染为热力图。将鼠标悬停在任意单元上，即可查看 bucket 边界和事件数。

    <Image img="/images/clickstack/dashboards/heatmap-tile-rendered.png" alt="显示 payment 服务在 24 小时内 span 耗时分布的热力图仪表盘卡片" size="lg" />
    ...
    <Image img="/images/clickstack/dashboards/heatmap-tile-drilldown.png" alt="点击热力图单元后显示 View in Event Deltas 操作" size="lg" />
  ...
  <Step>
    ...
    <Image img="/images/use-cases/observability/hyperdx-dashboard-filter.png" alt="带筛选条件的仪表板" size="lg" />
    ...
    <Image img="/images/use-cases/observability/hyperdx-dashboard-save.png" alt="保存仪表板" size="lg" />
  </Step>
</Steps>
...
<Image img="/images/use-cases/observability/hyperdx-dashboard-edit.png" alt="编辑仪表盘" size="lg" />
...
<Image img="/images/use-cases/observability/hyperdx-dashboard-search.png" alt="仪表盘搜索" size="lg" />
...
<Image img="/images/clickstack/dashboards/filter-dropdown.png" alt="显示可用服务名称的服务下拉过滤器" size="lg" />
...
<Steps>
  <Step>
    ...
    <Image img="/images/clickstack/dashboards/edit-filters.png" alt="仪表盘工具栏中的 Edit Filters 按钮" size="lg" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/dashboards/add-filter.png" alt="包含 Name、Data source 和 Filter expression 字段的 Add filter 对话框" size="md" />

    Filters 模态框会显示该仪表盘上已配置的所有过滤器。你可以在这里编辑或删除现有过滤器，也可以添加更多过滤器。

    <Image img="/images/clickstack/dashboards/saved-filters.png" alt="显示已配置 Services 过滤器的 Filters 模态框" size="md" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/dashboards/filtered-dashboard.png" alt="已按 frontend 服务过滤的仪表盘" size="lg" />
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/dashboards/save-filter-values.png" alt="显示 Save Query and Filters as Default 选项的仪表盘菜单" size="lg" />
  </Step>
</Steps>
...
<Image img="/images/clickstack/dashboards/drilldown.png" alt="显示“查看所有事件”和“按组筛选”选项的下钻上下文菜单" size="lg" />
...
此仪表板提供用于监控 ClickHouse 的可视化视图。要进入此仪表板，请从左侧菜单中选择。

<Image img="/images/use-cases/observability/hyperdx-dashboard-clickhouse.png" alt="ClickHouse 仪表盘" size="lg" />

此仪表板使用选项卡分别展示 **Selects**、**Inserts** 和 **ClickHouse 基础设施** 的监控信息。

<Info>
  **所需的系统表访问权限**

  此仪表板会查询 ClickHouse [系统表](/zh/reference/system-tables) 以显示关键指标。需要以下授权：
