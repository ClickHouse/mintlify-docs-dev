...
<Steps>
  ...
  <Step>
    ...
    * `P95 Duration`：`Duration` 的 95 分位数。将该列的数字格式设置为耗时格式，这样它会显示为 `288ms`，而不是原始纳秒值。
    ...
    <Image img="/images/clickstack/dashboards/row-click-catalog.png" alt="服务清单表，为每个服务显示 RED 列（Requests、Errors、P95 Duration），按请求量排序，上方还有请求随时间变化的趋势图" size="lg" />
  ...
  <Step>
    ...
    <Image img="/images/clickstack/dashboards/row-click-drilldown.png" alt="在点击某一行后显示的 Service Detail 仪表盘，其中 Service 过滤器已设置为被点击的服务，RED 关键指标、延迟百分位数以及各端点明细都已限定到该服务" size="lg" />
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/dashboards/row-click-search-drilldown.png" alt="点击行后，搜索页面会在链路追踪数据源上打开，并过滤到被点击的服务" size="lg" />
  </Step>
</Steps>
...
<Image img="/images/clickstack/dashboards/row-click-drawer.png" alt="在 Dashboard 模式下的 Row Click Action 抽屉面板中，Service Detail 仪表盘被选为目标端，并配置了从点击行中模板化出的 ServiceName 过滤器" size="lg" />
