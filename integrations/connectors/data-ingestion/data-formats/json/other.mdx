...
As noted earlier, for the [original person object](/integrations/connectors/data-ingestion/data-formats/json/schema#static-vs-dynamic-json), we can't ensure the structure of the `tags` column. We insert the original row (including `company.labels`, which we ignore for now), declaring the `Tags` column as a `String`:
...
Suppose we wish to count the number of papers released by year. Contrast the following query using only a string versus the [structured version](/integrations/connectors/data-ingestion/data-formats/json/inference#creating-tables) of the schema:
...
The [Nested type](/reference/data-types/nested-data-structures) can be used to model static objects which are rarely subject to change, offering an alternative to `Tuple` and `Array(Tuple)`. We generally recommend avoiding using this type for JSON as its behavior is often confusing. The primary benefit of `Nested` is that sub-columns can be used in ordering keys.
