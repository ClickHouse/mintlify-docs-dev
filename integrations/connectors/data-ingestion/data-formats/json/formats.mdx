...
In cases when we're dealing with [nested JSON objects](/assets/list-nested.json), we can additionally define an explicit schema and use complex types ([`Array`](/reference/data-types/array), [`JSON`](/integrations/connectors/data-ingestion/data-formats/json/intro) or [`Tuple`](/reference/data-types/tuple)) to load data:
