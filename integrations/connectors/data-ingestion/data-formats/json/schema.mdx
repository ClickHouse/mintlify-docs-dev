...
While [schema inference](/integrations/connectors/data-ingestion/data-formats/json/inference) can be used to establish an initial schema for JSON data and query JSON data files in place, e.g., in S3, you should aim to establish an optimized versioned schema for your data. We discuss the recommended approach for modeling JSON structures below.
...
1. **Primitive types** - If the key's value is a primitive type, irrespective of whether it is part of a sub-object or on the root, ensure you select its type according to general schema [design best practices](/guides/cloud-oss/data-modelling/schema-design) and [type optimization rules](/guides/cloud-oss/data-modelling/schema-design#optimizing-types). Arrays of primitives, such as `phone_numbers` below, can be modeled as `Array(<type>)` e.g., `Array(String)`.
...
Consider our [earlier person JSON](/integrations/connectors/data-ingestion/data-formats/json/schema#static-vs-dynamic-json) where the `company.labels` object was determined to be dynamic.
...
Type hints offer more than just a way to avoid unnecessary type inference - they eliminate storage and processing indirection entirely, as well as allowing [optimal primitive types](/guides/cloud-oss/data-modelling/schema-design#optimizing-types) to be specified. JSON paths with type hints are always stored just like traditional columns, bypassing the need for [**discriminator columns**](https://clickhouse.com/blog/a-new-powerful-json-data-type-for-clickhouse#storage-extension-for-dynamically-changing-data) or dynamic resolution during query time. 
...
<Image img="/static/images/integrations/data-ingestion/data-formats/json_column_per_type.png" size="md" alt="Column per JSON path" />
...
<Image img="/static/images/integrations/data-ingestion/data-formats/json_offsets.png" size="md" alt="JSON offsets" />
...
<Image img="/static/images/integrations/data-ingestion/data-formats/json_shared_column.png" size="md" alt="Shared JSON column" />
