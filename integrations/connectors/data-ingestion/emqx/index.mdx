...
## Connecting EMQX <a id="connecting-emqx"></a>
...
## Get your ClickHouse Cloud service <a id="get-your-clickhouse-cloudservice"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_1.png" size="sm" border alt="ClickHouse Cloud Service Deployment interface showing AWS region selection" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_2.png" size="sm" border alt="ClickHouse Cloud Connection Settings showing IP access configuration" />

Then you need to save your username and password for future use.

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_3.png" size="sm" border alt="ClickHouse Cloud Credentials screen showing username and password" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_4.png" size="lg" border alt="ClickHouse Cloud Running Instance dashboard with connection options" />

Click "Connect to SQL Console" to create database and table for integration with EMQX Cloud.

<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_5.png" size="lg" border alt="ClickHouse Cloud SQL Console interface" />
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_cloud_6.png" size="lg" border alt="ClickHouse Cloud Create Database and Table SQL query execution" />

## Create an MQTT service on EMQX Cloud <a id="create-an-mqtt-service-on-emqx-cloud"></a>

Creating a dedicated MQTT broker on EMQX Cloud is as easy as a few clicks.

### Get an account <a id="get-an-account"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_sign_up.png" size="lg" border alt="EMQX Cloud Signup Page with registration form" />

### Create an MQTT cluster <a id="create-an-mqtt-cluster"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_create_1.png" size="lg" border alt="EMQX Cloud Create Deployment Step 1 showing deployment options" />
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_overview.png" size="lg" border alt="EMQX Cloud Overview Dashboard showing broker metrics" />

### Add client credential <a id="add-client-credential"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_auth.png" size="lg" border alt="EMQX Cloud Authentication Setup interface for adding credentials" />

Click 'Confirm' and now we have a fully managed MQTT broker ready.

### Enable NAT gateway <a id="enable-nat-gateway"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/emqx_cloud_nat_gateway.png" size="lg" border alt="EMQX Cloud NAT Gateway Configuration panel" />
...
## Integration EMQX Cloud with ClickHouse Cloud <a id="integration-emqx-cloud-with-clickhouse-cloud"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_clickhouse.png" size="lg" border alt="EMQX Cloud ClickHouse Data Integration connector details" />

### Create ClickHouse resource <a id="create-clickhouse-resource"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_resource.png" size="lg" border alt="EMQX Cloud ClickHouse Resource Setup form with connection details" />

### Create a new rule <a id="create-a-new-rule"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_1.png" size="md" border alt="EMQX Cloud Data Integration Rule Creation Step 1 showing SQL query" />

You can use the SQL test to test and see the results.

<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_2.png" size="md" border alt="EMQX Cloud Data Integration Rule Creation Step 2 showing test results" />

Now click on the "NEXT" button. This step is to tell EMQX Cloud how to insert refined data into your ClickHouse database.

### Add a response action <a id="add-a-response-action"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_rule_action.png" size="md" border alt="EMQX Cloud Data Integration Rule Action Setup with SQL template" />

This is a template for inserting data into Clickhouse, you can see the variables are used here.

### View rules details <a id="view-rules-details"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/data_integration_details.png" size="md" border alt="EMQX Cloud Data Integration Rule Details showing configuration summary" />

All the MQTT messages sent to the `temp_hum/emqx` topic will be persisted into your ClickHouse Cloud database.

## Saving Data into ClickHouse <a id="saving-data-into-clickhouse"></a>
...
### Publish MQTT messages to EMQX Cloud <a id="publish-mqtt-messages-to-emqx-cloud"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-overview.png" size="lg" border alt="MQTTX Overview showing the client interface" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-new.png" size="lg" border alt="MQTTX New Connection Setup form with connection details" />
...
<Image img="/images/integrations/data-ingestion/emqx/mqttx-publish.png" size="lg" border alt="MQTTX Publish MQTT Messages interface showing message composition" />

### View rules monitoring <a id="view-rules-monitoring"></a>

Check the rule monitoring and add one to the number of success.

<Image img="/images/integrations/data-ingestion/emqx/rule_monitor.png" size="lg" border alt="EMQX Cloud Rule Monitoring dashboard showing message processing metrics" />

### Check the data persisted <a id="check-the-data-persisted"></a>
...
<Image img="/images/integrations/data-ingestion/emqx/clickhouse_result.png" size="lg" border alt="ClickHouse Query Results showing persisted IoT data" />
