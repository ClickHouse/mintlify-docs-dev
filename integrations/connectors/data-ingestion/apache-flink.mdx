...
You can find the list of available ClickHouse input formats in [this documentation page](/reference/formats#formats-overview) and [ClickHouseFormat.java](https://github.com/ClickHouse/clickhouse-java/blob/main/clickhouse-data/src/main/java/com/clickhouse/data/ClickHouseFormat.java).
...
- We recommend setting `maxBatchSize` to at least 1000 and ideally between 10,000 to 100,000. See [this guide on bulk inserts](/concepts/operations/insert/bulkinserts) for more information.
- To do OLTP-style deduplication or upsert to ClickHouse, refer to [this documentation page](/concepts/operations/insert/deduplication#options-for-deduplication). _Note: this is not to be confused with batch deduplication that happens on retries._
