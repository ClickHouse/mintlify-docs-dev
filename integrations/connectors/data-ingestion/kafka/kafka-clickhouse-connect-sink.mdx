...
| `keeperOnCluster`                               | Allows configuration of ON CLUSTER parameter for self-hosted instances (e.g. `ON CLUSTER clusterNameInConfigFileDefinition`) for exactly-once connect_state table (see [Distributed DDL Queries](/core/reference/statements/distributed-ddl)   | `""`                                                     |
...
A record is converted into JSON and sent to ClickHouse as a value in [JSONEachRow](/core/reference/formats/JSON/JSONEachRow) format.
...
From CH version 26.1 onwards, this schema is supported when `allow_experimental_nullable_tuple_type=1` (see [this documentation page](/core/reference/settings/session-settings#allow_experimental_nullable_tuple_type)).
...
The connector supports the String Converter in different ClickHouse formats: [JSON](/core/reference/formats/JSON/JSONEachRow), [CSV](/core/reference/formats/CSV/CSV), and [TSV](/core/reference/formats/TabSeparated/TabSeparated).
