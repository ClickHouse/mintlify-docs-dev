...
* Deployment and management via ClickHouse Cloud UI, [Open API](/products/cloud/features/admin-features/api/api-overview), or [Terraform](https://registry.terraform.io/providers/ClickHouse/clickhouse/3.3.3-alpha2/docs/resources/clickpipe)
...
* [**Confluent Cloud**](/integrations/connectors/data-ingestion/kafka/confluent) - Confluent Platform provides an option to upload and [run ClickHouse Connector Sink on Confluent Cloud](/integrations/connectors/data-ingestion/kafka/confluent/custom-connector) or use [HTTP Sink Connector for Confluent Platform](/integrations/connectors/data-ingestion/kafka/confluent/kafka-connect-http) that integrates Apache Kafka with an API via HTTP or HTTPS.
...
* **Custom code** - Custom code using Kafka and ClickHouse [client libraries](/integrations/language-clients) may be appropriate in cases where custom processing of events is required.
