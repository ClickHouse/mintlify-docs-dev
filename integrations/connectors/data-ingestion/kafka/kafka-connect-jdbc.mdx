...
* `batch.size` - The number of rows to send in a single batch. Ensure this set is to an appropriately large number. Per ClickHouse [recommendations](/core/reference/statements/insert-into#performance-considerations) a value of 1000 should be considered a minimum.
