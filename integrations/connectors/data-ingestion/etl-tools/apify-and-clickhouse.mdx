...
<Note>
Apify also provides a [Python client](https://docs.apify.com/api/client/python). If you prefer Python, install `apify-client` via pip and use [clickhouse-connect](/integrations/language-clients/python) for ClickHouse.
</Note>
</Step>
<Step>
### Create a target table in ClickHouse {#4-create-a-target-table}

Create a table to hold the scraped data. The schema depends on the Actor you use. This example uses [MergeTree](/core/reference/engines/table-engines/mergetree-family/mergetree) for a product scraping Actor:
...
Use [`JSONEachRow`](/core/reference/formats/JSON/JSONEachRow) format when inserting into ClickHouse. It maps directly to Apify's JSON output with no transformation needed.
