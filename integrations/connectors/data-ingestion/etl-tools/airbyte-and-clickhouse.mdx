...
<Steps>
<Step>
## Download and run Airbyte <a id="1-download-and-run-airbyte"></a>
...
4. Once you see the Airbyte banner in your terminal, you can connect to <a href="http://localhost:8000" target="_blank">localhost:8000</a>
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_01.png" size="lg" border alt="Airbyte banner" />
...
<Step>
## Add ClickHouse as a destination <a id="2-add-clickhouse-as-a-destination"></a>
...
2. Within Airbyte, select the "Destinations" page and add a new destination:
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_02.png" size="lg" border alt="Add a destination in Airbyte" />
...
3. Select ClickHouse from the "Destination type" drop-down list, and Fill out the "Set up the destination" form by providing your ClickHouse hostname and ports, database name, username and password and select if it's an SSL connection (equivalent to the `--secure` flag in the  `clickhouse-client`):
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_03.png" size="lg" border alt="ClickHouse destination creation in Airbyte" />
...
<Step>
## Add a dataset as a source <a id="3-add-a-dataset-as-a-source"></a>
...
1. Within Airbyte, select the "Sources" page and add a new source of type file.
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_04.png" size="lg" border alt="Add a source in Airbyte" />
...
2. Fill out the "Set up the source" form by naming the source and providing the URL of the NYC Taxi Jan 2022 file (see below). Make sure to pick `parquet` as file format, `HTTPS Public Web` as Storage Provider and `nyc_taxi_2022` as Dataset Name.
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/airbyte_05.png" size="lg" border alt="ClickHouse source creation in Airbyte" />

3. Congratulations! You have now added a source file in Airbyte.
</Step>
<Step>
## Create a connection and load the dataset into ClickHouse <a id="4-create-a-connection-and-load-the-dataset-into-clickhouse"></a>

1. Within Airbyte, select the "Connections" page and add a new connection

<Image img="/images/integrations/data-ingestion/etl-tools/airbyte_06.png" size="lg" border alt="Add a connection in Airbyte" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/airbyte_07.png" size="lg" border alt="Connection creation in Airbyte" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/airbyte_08.png" size="lg" border alt="Sync now in Airbyte" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/airbyte_09.png" size="lg" border alt="Completed successfully" />
