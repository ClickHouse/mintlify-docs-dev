...
## How the target table is managed <a id="target-table-management"></a>
...
## Materialization with implicit target <a id="implicit-target"></a>
...
### Multiple materialized views <a id="multiple-materialized-views"></a>
...
### How to iterate the target table schema <a id="how-to-iterate-the-target-table-schema"></a>
...
### Data catch-up <a id="data-catch-up"></a>
...
## Materialization with explicit target (Beta) <a id="explicit-target"></a>
...
### Benefits <a id="explicit-target-benefits"></a>
...
### Limitations <a id="explicit-target-limitations"></a>
...
### Usage <a id="explicit-target-usage"></a>
...
### Configuration options <a id="explicit-target-configuration"></a>
...
### Common operations <a id="explicit-target-common-operations"></a>

#### Full refresh with explicit targets <a id="explicit-target-full-refresh"></a>
...
#### Changing the target table <a id="explicit-target-changing"></a>
...
### Troubleshooting common issues <a id="explicit-target-troubleshooting"></a>

#### Target table is empty while/after `run` is executed <a id="target-table-empty"></a>
...
#### `dbt run --full-refresh` in a target table with `repopulate_from_mvs_on_full_refresh=True` uses the logic from old materialized view versions, not from the SQL that is currently in the project <a id="full-refresh-with-repopulate-from-mvs-on-full-refresh"></a>
...
#### There's duplicate data after a run is executed <a id="duplicate-data"></a>
Possible reasons:
...
#### Data loss during active ingestion after a `dbt run --full-refresh` is executed <a id="data-loss-active-ingestion"></a>
...
#### Check the current target of an MV in ClickHouse <a id="check-mv-target"></a>
...
#### Check if dbt recognizes a table as a materialized view target <a id="check-dbt-recognition"></a>
...
### Migrating from implicit to explicit target <a id="migration-implicit-to-explicit"></a>
...
## Behavior comparison between implicit and explicit target approaches <a id="behavior-comparison"></a>

### How they behave in general <a id="general-behavior"></a>
...
### Behavior during active ingestion <a id="behavior-during-active-ingestion"></a>
...
#### Implicit target operations <a id="ingestion-implicit-target"></a>
...
#### Explicit target operations <a id="ingestion-explicit-target"></a>
...
## Refreshable Materialized Views <a id="refreshable-materialized-views"></a>
...
### Example with implicit target <a id="refreshable-implicit-example"></a>
...
### Example with explicit target <a id="refreshable-explicit-example"></a>
...
### Limitations <a id="refreshable-limitations"></a>
