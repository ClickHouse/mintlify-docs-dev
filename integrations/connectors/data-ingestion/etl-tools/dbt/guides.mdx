...
These guides are designed to be used in conjunction with the rest of the [documentation](/integrations/connectors/data-ingestion/etl-tools/dbt), the [features and configurations](/integrations/connectors/data-ingestion/etl-tools/dbt/features-and-configurations) and the [materializations reference](/integrations/connectors/data-ingestion/etl-tools/dbt/materializations).

## Setup {#setup}

Follow the instructions in the [Setup of dbt and the ClickHouse adapter](/integrations/connectors/data-ingestion/etl-tools/dbt) section to prepare your environment.
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_01.png" size="lg" alt="IMDB table schema" />
...
3. At this point, you will need the text editor of your choice. In the examples below, we use the popular VS Code. Opening the IMDB directory, you should see a collection of yml and sql files:
    ...
    <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_02.png" size="lg" alt="New dbt project" />

4. Update your `dbt_project.yml` file to specify our first model - `actor_summary` and set profile to `clickhouse_imdb`.

    <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_03.png" size="lg" alt="dbt profile" />

    <Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_04.png" size="lg" alt="dbt profile" />
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_05.png" size="lg" alt="incremental updates dbt" />

This strategy may encounter challenges on very large models. For further details see [Limitations](/integrations/connectors/data-ingestion/etl-tools/dbt#limitations).
...
Historically ClickHouse has had only limited support for updates and deletes, in the form of asynchronous [Mutations](/reference/statements/alter). These can be extremely IO-intensive and should generally be avoided.

ClickHouse 22.8 introduced [lightweight deletes](/snippets/delete) and ClickHouse 25.7 introduced [lightweight updates](/reference/statements/update). With the introduction of these features, modifications from single update queries, even when being materialized asynchronously, will occur instantly from the user's perspective.
...
<Image img="/images/integrations/data-ingestion/etl-tools/dbt/dbt_06.png" size="lg" alt="lightweight delete incremental" />
...
dbt provides the ability to load data from CSV files. This capability isn't suited to loading large exports of a database and is more designed for small files typically used for code tables and [dictionaries](/concepts/features/dictionaries), e.g. mapping country codes to country names. For a simple example, we generate and then upload a list of genre codes using the seed functionality.
