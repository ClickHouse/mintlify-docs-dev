...
Historically ClickHouse has had only limited support for updates and deletes, in the form of asynchronous [Mutations](/core/reference/statements/alter). These can be extremely IO-intensive and should generally be avoided.
...
dbt provides the ability to load data from CSV files. This capability isn't suited to loading large exports of a database and is more designed for small files typically used for code tables and [dictionaries](/core/concepts/features/dictionaries), e.g. mapping country codes to country names. For a simple example, we generate and then upload a list of genre codes using the seed functionality.
