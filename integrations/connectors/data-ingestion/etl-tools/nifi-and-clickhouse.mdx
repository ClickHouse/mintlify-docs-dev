...
1. To configure a Controller Service in Apache NiFi, visit the NiFi Flow Configuration page by clicking on the "gear" button
    ...
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_01.png" size="sm" border alt="NiFi Flow Configuration page with gear button highlighted" />

2. Select the Controller Services tab and add a new Controller Service by clicking on the `+` button at the top right

    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_02.png" size="lg" border alt="Controller Services tab with add button highlighted" />

3. Search for `DBCPConnectionPool` and click on the "Add" button

    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_03.png" size="lg" border alt="Controller Service selection dialog with DBCPConnectionPool highlighted" />
...
4. The newly added `DBCPConnectionPool` will be in an Invalid state by default. Click on the "gear" button to start configuring
    ...
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_04.png" size="lg" border alt="Controller Services list showing invalid DBCPConnectionPool with gear button highlighted" />
...
6. In the Settings section, change the name of the Controller Service to "ClickHouse JDBC" for easy reference
    ...
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_05.png" size="lg" border alt="DBCPConnectionPool configuration dialog showing properties filled in" />

7. Activate the `DBCPConnectionPool` Controller Service by clicking on the "lightning" button and then the "Enable" button

    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_06.png" size="lg" border alt="Controller Services list with lightning button highlighted" />

    <br/>

    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_07.png" size="lg" border alt="Enable Controller Service confirmation dialog" />

8. Check the Controller Services tab and ensure that the Controller Service is enabled

    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_08.png" size="lg" border alt="Controller Services list showing enabled ClickHouse JDBC service" />
...
1. Add an ​`​ExecuteSQL` processor, along with the appropriate upstream and downstream processors
    ...
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_09.png" size="md" border alt="NiFi canvas showing ExecuteSQL processor in a workflow" />
...
3. Start the `​​ExecuteSQL` processor
    ...
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_10.png" size="lg" border alt="ExecuteSQL processor configuration with properties filled in" />

4. To confirm that the query has been processed successfully, inspect one of the `FlowFile` in the output queue

    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_11.png" size="lg" border alt="List queue dialog showing flowfiles ready for inspection" />

5. Switch view to "formatted" to view the result of the output `FlowFile`

    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_12.png" size="lg" border alt="FlowFile content viewer showing query results in formatted view" />
...
3. To confirm that multiple records are merged into one, examine the input and output of the `MergeRecord` processor. Note that the output is an array of multiple input records
    ...
    Input
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_13.png" size="sm" border alt="MergeRecord processor input showing single records" />

    Output
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_14.png" size="sm" border alt="MergeRecord processor output showing merged array of records" />
...
4. To confirm that each insert contains multiple rows, check that the row count in the table is incrementing by at least the value of "Minimum Number of Records" defined in `MergeRecord`.
    ...
    <Image img="/static/images/integrations/data-ingestion/etl-tools/nifi_15.png" size="sm" border alt="Query results showing row count in the destination table" />
