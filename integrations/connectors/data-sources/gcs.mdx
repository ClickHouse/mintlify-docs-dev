...
## GCS backed MergeTree <a id="gcs-backed-mergetree"></a>
...
#### Storage configuration > disks > gcs <a id="storage_configuration--disks--gcs"></a>
...
```
#### Storage configuration > disks > cache <a id="storage_configuration--disks--cache"></a>
...
```
#### Storage configuration > policies > gcs_main <a id="storage_configuration--policies--gcs_main"></a>
...
## Using Google Cloud Storage (GCS) <a id="gcs-multi-region"></a>
...
### Prepare virtual machines <a id="prepare-vms"></a>
...
### Create two buckets <a id="create-two-buckets"></a>
...
#### Remote ClickHouse Keeper servers <a id="remote-clickhouse-keeper-servers"></a>
...
#### Remote ClickHouse servers <a id="remote-clickhouse-servers"></a>
...
#### Replica identification <a id="replica-identification"></a>
...
#### Storage in GCS <a id="storage-in-gcs"></a>
...
### Start ClickHouse Keeper <a id="start-clickhouse-keeper"></a>
...
#### Verify disk configuration <a id="verify-disk-configuration"></a>
...
```
#### Verify that tables created on the cluster are created on both nodes <a id="verify-that-tables-created-on-the-cluster-are-created-on-both-nodes"></a>
```sql highlight={1,18}
...
#### Verify that data can be inserted <a id="verify-that-data-can-be-inserted"></a>
...
#### Verify that the storage policy `gcs_main` is used for the table. <a id="verify-that-the-storage-policy-gcs_main-is-used-for-the-table"></a>
```sql
SELECT
    engine,
    data_paths,
...
#### Verify in Google Cloud console <a id="verify-in-google-cloud-console"></a>
...
#### Bucket for replica one <a id="bucket-for-replica-one"></a>

<Image img="/images/integrations/data-ingestion/s3/GCS-examine-bucket-1.png" size="lg" border alt="Replica one bucket in Google Cloud Storage showing folder structure with data partitions" />

#### Bucket for replica two <a id="bucket-for-replica-two"></a>

<Image img="/images/integrations/data-ingestion/s3/GCS-examine-bucket-2.png" size="lg" border alt="Replica two bucket in Google Cloud Storage showing folder structure with data partitions" />

<!-- anchors from _GCS_authentication_and_bucket.mdx -->
<a id="ch_bucket_us_east1"></a>
<a id="ch_bucket_us_east4"></a>
<a id="generate-an-access-key"></a>
<a id="create-a-service-account-hmac-key-and-secret"></a>
<a id="add-a-new-service-account"></a>
<!-- anchors from _GCS_authentication_and_bucket.mdx -->
<a id="ch_bucket_us_east1"></a>
<a id="ch_bucket_us_east4"></a>
<a id="generate-an-access-key"></a>
