...
* Uses [`RowBinary`](/core/reference/formats/RowBinary/RowBinary) format over the HTTP transport.
  * There are plans to switch to [`Native`](/core/reference/formats/Native) over TCP.
...
You can apply various [ClickHouse settings](/core/reference/settings/session-settings) using the `with_option` method. For example:
