...
- It is aliased to [`toTimeWithFixedDate`](/core/reference/functions/regular-functions/date-time-functions#toTimeWithFixedDate).
...
- If a date is passed as a string in a format that includes a timezone, then a conversion function is needed. Usually [`parseDateTimeBestEffort`](/core/reference/functions/regular-functions/type-conversion-functions#parseDateTimeBestEffort) is used.
