...
| compress                 | bool or str | True                          | Enable compression for ClickHouse HTTP inserts and query results. See [Additional Options (Compression)](/integrations/language-clients/python/reference/additional-options#compression)                                                                                                           |
...
| apply_server_timezone    | bool        | True                          | Use server timezone for timezone aware query results. See [Timezone Precedence](/integrations/language-clients/python/reference/advanced-querying#time-zones)                                                                                                                                      |
...
| external_data | ExternalData     | *None*     | An `ExternalData` object containing file or binary data to use with the query. See [Advanced Queries (External Data)](/integrations/language-clients/python/reference/advanced-querying#external-data)     |
...
| external_data       | ExternalData     | *None*     | An ExternalData object containing file or binary data to use with the query. See [Advanced Queries (External Data)](/integrations/language-clients/python/reference/advanced-querying#external-data)                            |
| context             | QueryContext     | *None*     | A reusable QueryContext object can be used to encapsulate the above method arguments. See [Advanced Queries (QueryContexts)](/integrations/language-clients/python/reference/advanced-querying#querycontexts)                   |
| transport_settings  | dict             | *None*     | Optional dictionary of HTTP headers to include with this request. Each key-value pair is added as an HTTP header (e.g., `{'X-Custom-Header': 'value'}`). Useful for proxy authentication, request tracing, or passing headers required by intermediate infrastructure. |
...
The complete details of streaming query results (using StreamContext objects) are outlined in [Advanced Queries (Streaming Queries)](/integrations/language-clients/python/reference/advanced-querying#streaming-queries).

## Consuming query results with NumPy, Pandas or Arrow {#consuming-query-results-with-numpy-pandas-or-arrow}

ClickHouse Connect provides specialized query methods for NumPy, Pandas, and Arrow data formats. For detailed information on using these methods, including examples, streaming capabilities, and advanced type handling, see [Advanced Querying (NumPy, Pandas and Arrow Queries)](/integrations/language-clients/python/reference/advanced-querying#numpy-pandas-and-arrow-queries).

## Client streaming query methods {#client-streaming-query-methods}

For streaming large result sets, ClickHouse Connect provides multiple streaming methods. See [Advanced Queries (Streaming Queries)](/integrations/language-clients/python/reference/advanced-querying#streaming-queries) for details and examples.
...
| settings           | dict                              | *None*     | See [settings description](#settings-argument).                                                                                                                                               |
| context            | InsertContext                     | *None*     | A reusable InsertContext object can be used to encapsulate the above method arguments. See [Advanced Inserts (InsertContexts)](/integrations/language-clients/python/reference/advanced-inserting#insertcontexts)                          |
| transport_settings | dict                              | *None*     | Optional dictionary of HTTP headers to include with this request. Each key-value pair is added as an HTTP header (e.g., `{'X-Custom-Header': 'value'}`). Useful for proxy authentication, request tracing, or passing headers required by intermediate infrastructure. |
...
For specialized insert methods that work with Pandas DataFrames, PyArrow Tables, and Arrow-backed DataFrames, see [Advanced Inserting (Specialized Insert Methods)](/integrations/language-clients/python/reference/advanced-inserting#specialized-insert-methods).
...
For inserting data directly from files into ClickHouse tables, see [Advanced Inserting (File Inserts)](/integrations/language-clients/python/reference/advanced-inserting#file-inserts).

## Raw API {#raw-api}

For advanced use cases requiring direct access to ClickHouse HTTP interfaces without type transformations, see [Advanced Usage (Raw API)](/integrations/language-clients/python/reference/advanced-usage#raw-api).
...
For information on using ClickHouse Connect in multithreaded, multiprocess, and async/event-driven applications, see [Advanced Usage (Multithreaded, multiprocess, and async/event driven use cases)](/integrations/language-clients/python/reference/advanced-usage#multithreaded-multiprocess-and-asyncevent-driven-use-cases).

## AsyncClient wrapper {#asyncclient-wrapper}

For information on using the AsyncClient wrapper for asyncio environments, see [Advanced Usage (AsyncClient wrapper)](/integrations/language-clients/python/reference/advanced-usage#asyncclient-wrapper).

## Managing ClickHouse Session IDs {#managing-clickhouse-session-ids}

For information on managing ClickHouse session IDs in multi-threaded or concurrent applications, see [Advanced Usage (Managing ClickHouse Session IDs)](/integrations/language-clients/python/reference/advanced-usage#managing-clickhouse-session-ids).

## Customizing the HTTP connection pool {#customizing-the-http-connection-pool}

For information on customizing the HTTP connection pool for large multi-threaded applications, see [Advanced Usage (Customizing the HTTP connection pool)](/integrations/language-clients/python/reference/advanced-usage#customizing-the-http-connection-pool).
