...
| parameters    | dict or iterable | *None*     | See [parameters description](/integrations/language-clients/python/reference/driver-api#parameters-argument).                                                                                        |
| settings      | dict             | *None*     | See [settings description](/integrations/language-clients/python/reference/driver-api#settings-argument).                                                                                            |
...
Because each query or insert executed maintains state in its own `QueryContext` or `InsertContext` object, respectively, these helper objects aren't thread-safe, and they shouldn't be shared between multiple processing streams. See the additional discussion about context objects in the [QueryContexts](/integrations/language-clients/python/reference/advanced-querying#querycontexts) and [InsertContexts](/integrations/language-clients/python/reference/advanced-inserting#insertcontexts) sections.
