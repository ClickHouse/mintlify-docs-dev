...
description: 'Release notes for the ClickHouse JS client.'
keywords: ['clickhouse', 'js', 'JavaScript', 'NodeJS', 'release notes', 'changelog']
noindex: true
---

<Update label="2026-01-07" description="1.16.0">
## New features
...
<Update label="2026-01-07" description="1.15.0">
## New features
...
<Update label="2025-11-20" description="1.14.0">
## New features
...
<Update label="2025-11-12" description="1.13.0">
## New features
...
<Update label="2025-08-15" description="1.12.1">
## Improvements
...
<Update label="2025-07-24" description="1.12.0">
## Types
...
<Update label="2025-06-06" description="1.11.2">
...
- Added a new configuration option: `capture_enhanced_stack_trace`; see [the JS doc](https://github.com/ClickHouse/clickhouse-js/blob/1.11.2/packages/client-node/src/config.ts#L45-L54) in the Node.js client package. Note that it is disabled by default due to a possible performance impact. ([#427])
...
<Update label="2025-04-17" description="1.11.1">
## Bug fixes
...
- (Node.js only) Added support for streaming on 32-bit platforms. ([#403](https://github.com/ClickHouse/clickhouse-js/pull/403), [shevchenkonik](https://github.com/shevchenkonik))
</Update>

<Update label="2025-03-17" description="1.11.0">
## New features

- It is now possible to provide custom HTTP headers when calling the `query`/`insert`/`command`/`exec` methods using the `http_headers` option. NB: `http_headers` specified this way will override `http_headers` set on the client instance level. ([#394](https://github.com/ClickHouse/clickhouse-js/issues/394), [@DylanRJohnston](https://github.com/DylanRJohnston))
...
<Update label="2025-01-16" description="1.10.1">
## Bug fixes
...
<Update label="2024-12-17" description="1.10.0">
## New features

- Added support for JWT authentication (ClickHouse Cloud feature) in both Node.js and Web API packages ([#270](https://github.com/ClickHouse/clickhouse-js/issues/270)). JWT token can be set via `access_token` client configuration option.
...
<Update label="2024-12-10" description="1.9.1">
...
<Update label="2024-11-29" description="1.9.0">
## New features
...
<Update label="2024-11-14" description="1.8.1">
## Bug fixes
...
<Update label="2024-11-07" description="1.8.0">
## New features

- Added support for specifying roles via request query parameters. See [this example](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/role.ts) for more details. ([@pulpdrew](https://github.com/pulpdrew), [#328](https://github.com/ClickHouse/clickhouse-js/pull/328))
</Update>

<Update label="2024-10-18" description="1.7.0">
## Bug fixes
...
- Added `JSONEachRowWithProgress` format support, `ProgressRow` interface, and `isProgressRow` type guard. See [this Node.js example](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_json_each_row_with_progress.ts) for more details. It should work similarly with the Web version.
...
<Update label="2024-09-12" description="1.6.0">
## New features
...
- (Node.js) Fixed unhandled exceptions produced when calling `ResultSet.json` if the response data was not, in fact, a valid JSON. ([#311](https://github.com/ClickHouse/clickhouse-js/pull/311))
</Update>

<Update label="2024-08-22" description="1.5.0">
## New features

- (Node.js only) It is now possible to disable the automatic decompression of the response stream with the `exec` method. See `ExecParams.decompress_response_stream` for more details. ([#298](https://github.com/ClickHouse/clickhouse-js/issues/298)).
</Update>

<Update label="2024-08-07" description="1.4.1">
## Improvements
...
<Update label="2024-07-08" description="1.3.0">
## New features
...
<Update label="2024-06-19" description="1.2.0">
## New features
...
- (Experimental) Added an option to provide a custom HTTP Agent in the client configuration via the `http_agent` option ([#283](https://github.com/ClickHouse/clickhouse-js/issues/283), related: [#278](https://github.com/ClickHouse/clickhouse-js/issues/278)). The following conditions apply if a custom HTTP Agent is provided:
  - The `max_open_connections` and `tls` options will have _no effect_ and will be ignored by the client, as those are part of the underlying HTTP Agent configuration.
...
See the [doc entry](/integrations/language-clients/javascript/usage#custom-httphttps-agent-experimental-nodejs-only) regarding custom HTTP(s) Agent usage with code samples.

If you have feedback on these experimental features, please let us know by creating [an issue](https://github.com/ClickHouse/clickhouse-js/issues) in the repository or send a message in the [Community Slack](https://clickhouse.com/slack) (`#clickhouse-js` channel).
</Update>

<Update label="2024-06-06" description="1.1.0">
## New features
...
<Update label="2024-05-20" description="1.0.2">
## Bug fixes
...
- (Node.js) Removed an unnecessary error log in the `ResultSet.stream` method if the request was aborted or the result set was closed ([#263](https://github.com/ClickHouse/clickhouse-js/issues/263)).
...
<Update label="2024-04-02" description="1.0.1">
A hotfix for both Node.js and Web packages.
See the [1.0.0](https://github.com/ClickHouse/clickhouse-js/releases/tag/1.0.0) entry that contains the entire 1.0.x changelog. 
...
<Update label="2024-04-02" description="1.0.0">
...
If you are interested in more details, see the [related test](https://github.com/ClickHouse/clickhouse-js/blob/main/packages/client-node/__tests__/integration/node_query_format_types.test.ts) (featuring a great ESLint plugin [expect-types](https://github.com/JoshuaKGoldberg/eslint-plugin-expect-type)) in the client package.
...
See also: [URL configuration example](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/url_configuration.ts).
...
- Added `http_headers` configuration parameter as a direct replacement for `additional_headers`. Functionally, it is the same, and the change is purely cosmetic, as we'd like to leave an option to implement a TCP connection open in the future.
</Update>

<Update label="2024-03-26" description="0.3.1">
### Bug fixes

- Fixed an issue where query parameters containing tabs or newline characters were not encoded properly (#249).
</Update>

<Update label="2024-03-18" description="0.3.0">
...
- The client ensures proper `Connection` header value considering Keep-Alive settings. If Keep-Alive is disabled, its value is now forced to ["close"](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Connection#close).
</Update>

<Update label="2024-02-20" description="0.2.10">
### New features
...
<Update label="2024-01-19" description="0.2.9">
### New features

- It is now possible to set additional HTTP headers for outgoing ClickHouse requests. This might be useful if, for example, you have a reverse proxy with authorization. ([@teawithfruit](https://github.com/teawithfruit))
...
<Update label="2024-01-12" description="0.2.8">
### New features
...
- [Including specific columns or excluding certain ones instead](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/insert_exclude_columns.ts)
- [Leveraging this feature](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/insert_ephemeral_columns.ts) when working with [ephemeral columns](/reference/statements/create/table#ephemeral) ([#217](https://github.com/ClickHouse/clickhouse-js/issues/217))
</Update>

<Update label="2023-12-13" description="0.2.7">
### New features

- (Node.js only) `X-ClickHouse-Summary` response header is now parsed when working with `insert`/`exec`/`command` methods. See the [related test](https://github.com/ClickHouse/clickhouse-js/blob/main/packages/client-node/__tests__/integration/node_summary.test.ts) for more details.
NB: it is guaranteed to be correct only for non-streaming scenarios. 
The web version does not currently support this due to CORS limitations. ([#210](https://github.com/ClickHouse/clickhouse-js/issues/210))
...
<Update label="2023-11-16" description="0.2.6">
### New features

- Added [Parquet format](/integrations/connectors/data-ingestion/data-formats/parquet) streaming support to the Node.js client. Examples: [insert from a file](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_file_stream_parquet.ts), [select into a file](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_parquet_as_file.ts).
</Update>

<Update label="2023-10-30" description="0.2.5">
### Bug fixes
...
<Update label="2023-10-17" description="0.2.4">
...
<Update label="2023-10-11" description="0.2.3">
No changes in web/common modules.
...
<Update label="2023-09-02" description="0.2.2">
### New features

- Added missing `default_format` setting, which allows to perform `exec` calls without the `FORMAT` clause. See [the example](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/default_format_setting.ts).
</Update>

<Update label="2023-08-10" description="0.2.1">
### Breaking changes
...
<Update label="2023-07-26" description="0.2.0">
Introduces web client (using native [fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) and [WebStream](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) APIs) without Node.js modules in the common interfaces. 
...
<Update label="2023-07-19" description="0.2.0-beta1">
Introduces browser client (using native [fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) and [WebStream](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) APIs) with no Node.js modules in the common interfaces. 
No polyfills are required.
...
<Update label="2023-07-03" description="0.1.1">
## New features
...
<Update label="2023-06-22" description="0.1.0">
## Breaking changes
...
<Update label="2023-05-17" description="0.0.16">
### Breaking changes
* Node.js 14 [EOL](https://endoflife.date/nodejs) as its maintenance phase has ended in April 2023. Node.js 16+ is now required to use the client.

### Bug fixes
* Fix NULL parameter binding. As the HTTP interface expects `\N` instead of a `'NULL'` string, it is now correctly handled for both `null` and _explicitly_ `undefined` parameters. See the [test scenarios](https://github.com/ClickHouse/clickhouse-js/blob/f1500e188600d85ddd5ee7d2a80846071c8cf23e/__tests__/integration/select_query_binding.test.ts#L273-L303) for more details.
</Update>

<Update label="2023-04-26" description="0.0.15">
### Bug fixes
* Fix Node.JS 19.x/20.x timeout error (@olexiyb)
</Update>

<Update label="2023-03-22" description="0.0.14">
### New features
* Added support for `JSONStrings`, `JSONCompact`, `JSONCompactStrings`, `JSONColumnsWithMetadata` formats (@andrewzolotukhin).
</Update>

<Update label="2023-03-14" description="0.0.13">
### New features
* `query_id` can now be overridden for all main client's methods: `query`, `exec`, `insert`.
</Update>

<Update label="2023-02-01" description="0.0.12">
### New features
...
<Update label="2022-12-08" description="0.0.11">
### Breaking changes
...
<Update label="2022-11-14" description="0.0.10">
Remove request listeners synchronously (#124) - closed issue #123
</Update>

<Update label="2022-10-25" description="0.0.9">
See #121 
