...
If something is unclear or missing from the examples or from the following documentation, feel free to [contact us](/docs/language-clients/javascript/usage#contact-us).
...
| **pathname**?: string                                                    | An optional pathname to add to the ClickHouse URL after it is parsed by the client. | `''`                    | [Proxy with a pathname docs](/docs/language-clients/javascript/usage#proxy-with-a-pathname)                                |
| **request_timeout**?: number                                             | The request timeout in milliseconds.                                                | `30_000`                | -                                                                                          |
| **compression**?: `{ **response**?: boolean; **request**?: boolean }`    | Enable compression.                                                                 | -                       | [Compression docs](/docs/language-clients/javascript/usage#compression)                                                    |
...
| **log**?: `{ **LoggerClass**?: Logger, **level**?: ClickHouseLogLevel }` | Internal client logs configuration.                                                 | -                       | [Logging docs](/docs/language-clients/javascript/usage#logging-nodejs-only)                                                |
...
| **http_headers**?: `Record<string, string>`                              | Additional HTTP headers for outgoing ClickHouse requests.                           | -                       | [Reverse proxy with authentication docs](/docs/language-clients/javascript/usage#reverse-proxy-with-authentication)        |
...
| **tls**?: `{ **ca_cert**: Buffer, **cert**?: Buffer, **key**?: Buffer }`                              | Configure TLS certificates.                                 | -             | [TLS docs](/docs/language-clients/javascript/usage#tls-certificates-nodejs-only)                                                     |
| **keep_alive**?: `{ **enabled**?: boolean, **idle_socket_ttl**?: number }`                            | -                                                           | -             | [Keep Alive docs](/docs/language-clients/javascript/usage#keep-alive-configuration-nodejs-only)                                      |
| **http_agent**?: http.Agent \| https.Agent| Custom HTTP agent for the client.                           | -             | [HTTP agent docs](/docs/language-clients/javascript/usage#custom-httphttps-agent-experimental-nodejs-only)                           |
| **set_basic_auth_header**?: boolean                                       | Set the `Authorization` header with basic auth credentials. | `true`        | [this setting usage in the HTTP agent docs](/docs/language-clients/javascript/usage#custom-httphttps-agent-experimental-nodejs-only) |
...
See also: [Keep-Alive configuration](/docs/language-clients/javascript/usage#keep-alive-configuration-nodejs-only).
...
Please refer to the list of the [supported data formats](/docs/language-clients/javascript/usage#supported-data-formats) to determine what the best format is for streaming in your case. For example, if you want to stream JSON objects, you could choose [JSONEachRow](/interfaces/formats/JSONEachRow), and each row will be parsed as a JS object, or, perhaps, a more compact [JSONCompactColumns](/interfaces/formats/JSONCompactColumns) format that will result in each row being a compact array of values. See also: [streaming files](/docs/language-clients/javascript/usage#streaming-files-nodejs-only).
...
It can work with either a `Stream.Readable` or a plain `Array<T>`, depending on the [data format](/docs/language-clients/javascript/usage#supported-data-formats) specified to the `insert` method. See also this section about the [file streaming](/docs/language-clients/javascript/usage#streaming-files-nodejs-only).
...
[Source code](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_file_stream_csv.ts). See also: [file streaming](/docs/language-clients/javascript/usage#streaming-files-nodejs-only).
