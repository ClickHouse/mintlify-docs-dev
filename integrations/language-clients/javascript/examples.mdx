...
If something is unclear or missing from the examples or from the following documentation, feel free to [contact us](/integrations/language-clients/javascript/usage#contact-us).
...
| **pathname**?: string                                                    | An optional pathname to add to the ClickHouse URL after it is parsed by the client. | `''`                    | [Proxy with a pathname docs](/integrations/language-clients/javascript/usage#proxy-with-a-pathname)                                |
| **request_timeout**?: number                                             | The request timeout in milliseconds.                                                | `30_000`                | -                                                                                          |
| **compression**?: `{ **response**?: boolean; **request**?: boolean }`    | Enable compression.                                                                 | -                       | [Compression docs](/integrations/language-clients/javascript/usage#compression)                                                    |
...
| **log**?: `{ **LoggerClass**?: Logger, **level**?: ClickHouseLogLevel }` | Internal client logs configuration.                                                 | -                       | [Logging docs](/integrations/language-clients/javascript/usage#logging-nodejs-only)                                                |
...
| **http_headers**?: `Record<string, string>`                              | Additional HTTP headers for outgoing ClickHouse requests.                           | -                       | [Reverse proxy with authentication docs](/integrations/language-clients/javascript/usage#reverse-proxy-with-authentication)        |
| **roles**?: string \|  string[]                                          | ClickHouse role name(s) to attach to the outgoing requests.                         | -                       | [Using roles with the HTTP interface](/integrations/connectors/data-integrations/drivers-and-interfaces/http#setting-role-with-query-parameters) |
...
| **tls**?: `{ **ca_cert**: Buffer, **cert**?: Buffer, **key**?: Buffer }`                              | Configure TLS certificates.                                 | -             | [TLS docs](/integrations/language-clients/javascript/usage#tls-certificates-nodejs-only)                                                     |
| **keep_alive**?: `{ **enabled**?: boolean, **idle_socket_ttl**?: number }`                            | -                                                           | -             | [Keep Alive docs](/integrations/language-clients/javascript/usage#keep-alive-configuration-nodejs-only)                                      |
| **http_agent**?: http.Agent \| https.Agent| Custom HTTP agent for the client.                           | -             | [HTTP agent docs](/integrations/language-clients/javascript/usage#custom-httphttps-agent-experimental-nodejs-only)                           |
| **set_basic_auth_header**?: boolean                                       | Set the `Authorization` header with basic auth credentials. | `true`        | [this setting usage in the HTTP agent docs](/integrations/language-clients/javascript/usage#custom-httphttps-agent-experimental-nodejs-only) |
...
See also: [Keep-Alive configuration](/integrations/language-clients/javascript/usage#keep-alive-configuration-nodejs-only).
...
Please refer to the list of the [supported data formats](/integrations/language-clients/javascript/usage#supported-data-formats) to determine what the best format is for streaming in your case. For example, if you want to stream JSON objects, you could choose [JSONEachRow](/reference/formats/JSON/JSONEachRow), and each row will be parsed as a JS object, or, perhaps, a more compact [JSONCompactColumns](/reference/formats/JSON/JSONCompactColumns) format that will result in each row being a compact array of values. See also: [streaming files](/integrations/language-clients/javascript/usage#streaming-files-nodejs-only).
...
It can work with either a `Stream.Readable` or a plain `Array<T>`, depending on the [data format](/integrations/language-clients/javascript/usage#supported-data-formats) specified to the `insert` method. See also this section about the [file streaming](/integrations/language-clients/javascript/usage#streaming-files-nodejs-only).
...
[Source code](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_file_stream_csv.ts). See also: [file streaming](/integrations/language-clients/javascript/usage#streaming-files-nodejs-only).
