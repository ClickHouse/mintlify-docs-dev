...
For a full list of available settings, see [here](/core/reference/settings/session-settings).
</Note>
...
Common formats: `JSONEachRow`, `CSV`, `TSV`, `Parquet`, `Native`. See the [formats documentation](/core/reference/formats) for all options.

---

### Raw stream insert {#raw-stream-insert}

Use `InsertRawStreamAsync` to insert data directly from file or memory streams in formats like CSV, JSON, Parquet, or any [supported ClickHouse format](/core/reference/formats).
...
<Note>
See the [format settings documentation](/core/reference/settings/formats) for options to control data ingestion behavior.
</Note>
