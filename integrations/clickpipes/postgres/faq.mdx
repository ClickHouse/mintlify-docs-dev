...
### Are transaction rollbacks replicated to ClickHouse? <a id="are-transaction-rollbacks-replicated"></a>

No. CDC replicates only committed transactions. Rolled-back transactions are never sent to ClickHouse.

### Can I keep data in ClickHouse longer than in my source Postgres? <a id="retain-data-longer-in-clickhouse"></a>
...
### How can I enrich data as it flows from Postgres to ClickHouse? <a id="data-enrichment"></a>
...
### Can I replicate from multiple Postgres instances into one or more ClickHouse services? <a id="multi-region-multi-source"></a>
...
### How does idling affect my Postgres CDC ClickPipe? <a id="how-does-idling-affect-my-postgres-cdc-clickpipe"></a>
...
### How are TOAST columns handled in ClickPipes for Postgres? <a id="how-are-toast-columns-handled-in-clickpipes-for-postgres"></a>

Please refer to the [Handling TOAST Columns](/integrations/clickpipes/postgres/toast) page for more information.

### How are generated columns handled in ClickPipes for Postgres? <a id="how-are-generated-columns-handled-in-clickpipes-for-postgres"></a>
...
### Do tables need to have primary keys to be part of Postgres CDC? <a id="do-tables-need-to-have-primary-keys-to-be-part-of-postgres-cdc"></a>
...
### Do you support partitioned tables as part of Postgres CDC? <a id="do-you-support-partitioned-tables-as-part-of-postgres-cdc"></a>
...
### Can I connect Postgres databases that don't have a public IP or are in private networks? <a id="can-i-connect-postgres-databases-that-dont-have-a-public-ip-or-are-in-private-networks"></a>
...
### How do you handle UPDATEs and DELETEs? <a id="how-do-you-handle-updates-and-deletes"></a>
...
### Can I update primary key columns in PostgreSQL? <a id="can-i-update-primary-key-columns-in-postgresql"></a>
...
### Do you support schema changes? <a id="do-you-support-schema-changes"></a>
...
### What are the costs for ClickPipes for Postgres CDC? <a id="what-are-the-costs-for-clickpipes-for-postgres-cdc"></a>
...
### My replication slot size is growing or not decreasing; what might be the issue? <a id="my-replication-slot-size-is-growing-or-not-decreasing-what-might-be-the-issue"></a>
...
### How are Postgres data types mapped to ClickHouse? <a id="how-are-postgres-data-types-mapped-to-clickhouse"></a>
...
### Can I define my own data type mapping while replicating data from Postgres to ClickHouse? <a id="can-i-define-my-own-data-type-mapping-while-replicating-data-from-postgres-to-clickhouse"></a>
...
### How are JSON and JSONB columns replicated from Postgres? <a id="how-are-json-and-jsonb-columns-replicated-from-postgres"></a>
...
### What happens to inserts when a mirror is paused? <a id="what-happens-to-inserts-when-a-mirror-is-paused"></a>
...
### Can ClickPipe creation be automated or done via API or CLI? <a id="can-clickpipe-creation-be-automated-or-done-via-api-or-cli"></a>

A Postgres ClickPipe can also be created and managed via [OpenAPI](/products/cloud/features/admin-features/api/openapi) endpoints. This feature is in beta, and the API reference can be found [here](/api-reference/organization/get-list-of-available-organizations#tag/beta). We're actively working on Terraform support to create Postgres ClickPipes as well.

### How do I speed up my initial load? <a id="how-do-i-speed-up-my-initial-load"></a>
...
### How should I scope my publications when setting up replication? <a id="how-should-i-scope-my-publications-when-setting-up-replication"></a>
...
### Recommended `max_slot_wal_keep_size` settings <a id="recommended-max_slot_wal_keep_size-settings"></a>
...
#### How to calculate the right value <a id="how-to-calculate-the-right-value"></a>

To determine the right setting, measure the WAL generation rate:

##### For PostgreSQL 10+ <a id="for-postgresql-10"></a>

```sql
SELECT pg_wal_lsn_diff(pg_current_wal_insert_lsn(), '0/0') / 1024 / 1024 AS wal_generated_mb;
```

##### For PostgreSQL 9.6 and below: <a id="for-postgresql-96-and-below"></a>
...
### I'm seeing a ReceiveMessage EOF error in the logs. What does it mean? <a id="im-seeing-a-receivemessage-eof-error-in-the-logs-what-does-it-mean"></a>
...
### My replication slot is invalidated. What should I do? <a id="my-replication-slot-is-invalidated-what-should-i-do"></a>
...
### I am seeing out of memory (OOMs) on ClickHouse while my ClickPipe is ingesting data. Can you help? <a id="i-am-seeing-out-of-memory-ooms-on-clickhouse-while-my-clickpipe-is-ingesting-data-can-you-help"></a>
...
### I am seeing an `invalid snapshot identifier` during the initial load. What should I do? <a id="i-am-seeing-an-invalid-snapshot-identifier-during-the-initial-load-what-should-i-do"></a>
...
### What happens if I drop a publication in Postgres? <a id="what-happens-if-i-drop-a-publication-in-postgres"></a>
...
### What if I am seeing `Unexpected Datatype` errors or `Cannot parse type XX ...` <a id="what-if-i-am-seeing-unexpected-datatype-errors"></a>
...
### I'm seeing errors like `invalid memory alloc request size <XXX>` during replication/slot creation <a id="postgres-invalid-memalloc-bug"></a>
...
### I need to maintain a complete historical record in ClickHouse, even when the data is deleted from the source Postgres database. Can I completely ignore DELETE and TRUNCATE operations from Postgres in ClickPipes? <a id="ignore-delete-truncate"></a>
...
### Why can I not replicate my table which has a dot in it? <a id="replicate-table-dot"></a>
...
### Initial load completed but there is no/missing data on ClickHouse. What could be the issue? <a id="initial-load-issue"></a>
...
### Can I have the ClickPipe create a replication slot with failover enabled? <a id="failover-slot"></a>
...
<Image img="/images/integrations/data-ingestion/clickpipes/postgres/failover_slot.png" border size="md"/>
...
### I am seeing errors like `Internal error encountered during logical decoding of aborted sub-transaction` <a id="transient-logical-decoding-errors"></a>
...
### I am seeing errors like `error converting new tuple to map` or `error parsing logical message` during CDC replication <a id="logical-message-processing-errors"></a>
...
### Can I include columns I initially excluded from replication? <a id="include-excluded-columns"></a>
...
### I am noticing that my ClickPipe has entered Snapshot but data isn't flowing in, what could be the issue? <a id="snapshot-no-data-flow"></a>
...
#### Parallel snapshotting is taking time to obtain partitions <a id="parallel-snapshotting-taking-time"></a>
...
#### Replication slot creation is transaction locked <a id="replication-slot-creation-transaction-locked"></a>
