...
## Enable binary log retention <a id="enable-binlog-retention-aurora"></a>
...
### 1. Enable binary logging via automated backup <a id="enable-binlog-logging"></a>
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/rds-backups.png" alt="Enabling automated backups in Aurora" size="lg" border/>

We recommend setting the **Backup retention period** to a reasonably long value, depending on the replication use case.

### 2. Increase the binlog retention interval <a id="binlog-retention-interval"></a>
...
## Configure binlog settings <a id="binlog-settings"></a>
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/aurora_config.png" alt="Where to find parameter group in Aurora" size="lg" border/>

<br/>
Click the parameter group link, which will take you to its dedicated page. You should see an **Edit** button in the top right.

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/edit_button.png" alt="Edit parameter group" size="lg" border/>
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_format.png" alt="Binlog format to ROW" size="lg" border/>

2. `binlog_row_metadata` to `FULL`.

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_metadata.png" alt="Binlog row metadata" size="lg" border/>

3. `binlog_row_image` to `FULL`.

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/parameter_group/binlog_row_image.png" alt="Binlog row image" size="lg" border/>
...
## Enable GTID mode (recommended) <a id="gtid-mode"></a>
...
<Image img="/images/integrations/data-ingestion/clickpipes/mysql/enable_gtid.png" alt="GTID enabled" size="lg" border/>

## Configure a database user <a id="configure-database-user"></a>
...
## Configure network access <a id="configure-network-access"></a>

### IP-based access control <a id="ip-based-access-control"></a>

To restrict traffic to your Aurora MySQL instance, add the [documented static NAT IPs](/integrations/clickpipes/home#list-of-static-ips) to the **Inbound rules** of your Aurora security group.

<Image img="/images/integrations/data-ingestion/clickpipes/mysql/source/rds/security-group-in-rds-mysql.png" alt="Where to find security group in Aurora MySQL?" size="lg" border/>

<Image img="/images/integrations/data-ingestion/clickpipes/postgres/source/rds/edit_inbound_rules.png" alt="Edit inbound rules for the above security group" size="lg" border/>

### Private access via AWS PrivateLink <a id="private-access-via-aws-privatelink"></a>
